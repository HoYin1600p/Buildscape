...
public class BrainDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void renderBrainInfo(final Entity entity, final DebugBrainDump brainDump) {
      ...
      if (selected) {
         ...
         Gizmos.billboardTextOverMob(entity, row, "health: " + String.format(Locale.ROOT, "%.1f", brainDump.health()) + " / " + String.format(Locale.ROOT, "%.1f", brainDump.maxHealth()), color, 0.32F);
         ++row;
      }
