...
public class FallingParticle extends SingleQuadParticle {
   ...
   private float rotSpeed = (float)Math.toRadians(this.random.nextBoolean() ? -30.0D : 30.0D);
   private final float spinAcceleration = (float)Math.toRadians(this.random.nextBoolean() ? -5.0D : 5.0D);
   ...
   public void tick() {
      ...
      if (!this.removed) {
         ...
         double za = 0.0D;
         if (this.flowAway) {
            xa += this.xaFlowScale * Math.pow((double)relativeAge, 1.25D);
            za += this.zaFlowScale * Math.pow((double)relativeAge, 1.25D);
         }
