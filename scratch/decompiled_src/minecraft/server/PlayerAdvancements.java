...
public class PlayerAdvancements {
   ...
   private final Map activeTriggers = new IdentityHashMap();
      ...
      if (!advancementProgress.isDone()) {
         for(Map.Entry entry : holder.value().criteria().entrySet()) {
            ...
            if (criterionProgress != null && !criterionProgress.isDone()) {
               this.addListener((Criterion)entry.getValue(), new PlayerAdvancements.TriggerInstanceKey(holder, (String)entry.getKey()));
            }
         }
   ...
   private void unregisterListeners(final AdvancementHolder holder) {
      ...
      for(Map.Entry entry : holder.value().criteria().entrySet()) {
         ...
         if (criterionProgress != null && (criterionProgress.isDone() || advancementProgress.isDone())) {
            this.removeListener(((Criterion)entry.getValue()).trigger(), new PlayerAdvancements.TriggerInstanceKey(holder, (String)entry.getKey()));
         }
      }
   ...
   private Map getOrCreateTriggerMapForType(final CriterionTrigger type) {
      return (Map)this.activeTriggers.computeIfAbsent(type, (var0) -> new HashMap());
   }

   private void addListener(final Criterion typeAndInstance, final PlayerAdvancements.TriggerInstanceKey criterion) {
      this.getOrCreateTriggerMapForType(typeAndInstance.trigger()).put(criterion, typeAndInstance.triggerInstance());
   }

   public @Nullable Map getTriggerMapForType(final CriterionTrigger type) {
      return (Map)this.activeTriggers.get(type);
   }

   private void removeListener(final CriterionTrigger type, final PlayerAdvancements.TriggerInstanceKey criterion) {
      Map map = this.getTriggerMapForType(type);
      if (map != null) {
   ...
   private static record Data(Map map) {
      public static final Codec CODEC = Codec.unboundedMap(Identifier.CODEC, AdvancementProgress.CODEC).xmap(PlayerAdvancements.Data::new, PlayerAdvancements.Data::map);
