...
public record AnimationFrame(int index, Optional time) {
   ...
   public static final Codec CODEC = Codec.either(ExtraCodecs.NON_NEGATIVE_INT, FULL_CODEC).xmap((either) -> (AnimationFrame)either.map(AnimationFrame::new, (v) -> v), (frame) -> frame.time.isPresent() ? Either.right(frame) : Either.left(frame.index));
