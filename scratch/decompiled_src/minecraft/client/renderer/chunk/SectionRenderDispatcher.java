...
public class SectionRenderDispatcher {
   ...
   private final Consumer onSectionMeshUpdate;
   private final AtomicReference cameraPosition = new AtomicReference(Vec3.ZERO);
   private volatile SectionCompiler sectionCompiler;
   private final StagingBuffer stagingBuffer;
   private final Map chunkUberBuffers;
   private final ReentrantLock copyLock = new ReentrantLock();

   public SectionRenderDispatcher(final TracingExecutor executor, final RenderBuffers renderBuffers, final SectionCompiler sectionCompiler, final Consumer onSectionMeshUpdate) {
      ...
      this.chunkUberBuffers = Util.makeEnumMap(ChunkSectionLayer.class, (layer) -> {
         ...
         UberGpuBuffer vertexUberBuffer = new UberGpuBuffer(layer.label(), 32, 134217728, vertexFormat.getVertexSize(), this.stagingBuffer);
         UberGpuBuffer indexUberBuffer = new UberGpuBuffer(layer.label(), 64, 33554432, 8, this.stagingBuffer);
   ...
   public void uploadTerrainBuffersToGpu() {
      ...
      StagingBuffer.Uploader uploader = this.stagingBuffer.startUploading(device.createCommandEncoder());
   ...
   public class RenderSection implements RotatingSectionStorage.Value {
      ...
      private boolean addSectionBuffersToUberBuffer(final ChunkSectionLayer layer, final CompiledSectionMesh key, final @Nullable ByteBuffer vertexBuffer, final @Nullable ByteBuffer indexBuffer) {
         ...
         try {
            ...
            if (draw != null) {
               SectionRenderDispatcher.SectionUberBuffers sectionBuffers = (SectionRenderDispatcher.SectionUberBuffers)SectionRenderDispatcher.this.chunkUberBuffers.get(layer);

               assert sectionBuffers != null;

               if (vertexBuffer != null) {
                  UberGpuBuffer.UploadCallback callback = (mesh) -> this.vertexBufferUploadCallback(mesh, layer);
                  success &= sectionBuffers.vertexBuffer.addAllocation(key, callback, vertexBuffer);
               }

               if (indexBuffer != null) {
                  boolean sortedIndexBuffer = vertexBuffer == null;
                  UberGpuBuffer.UploadCallback callback = (mesh) -> this.indexBufferUploadCallback(mesh, layer, sortedIndexBuffer);
      ...
      public abstract class SectionTask {
         ...
         public static enum SectionTaskResult {
            ...
            // $FF: synthetic method
            private static SectionRenderDispatcher.RenderSection.SectionTask.SectionTaskResult[] $values() {
               return new SectionRenderDispatcher.RenderSection.SectionTask.SectionTaskResult[]{SUCCESSFUL, CANCELLED};
            }
         }
      }
   }
   ...
   private static record SectionUberBuffers(UberGpuBuffer vertexBuffer, UberGpuBuffer indexBuffer) {
   }
}
