...
public class GameTestMainUtil {
   ...
   private static final OptionSpec universe = parser.accepts("universe", "The path to where the test server world will be created. Any existing folder will be replaced.").withRequiredArg().defaultsTo("gametestserver", new String[0]);
   private static final OptionSpec report = parser.accepts("report", "Exports results in a junit-like XML report at the given path.").withRequiredArg().ofType(File.class);
   private static final OptionSpec tests = parser.accepts("tests", "Which test(s) to run (namespaced ID selector using wildcards). Empty means run all.").withRequiredArg();
   private static final OptionSpec verify = parser.accepts("verify", "Runs the tests specified with `test` or `testNamespace` 100 times for each 90 degree rotation step").withRequiredArg().ofType(Boolean.class).defaultsTo(false, new Boolean[0]);
   private static final OptionSpec repeatCount = parser.accepts("repeatCount", "Runs each of the specified tests this many times").withRequiredArg().ofType(Integer.class).defaultsTo(1, new Integer[0]);
   private static final OptionSpec packs = parser.accepts("packs", "A folder of datapacks to include in the world").withRequiredArg();
   private static final OptionSpec help = parser.accepts("help").forHelp();
   ...
   public static void runGameTestServer(final String[] args, final Consumer onUniverseCreated) throws Exception {
      ...
      if (options.has(help)) {
         ...
         MinecraftServer.spin((thread) -> GameTestServer.create(thread, levelStorageSource, packRepository, optionalFromOption(options, tests), options.valueOf(verify), options.valueOf(repeatCount)));
      }
   }
   ...
   private static void copyPacks(final String serverPath, final String packSourcePath) throws IOException {
      ...
      if (Files.exists(sourceFolder, new LinkOption[0])) {
         Stream list = Files.list(sourceFolder);
