...
public enum GameType implements StringRepresentable {
   ...
   public static final StreamCodec OPTIONAL_STREAM_CODEC = ByteBufCodecs.OPTIONAL_VAR_INT.map((id) -> id.isPresent() ? Optional.of(byId(id.getAsInt())) : Optional.empty(), (gameType) -> gameType.isPresent() ? OptionalInt.of(((GameType)gameType.get()).getId()) : OptionalInt.empty());
   /** @deprecated */
   @Deprecated
