...
public class ForceLoadCommand {
   ...
   private static final CommandResponseTracker.MessagesWithArg RESPONSE_ADD = CommandResponseTracker.messages(ERROR_ALL_ADDED, (chunkPos, var1, params) -> Component.translatable("commands.forceload.added.single", Component.translationArg(chunkPos), Component.translationArg(params.dimension().identifier())), (chunkCount, var1, params) -> Component.translatable("commands.forceload.added.multiple", chunkCount, Component.translationArg(params.dimension().identifier()), Component.translationArg(params.min()), Component.translationArg(params.max())));
   private static final CommandResponseTracker.MessagesWithArg RESPONSE_REMOVE = CommandResponseTracker.messages(ERROR_NONE_REMOVED, (chunkPos, var1, params) -> Component.translatable("commands.forceload.removed.single", Component.translationArg(chunkPos), Component.translationArg(params.dimension().identifier())), (chunkCount, var1, params) -> Component.translatable("commands.forceload.removed.multiple", chunkCount, Component.translationArg(params.dimension().identifier()), Component.translationArg(params.min()), Component.translationArg(params.max())));
   ...
   private static int changeForceLoad(final CommandSourceStack source, final ColumnPos from, final ColumnPos to, final boolean add) throws CommandSyntaxException {
      ...
      if (minX >= -30000000 && minZ >= -30000000 && maxX < 30000000 && maxZ < 30000000) {
         ...
         if (chunkCount > 256L) {
            ...
            return tracker.sendFeedback(source, true, add ? RESPONSE_ADD : RESPONSE_REMOVE, new ForceLoadCommand.ForceLoadParams(dimension, minChunkX, minChunkZ, maxChunkX, maxChunkZ));
         }
      } else {
   ...
   private static record ForceLoadParams(ResourceKey dimension, int minChunkX, int minChunkZ, int maxChunkX, int maxChunkZ) {
      public ChunkPos min() {
