...
public abstract class AbstractScrollArea extends AbstractWidget {
   ...
   public AbstractScrollArea(final int x, final int y, final int width, final int height, final Component message, final AbstractScrollArea.ScrollbarSettings scrollbarSettings) {
   ...
   public boolean mouseDragged(final MouseButtonEvent event, final double dx, final double dy) {
      if (this.scrolling) {
         if (event.y() < (double)this.getY()) {
            ...
            double yDragScale = Math.max(1.0D, max / (double)(this.height - barHeight));
   ...
   public void setScrollAmount(final double scrollAmount) {
      this.scrollAmount = Mth.clamp(scrollAmount, 0.0D, (double)this.maxScrollAmount());
   }
   ...
   public static AbstractScrollArea.ScrollbarSettings defaultSettings(final int scrollRate) {
      return new AbstractScrollArea.ScrollbarSettings(SCROLLER_SPRITE, (Identifier)null, SCROLLER_BACKGROUND_SPRITE, 6, 32, scrollRate, true);
   }
