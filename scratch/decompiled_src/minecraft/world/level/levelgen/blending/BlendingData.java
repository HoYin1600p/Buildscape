...
public class BlendingData {
   ...
   private static final List SURFACE_BLOCKS = List.of(Blocks.PODZOL, Blocks.GRAVEL, Blocks.GRASS_BLOCK, Blocks.STONE, Blocks.COARSE_DIRT, Blocks.SAND, Blocks.RED_SAND, Blocks.MYCELIUM, Blocks.SNOW_BLOCK, Blocks.TERRACOTTA, Blocks.DIRT);
   ...
   private BlendingData(final int minSection, final int maxSection, final Optional heights) {
      this.heights = (float[])heights.orElseGet(() -> (float[])Util.make(new float[CELL_COLUMN_COUNT], (i) -> Arrays.fill(i, Float.MAX_VALUE)));
   ...
   public BlendingData.Packed pack() {
      ...
      return new BlendingData.Packed(this.areaWithOldGeneration.getMinSectionY(), this.areaWithOldGeneration.getMaxSectionY() + 1, hasHeight ? Optional.of(FloatArrays.copy(this.heights)) : Optional.empty());
   }
   ...
   public static Set sideByGenerationAge(final WorldGenLevel region, final int chunkX, final int chunkZ, final boolean wantedOldGen) {
      Set sides = EnumSet.noneOf(CompositeDirection.Direction8.class);
   ...
   protected void iterateBiomes(final int minCellX, final int quartY, final int minCellZ, final BlendingData.BiomeConsumer biomeConsumer) {
      if (quartY >= QuartPos.fromBlock(this.areaWithOldGeneration.getMinY()) && quartY <= QuartPos.fromBlock(this.areaWithOldGeneration.getMaxY())) {
         ...
         for(int i = 0; i < this.biomes.size(); ++i) {
            List biomeCell = (List)this.biomes.get(i);
            if (biomeCell != null) {
               Holder value = (Holder)biomeCell.get(quartIndex);
               if (value != null) {
   ...
   protected void iterateDensities(final int minCellX, final int minCellZ, final int fromCellY, final int toCellY, final BlendingData.DensityConsumer densityConsumer) {
   ...
   public static record Packed(int minSection, int maxSection, Optional heights) {
      private static final Codec FLOAT_ARRAY_CODEC = Codec.FLOAT.listOf().xmap(Floats::toArray, Floats::asList);
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.INT.fieldOf("min_section").forGetter(BlendingData.Packed::minSection), Codec.INT.fieldOf("max_section").forGetter(BlendingData.Packed::maxSection), FLOAT_ARRAY_CODEC.lenientOptionalFieldOf("heights").forGetter(BlendingData.Packed::heights)).apply(i, BlendingData.Packed::new)).validate(BlendingData.Packed::validateArraySize);

      private static DataResult validateArraySize(final BlendingData.Packed blendingData) {
