...
public final class GuiItemRenderState implements ScreenArea {
   ...
   private @Nullable ScreenRectangle calculateOversizedItemBounds() {
      ...
      int actualXSize = Mth.ceil(aabb.getXsize() * 16.0D);
      int actualYSize = Mth.ceil(aabb.getYsize() * 16.0D);
      if (actualXSize <= 16 && actualYSize <= 16) {
         return null;
      } else {
         float xOffset = (float)(aabb.minX * 16.0D);
         float yOffset = (float)(aabb.maxY * 16.0D);
