...
public abstract class AgeableMob extends PathfinderMob {
   private static final EntityDataAccessor DATA_BABY_ID = SynchedEntityData.defineId(AgeableMob.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor AGE_LOCKED = SynchedEntityData.defineId(AgeableMob.class, EntityDataSerializers.BOOLEAN);
   ...
   public static void setAgeLocked(final Mob mob, final Supplier isAgedLocked, final Player player, final ItemStack itemInHand, final Consumer setAgeLockData) {
      ...
      mob.level().playSound((Entity)null, mob.blockPosition(), isAgeLocked ? SoundEvents.GOLDEN_DANDELION_USE : SoundEvents.GOLDEN_DANDELION_UNUSE, SoundSource.PLAYERS, 1.0F, 1.0F);
   }
   ...
   public static int makeAgeLockedParticle(final Level level, final Mob mob, int ageLockParticleTimer, final boolean isAgeLocked) {
      if (ageLockParticleTimer > 0) {
         if (level.isClientSide() && ageLockParticleTimer % 2 == 0) {
            ...
            Vec3 spawnPosition = new Vec3(mob.getRandomX(1.0D), mob.getRandomY(0.2D) + (double)mob.getBbHeight() + (double)yParticleOffset, mob.getRandomZ(1.0D));
            level.addParticle(isAgeLocked ? ParticleTypes.PAUSE_MOB_GROWTH : ParticleTypes.RESET_MOB_GROWTH, spawnPosition.x, spawnPosition.y, spawnPosition.z, 0.0D, 0.0D, 0.0D);
         }
