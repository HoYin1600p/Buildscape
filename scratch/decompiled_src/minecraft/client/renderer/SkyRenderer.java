...
public class SkyRenderer implements AutoCloseable {
      ...
      try {
         ...
         for(int i = 0; i < 1500; ++i) {
            ...
            if (!(lengthSq <= 0.010000001F) && !(lengthSq >= 1.0F)) {
               ...
               float zRot = (float)(random.nextDouble() * (double)(float)Math.PI * 2.0D);
   ...
   private static GpuBuffer buildEndSky() {
      ...
      try {
         ...
         for(int i = 0; i < 6; ++i) {
            ...
            switch (i) {
               case 1:
                  pose.rotationX(((float)Math.PI / 2F));
                  break;
               case 2:
                  pose.rotationX((-(float)Math.PI / 2F));
                  break;
               case 3:
                  ...
                  break;
               case 4:
                  pose.rotationZ(((float)Math.PI / 2F));
                  break;
               case 5:
                  pose.rotationZ((-(float)Math.PI / 2F));
            }
   ...
   public void renderSkyDisc(final int skyColor) {
      ...
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Sky disc", colorTexture, Optional.empty(), depthTexture, OptionalDouble.empty());
   ...
   public void extractRenderState(final ClientLevel level, final float partialTicks, final Camera camera, final SkyRenderState state) {
      ...
      if (state.skybox != DimensionType.Skybox.NONE) {
         if (state.skybox == DimensionType.Skybox.END) {
            ...
            state.sunAngle = attributeProbe.getValue(EnvironmentAttributes.SUN_ANGLE, partialTicks) * ((float)Math.PI / 180F);
            state.moonAngle = attributeProbe.getValue(EnvironmentAttributes.MOON_ANGLE, partialTicks) * ((float)Math.PI / 180F);
            state.starAngle = attributeProbe.getValue(EnvironmentAttributes.STAR_ANGLE, partialTicks) * ((float)Math.PI / 180F);
   ...
   private boolean shouldRenderDarkDisc(final float deltaPartialTick, final ClientLevel level) {
      return Minecraft.getInstance().player.getEyePosition(deltaPartialTick).y - level.getLevelData().getHorizonHeight(level) < 0.0D && !Minecraft.getInstance().player.isUnderWater();
   }
   ...
   public void renderDarkDisc() {
      ...
      GpuTextureView colorTexture = this.renderTarget.getColorTextureView();
      GpuTextureView depthTexture = this.renderTarget.getDepthTextureView();
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Sky dark", colorTexture, Optional.empty(), depthTexture, OptionalDouble.empty());
   ...
   private void renderSun(final float rainBrightness, final PoseStack poseStack) {
      ...
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Sky sun", color, Optional.empty(), depth, OptionalDouble.empty());
   ...
   private void renderMoon(final MoonPhase moonPhase, final float rainBrightness, final PoseStack poseStack) {
      ...
      GpuTextureView depth = this.renderTarget.getDepthTextureView();
      GpuBuffer indexBuffer = this.quadIndices.getBuffer(6);
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Sky moon", color, Optional.empty(), depth, OptionalDouble.empty());
   ...
   private void renderStars(final float starBrightness, final PoseStack poseStack) {
      ...
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Stars", colorTexture, Optional.empty(), depthTexture, OptionalDouble.empty());
   ...
   public void renderSunriseAndSunset(final PoseStack poseStack, final float sunAngle, final int sunriseAndSunsetColor) {
      ...
      if (!(alpha <= 0.001F)) {
         ...
         GpuTextureView color = this.renderTarget.getColorTextureView();
         GpuTextureView depth = this.renderTarget.getDepthTextureView();
         RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Sunrise sunset", color, Optional.empty(), depth, OptionalDouble.empty());
   ...
   public void renderEndSky() {
      ...
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "End sky", colorTexture, Optional.empty(), depthTexture, OptionalDouble.empty());
   ...
   public void renderEndFlash(final PoseStack poseStack, final float intensity, final float xAngle, final float yAngle) {
      ...
      GpuTextureView depth = this.renderTarget.getDepthTextureView();
      GpuBuffer indexBuffer = this.quadIndices.getBuffer(6);
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "End flash", color, Optional.empty(), depth, OptionalDouble.empty());
