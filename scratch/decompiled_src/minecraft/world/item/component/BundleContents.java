...
public final class BundleContents implements ContainerComponent, TooltipComponent {
   public static final BundleContents EMPTY = new BundleContents(List.of());
   public static final Codec CODEC = ItemStackTemplate.CODEC.listOf().xmap(BundleContents::new, (contents) -> contents.items);
   public static final StreamCodec STREAM_CODEC = ItemStackTemplate.STREAM_CODEC.apply(ByteBufCodecs.list()).map(BundleContents::new, (contents) -> contents.items);
   private static final Fraction BUNDLE_IN_BUNDLE_WEIGHT = Fraction.getFraction(1, 16);
   private static final int NO_STACK_INDEX = -1;
   public static final int NO_SELECTED_ITEM_INDEX = -1;
   public static final DataResult BEEHIVE_WEIGHT = DataResult.success(Fraction.ONE);
   ...
   private static DataResult computeContentWeight(final List items) {
      try {
         ...
         for(ItemInstance stack : items) {
            DataResult itemWeight = getWeight(stack);
   ...
   private static DataResult getWeight(final ItemInstance item) {
      ...
      if (bundle != null) {
         ...
         List bees = ((Bees)item.getOrDefault(DataComponents.BEES, Bees.EMPTY)).bees();
   ...
   public BundleContents copyWithContents(final Stream newContents) {
   ...
   public BundleContents.Mutable asMutable() {
      DataResult currentWeight = (DataResult)this.weight.get();
   ...
   public static class Mutable extends SimpleMutableContainer {
      ...
      private Mutable(final List items, final Fraction weight, final int selectedItem) {
         super(items);
         this.weight = weight;
         this.selectedItem = selectedItem;
      }
      ...
      public int tryInsert(final ItemStack itemsToAdd) {
         if (!BundleContents.canItemBeInBundle(itemsToAdd)) {
            ...
            DataResult maybeItemWeight = BundleContents.getWeight(itemsToAdd);
      ...
      public int tryTransfer(final Slot slot, final Player player) {
         ...
         DataResult itemWeight = BundleContents.getWeight(other);
         if (itemWeight.isError()) {
            return 0;
         } else {
      ...
      private static @Nullable Fraction getWeightWithAddedItems(final Fraction weight, final ItemStack itemsToAdd) {
         if (BundleContents.canItemBeInBundle(itemsToAdd)) {
            DataResult itemWeight = BundleContents.getWeight(itemsToAdd);
            if (itemWeight.isError()) {
               return null;
            }
