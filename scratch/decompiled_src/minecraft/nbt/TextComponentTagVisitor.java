...
public class TextComponentTagVisitor implements TagVisitor {
   ...
   public void visitCompound(final CompoundTag tag) {
      if (tag.isEmpty()) {
         ...
         Collection keys;
         if (this.sortKeys) {
            List keyCopy = new ArrayList(tag.keySet());
   ...
   public static enum Token {
      ...
      // $FF: synthetic method
      private static TextComponentTagVisitor.Token[] $values() {
         return new TextComponentTagVisitor.Token[]{FOLDED, ELEMENT_SEPARATOR, LIST_CLOSE, LIST_OPEN, LIST_TYPE_SEPARATOR, STRUCT_CLOSE, STRUCT_OPEN, NAME_VALUE_SEPARATOR, BYTE_SUFFIX, BYTE_ARRAY_PREFIX, SHORT_SUFFIX, INT_ARRAY_PREFIX, LONG_SUFFIX, LONG_ARRAY_PREFIX, FLOAT_SUFFIX, DOUBLE_SUFFIX};
      }
   }
}
