...
public class FirstPersonHandsAndItemsRenderer {
   ...
   private void applyEatTransform(final PoseStack poseStack, final float partialTicks, final HumanoidArm arm, final float useItemRemainingTicks, final int useDuration) {
      ...
      float eatJiggle = 1.0F - (float)Math.pow((double)scaledUsageTime, 27.0D);
   ...
   private void applyBrushTransform(final PoseStack poseStack, final float frameInterp, final HumanoidArm arm, final float useItemRemainingTicks) {
   ...
   public void submitHandsWithItems(final float frameInterp, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final PlayerRenderState playerState, final FirstPersonHandsAndItemsRenderState state) {
      AvatarRenderState avatarRenderState = playerState.avatarRenderState;
      if (avatarRenderState != null && state.handRenderSelection != null) {
         ...
         poseStack.mulPose(Axis.XP.rotationDegrees((state.viewXRot - state.xBob) * 0.1F));
         poseStack.mulPose(Axis.YP.rotationDegrees((state.viewYRot - state.yBob) * 0.1F));
         if (state.handRenderSelection.renderMainHand) {
            float mainHandAttack = attackHand == InteractionHand.MAIN_HAND ? attackValue : 0.0F;
            float mainhandInverseArmHeight = state.mainHandSwapScale * (1.0F - Mth.lerp(frameInterp, state.oldMainHandHeight, state.mainHandHeight));
            this.submitArmWithItem(playerState, state, frameInterp, xRot, InteractionHand.MAIN_HAND, mainHandAttack, state.mainHandItem, mainhandInverseArmHeight, poseStack, submitNodeCollector, playerState.avatarRenderState.lightCoords);
         }

         if (state.handRenderSelection.renderOffHand) {
            float offHandAttack = attackHand == InteractionHand.OFF_HAND ? attackValue : 0.0F;
            float offhandInverseArmHeight = state.offHandSwapScale * (1.0F - Mth.lerp(frameInterp, state.oldOffHandHeight, state.offHandHeight));
            this.submitArmWithItem(playerState, state, frameInterp, xRot, InteractionHand.OFF_HAND, offHandAttack, state.offHandItem, offhandInverseArmHeight, poseStack, submitNodeCollector, playerState.avatarRenderState.lightCoords);
         }
   ...
   private void swingArm(final float animation, final PoseStack poseStack, final int invert, final HumanoidArm arm) {
      ...
      float ySwingPosition = 0.2F * Mth.sin((double)(Mth.sqrt(animation) * ((float)Math.PI * 2F)));
