...
public class RamTarget extends Behavior {
   ...
   protected void start(final ServerLevel level, final Goat body, final long timestamp) {
      ...
      this.ramDirection = (new Vec3((double)curPos.getX() - ramTargetPos.x(), 0.0D, (double)curPos.getZ() - ramTargetPos.z())).normalize();
   ...
   protected void tick(final ServerLevel level, final Goat body, final long timestamp) {
      List nearbyEntities = level.getNearbyEntities(LivingEntity.class, this.ramTargeting, body, body.getBoundingBox());
      Brain brain = body.getBrain();
      if (!nearbyEntities.isEmpty()) {
         ...
         boolean lostOrReachedTarget = walkTarget.isEmpty() || ramTarget.isEmpty() || ((WalkTarget)walkTarget.get()).getTarget().currentPosition().closerThan((Position)ramTarget.get(), 0.25D);
   ...
   private boolean hasRammedHornBreakingBlock(final ServerLevel level, final Goat body) {
      Vec3 horizontalMovementNormalized = body.getDeltaMovement().multiply(1.0D, 0.0D, 1.0D).normalize();
