...
public class LocalTime implements SelectItemModelProperty {
   ...
   public static final Codec VALUE_CODEC = Codec.STRING;
   private static final Codec TIME_ZONE_CODEC = VALUE_CODEC.comapFlatMap((s) -> {
      TimeZone tz = TimeZone.getTimeZone(s);
      return tz.equals(TimeZone.UNKNOWN_ZONE) ? DataResult.error(() -> "Unknown timezone: " + s) : DataResult.success(tz);
   }, TimeZone::getID);
   private static final MapCodec DATA_MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.STRING.fieldOf("pattern").forGetter((o) -> o.format), Codec.STRING.optionalFieldOf("locale", "").forGetter((o) -> o.localeId), TIME_ZONE_CODEC.optionalFieldOf("time_zone").forGetter((o) -> o.timeZone)).apply(i, LocalTime.Data::new));
   public static final SelectItemModelProperty.Type TYPE = SelectItemModelProperty.Type.create(DATA_MAP_CODEC.flatXmap(LocalTime::create, (d) -> DataResult.success(d.data)), VALUE_CODEC);
   ...
   public static LocalTime create(final String format, final String localeId, final Optional timeZone) {
      return (LocalTime)create(new LocalTime.Data(format, localeId, timeZone)).getOrThrow((msg) -> new IllegalStateException("Failed to validate format: " + msg));
   }

   private static DataResult create(final LocalTime.Data data) {
      ...
      try {
         ...
         return DataResult.error(() -> "Invalid time format '" + String.valueOf(parsedFormat) + "': " + var5.getMessage());
      }
