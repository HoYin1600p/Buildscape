...
public class RecipeButton extends AbstractWidget {
   ...
   private List selectedEntries = List.of();
   ...
   public void init(final RecipeCollection collection, final boolean isFiltering, final RecipeBookPage page, final ContextMap resolutionContext) {
      ...
      List fittingRecipes = collection.getSelectedRecipes(isFiltering ? RecipeCollection.CraftableStatus.CRAFTABLE : RecipeCollection.CraftableStatus.ANY);
      ...
      List newlyShownRecipes = fittingRecipes.stream().map(RecipeDisplayEntry::id).filter(page.getRecipeBook()::willHighlight).toList();
   ...
   public List getTooltipText(final ItemStack displayStack) {
      List texts = new ArrayList(Screen.getTooltipFromItem(Minecraft.getInstance(), displayStack));
      if (this.hasMultipleRecipes()) {
