...
public abstract class LegacyBlockRenameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Type expectedType = DSL.named(References.BLOCK_NAME.typeName(), NamespacedSchema.namespacedString());
      if (!Objects.equals(blockType, expectedType)) {
         ...
         TypeRewriteRule flatBlockStateRule = this.fixTypeEverywhereTyped(this.name + " for flat_block_state", this.getInputSchema().getType(References.FLAT_BLOCK_STATE), (input) -> input.update(DSL.remainderFinder(), (tag) -> (Dynamic)DataFixUtils.orElse(tag.asString().result().map(this::fixFlatBlockState).map(tag::createString), tag)));
