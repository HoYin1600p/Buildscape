...
public class WoodlandMansionPieces {
   ...
   private static class MansionGrid {
      ...
      private void setupThirdFloor() {
         List potentialRooms = new ArrayList();
         ...
         if (potentialRooms.isEmpty()) {
            ...
            WoodlandMansionPieces.MansionGrid.GridPos roomPos = (WoodlandMansionPieces.MansionGrid.GridPos)potentialRooms.get(this.random.nextInt(potentialRooms.size()));
            ...
            List potentialCorridors = new ArrayList();
   ...
   private static class MansionPiecePlacer {
      ...
      public void createMansion(final BlockPos origin, final Rotation rotation, final List pieces, final WoodlandMansionPieces.MansionGrid mansion) {
         ...
         this.createRoof(pieces, origin.above(27), rotation, thirdGrid, (WoodlandMansionPieces.SimpleGrid)null);
         if (!pieces.isEmpty()) {
         }

         WoodlandMansionPieces.FloorRoomCollection[] roomCollections = new WoodlandMansionPieces.FloorRoomCollection[]{new WoodlandMansionPieces.FirstFloorRoomCollection(), new WoodlandMansionPieces.SecondFloorRoomCollection(), new WoodlandMansionPieces.ThirdFloorRoomCollection()};
               ...
               for(int x = 0; x < grid.width; ++x) {
                  if (grid.get(x, y) == 1) {
                     ...
                     if (grid.get(x, y - 1) == 1 || (rooms.get(x, y - 1) & 8388608) == 8388608) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "carpet_north", pos.relative(rotation.rotate(Direction.EAST), 1).above(), rotation));
                     }

                     if (grid.get(x + 1, y) == 1 || (rooms.get(x + 1, y) & 8388608) == 8388608) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "carpet_east", pos.relative(rotation.rotate(Direction.SOUTH), 1).relative(rotation.rotate(Direction.EAST), 5).above(), rotation));
                     }

                     if (grid.get(x, y + 1) == 1 || (rooms.get(x, y + 1) & 8388608) == 8388608) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, southPiece, pos.relative(rotation.rotate(Direction.SOUTH), 5).relative(rotation.rotate(Direction.WEST), 1), rotation));
                     }

                     if (grid.get(x - 1, y) == 1 || (rooms.get(x - 1, y) & 8388608) == 8388608) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, westPiece, pos.relative(rotation.rotate(Direction.WEST), 1).relative(rotation.rotate(Direction.NORTH), 1), rotation));
                     }
                  }
               }
            }
               ...
               for(int x = 0; x < grid.width; ++x) {
                  ...
                  if (grid.get(x, y) == 2 || thirdFloorStartRoom) {
                     ...
                     if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x - 1, y) && !mansion.isRoomId(grid, x - 1, y, floorNum, roomId)) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, doorDir == Direction.WEST ? doorPiece : wallPiece, roomPos, rotation));
                     }

                     if (grid.get(x + 1, y) == 1 && !thirdFloorStartRoom) {
                        BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 8);
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, doorDir == Direction.EAST ? doorPiece : wallPiece, pos, rotation));
                     }
                     ...
                     if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y + 1) && !mansion.isRoomId(grid, x, y + 1, floorNum, roomId)) {
                        ...
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, doorDir == Direction.SOUTH ? doorPiece : wallPiece, pos, rotation.getRotated(Rotation.CLOCKWISE_90)));
                     }

                     if (grid.get(x, y - 1) == 1 && !thirdFloorStartRoom) {
                        BlockPos pos = roomPos.relative(rotation.rotate(Direction.NORTH), 1);
                        pos = pos.relative(rotation.rotate(Direction.EAST), 7);
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, doorDir == Direction.NORTH ? doorPiece : wallPiece, pos, rotation.getRotated(Rotation.CLOCKWISE_90)));
                     }
      ...
      private void traverseOuterWalls(final List pieces, final WoodlandMansionPieces.PlacementData data, final WoodlandMansionPieces.SimpleGrid grid, Direction gridDirection, final int startX, final int startY, final int endX, final int endY) {
         int gridX = startX;
         int gridY = startY;
      ...
      private void createRoof(final List pieces, final BlockPos roofOrigin, final Rotation rotation, final WoodlandMansionPieces.SimpleGrid grid, final WoodlandMansionPieces.@Nullable SimpleGrid aboveGrid) {
         for(int y = 0; y < grid.height; ++y) {
            for(int x = 0; x < grid.width; ++x) {
               ...
               if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y) && !isAbove) {
                  ...
                  if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y - 1)) {
                     ...
                     pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_front", p2, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
                  }
         ...
         if (aboveGrid != null) {
            for(int y = 0; y < grid.height; ++y) {
               for(int x = 0; x < grid.width; ++x) {
                  ...
                  var17 = var17.relative(rotation.rotate(Direction.EAST), (x - this.startX) * 8);
                  ...
                  if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y) && isAbove) {
                     ...
                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y - 1)) {
                        ...
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "small_wall", p2, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
                     }
                     ...
                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x + 1, y)) {
                        ...
                        if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y + 1)) {
                           ...
                           p2 = p2.relative(rotation.rotate(Direction.SOUTH), 7);
                           pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "small_wall_corner", p2, rotation.getRotated(Rotation.CLOCKWISE_90)));
                        }
                     }
                     ...
                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x - 1, y)) {
                        if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y - 1)) {
                           ...
                           p2 = p2.relative(rotation.rotate(Direction.NORTH), 1);
                           pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "small_wall_corner", p2, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
                        }

                        if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y + 1)) {
                           BlockPos p2 = var17.relative(rotation.rotate(Direction.WEST), 1);
                           p2 = p2.relative(rotation.rotate(Direction.SOUTH), 8);
                           pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "small_wall_corner", p2, rotation.getRotated(Rotation.CLOCKWISE_180)));
                        }
                     }
                  }
               }
            ...
            for(int x = 0; x < grid.width; ++x) {
               ...
               if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y) && !isAbove) {
                  if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x + 1, y)) {
                     ...
                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y - 1)) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_corner", p2, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
                     } else if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x + 1, y - 1)) {
                        BlockPos p3 = var19.relative(rotation.rotate(Direction.EAST), 9);
                        p3 = p3.relative(rotation.rotate(Direction.NORTH), 2);
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_inner_corner", p3, rotation.getRotated(Rotation.CLOCKWISE_90)));
                     }
                  }

                  if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x - 1, y)) {
                     BlockPos p2 = var19.relative(rotation.rotate(Direction.EAST), 0);
                     ...
                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y + 1)) {
                        ...
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_inner_corner", p3, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
                     }

                     if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, x, y - 1)) {
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_corner", p2, rotation.getRotated(Rotation.CLOCKWISE_180)));
                     } else if (WoodlandMansionPieces.MansionGrid.isHouse(grid, x - 1, y - 1)) {
                        BlockPos p3 = p2.relative(rotation.rotate(Direction.SOUTH), 1);
                        pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_inner_corner", p3, rotation.getRotated(Rotation.CLOCKWISE_180)));
                     }
                  }
               }
            }
      ...
      private void entrance(final List pieces, final WoodlandMansionPieces.PlacementData data) {
      ...
      private void traverseWallPiece(final List pieces, final WoodlandMansionPieces.PlacementData data) {
      ...
      private void traverseTurn(final List pieces, final WoodlandMansionPieces.PlacementData data) {
      ...
      private void traverseInnerTurn(final List pieces, final WoodlandMansionPieces.PlacementData data) {
      ...
      private void addRoom1x1(final List pieces, final BlockPos roomPos, final Rotation rotation, final Direction doorDir, final WoodlandMansionPieces.FloorRoomCollection rooms) {
      ...
      private void addRoom1x2(final List pieces, final BlockPos roomPos, final Rotation rotation, final Direction roomDir, final Direction doorDir, final WoodlandMansionPieces.FloorRoomCollection rooms, final boolean isStairsRoom) {
         if (doorDir == Direction.EAST && roomDir == Direction.SOUTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation));
         } else if (doorDir == Direction.EAST && roomDir == Direction.NORTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 6);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation, Mirror.LEFT_RIGHT));
         } else if (doorDir == Direction.WEST && roomDir == Direction.NORTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 7);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 6);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_180)));
         } else if (doorDir == Direction.WEST && roomDir == Direction.SOUTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 7);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation, Mirror.FRONT_BACK));
         } else if (doorDir == Direction.SOUTH && roomDir == Direction.EAST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_90), Mirror.LEFT_RIGHT));
         } else if (doorDir == Direction.SOUTH && roomDir == Direction.WEST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 7);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_90)));
         } else if (doorDir == Direction.NORTH && roomDir == Direction.WEST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 7);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 6);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_90), Mirror.FRONT_BACK));
         } else if (doorDir == Direction.NORTH && roomDir == Direction.EAST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 6);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2SideEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
         } else if (doorDir == Direction.SOUTH && roomDir == Direction.NORTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
            pos = pos.relative(rotation.rotate(Direction.NORTH), 8);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2FrontEntrance(this.random, isStairsRoom), pos, rotation));
         } else if (doorDir == Direction.NORTH && roomDir == Direction.SOUTH) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 7);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 14);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2FrontEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_180)));
         } else if (doorDir == Direction.WEST && roomDir == Direction.EAST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 15);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2FrontEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.CLOCKWISE_90)));
         } else if (doorDir == Direction.EAST && roomDir == Direction.WEST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.WEST), 7);
            pos = pos.relative(rotation.rotate(Direction.SOUTH), 6);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2FrontEntrance(this.random, isStairsRoom), pos, rotation.getRotated(Rotation.COUNTERCLOCKWISE_90)));
         } else if (doorDir == Direction.UP && roomDir == Direction.EAST) {
            BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 15);
            pieces.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, rooms.get1x2Secret(this.random), pos, rotation.getRotated(Rotation.CLOCKWISE_90)));
      ...
      private void addRoom2x2(final List pieces, final BlockPos roomPos, final Rotation rotation, final Direction roomDir, final Direction doorDir, final WoodlandMansionPieces.FloorRoomCollection rooms) {
         int east = 0;
         int south = 0;
      ...
      private void addRoom2x2Secret(final List pieces, final BlockPos roomPos, final Rotation rotation, final WoodlandMansionPieces.FloorRoomCollection rooms) {
         BlockPos pos = roomPos.relative(rotation.rotate(Direction.EAST), 1);
   ...
   public static class WoodlandMansionPiece extends TemplateStructurePiece {
      ...
      protected void handleDataMarker(final String markerId, final BlockPos position, final ServerLevelAccessor level, final RandomSource random, final BoundingBox chunkBB) {
         if (markerId.startsWith("Chest")) {
            ...
            switch (markerId) {
               case "Mage":
                  mobs.add((Mob)EntityTypes.EVOKER.create(level.getLevel(), EntitySpawnReason.STRUCTURE));
                  break;
               case "Warrior":
                  mobs.add((Mob)EntityTypes.VINDICATOR.create(level.getLevel(), EntitySpawnReason.STRUCTURE));
                  break;
               case "Group of Allays":
                  int numberOfAllays = level.getRandom().nextInt(3) + 1;

                  for(int i = 0; i < numberOfAllays; ++i) {
                     mobs.add((Mob)EntityTypes.ALLAY.create(level.getLevel(), EntitySpawnReason.STRUCTURE));
                  }
                  break;
               default:
                  return;
