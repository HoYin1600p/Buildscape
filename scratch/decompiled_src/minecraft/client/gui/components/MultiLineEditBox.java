...
public class MultiLineEditBox extends AbstractTextAreaWidget {
   ...
   private MultiLineEditBox(final Font font, final int x, final int y, final int width, final int height, final Component placeholder, final Component narration, final int textColor, final boolean textShadow, final int cursorColor, final boolean showBackground, final boolean showDecorations) {
      super(x, y, width, height, narration, AbstractScrollArea.defaultSettings((int)(9.0D / 2.0D)), showBackground, showDecorations);
      this.font = font;
   ...
   private void scrollToCursor() {
      ...
      MultilineTextField.StringView firstFullyVisibleLine = this.textField.getLineView((int)(scrollAmount / 9.0D));
      if (this.textField.cursor() <= firstFullyVisibleLine.beginIndex()) {
         scrollAmount = (double)(this.textField.getLineAtCursor() * 9);
      } else {
         MultilineTextField.StringView lastFullyVisibleLine = this.textField.getLineView((int)((scrollAmount + (double)this.height) / 9.0D) - 1);
