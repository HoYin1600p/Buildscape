...
public class SporeBlossomBlock extends Block {
   ...
   public void animateTick(final BlockState state, final Level level, final BlockPos pos, final RandomSource random) {
      ...
      double yFalling = (double)plantY + 0.7D;
      double zFalling = (double)plantZ + random.nextDouble();
      level.addParticle(ParticleTypes.FALLING_SPORE_BLOSSOM, xFalling, yFalling, zFalling, 0.0D, 0.0D, 0.0D);
      ...
      for(int i = 0; i < 14; ++i) {
         ...
         if (!particlePosState.isCollisionShapeFullBlock(level, ambientPos)) {
            level.addParticle(ParticleTypes.SPORE_BLOSSOM_AIR, (double)ambientPos.getX() + random.nextDouble(), (double)ambientPos.getY() + random.nextDouble(), (double)ambientPos.getZ() + random.nextDouble(), 0.0D, 0.0D, 0.0D);
         }
      }
