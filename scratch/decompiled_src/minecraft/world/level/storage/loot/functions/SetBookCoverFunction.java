...
public class SetBookCoverFunction extends LootItemConditionalFunction {
   ...
   public SetBookCoverFunction(final Optional condition, final Optional title, final Optional author, final Optional generation) {
      super(condition);
      this.author = author;
      this.title = title;
   ...
   private WrittenBookContent apply(final WrittenBookContent original) {
      return new WrittenBookContent((Filterable)this.title.orElseGet(original::title), (String)this.author.orElseGet(original::author), this.generation.orElseGet(original::generation), original.pages(), original.resolved());
   }
