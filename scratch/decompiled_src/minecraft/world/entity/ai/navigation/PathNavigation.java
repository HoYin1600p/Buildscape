...
public abstract class PathNavigation {
   ...
   protected void followThePath() {
      ...
      double xDistance = Math.abs(this.mob.getX() - ((double)currentNodePos.getX() + 0.5D));
      double yDistance = Math.abs(this.mob.getY() - (double)currentNodePos.getY());
      double zDistance = Math.abs(this.mob.getZ() - ((double)currentNodePos.getZ() + 0.5D));
   ...
   private boolean shouldTargetNextNodeInDirection(final Vec3 mobPosition) {
      if (this.path.getNextNodeIndex() + 1 >= this.path.getNodeCount()) {
         ...
         if (!mobPosition.closerThan(currentNode, 2.0D)) {
            return false;
            ...
            boolean withinCurrentBlock = mobToCurrentSqr < 0.5D;
            if (!closerToNextThanCurrent && !withinCurrentBlock) {
               ...
               return pathDirection.dot(mobDirection) < 0.0D;
            }
         }
      }
   }
   ...
   protected void doStuckDetection(final Vec3 mobPos) {
      ...
      if (this.path != null && !this.path.isDone()) {
         ...
         if (pos.equals(this.timeoutCachedNode)) {
            ...
            this.timeoutLimit = this.mob.getSpeed() > 0.0F ? distToNode / (double)this.mob.getSpeed() * 20.0D : 0.0D;
         }
   ...
   protected static boolean isClearForMovementBetween(final Mob mob, final Vec3 startPos, final Vec3 stopPos, final boolean blockedByFluids) {
      Vec3 to = new Vec3(stopPos.x, stopPos.y + (double)mob.getBbHeight() * 0.5D, stopPos.z);
   ...
   public boolean shouldRecomputePath(final BlockPos pos) {
      if (this.hasDelayedRecomputation) {
         ...
         Vec3 middlePos = new Vec3(((double)target.x + this.mob.getX()) / 2.0D, ((double)target.y + this.mob.getY()) / 2.0D, ((double)target.z + this.mob.getZ()) / 2.0D);
