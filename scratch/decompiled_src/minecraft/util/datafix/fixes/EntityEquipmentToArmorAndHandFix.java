...
public class EntityEquipmentToArmorAndHandFix extends DataFix {
   ...
   private TypeRewriteRule cap(final Type oldItemStackType, final Type newItemStackType) {
      Type oldEquipmentType = DSL.named(References.ENTITY_EQUIPMENT.typeName(), DSL.optional(DSL.field("Equipment", DSL.list(oldItemStackType))));
      Type newEquipmentType = DSL.named(References.ENTITY_EQUIPMENT.typeName(), DSL.and(DSL.optional(DSL.field("ArmorItems", DSL.list(newItemStackType))), DSL.optional(DSL.field("HandItems", DSL.list(newItemStackType))), DSL.optional(DSL.field("body_armor_item", newItemStackType)), DSL.optional(DSL.field("saddle", newItemStackType))));
      if (!oldEquipmentType.equals(this.getInputSchema().getType(References.ENTITY_EQUIPMENT))) {
         ...
         return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix - drop chances", this.getInputSchema().getType(References.ENTITY), (typed) -> typed.update(DSL.remainderFinder(), EntityEquipmentToArmorAndHandFix::fixDropChances)), this.fixTypeEverywhere("EntityEquipmentToArmorAndHandFix - equipment", oldEquipmentType, newEquipmentType, (ops) -> {
            Object emptyStack = (ItemStackNew)((Pair)newItemStackType.read((new Dynamic(ops)).emptyMap()).result().orElseThrow(() -> new IllegalStateException("Could not parse newly created empty itemstack."))).getFirst();
            ...
            return (named) -> named.mapSecond((equipmentField) -> {
                  ...
                  Either handItems = Either.right(DSL.unit());
                  Either armorItems = Either.right(DSL.unit());
                  if (!items.isEmpty()) {
                     handItems = Either.left(Lists.newArrayList(new Object[]{items.getFirst(), emptyStack}));
                  }

                  if (items.size() > 1) {
                     List armor = Lists.newArrayList(new Object[]{emptyStack, emptyStack, emptyStack, emptyStack});
   ...
   private static Dynamic fixDropChances(Dynamic tag) {
      ...
      if (dropChances.isPresent()) {
         Iterator chances = Stream.concat(((Stream)dropChances.get()).map((value) -> value.asFloat(0.0F)), Stream.generate(() -> 0.0F)).iterator();
