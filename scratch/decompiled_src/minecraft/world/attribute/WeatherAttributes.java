...
public class WeatherAttributes {
   ...
   private static final Set WEATHER_ATTRIBUTES = Sets.union(RAIN.keySet(), THUNDER.keySet());
   ...
   private static void addLayer(final EnvironmentAttributeSystem.Builder system, final WeatherAttributes.WeatherAccess weatherAccess, final EnvironmentAttribute attribute) {
      ...
      system.addTimeBasedLayer(attribute, (result, cacheTickId) -> {
         ...
         if (rainEntry != null && rainLevel > 0.0F) {
            Object rainValue = (Value)rainEntry.applyModifier(result);
            result = attribute.type().stateChangeLerp().apply(rainLevel, result, rainValue);
         }

         if (thunderEntry != null && thunderLevel > 0.0F) {
            Object thunderValue = (Value)thunderEntry.applyModifier(result);
