...
public final class Shapes {
   ...
   static int findBits(final double min, final double max) {
      if (!(min < -1.0E-7D) && !(max > 1.0000001D)) {
         for(int bits = 0; bits <= 3; ++bits) {
            ...
            boolean foundMin = Math.abs(shMin - (double)Math.round(shMin)) < 1.0E-7D * (double)intervals;
            boolean foundMax = Math.abs(shMax - (double)Math.round(shMax)) < 1.0E-7D * (double)intervals;
         ...
         if (!firstEmpty && !secondEmpty) {
            if (first == second) {
               ...
               for(Direction.Axis axis : AxisCycle.AXIS_VALUES) {
                  if (first.max(axis) < second.min(axis) - 1.0E-7D) {
                     return firstOnlyMatters || secondOnlyMatters;
                  }

                  if (second.max(axis) < first.min(axis) - 1.0E-7D) {
                     return firstOnlyMatters || secondOnlyMatters;
                  }
               }
   ...
   public static boolean blockOccludes(final VoxelShape shape, final VoxelShape occluder, final Direction direction) {
      if (shape == block() && occluder == block()) {
         ...
         return DoubleMath.fuzzyEquals(first.max(axis), 1.0D, 1.0E-7D) && DoubleMath.fuzzyEquals(second.min(axis), 0.0D, 1.0E-7D) && !joinIsNotEmpty(new SliceShape(first, axis, first.shape.getSize(axis) - 1), new SliceShape(second, axis, 0), op);
      }
   }
   ...
   public static boolean mergedFaceOccludes(final VoxelShape shape, final VoxelShape occluder, final Direction direction) {
      if (shape != block() && occluder != block()) {
         ...
         VoxelShape first = sign == Direction.AxisDirection.POSITIVE ? shape : occluder;
         VoxelShape second = sign == Direction.AxisDirection.POSITIVE ? occluder : shape;
         if (!DoubleMath.fuzzyEquals(first.max(axis), 1.0D, 1.0E-7D)) {
            first = empty();
         }

         if (!DoubleMath.fuzzyEquals(second.min(axis), 0.0D, 1.0E-7D)) {
            second = empty();
         }
   ...
   static IndexMerger createIndexMerger(final int cost, final DoubleList first, final DoubleList second, final boolean firstOnlyMatters, final boolean secondOnlyMatters) {
      ...
      if (first.getDouble(firstSize) < second.getDouble(0) - 1.0E-7D) {
         return new NonOverlappingMerger(first, second, false);
      } else if (second.getDouble(secondSize) < first.getDouble(0) - 1.0E-7D) {
