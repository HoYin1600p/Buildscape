...
public enum ChatTrustLevel implements StringRepresentable {
   ...
   private static boolean containsModifiedStyle(final Component decoratedContent) {
      return decoratedContent.visit((style, contents) -> isModifiedStyle(style) ? Optional.of(true) : Optional.empty(), Style.EMPTY).orElse(false);
   }
   ...
   public @Nullable GuiMessageTag createTag(final PlayerChatMessage message) {
      ...
      switch (this.ordinal()) {
         case 1:
            var10000 = GuiMessageTag.chatModified(message.signedContent());
            break;
         case 2:
