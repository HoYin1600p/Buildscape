...
public record TextInput(int width, Component label, boolean labelVisible, String initial, int maxLength, Optional multiline) implements InputControl {
   ...
   public static record MultilineOptions(Optional maxLines, Optional height) {
      ...
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(ExtraCodecs.POSITIVE_INT.optionalFieldOf("max_lines").forGetter(TextInput.MultilineOptions::maxLines), ExtraCodecs.intRange(1, 512).optionalFieldOf("height").forGetter(TextInput.MultilineOptions::height)).apply(i, TextInput.MultilineOptions::new));
   }
}
