...
public class WeatherEffectRenderer implements AutoCloseable {
   ...
   private WeatherEffectRenderer.ColumnInstance createRainColumnInstance(final RandomSource random, final long ticks, final int x, final int bottomY, final int topY, final int z, final int lightCoords, final float partialTicks) {
   ...
   private WeatherEffectRenderer.ColumnInstance createSnowColumnInstance(final RandomSource random, final long ticks, final int x, final int bottomY, final int topY, final int z, final int lightCoords, final float partialTicks) {
      int wrappedTicks = (int)(ticks & 131071L);
   ...
   private void prepareInstances(final VertexConsumer builder, final List columns, final Vec3 cameraPos, final float maxAlpha, final int radius, final float intensity) {
      if (!columns.isEmpty()) {
         float radiusSq = (float)(radius * radius);

         for(WeatherEffectRenderer.ColumnInstance column : columns) {
            float relativeX = (float)((double)column.x + 0.5D - cameraPos.x);
            float relativeZ = (float)((double)column.z + 0.5D - cameraPos.z);
