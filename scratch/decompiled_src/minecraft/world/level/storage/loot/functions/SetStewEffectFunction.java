...
public class SetStewEffectFunction extends LootItemConditionalFunction {
   private static final Codec EFFECTS_LIST = SetStewEffectFunction.EffectEntry.CODEC.listOf().validate((entries) -> {
      Set seenEffects = new ObjectOpenHashSet();
   ...
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> commonFields(i).and(EFFECTS_LIST.optionalFieldOf("effects", List.of()).forGetter((f) -> f.effects)).apply(i, SetStewEffectFunction::new));
   private final List effects;

   private SetStewEffectFunction(final Optional condition, final List effects) {
      super(condition);
   ...
   public ItemStack run(final ItemStack itemStack, final LootContext context) {
      if (itemStack.is(Items.SUSPICIOUS_STEW) && !this.effects.isEmpty()) {
         SetStewEffectFunction.EffectEntry entry = (SetStewEffectFunction.EffectEntry)Util.getRandom(this.effects, context.getRandom());
         Holder effect = entry.effect();
   ...
   public static class Builder extends LootItemConditionalFunction.Builder {
      ...
      public SetStewEffectFunction.Builder withEffect(final Holder effect, final Holder duration) {
   ...
   private static record EffectEntry(Holder effect, Holder duration) implements LootContextUser {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(MobEffect.CODEC.fieldOf("type").forGetter(SetStewEffectFunction.EffectEntry::effect), NumberProviders.CODEC.fieldOf("duration").forGetter(SetStewEffectFunction.EffectEntry::duration)).apply(i, SetStewEffectFunction.EffectEntry::new));
