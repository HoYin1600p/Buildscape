...
public class PeriodicNotificationManager extends SimplePreparableReloadListener implements AutoCloseable {
   private static final Codec CODEC = Codec.unboundedMap(Codec.STRING, RecordCodecBuilder.create((i) -> i.group(Codec.LONG.optionalFieldOf("delay", 0L).forGetter(PeriodicNotificationManager.Notification::delay), Codec.LONG.fieldOf("period").forGetter(PeriodicNotificationManager.Notification::period), Codec.STRING.fieldOf("title").forGetter(PeriodicNotificationManager.Notification::title), Codec.STRING.fieldOf("message").forGetter(PeriodicNotificationManager.Notification::message)).apply(i, PeriodicNotificationManager.Notification::new)).listOf());
   ...
   protected void apply(final Map preparations, final ResourceManager manager, final ProfilerFiller profiler) {
      List notifications = (List)preparations.entrySet().stream().filter((e) -> this.selector.apply((String)e.getKey())).map(Map.Entry::getValue).flatMap(Collection::stream).collect(Collectors.toList());
   ...
   private static class NotificationTask extends TimerTask {
      ...
      public PeriodicNotificationManager.NotificationTask reset(final List notifications, final long period) {
         this.cancel();
