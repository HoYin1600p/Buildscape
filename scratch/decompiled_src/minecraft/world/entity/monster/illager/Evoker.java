...
public class Evoker extends SpellcasterIllager {
   ...
   private class EvokerAttackSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
      ...
      protected void performSpellCasting() {
         ...
         double maxY = Math.max(target.getY(), Evoker.this.getY()) + 1.0D;
         ...
         if (Evoker.this.distanceToSqr(target) < 9.0D) {
            for(int i = 0; i < 5; ++i) {
               ...
               this.createSpellEntity(Evoker.this.getX() + (double)Mth.cos((double)angle) * 1.5D, Evoker.this.getZ() + (double)Mth.sin((double)angle) * 1.5D, minY, maxY, angle, 0);
            }

            for(int i = 0; i < 8; ++i) {
               float angle = angleTowardsTarget + (float)i * (float)Math.PI * 2.0F / 8.0F + 1.2566371F;
               this.createSpellEntity(Evoker.this.getX() + (double)Mth.cos((double)angle) * 2.5D, Evoker.this.getZ() + (double)Mth.sin((double)angle) * 2.5D, minY, maxY, angle, 3);
            }
         } else {
            for(int i = 0; i < 16; ++i) {
               double reach = 1.25D * (double)(i + 1);
               int spellSpeed = 1 * i;
   ...
   private class EvokerSummonSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
      ...
      public boolean canUse() {
         if (!super.canUse()) {
            ...
            int vexes = getServerLevel(Evoker.this.level()).getNearbyEntities(Vex.class, this.vexCountTargeting, Evoker.this, Evoker.this.getBoundingBox().inflate(16.0D)).size();
   ...
   public class EvokerWololoSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
      ...
      public EvokerWololoSpellGoal() {
         Objects.requireNonNull(Evoker.this);
         super();
         this.wololoTargeting = TargetingConditions.forNonCombat().range(16.0D).selector((target, level) -> ((Sheep)target).getColor() == DyeColor.BLUE);
      }
      ...
      public boolean canUse() {
         if (Evoker.this.getTarget() != null) {
            ...
            if (!level.getGameRules().get(GameRules.MOB_GRIEFING)) {
               ...
               List entities = level.getNearbyEntities(Sheep.class, this.wololoTargeting, Evoker.this, Evoker.this.getBoundingBox().inflate(16.0D, 4.0D, 16.0D));
               if (entities.isEmpty()) {
                  return false;
               } else {
