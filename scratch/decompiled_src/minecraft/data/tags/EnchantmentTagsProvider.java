...
public abstract class EnchantmentTagsProvider extends TagsProvider {
   ...
   protected void tooltipOrder(final HolderLookup.Provider registries, final ResourceKey... order) {
      ...
      Set set = Set.of(order);
      List unlisted = (List)registries.lookupOrThrow(Registries.ENCHANTMENT).listElements().filter((e) -> !set.contains(e.unwrapKey().get())).map(Holder::getRegisteredName).collect(Collectors.toList());
      if (!unlisted.isEmpty()) {
