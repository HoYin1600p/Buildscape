...
public abstract class BlockLootSubProvider implements LootTableSubProvider {
   ...
   protected final HolderGetter blocks;
   protected final HolderGetter predicates;
   ...
   protected BlockLootSubProvider(final Set explosionResistant, final FeatureFlagSet enabledFeatures, final LootTableSubProvider.Context output) {
   ...
   private static LootTable.Builder createSelfDropDispatchTable(final Block original, final Holder condition, final LootPoolEntryContainer.Builder entry) {
   ...
   protected LootTable.Builder createSingleItemTable(final ItemLike drop, final Holder count) {
   ...
   protected LootTable.Builder createSingleItemTableWithSilkTouch(final Block original, final ItemLike drop, final Holder count) {
   ...
   protected LootTable.Builder createMultifaceBlockDrops(final Block block, final Holder condition) {
   ...
   public LootTable.Builder createSegmentedBlockDrops(final Block block) {
      if (block instanceof SegmentableBlock segmentableBlock) {
         return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add((LootPoolEntryContainer.Builder)this.applyExplosionDecay(block, LootItem.lootTableItem(block).apply(IntStream.rangeClosed(1, 4).boxed().toList(), (count) -> (LootItemFunction.Builder)SetItemCountFunction.setCount(ConstantValue.exactly((float)count.intValue())).when(MatchBlock.blockMatches(this.blocks, block, StatePropertiesPredicate.Builder.properties().hasProperty(segmentableBlock.getSegmentAmountProperty(), count)))))));
      } else {
         return noDrop();
      }
   }
