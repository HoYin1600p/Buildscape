...
public class LayerDefinitions {
   ...
   public static Map createRoots() {
      ...
      ArmorModelSet humanoidArmor = HumanoidModel.createArmorMeshSet(INNER_ARMOR_DEFORMATION, OUTER_ARMOR_DEFORMATION).map((mesh) -> LayerDefinition.create(mesh, 64, 32));
      ArmorModelSet humanoidBabyArmor = HumanoidModel.createBabyArmorMeshSet(BABY_INNER_ARMOR_DEFORMATION, BABY_OUTER_ARMOR_DEFORMATION, PartPose.ZERO).map((mesh) -> LayerDefinition.create(mesh, 64, 64));
      ArmorModelSet playerArmor = PlayerModel.createArmorMeshSet(INNER_ARMOR_DEFORMATION, OUTER_ARMOR_DEFORMATION).map((mesh) -> LayerDefinition.create(mesh, 64, 32));
      ArmorModelSet piglinArmor = PiglinModel.createArmorMeshSet(INNER_ARMOR_DEFORMATION, new CubeDeformation(1.02F)).map((mesh) -> LayerDefinition.create(mesh, 64, 32));
      ArmorModelSet piglinBabyArmor = AbstractPiglinModel.createBabyArmorMeshSet(BABY_PIGLIN_INNER_ARMOR_DEFORMATION, BABY_PIGLIN_OUTER_ARMOR_DEFORMATION, BABY_PIGLIN_ARMOR_ARM_OFFSET).map((mesh) -> LayerDefinition.create(mesh, 64, 64));
      ...
      ArmorModelSet armorStandArmor = ArmorStandArmorModel.createArmorLayerSet(INNER_ARMOR_DEFORMATION, OUTER_ARMOR_DEFORMATION);
      ...
      ArmorModelSet zombieVillagerArmor = ZombieVillagerModel.createArmorLayerSet(INNER_ARMOR_DEFORMATION, OUTER_ARMOR_DEFORMATION);
      ...
      List missingDefinitions = ModelLayers.getKnownLocations().filter((l) -> !definitions.containsKey(l)).toList();
