...
public class UnihexProvider implements GlyphProvider {
   ...
   static void readFromStream(final InputStream input, final UnihexProvider.ReaderOutput output) throws IOException {
      ...
      while(true) {
         ...
         switch (dataDigitCount) {
            case 32:
               var10000 = UnihexProvider.ByteContents.read(line, buffer);
               break;
            case 64:
               var10000 = UnihexProvider.ShortContents.read(line, buffer);
               break;
            case 96:
               var10000 = UnihexProvider.IntContents.read24(line, buffer);
               break;
            case 128:
               var10000 = UnihexProvider.IntContents.read32(line, buffer);
               break;
            default:
               throw new IllegalArgumentException("Invalid entry at line " + line + ": expected hex number describing (8,16,24,32) x 16 bitmap, followed by a new line");
         }

         UnihexProvider.LineData contents = var10000;
   ...
   public static class Definition implements GlyphProviderDefinition {
      public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Identifier.CODEC.fieldOf("hex_file").forGetter((o) -> o.hexFile), UnihexProvider.OverrideRange.CODEC.listOf().optionalFieldOf("size_overrides", List.of()).forGetter((o) -> o.sizeOverrides)).apply(i, UnihexProvider.Definition::new));
      ...
      private UnihexProvider loadData(final InputStream zipFile) throws IOException {
         CodepointMap bits = new CodepointMap((x$0) -> new UnihexProvider.LineData[x$0], (x$0) -> new UnihexProvider.LineData[x$0][]);
         ...
         try {
            ...
            CodepointMap glyphs = new CodepointMap((x$0) -> new UnihexProvider.Glyph[x$0], (x$0) -> new UnihexProvider.Glyph[x$0][]);
            ...
            for(UnihexProvider.OverrideRange sizeOverride : this.sizeOverrides) {
               ...
               for(int c = from; c <= to; ++c) {
                  UnihexProvider.LineData codepointBits = (UnihexProvider.LineData)bits.remove(c);
   ...
   public static record Dimensions(int left, int right) {
      public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.INT.fieldOf("left").forGetter(UnihexProvider.Dimensions::left), Codec.INT.fieldOf("right").forGetter(UnihexProvider.Dimensions::right)).apply(i, UnihexProvider.Dimensions::new));
   ...
   private static record OverrideRange(int from, int to, UnihexProvider.Dimensions dimensions) {
      private static final Codec RAW_CODEC = RecordCodecBuilder.create((i) -> i.group(ExtraCodecs.CODEPOINT.fieldOf("from").forGetter(UnihexProvider.OverrideRange::from), ExtraCodecs.CODEPOINT.fieldOf("to").forGetter(UnihexProvider.OverrideRange::to), UnihexProvider.Dimensions.MAP_CODEC.forGetter(UnihexProvider.OverrideRange::dimensions)).apply(i, UnihexProvider.OverrideRange::new));
      public static final Codec CODEC = RAW_CODEC.validate((o) -> o.from >= o.to ? DataResult.error(() -> "Invalid range: [" + o.from + ";" + o.to + "]") : DataResult.success(o));
   }
