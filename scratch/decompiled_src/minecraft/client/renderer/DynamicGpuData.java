...
public class DynamicGpuData implements AutoCloseable {
   ...
   private final DynamicGpuDataStorage transforms;
   private final DynamicGpuDataStorage terrain;
   private @Nullable DynamicGpuDataStorage chunkSections = null;
   private @Nullable DynamicGpuDataStorage chunkSectionsCommandBuffer = null;

   public DynamicGpuData() {
      this.transforms = new DynamicGpuDataStorage("Dynamic Transforms UBO", TRANSFORM_UBO_SIZE, 128, 2);
      this.terrain = new DynamicGpuDataStorage("Terrain UBO", TERRAIN_TRANSFORM_UBO_SIZE, 128, 1);
   }
   ...
   public GpuBufferSlice writeTransform(final DynamicGpuData.Transform transform) {
      return this.transforms.writeData((DynamicGpuDataStorage.DynamicGpuData)transform);
   }

   public GpuBufferSlice[] writeTransforms(final DynamicGpuData.Transform... transforms) {
      return this.transforms.writeData((DynamicGpuDataStorage.DynamicGpuData[])transforms);
   }
   ...
   public GpuBufferSlice writeTerrainTransform(final Matrix4fc modelView, final int textureAtlasWidth, final int textureAtlasHeight) {
      return this.terrain.writeData((DynamicGpuDataStorage.DynamicGpuData)(new DynamicGpuData.TerrainTransform(modelView, textureAtlasWidth, textureAtlasHeight)));
   }
   ...
   public GpuBufferSlice[] writeChunkSections(final DynamicGpuData.ChunkSectionInfo... infos) {
      ...
      if (this.chunkSections == null) {
         this.chunkSections = new DynamicGpuDataStorage("Chunk Sections UBO", CHUNK_SECTION_UBO_SIZE, 128, 2);
      }

      return this.chunkSections.writeData((DynamicGpuDataStorage.DynamicGpuData[])infos);
   }

   public GpuBufferSlice writeChunkSectionsInstanced(final List infos) {
      ...
      if (this.chunkSections == null) {
         this.chunkSections = new DynamicGpuDataStorage("Chunk Sections Instanced", CHUNK_SECTION_UBO_SIZE, 32, 2);
      }

      return this.chunkSections.writeDataBatched(infos);
   }

   public GpuBufferSlice[] writeChunkSectionCommands(final List draws) {
      if (this.chunkSectionsCommandBuffer == null) {
         this.chunkSectionsCommandBuffer = new DynamicGpuDataStorage("Chunk Sections Command Buffer", 20, 512, 2);
      }
