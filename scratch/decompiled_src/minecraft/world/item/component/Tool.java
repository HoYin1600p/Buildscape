...
public record Tool(List rules, float defaultMiningSpeed, int damagePerBlock, boolean canDestroyBlocksInCreative) {
   ...
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(Tool.Rule.STREAM_CODEC.apply(ByteBufCodecs.list()), Tool::rules, ByteBufCodecs.FLOAT, Tool::defaultMiningSpeed, ByteBufCodecs.VAR_INT, Tool::damagePerBlock, ByteBufCodecs.BOOL, Tool::canDestroyBlocksInCreative, Tool::new);
   ...
   public static record Rule(HolderSet blocks, Optional speed, Optional correctForDrops) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(RegistryCodecs.holderSet(Registries.BLOCK).fieldOf("blocks").forGetter(Tool.Rule::blocks), ExtraCodecs.POSITIVE_FLOAT.optionalFieldOf("speed").forGetter(Tool.Rule::speed), Codec.BOOL.optionalFieldOf("correct_for_drops").forGetter(Tool.Rule::correctForDrops)).apply(i, Tool.Rule::new));
      public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.holderSet(Registries.BLOCK), Tool.Rule::blocks, ByteBufCodecs.FLOAT.apply(ByteBufCodecs::optional), Tool.Rule::speed, ByteBufCodecs.BOOL.apply(ByteBufCodecs::optional), Tool.Rule::correctForDrops, Tool.Rule::new);

      public static Tool.Rule minesAndDrops(final HolderSet blocks, final float speed) {
         return new Tool.Rule(blocks, Optional.of(speed), Optional.of(true));
      }

      public static Tool.Rule deniesDrops(final HolderSet blocks) {
         return new Tool.Rule(blocks, Optional.empty(), Optional.of(false));
      }

      public static Tool.Rule overrideSpeed(final HolderSet blocks, final float speed) {
