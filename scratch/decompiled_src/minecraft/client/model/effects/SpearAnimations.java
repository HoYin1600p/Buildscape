...
public class SpearAnimations {
   ...
   public static void thirdPersonHandUse(final ModelPart arm, final ModelPart head, final HumanoidArm holdingArm, final ItemStack item, final HumanoidRenderState state) {
   ...
   public static void thirdPersonUseItem(final ArmedEntityRenderState state, final PoseStack poseStack, final float timeHeld, final HumanoidArm arm, final ItemStack actualItem) {
      ...
      if (kineticWeapon != null && timeHeld != 0.0F) {
         ...
         poseStack.translate(0.0D, (double)(-hitFeedback) * 0.4D, (double)(-kineticWeapon.forwardMovement() * (raiseProgressModified - params.raiseBackProgress()) + hitFeedback));
   ...
   public static void thirdPersonAttackHand(final HumanoidModel model, final float animation, final HumanoidArm arm) {
      ...
      ModelPart var10000 = model.getArm(arm);
   ...
   public static void firstPersonUse(final float ticksSinceKineticHitFeedback, final PoseStack poseStack, final float timeHeld, final HumanoidArm arm, final ItemStack itemStack) {
      ...
      if (kineticWeapon != null) {
         SpearAnimations.UseParams params = SpearAnimations.UseParams.fromKineticWeapon(kineticWeapon, timeHeld);
         int invert = arm == HumanoidArm.RIGHT ? 1 : -1;
         poseStack.translate((double)((float)invert * (params.raiseProgress() * 0.15F + params.raiseProgressEnd() * -0.05F + params.swayProgress() * -0.1F + params.swayScaleSlow() * 0.005F)), (double)(params.raiseProgress() * -0.075F + params.raiseProgressMiddle() * 0.075F + params.swayScaleFast() * 0.01F), (double)params.raiseProgressStart() * 0.05D + (double)params.raiseProgressEnd() * -0.05D + (double)(params.swayScaleSlow() * 0.005F));
   ...
   public static void firstPersonAttack(final float attack, final PoseStack poseStack, final int invert, final HumanoidArm arm) {
      ...
      poseStack.translate(0.0D, 0.0D, -0.25D * (double)(endingAmount - middleAmount));
   }
   ...
   private static record UseParams(float raiseProgress, float raiseProgressStart, float raiseProgressMiddle, float raiseProgressEnd, float swayProgress, float lowerProgress, float raiseBackProgress, float swayIntensity, float swayScaleSlow, float swayScaleFast) {
      public static SpearAnimations.UseParams fromKineticWeapon(final KineticWeapon kineticWeapon, final float time) {
         ...
         return new SpearAnimations.UseParams(raiseProgress, raiseProgressStart, raiseProgressMiddle, raiseProgressEnd, swayProgress, lowerProgress, raiseBackProgress, swayIntensity, swayScaleSlow, swayScaleFast);
      }
   }
}
