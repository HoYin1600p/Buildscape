...
public class EyeblossomBlock extends FlowerBlock {
   ...
   public static enum Type {
      ...
      public void spawnTransformParticle(final ServerLevel level, final BlockPos pos, final RandomSource random) {
         ...
         double lifetime = 0.5D + random.nextDouble();
         Vec3 velocity = new Vec3(random.nextDouble() - 0.5D, random.nextDouble() + 1.0D, random.nextDouble() - 0.5D);
         Vec3 target = start.add(velocity.scale(lifetime));
         TrailParticleOption particle = new TrailParticleOption(target, this.particleColor, (int)(20.0D * lifetime));
         level.sendParticles(particle, start.x, start.y, start.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
      }
