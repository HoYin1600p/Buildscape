...
public class Breeze extends Monster {
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.HURT_BY, SensorType.NEAREST_PLAYERS, SensorType.BREEZE_ATTACK_ENTITY_SENSOR), BreezeAi::getActivities);
   ...
   protected Brain makeBrain(final Brain.Packed input) {
      Brain brain = BRAIN_PROVIDER.makeBrain(this, input);
   ...
   public void emitJumpTrailParticles() {
      if (++this.jumpTrailStartedTick <= 5) {
         ...
         Vec3 centered = this.position().add(movement).add(0.0D, (double)0.1F, 0.0D);

         for(int i = 0; i < 3; ++i) {
            this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, ground), centered.x, centered.y, centered.z, 0.0D, 0.0D, 0.0D);
         }
      ...
      if (!this.isPassenger()) {
         ...
         if (ground.getRenderShape() != RenderShape.INVISIBLE) {
            for(int i = 0; i < amount; ++i) {
               this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, ground), position.x, position.y, position.z, 0.0D, 0.0D, 0.0D);
            }
   ...
   public boolean withinInnerCircleRange(final Vec3 target) {
      ...
      return target.closerThan(ourPosition, 4.0D, 10.0D);
   }
