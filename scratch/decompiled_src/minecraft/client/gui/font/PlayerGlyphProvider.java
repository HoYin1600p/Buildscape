...
public class PlayerGlyphProvider {
   ...
   private final LoadingCache wrapperCache = CacheBuilder.newBuilder().expireAfterAccess(PlayerSkinRenderCache.CACHE_DURATION).build(new CacheLoader() {
      {
         Objects.requireNonNull(PlayerGlyphProvider.this);
      }

      public GlyphSource load(final FontDescription.PlayerSprite playerInfo) {
         final Supplier skin = PlayerGlyphProvider.this.playerSkinRenderCache.createLookup(playerInfo.profile());
