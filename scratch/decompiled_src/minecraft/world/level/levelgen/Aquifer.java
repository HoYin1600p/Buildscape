...
public interface Aquifer {
   ...
   public static record Config(DensityFunction barrierNoise, DensityFunction fluidLevelFloodednessNoise, DensityFunction fluidLevelSpreadNoise, DensityFunction lavaNoise, DensityFunction exclusion, DensityFunction surfaceLevel) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(DensityFunction.CODEC.fieldOf("barrier").forGetter(Aquifer.Config::barrierNoise), DensityFunction.CODEC.fieldOf("fluid_level_floodedness").forGetter(Aquifer.Config::fluidLevelFloodednessNoise), DensityFunction.CODEC.fieldOf("fluid_level_spread").forGetter(Aquifer.Config::fluidLevelSpreadNoise), DensityFunction.CODEC.fieldOf("lava").forGetter(Aquifer.Config::lavaNoise), DensityFunction.CODEC.fieldOf("exclusion").forGetter(Aquifer.Config::exclusion), DensityFunction.CODEC.fieldOf("surface_level").forGetter(Aquifer.Config::surfaceLevel)).apply(i, Aquifer.Config::new));

      public Aquifer create(final NoiseChunk noiseChunk, final ChunkPos pos, final PositionalRandomFactory positionalRandomFactory, final int minBlockY, final int yBlockSize, final Aquifer.FluidPicker fluidRule) {
         return new Aquifer.NoiseBasedAquifer(noiseChunk, pos, this, positionalRandomFactory, minBlockY, yBlockSize, fluidRule);
      }

      public Aquifer.Config mapAll(final DensityFunction.Visitor visitor) {
         return new Aquifer.Config(this.barrierNoise.mapAll(visitor), this.fluidLevelFloodednessNoise.mapAll(visitor), this.fluidLevelSpreadNoise.mapAll(visitor), this.lavaNoise.mapAll(visitor), this.exclusion.mapAll(visitor), this.surfaceLevel.mapAll(visitor));
      }
   }
   ...
   public static class NoiseBasedAquifer implements Aquifer {
      ...
      private NoiseBasedAquifer(final NoiseChunk noiseChunk, final ChunkPos pos, final Aquifer.Config config, final PositionalRandomFactory positionalRandomFactory, final int minBlockY, final int yBlockSize, final Aquifer.FluidPicker globalFluidPicker) {
      ...
      private static double similarity(final int distanceSqr1, final int distanceSqr2) {
         double threshold = 25.0D;
         return 1.0D - (double)(distanceSqr2 - distanceSqr1) / 25.0D;
      }

      private double calculatePressure(final DensityFunction.FunctionContext context, final MutableDouble barrierNoiseValue, final Aquifer.FluidStatus statusClosest1, final Aquifer.FluidStatus statusClosest2) {
         int posY = context.blockY();
         ...
         if ((!type1.is(Blocks.LAVA) || !type2.is(Blocks.WATER)) && (!type1.is(Blocks.WATER) || !type2.is(Blocks.LAVA))) {
            ...
            if (fluidYDiff == 0) {
               ...
               double averageFluidY = 0.5D * (double)(statusClosest1.fluidLevel + statusClosest2.fluidLevel);
               double howFarAboveAverageFluidPoint = (double)posY + 0.5D - averageFluidY;
               double baseValue = (double)fluidYDiff / 2.0D;
               double topBias = 0.0D;
               double furthestRocksFromTopBias = 2.5D;
               double furthestHolesFromTopBias = 1.5D;
               double bottomBias = 3.0D;
               double furthestRocksFromBottomBias = 10.0D;
               double furthestHolesFromBottomBias = 3.0D;
               double distanceFromBarrierEdgeTowardsMiddle = baseValue - Math.abs(howFarAboveAverageFluidPoint);
               double gradient;
               if (howFarAboveAverageFluidPoint > 0.0D) {
                  double centerPoint = 0.0D + distanceFromBarrierEdgeTowardsMiddle;
                  if (centerPoint > 0.0D) {
                     gradient = centerPoint / 1.5D;
                  } else {
                     gradient = centerPoint / 2.5D;
                  }
               } else {
                  double centerPoint = 3.0D + distanceFromBarrierEdgeTowardsMiddle;
                  if (centerPoint > 0.0D) {
                     gradient = centerPoint / 3.0D;
                  } else {
                     gradient = centerPoint / 10.0D;
                  }
               }
               ...
               return 2.0D * (noiseValue + gradient);
            }
         } else {
            return 2.0D;
         }
      ...
      private int computeSurfaceLevel(final int x, final int y, final int z, final Aquifer.FluidStatus globalFluid, final int lowestSurfaceLevel, final boolean surfaceAtCenterIsUnderGlobalFluidLevel) {
         ...
         if ((double)this.exclusion.compute(context) > 0.0D) {
            partiallyFloodedness = -1.0D;
            fullyFloodidness = -1.0D;
         } else {
            int distanceBelowSurface = this.adjustSurfaceLevel(lowestSurfaceLevel) - y;
            int floodednessMaxDepth = 64;
            double floodednessFactor = surfaceAtCenterIsUnderGlobalFluidLevel ? Mth.clampedMap((double)distanceBelowSurface, 0.0D, 64.0D, 1.0D, 0.0D) : 0.0D;
            double floodednessNoiseValue = Mth.clamp((double)this.fluidLevelFloodednessNoise.compute(context), -1.0D, 1.0D);
            double fullyFloodedThreshold = Mth.map(floodednessFactor, 1.0D, 0.0D, -0.3D, 0.8D);
            double partiallyFloodedThreshold = Mth.map(floodednessFactor, 1.0D, 0.0D, -0.8D, 0.4D);
         ...
         if (fullyFloodidness > 0.0D) {
            fluidSurfaceLevel = globalFluid.fluidLevel;
         } else if (partiallyFloodedness > 0.0D) {
