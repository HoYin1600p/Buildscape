...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
   ...
   private static final CompletableFuture UNLOADED_CHUNK_LIST_FUTURE = CompletableFuture.completedFuture(UNLOADED_CHUNK_LIST_RESULT);
   ...
   private final List pendingGenerationTasks = new ArrayList();
   ...
   private final Int2ObjectMap entityMap = new Int2ObjectOpenHashMap();
   ...
   private final Queue unloadQueue = Queues.newConcurrentLinkedQueue();
   ...
   CompletableFuture getChunkRangeFuture(final ChunkHolder centerChunk, final int range, final IntFunction distanceToStatus) {
      if (range == 0) {
         ...
         List deps = new ArrayList(chunkCount);
         ...
         return Util.sequence(deps).thenApply((chunkResults) -> {
            List chunks = new ArrayList(chunkResults.size());
   ...
   public ReportedException debugFuturesAndCreateReportedException(final IllegalStateException exception, final String details) {
      ...
      Consumer addToDebug = (holder) -> holder.getAllFutures().forEach((pair) -> {
   ...
   protected void saveAllChunks(final boolean flushStorage) {
      if (flushStorage) {
         List chunksToSave = this.visibleChunkMap.values().stream().filter(ChunkHolder::wasAccessibleSinceLastSave).peek(ChunkHolder::refreshAccessibility).toList();
   ...
   private CompletableFuture scheduleChunkLoad(final ChunkPos pos) {
      CompletableFuture chunkDataFuture = this.readChunk(pos).thenApplyAsync((chunkData) -> chunkData.map((tag) -> {
   ...
   public CompletableFuture prepareTickingChunk(final ChunkHolder chunk) {
      CompletableFuture future = this.getChunkRangeFuture(chunk, 1, (distance) -> ChunkStatus.FULL);
   ...
   private boolean save(final ChunkAccess chunk) {
      ...
      if (!chunk.tryMarkSaved()) {
         ...
         try {
            ...
            CompletableFuture encodedData = CompletableFuture.supplyAsync(data::write, Util.backgroundExecutor());
   ...
   void dumpChunks(final Writer output) throws IOException {
      ...
      while(var3.hasNext()) {
         ...
         Optional chunk = Optional.ofNullable(holder.getLatestChunk());
         Optional fullChunk = chunk.flatMap((chunkAccess) -> {
            Optional var10000;
   ...
   private static String printFuture(final CompletableFuture future) {
      try {
         ChunkResult result = (ChunkResult)future.getNow((Object)null);
         if (result != null) {
   ...
   public static CompoundTag getChunkDataFixContextTag(final ResourceKey dimension, final Optional generatorIdentifier) {
   ...
   public void collectSpawningChunks(final List output) {
   ...
   public void forEachBlockTickingChunk(final Consumer tickingChunkConsumer) {
   ...
   public List getPlayers(final ChunkPos pos, final boolean borderOnly) {
      Set allPlayers = this.playerMap.getAllPlayers();
   ...
   protected void tick() {
      ...
      List movedPlayers = Lists.newArrayList();
   ...
   public void resendBiomesForChunks(final List chunks) {
      Map chunksForPlayers = new HashMap();
   ...
   private class TrackedEntity implements ServerEntity.Synchronizer {
      ...
      public boolean equals(final Object obj) {
         if (obj instanceof ChunkMap.TrackedEntity trackedEntity) {
