...
public class EntityUUIDFix extends AbstractUUIDFix {
   ...
   private static Dynamic updateFox(final Dynamic tag) {
      Optional trustedUUIDs = tag.get("TrustedUUIDs").result().map((uuidTags) -> tag.createList(uuidTags.asStream().map((uuidTag) -> (Dynamic)createUUIDFromML(uuidTag).orElseGet(() -> {
