      ...
      if (clickedFace != Direction.UP) {
         ...
         if (!Cushion.canBePlacedAt(level, spawnAABB)) {
            ...
            if (level instanceof ServerLevel) {
               ...
               Cushion cushion = (Cushion)EntityTypes.CUSHION.create(serverLevel, entityConfig, blockPos, EntitySpawnReason.SPAWN_ITEM_USE, true, true);
               if (cushion == null) {
                  return InteractionResult.FAIL;
               }
   ...
   private static UseOnContext recalculateContextForSpecialCollisionShapes(final UseOnContext context) {
      ...
      if (player == null) {
         ...
         if (!clickedState.is(BlockTags.CUSHION_USES_COLLISION_SHAPE)) {
            ...
            Vec3 rayTo = context.getClickLocation().add(ray.normalize().scale(0.001D));
