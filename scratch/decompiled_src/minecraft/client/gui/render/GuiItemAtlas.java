...
public class GuiItemAtlas implements AutoCloseable {
   ...
   private final DynamicAtlasAllocator allocator;
   ...
   public GuiItemAtlas(final FeatureRenderDispatcher featureRenderDispatcher, final int textureSize, final int slotTextureSize) {
      ...
      device.createCommandEncoder().clearColorAndDepthTextures(this.texture, GuiRenderer.CLEAR_COLOR, this.depthTexture, 0.0D);
   }
   ...
   public boolean tryPrepareFor(final Set items) {
   ...
   private void drawToSlot(final int slotX, final int slotY, final boolean clear, final ItemStackRenderState item) {
      ...
      GpuDevice device = RenderSystem.getDevice();
      if (clear) {
         device.createCommandEncoder().clearColorAndDepthTextures(this.texture, GuiRenderer.CLEAR_COLOR, this.depthTexture, 0.0D, left, this.textureSize - bottom, this.slotTextureSize, this.slotTextureSize, 0);
      }
      ...
      Lighting.Entry lighting = item.usesBlockLight() ? Entry.ITEMS_3D : Entry.ITEMS_FLAT;
