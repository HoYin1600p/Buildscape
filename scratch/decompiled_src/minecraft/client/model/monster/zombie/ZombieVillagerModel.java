...
public class ZombieVillagerModel extends HumanoidModel implements VillagerLikeModel {
   ...
   public static ArmorModelSet createArmorLayerSet(final CubeDeformation innerDeformation, final CubeDeformation outerDeformation) {
      return createArmorMeshSet(ZombieVillagerModel::createBaseArmorMesh, ADULT_ARMOR_PARTS_PER_SLOT, innerDeformation, outerDeformation).map((mesh) -> LayerDefinition.create(mesh, 64, 32));
   }
   ...
   protected void setupAttackAnimation(final ZombieVillagerRenderState state) {
