...
public class BlockCollisions extends AbstractIterator {
   ...
   public BlockCollisions(final CollisionGetter collisionGetter, final CollisionContext context, final AABB box, final boolean onlySuffocatingBlocks, final BiFunction resultProvider) {
      ...
      int x0 = Mth.floor(box.minX - 1.0E-7D) - 1;
      int x1 = Mth.floor(box.maxX + 1.0E-7D) + 1;
      int y0 = Mth.floor(box.minY - 1.0E-7D) - 1;
      int y1 = Mth.floor(box.maxY + 1.0E-7D) + 1;
      int z0 = Mth.floor(box.minZ - 1.0E-7D) - 1;
      int z1 = Mth.floor(box.maxZ + 1.0E-7D) + 1;
      ...
      while(true) {
         if (this.cursor.advance()) {
            ...
            if (blockShape == Shapes.block()) {
               if (!this.box.intersects((double)x, (double)y, (double)z, (double)x + 1.0D, (double)y + 1.0D, (double)z + 1.0D)) {
                  continue;
               }
