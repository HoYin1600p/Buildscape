...
public class StackedContents {
   ...
   public boolean tryPick(final List ingredients, final int amount, final StackedContents.@Nullable Output output) {
      return (new StackedContents.RecipePicker(ingredients)).tryPick(amount, output);
   }

   public int tryPickAll(final List ingredients, final int maxSize, final StackedContents.@Nullable Output output) {
   ...
   private List getUniqueAvailableIngredientItems(final Iterable ingredients) {
      ...
      while(var3.hasNext()) {
         Reference2IntMap.Entry availableItem = (Reference2IntMap.Entry)var3.next();
   ...
   private static boolean anyIngredientMatches(final Iterable ingredients, final Object item) {
      for(StackedContents.IngredientInfo ingredient : ingredients) {
   ...
   private class RecipePicker {
      ...
      private void setInitialConnections() {
         for(int ingredient = 0; ingredient < this.ingredientCount; ++ingredient) {
            StackedContents.IngredientInfo ingredientInfo = (StackedContents.IngredientInfo)this.ingredients.get(ingredient);
      ...
      public boolean tryPick(final int capacity, final StackedContents.@Nullable Output output) {
         if (capacity <= 0) {
            return true;
         } else {
      ...
      public int tryPickAll(final int maxSize, final StackedContents.@Nullable Output output) {
         int min = 0;
         int max = Math.min(maxSize, StackedContents.this.getResultUpperBound(this.ingredients)) + 1;

         while(true) {
            int mid = (min + max) / 2;
            if (this.tryPick(mid, (StackedContents.Output)null)) {
               if (max - min <= 1) {
                  if (mid > 0) {
