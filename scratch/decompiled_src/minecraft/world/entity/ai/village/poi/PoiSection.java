...
public class PoiSection {
   ...
   private boolean add(final PoiRecord record) {
      ...
      Holder poiType = record.getPoiType();
   ...
   public void refresh(final Consumer updater) {
      if (!this.isValid) {
         Short2ObjectMap oldRecords = new Short2ObjectOpenHashMap(this.records);
         this.clear();
   ...
   public static record Packed(boolean isValid, List records) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.BOOL.lenientOptionalFieldOf("Valid", false).forGetter(PoiSection.Packed::isValid), PoiRecord.Packed.CODEC.listOf().fieldOf("Records").forGetter(PoiSection.Packed::records)).apply(i, PoiSection.Packed::new));
