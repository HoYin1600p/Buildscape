...
public class AttributeModifierIdFix extends DataFix {
   ...
   private static Stream fixModifiers(final Stream modifiers) {
      Map converted = new Object2ObjectArrayMap();
      modifiers.forEach((modifier) -> {
         ...
         if (idFromUUID != null) {
            ...
            if (preExisting == null) {
               ...
               double amount = preExisting.get("amount").asDouble(0.0D);
               double added = modifier.get("amount").asDouble(0.0D);
   ...
   private static Dynamic fixItemStackComponents(final Dynamic components) {
      return components.update("minecraft:attribute_modifiers", (attributeModifiers) -> attributeModifiers.update("modifiers", (modifiers) -> (Dynamic)DataFixUtils.orElse(modifiers.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(modifiers::createList), modifiers)));
   }
   ...
   private static Typed fixEntity(final Typed entity) {
      return entity.update(DSL.remainderFinder(), (tag) -> tag.renameAndFixField("Attributes", "attributes", (attributeList) -> (Dynamic)DataFixUtils.orElse(attributeList.asStreamOpt().result().map((s) -> s.map(AttributeModifierIdFix::fixAttribute)).map(attributeList::createList), attributeList)));
   }
