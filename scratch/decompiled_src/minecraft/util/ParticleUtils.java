...
public class ParticleUtils {
   public static void spawnParticlesOnBlockFaces(final Level level, final BlockPos pos, final ParticleOptions particle, final IntProvider particlesPerFaceRange) {
      ...
      for(Direction direction : Direction.values()) {
         spawnParticlesOnBlockFace(level, pos, particle, particlesPerFaceRange, direction, () -> getRandomSpeedRanges(random), 0.55D);
      }
   ...
   public static void spawnParticlesAlongAxis(final Direction.Axis attachedAxis, final Level level, final BlockPos pos, final double radius, final ParticleOptions particle, final UniformInt sparkCount) {
      ...
      for(int i = 0; i < particleCount; ++i) {
         double x = centerOfBlock.x + Mth.nextDouble(random, -1.0D, 1.0D) * (stepX ? 0.5D : radius);
         double y = centerOfBlock.y + Mth.nextDouble(random, -1.0D, 1.0D) * (stepY ? 0.5D : radius);
         double z = centerOfBlock.z + Mth.nextDouble(random, -1.0D, 1.0D) * (stepZ ? 0.5D : radius);
         double xBaseSpeed = stepX ? Mth.nextDouble(random, -1.0D, 1.0D) : 0.0D;
         double yBaseSpeed = stepY ? Mth.nextDouble(random, -1.0D, 1.0D) : 0.0D;
         double zBaseSpeed = stepZ ? Mth.nextDouble(random, -1.0D, 1.0D) : 0.0D;
   ...
   public static void spawnParticleOnFace(final Level level, final BlockPos pos, final Direction face, final ParticleOptions particle, final Vec3 speed, final double stepFactor) {
      ...
      RandomSource random = level.getRandom();
      double x = centerOfBlock.x + (stepX == 0 ? Mth.nextDouble(random, -0.5D, 0.5D) : (double)stepX * stepFactor);
      double y = centerOfBlock.y + (stepY == 0 ? Mth.nextDouble(random, -0.5D, 0.5D) : (double)stepY * stepFactor);
      double z = centerOfBlock.z + (stepZ == 0 ? Mth.nextDouble(random, -0.5D, 0.5D) : (double)stepZ * stepFactor);
      double xBaseSpeed = stepX == 0 ? speed.x() : 0.0D;
      double yBaseSpeed = stepY == 0 ? speed.y() : 0.0D;
      double zBaseSpeed = stepZ == 0 ? speed.z() : 0.0D;
      level.addParticle(particle, x, y, z, xBaseSpeed, yBaseSpeed, zBaseSpeed);
   }
   ...
   public static void spawnParticleInBlock(final LevelAccessor level, final BlockPos pos, final int count, final ParticleOptions particle) {
      double spreadWidth = 0.5D;
      BlockState blockState = level.getBlockState(pos);
      double spreadHeight = blockState.isAir() ? 1.0D : blockState.getShape(level, pos).max(Direction.Axis.Y);
      spawnParticles(level, pos, count, 0.5D, spreadHeight, true, particle);
   }
   ...
   public static void spawnParticles(final LevelAccessor level, final BlockPos pos, final int count, final double spreadWidth, final double spreadHeight, final boolean allowFloatingParticles, final ParticleOptions particle) {
      ...
      for(int i = 0; i < count; ++i) {
         double xVelocity = random.nextGaussian() * 0.02D;
         double yVelocity = random.nextGaussian() * 0.02D;
         double zVelocity = random.nextGaussian() * 0.02D;
         double spreadStartOffset = 0.5D - spreadWidth;
         double x = (double)pos.getX() + spreadStartOffset + random.nextDouble() * spreadWidth * 2.0D;
         double y = (double)pos.getY() + random.nextDouble() * spreadHeight;
         double z = (double)pos.getZ() + spreadStartOffset + random.nextDouble() * spreadWidth * 2.0D;
   ...
   public static void spawnSmashAttackParticles(final LevelAccessor level, final BlockPos pos, final int count) {
      ...
      for(int i = 0; (float)i < (float)count / 1.5F; ++i) {
         double x = center.x + 3.5D * Math.cos((double)i) + level.getRandom().nextGaussian() / 2.0D;
         double y = center.y;
         double z = center.z + 3.5D * Math.sin((double)i) + level.getRandom().nextGaussian() / 2.0D;
