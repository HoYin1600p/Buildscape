...
public abstract class AbstractCubeMob extends AgeableMob {
   protected static final EntityDataAccessor ID_SIZE = SynchedEntityData.defineId(AbstractCubeMob.class, EntityDataSerializers.INT);
   ...
   protected AbstractCubeMob(final EntityType type, final Level level) {
      super(type, level);
      ...
      if (this.onGround() && !this.wasOnGround) {
         ...
         for(int i = 0; (float)i < size * 16.0F; ++i) {
            ...
            if (particleType != null) {
               this.level().addParticle(particleType, this.getX() + (double)xd, this.getY(), this.getZ() + (double)zd, 0.0D, 0.0D, 0.0D);
            }
         }
   ...
   protected void setUpSplitCube(final AbstractCubeMob cubeMob, final int halfSize, final float xd, final float zd) {
      ...
      cubeMob.snapTo(this.getX() + (double)xd, this.getY() + 0.5D, this.getZ() + (double)zd, this.random.nextFloat() * 360.0F, 0.0F);
   }
   ...
   protected void dealDamage(final LivingEntity target) {
      Level damageSource = this.level();
      if (damageSource instanceof ServerLevel level) {
   ...
   private static class CubeMobFloatGoal extends Goal {
      ...
      public void tick() {
         ...
         if (var2 instanceof AbstractCubeMob.CubeMobMoveControl cubeMobMoveControl) {
            cubeMobMoveControl.setWantedMovement(1.2D);
         }
   ...
   private static class CubeMobKeepOnJumpingGoal extends Goal {
      ...
      public void tick() {
         MoveControl var2 = this.cubeMob.getMoveControl();
         if (var2 instanceof AbstractCubeMob.CubeMobMoveControl cubeMobMoveControl) {
            cubeMobMoveControl.setWantedMovement(1.0D);
         }

      }
   }

   protected static class CubeMobMoveControl extends MoveControl {
      private float yRot;
      private int jumpDelay;
      private boolean isAggressive;

      public CubeMobMoveControl(final AbstractCubeMob cubeMob) {
         super(cubeMob);
   ...
   private static class CubeMobRandomDirectionGoal extends Goal {
      ...
      public boolean canUse() {
         return this.cubeMob.getTarget() == null && (this.cubeMob.onGround() || this.cubeMob.isInWater() || this.cubeMob.isInLava() || this.cubeMob.hasEffect(MobEffects.LEVITATION)) && this.cubeMob.getMoveControl() instanceof AbstractCubeMob.CubeMobMoveControl;
      }
