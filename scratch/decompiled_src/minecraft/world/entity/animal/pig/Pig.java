...
public class Pig extends Animal implements ItemSteerable {
   ...
   private static final EntityDataAccessor DATA_VARIANT_ID = SynchedEntityData.defineId(Pig.class, EntityDataSerializers.PIG_VARIANT);
   private static final EntityDataAccessor DATA_SOUND_VARIANT_ID = SynchedEntityData.defineId(Pig.class, EntityDataSerializers.PIG_SOUND_VARIANT);
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(4, new TemptGoal(this, 1.2D, (i) -> i.is(Items.CARROT_ON_A_STICK), false));
      this.goalSelector.addGoal(4, new TemptGoal(this, 1.2D, (i) -> i.is(ItemTags.PIG_FOOD), false));
   ...
   protected void defineSynchedData(final SynchedEntityData.Builder entityData) {
      ...
      Registry pigSoundVariants = this.registryAccess().lookupOrThrow(Registries.PIG_SOUND_VARIANT);
      ...
      entityData.define(DATA_SOUND_VARIANT_ID, (Holder)pigSoundVariants.get(PigSoundVariants.CLASSIC).or(pigSoundVariants::getAny).orElseThrow());
   }
   ...
   public @Nullable Pig getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Pig baby = (Pig)EntityTypes.PIG.create(level, EntitySpawnReason.BREEDING);
   ...
   public @Nullable Object get(final DataComponentType type) {
      if (type == DataComponents.PIG_VARIANT) {
         ...
         return type == DataComponents.PIG_SOUND_VARIANT ? castComponentValue(type, this.getSoundVariant()) : super.get(type);
      }
   }
