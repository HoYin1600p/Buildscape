...
public class VideoSettingsScreen extends OptionsSubScreen {
   ...
   private static OptionInstance[] displayOptions(final Options options, final boolean supportsExclusiveFullscreen) {
      List result = Lists.newArrayList(new OptionInstance[]{options.framerateLimit(), options.enableVsync(), options.inactivityFpsLimit(), options.guiScale(), options.fullscreen()});
   ...
   protected void addOptions() {
      ...
      if (supportsExclusiveFullscreen) {
         ...
         OptionInstance exclusiveFullscreenModeOption = new OptionInstance("options.fullscreen.exclusive.mode", OptionInstance.noTooltip(), (caption, value) -> {
            if (monitor == null) {
   ...
   public boolean mouseClicked(final MouseButtonEvent event, final boolean doubleClick) {
      if (super.mouseClicked(event, doubleClick)) {
         if (this.gpuWarnlistManager.isShowingWarning()) {
            List warningMessage = Lists.newArrayList(new Component[]{WARNING_MESSAGE, CommonComponents.NEW_LINE});
   ...
   public boolean mouseScrolled(final double x, final double y, final double scrollX, final double scrollY) {
      if (this.minecraft.hasControlDown()) {
         OptionInstance guiScale = this.options.guiScale();
         OptionInstance.ValueSet oldValue = guiScale.values();
         if (oldValue instanceof OptionInstance.ClampingLazyMaxIntRange) {
            OptionInstance.ClampingLazyMaxIntRange clampingLazyMaxIntRange = (OptionInstance.ClampingLazyMaxIntRange)oldValue;
            ...
            if (newValue != 0 && newValue <= clampingLazyMaxIntRange.maxInclusive() && newValue >= clampingLazyMaxIntRange.minInclusive()) {
               CycleButton cycleButton = (CycleButton)this.list.findOption(guiScale);
               if (cycleButton != null) {
   ...
   public void updateTransparencyButton() {
      if (this.list != null) {
         OptionInstance option = this.options.improvedTransparency();
