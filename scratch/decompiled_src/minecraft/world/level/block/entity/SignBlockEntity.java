...
public class SignBlockEntity extends BlockEntity {
   ...
   public SignTextSlot getSlotPlayerIsFacing(final Player player) {
      Block signPositionOffset = this.getBlockState().getBlock();
      if (signPositionOffset instanceof SignBlock sign) {
   ...
   public SignText getText(final SignTextSlot slot) {
      ...
      switch (slot) {
         case FRONT:
            var10000 = this.frontText;
            break;
         case BACK:
            var10000 = this.backText;
            break;
         default:
   ...
   public void updateSignText(final Player player, final SignTextSlot slot, final List lines) {
   ...
   public boolean updateText(final UnaryOperator function, final SignTextSlot slot) {
   ...
   private static SignText updateMessages(final List newLines, final SignText originalText, final boolean shouldFilter) {
      SignText.Mutable result = originalText.asMutable();
      List originalLines = originalText.getMessages(shouldFilter);
   ...
   public boolean playerIsTooFarAwayToEdit(final UUID player) {
      ...
      return editingPlayer == null || !editingPlayer.isWithinBlockInteractionRange(this.getBlockPos(), 4.0D);
   }
