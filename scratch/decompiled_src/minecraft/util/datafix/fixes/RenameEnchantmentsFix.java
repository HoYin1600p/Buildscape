...
public class RenameEnchantmentsFix extends DataFix {
   ...
   private Dynamic fixEnchantmentList(final Dynamic itemStack, final String field) {
      return itemStack.update(field, (tag) -> (Dynamic)tag.asStreamOpt().map((s) -> s.map((element) -> element.update("id", (id) -> (Dynamic)id.asString().map((stringId) -> element.createString((String)this.renames.getOrDefault(NamespacedSchema.ensureNamespaced(stringId), stringId))).mapOrElse(Function.identity(), (fail) -> id)))).map(tag::createList).mapOrElse(Function.identity(), (fail) -> tag));
   }
}
