...
public abstract class ChunkGenerator {
   public static final Codec CODEC = BuiltInRegistries.CHUNK_GENERATOR.byNameCodec().dispatchStable(ChunkGenerator::codec, Function.identity());
   ...
   public @Nullable Pair findNearestMapStructure(final ServerLevel level, final HolderSet wantedStructures, final BlockPos pos, final int maxSearchRadius, final boolean createReference) {
      if (SharedConstants.DEBUG_DISABLE_FEATURES) {
         ...
         if (placementScans.isEmpty()) {
            ...
            List randomSpreadEntries = new ArrayList(placementScans.size());
            ...
            for(Map.Entry entry : placementScans.entrySet()) {
               ...
               if (placement instanceof ConcentricRingsStructurePlacement) {
                  ...
                  Pair generating = this.getNearestGeneratedStructure((Set)entry.getValue(), level, structureManager, pos, createReference, rings);
                  if (generating != null) {
            ...
            if (!randomSpreadEntries.isEmpty()) {
               ...
               for(int radius = 0; radius <= maxSearchRadius; ++radius) {
                  ...
                  for(Map.Entry entry : randomSpreadEntries) {
                     ...
                     Pair structurePos = getNearestGeneratedStructure((Set)entry.getValue(), level, structureManager, chunkOriginX, chunkOriginZ, radius, createReference, generatorState.getLevelSeed(), randomPlacement);
   ...
   private @Nullable Pair getNearestGeneratedStructure(final Set structures, final ServerLevel level, final StructureManager structureManager, final BlockPos pos, final boolean createReference, final ConcentricRingsStructurePlacement rings) {
      List positions = level.getChunkSource().getGeneratorState().getRingPositionsFor(rings);
      if (positions == null) {
         throw new IllegalStateException("Somehow tried to find structures for a placement that doesn't exist");
      } else {
         Pair closestPos = null;
         ...
         for(ChunkPos chunkPos : positions) {
            ...
            if (isClosest) {
               Pair generating = getStructureGeneratingAt(structures, level, structureManager, createReference, rings, chunkPos);
               if (generating != null) {
      ...
      for(int x = -radius; x <= radius; ++x) {
         ...
         for(int z = -radius; z <= radius; ++z) {
            ...
            if (xEdge || zEdge) {
               ...
               Pair generating = getStructureGeneratingAt(structures, level, structureManager, createReference, config, chunkTarget);
               if (generating != null) {
                  return generating;
               }
            }
   ...
   public void applyBiomeDecoration(final WorldGenLevel level, final ChunkAccess chunk, final StructureManager structureManager) {
      ...
      if (!SharedConstants.debugVoidTerrain(centerPos)) {
         ...
         Registry structuresRegistry = level.registryAccess().lookupOrThrow(Registries.STRUCTURE);
         Map structuresByStep = (Map)structuresRegistry.stream().collect(Collectors.groupingBy((structure) -> structure.step().ordinal()));
         List featureList = (List)this.featuresPerStep.get();
         WorldgenRandom random = new WorldgenRandom(new XoroshiroRandomSource(RandomSupport.generateUniqueSeed()));
         long decorationSeed = random.setDecorationSeed(level.getSeed(), origin.getX(), origin.getZ());
         Set possibleBiomes = new ObjectArraySet();
         ...
         try {
            Registry featureRegistry = level.registryAccess().lookupOrThrow(Registries.PLACED_FEATURE);
            ...
            for(int stepIndex = 0; stepIndex < generationSteps; ++stepIndex) {
               ...
               if (structureManager.shouldGenerateStructures()) {
                  for(Structure structure : (List)structuresByStep.getOrDefault(stepIndex, Collections.emptyList())) {
                     ...
                     Supplier currentlyGenerating = () -> (String)structuresRegistry.getResourceKey(structure).map(Object::toString).orElseGet(structure::toString);
               ...
               if (stepIndex < featureStepCount) {
                  ...
                  for(Holder biome : possibleBiomes) {
                     List featuresInBiome = ((BiomeGenerationSettings)this.generationSettingsGetter.apply(biome)).features();
                     if (stepIndex < featuresInBiome.size()) {
                        HolderSet featuresInBiomeThisStep = (HolderSet)featuresInBiome.get(stepIndex);
                  ...
                  for(int featureIndex = 0; featureIndex < numberOfFeaturesInStep; ++featureIndex) {
                     ...
                     Supplier currentlyGenerating = () -> (String)featureRegistry.getResourceKey(feature).map(Object::toString).orElseGet(feature::toString);
   ...
   public abstract void buildSurface(StructureManager structureManager, RandomState randomState, ChunkAccess protoChunk, BiomeManager biomeManager, Blender blender, Set possibleBiomes);
   ...
   public WeightedList getMobsAt(final Level level, final StructureManager structureManager, final MobCategory mobCategory, final BlockPos pos) {
      Map structures = structureManager.getAllStructuresAt(pos);
      ...
      for(Map.Entry entry : structures.entrySet()) {
         ...
         if (override != null) {
            ...
            Predicate check = override.boundingBox() == StructureSpawnOverride.BoundingBoxType.PIECE ? (start) -> structureManager.structureHasPieceAt(pos, start) : (start) -> start.getBoundingBox().isInside(pos);
   ...
   public void createStructures(final RegistryAccess registryAccess, final ChunkGeneratorStructureState state, final StructureManager structureManager, final ChunkAccess centerChunk, final StructureTemplateManager structureTemplateManager, final ResourceKey level) {
      if (!SharedConstants.DEBUG_DISABLE_STRUCTURES) {
         ...
         state.possibleStructureSets().forEach((set) -> {
            ...
            List structures = ((StructureSet)set.value()).structures();
            ...
            if (featurePlacement.isStructureChunk(state, sourceChunkPos.x(), sourceChunkPos.z())) {
               if (structures.size() == 1) {
                  ...
                  ArrayList options = new ArrayList(structures.size());
   ...
   private boolean tryGenerateStructure(final StructureSet.StructureSelectionEntry selected, final StructureManager structureManager, final RegistryAccess registryAccess, final RandomState randomState, final StructureTemplateManager structureTemplateManager, final long seed, final ChunkAccess centerChunk, final ChunkPos sourceChunkPos, final SectionPos sectionPos, final ResourceKey level, final BiomeResolver biomeResolver) {
      ...
      HolderSet biomeAllowedForStructure = structure.biomes();
      Predicate biomePredicate = biomeAllowedForStructure::contains;
         ...
         for(int sourceZ = targetZ - 8; sourceZ <= targetZ + 8; ++sourceZ) {
            ...
            for(StructureStart start : level.getChunk(sourceX, sourceZ).getAllStarts().values()) {
               try {
                  ...
                  Optional configuredStructuresRegistry = level.registryAccess().lookup(Registries.STRUCTURE);
   ...
   public abstract CompletableFuture fillFromNoise(Blender blender, RandomState randomState, StructureManager structureManager, ChunkAccess chunk);
