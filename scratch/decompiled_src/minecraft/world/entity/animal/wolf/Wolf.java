...
public class Wolf extends TamableAnimal implements NeutralMob {
   ...
   private static final EntityDataAccessor DATA_ANGER_END_TIME = SynchedEntityData.defineId(Wolf.class, EntityDataSerializers.LONG);
   private static final EntityDataAccessor DATA_VARIANT_ID = SynchedEntityData.defineId(Wolf.class, EntityDataSerializers.WOLF_VARIANT);
   private static final EntityDataAccessor DATA_SOUND_VARIANT_ID = SynchedEntityData.defineId(Wolf.class, EntityDataSerializers.WOLF_SOUND_VARIANT);
   ...
   protected void defineSynchedData(final SynchedEntityData.Builder entityData) {
      ...
      Registry wolfSoundVariants = this.registryAccess().lookupOrThrow(Registries.WOLF_SOUND_VARIANT);
      entityData.define(DATA_VARIANT_ID, VariantUtils.getDefaultOrAny(this.registryAccess(), WolfVariants.DEFAULT));
      entityData.define(DATA_SOUND_VARIANT_ID, (Holder)wolfSoundVariants.get(WolfSoundVariants.CLASSIC).or(wolfSoundVariants::getAny).orElseThrow());
   ...
   public @Nullable SpawnGroupData finalizeSpawn(final ServerLevelAccessor level, final DifficultyInstance difficulty, final EntitySpawnReason spawnReason, @Nullable SpawnGroupData groupData) {
      if (groupData instanceof Wolf.WolfPackData wolfGroupData) {
         ...
         if (selectedVariant.isPresent()) {
            ...
            groupData = new Wolf.WolfPackData((Holder)selectedVariant.get());
         }
      }
   ...
   protected void actuallyHurt(final ServerLevel level, final DamageSource source, final float damage) {
      if (!this.canArmorAbsorb(source)) {
         ...
         if (Crackiness.WOLF_ARMOR.byDamage(damageBefore, maxDamage) != Crackiness.WOLF_ARMOR.byDamage(this.getBodyArmorItem())) {
            ...
            level.sendParticles(new ItemParticleOption(ParticleTypes.ITEM, Items.ARMADILLO_SCUTE), this.getX(), this.getY() + 1.0D, this.getZ(), 20, 0.2D, 0.1D, 0.2D, 0.1D);
         }
   ...
   public @Nullable Wolf getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Wolf baby = (Wolf)EntityTypes.WOLF.create(level, EntitySpawnReason.BREEDING);
