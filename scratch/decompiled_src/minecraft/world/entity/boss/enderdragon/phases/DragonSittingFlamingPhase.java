...
public class DragonSittingFlamingPhase extends AbstractDragonSittingPhase {
   ...
   public void doClientTick() {
      ...
      if (this.flameTicks % 2 == 0 && this.flameTicks < 10) {
         ...
         for(int i = 0; i < 8; ++i) {
            double px = particleX + this.dragon.getRandom().nextGaussian() / 2.0D;
            double py = particleY + this.dragon.getRandom().nextGaussian() / 2.0D;
            double pz = particleZ + this.dragon.getRandom().nextGaussian() / 2.0D;
   ...
   public void doServerTick(final ServerLevel level) {
      ...
      if (this.flameTicks >= 200) {
         ...
         Vec3 look = (new Vec3(this.dragon.head.getX() - this.dragon.getX(), 0.0D, this.dragon.head.getZ() - this.dragon.getZ())).normalize();
         float radius = 5.0F;
         double x = this.dragon.head.getX() + look.x * 5.0D / 2.0D;
         double z = this.dragon.head.getZ() + look.z * 5.0D / 2.0D;
         double initialY = this.dragon.head.getY(0.5D);
         double y = initialY;
