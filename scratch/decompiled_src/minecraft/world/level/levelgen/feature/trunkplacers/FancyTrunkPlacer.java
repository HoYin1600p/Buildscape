...
public class FancyTrunkPlacer extends TrunkPlacer {
   ...
   public List placeTrunk(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int treeHeight, final BlockPos origin, final TreeFeature tree) {
      int assumedFoliageHeight = 5;
      int height = treeHeight + 2;
      int trunkHeight = Mth.floor((double)height * 0.618D);
      placeBelowTrunkBlock(level, trunkSetter, random, origin.below(), tree);
      double foliageDensity = 1.0D;
      int clustersPerY = Math.min(1, Mth.floor(1.382D + Math.pow(1.0D * (double)height / 13.0D, 2.0D)));
      int trunkTop = origin.getY() + trunkHeight;
      int relativeY = height - 5;
      List foliageCoords = Lists.newArrayList();
         ...
         if (!(treeShape < 0.0F)) {
            for(int i = 0; i < clustersPerY; ++i) {
               ...
               if (this.makeLimb(level, trunkSetter, random, checkStart, checkEnd, false, tree)) {
                  ...
                  double branchHeight = (double)checkStart.getY() - Math.sqrt((double)(dx * dx + dz * dz)) * 0.381D;
   ...
   private boolean makeLimb(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final BlockPos startPos, final BlockPos endPos, final boolean doPlace, final TreeFeature tree) {
   ...
   private void makeBranches(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int height, final BlockPos origin, final List foliageCoords, final TreeFeature tree) {
