...
public record KineticWeapon(int contactCooldownTicks, int delayTicks, Optional dismountConditions, Optional knockbackConditions, Optional damageConditions, float forwardMovement, float damageMultiplier, Optional sound, Optional hitSound) {
   ...
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.VAR_INT, KineticWeapon::contactCooldownTicks, ByteBufCodecs.VAR_INT, KineticWeapon::delayTicks, KineticWeapon.Condition.STREAM_CODEC.apply(ByteBufCodecs::optional), KineticWeapon::dismountConditions, KineticWeapon.Condition.STREAM_CODEC.apply(ByteBufCodecs::optional), KineticWeapon::knockbackConditions, KineticWeapon.Condition.STREAM_CODEC.apply(ByteBufCodecs::optional), KineticWeapon::damageConditions, ByteBufCodecs.FLOAT, KineticWeapon::forwardMovement, ByteBufCodecs.FLOAT, KineticWeapon::damageMultiplier, SoundEvent.STREAM_CODEC.apply(ByteBufCodecs::optional), KineticWeapon::sound, SoundEvent.STREAM_CODEC.apply(ByteBufCodecs::optional), KineticWeapon::hitSound, KineticWeapon::new);
      ...
      if (ticksUsed >= this.delayTicks) {
         ...
         for(EntityHitResult hitResult : (Collection)ProjectileUtil.getHitEntitiesAlong(livingEntity, attackRange, (e) -> PiercingWeapon.canHitEntity(livingEntity, e), ClipContext.Block.COLLIDER).map((a) -> List.of(), (e) -> e)) {
            ...
            if (!wasStabbed) {
               ...
               double relativeSpeed = Math.max(0.0D, attackerSpeedProjection - targetSpeedProjection);
               boolean dealsDismount = this.dismountConditions.isPresent() && ((KineticWeapon.Condition)this.dismountConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, (double)actionFactor);
               boolean dealsKnockback = this.knockbackConditions.isPresent() && ((KineticWeapon.Condition)this.knockbackConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, (double)actionFactor);
               boolean dealsDamage = this.damageConditions.isPresent() && ((KineticWeapon.Condition)this.damageConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, (double)actionFactor);
   ...
   public static record Condition(int maxDurationTicks, float minSpeed, float minRelativeSpeed) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(ExtraCodecs.NON_NEGATIVE_INT.fieldOf("max_duration_ticks").forGetter(KineticWeapon.Condition::maxDurationTicks), Codec.FLOAT.optionalFieldOf("min_speed", 0.0F).forGetter(KineticWeapon.Condition::minSpeed), Codec.FLOAT.optionalFieldOf("min_relative_speed", 0.0F).forGetter(KineticWeapon.Condition::minRelativeSpeed)).apply(i, KineticWeapon.Condition::new));
      public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.VAR_INT, KineticWeapon.Condition::maxDurationTicks, ByteBufCodecs.FLOAT, KineticWeapon.Condition::minSpeed, ByteBufCodecs.FLOAT, KineticWeapon.Condition::minRelativeSpeed, KineticWeapon.Condition::new);
