...
public class ReloadableServerRegistries {
   ...
   public static CompletableFuture reload(final LayeredRegistryAccess context, final List updatedContextTags, final ResourceManager manager, final Executor executor) {
      List contextRegistriesWithTags = TagLoader.buildUpdatedLookups(context.getAccessForLoading(RegistryLayer.RELOADABLE), updatedContextTags);
   ...
   private static ReloadableServerRegistries.LoadResult createAndValidateFullContext(final LayeredRegistryAccess contextLayers, final HolderLookup.Provider contextLookupWithUpdatedTags, final RegistryAccess.Frozen reloadableRegistries) {
      LayeredRegistryAccess fullLayers = contextLayers.replaceFrom(RegistryLayer.RELOADABLE, reloadableRegistries);
