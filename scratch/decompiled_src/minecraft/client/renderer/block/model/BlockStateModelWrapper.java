...
public class BlockStateModelWrapper implements BlockModel {
   ...
   public BlockStateModelWrapper(final BlockStateModel model, final List tints, final Matrix4fc transformation) {
      this.model = model;
      this.tints = tints;
   ...
   public void update(final BlockModelRenderState output, final BlockState blockState, final BlockDisplayContext displayContext, final long seed) {
      List partList = output.setupModel(this.transformation, this.model.hasMaterialFlag(1));
   ...
   public static record Unbaked(BlockState model, List tints, Optional transformation) implements BlockModel.Unbaked {
