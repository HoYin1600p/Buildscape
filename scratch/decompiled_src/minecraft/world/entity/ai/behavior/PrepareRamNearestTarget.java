...
public class PrepareRamNearestTarget extends Behavior {
   ...
   protected void tick(final ServerLevel level, final PathfinderMob body, final long timestamp) {
      if (!this.ramCandidate.isEmpty()) {
         ...
         boolean didTargetMove = !((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getTarget().blockPosition().equals(((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getTargetPosition());
         if (didTargetMove) {
            level.broadcastEntityEvent(body, (byte)59);
            ...
            if (startRamPos.equals(((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getStartPosition())) {
               ...
               if (timestamp - this.reachedRamPositionTimestamp.get() >= (long)this.ramPrepareTime) {
                  body.getBrain().setMemory(MemoryModuleType.RAM_TARGET, this.getEdgeOfBlock(startRamPos, ((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getTargetPosition()));
   ...
   private Vec3 getEdgeOfBlock(final BlockPos startRamPos, final BlockPos targetPos) {
      ...
      double xOffset = 0.5D * (double)Mth.sign((double)(targetPos.getX() - startRamPos.getX()));
      double zOffset = 0.5D * (double)Mth.sign((double)(targetPos.getZ() - startRamPos.getZ()));
      return Vec3.atBottomCenterOf(targetPos).add(xOffset, 0.0D, zOffset);
   }
   ...
   private Optional calculateRammingStartPosition(final PathfinderMob body, final LivingEntity ramableTarget) {
      ...
      if (!this.isWalkableBlock(body, targetPos)) {
         ...
         List possibleRamPositions = Lists.newArrayList();
   ...
   private void chooseRamPosition(final PathfinderMob body, final LivingEntity ramableTarget) {
      ...
      this.ramCandidate = this.calculateRammingStartPosition(body, ramableTarget).map((pos) -> new PrepareRamNearestTarget.RamCandidate(pos, ramableTarget.blockPosition(), ramableTarget));
   }
