...
public class TooltipDisplayComponentFix extends DataFix {
   ...
   private static Typed fix(Typed typed, final OpticFinder canPlaceOnFinder, final OpticFinder canBreakFinder, final Type newCanPlaceOnType, final Type newCanBreakType) {
      Set hiddenTooltips = new HashSet();
      ...
      return typed.update(DSL.remainderFinder(), (remainder) -> {
         ...
         return hiddenTooltips.isEmpty() && !hideTooltip ? remainder : remainder.set("minecraft:tooltip_display", remainder.createMap(Map.of(remainder.createString("hide_tooltip"), remainder.createBoolean(hideTooltip), remainder.createString("hidden_components"), remainder.createList(hiddenTooltips.stream().map(remainder::createString)))));
      });
   }
