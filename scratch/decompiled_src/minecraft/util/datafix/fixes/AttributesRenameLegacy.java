...
public class AttributesRenameLegacy extends DataFix {
   ...
   private Typed fixItemStackTag(final Typed itemStack) {
      return itemStack.update(DSL.remainderFinder(), (tag) -> tag.update("AttributeModifiers", (modifiers) -> (Dynamic)DataFixUtils.orElse(modifiers.asStreamOpt().result().map((s) -> s.map((modifier) -> modifier.update("AttributeName", this::fixName))).map(modifiers::createList), modifiers)));
   }

   private Typed fixEntity(final Typed entity) {
      return entity.update(DSL.remainderFinder(), (tag) -> tag.update("Attributes", (attributeList) -> (Dynamic)DataFixUtils.orElse(attributeList.asStreamOpt().result().map((s) -> s.map((attribute) -> attribute.update("Name", this::fixName))).map(attributeList::createList), attributeList)));
   }
}
