...
public class HoneyBlock extends HalfTransparentBlock {
   ...
   private boolean isSlidingDown(final BlockPos pos, final Entity entity) {
      if (entity.onGround()) {
         ...
         double dx = Math.abs((double)pos.getX() + 0.5D - entity.getX());
         double dz = Math.abs((double)pos.getZ() + 0.5D - entity.getZ());
         double overlapDistance = 0.4375D + (double)(entity.getBbWidth() / 2.0F);
         return dx + 1.0E-7D > overlapDistance || dz + 1.0E-7D > overlapDistance;
      }
   }
   ...
   private void doSlideMovement(final Entity entity) {
      ...
      if (getOldDeltaY(entity.getDeltaMovement().y) < -0.13D) {
         double horizontalReductionFactor = -0.05D / getOldDeltaY(entity.getDeltaMovement().y);
         entity.setDeltaMovement(new Vec3(deltaMovement.x * horizontalReductionFactor, getNewDeltaY(-0.05D), deltaMovement.z * horizontalReductionFactor));
      } else {
         entity.setDeltaMovement(new Vec3(deltaMovement.x, getNewDeltaY(-0.05D), deltaMovement.z));
      }
