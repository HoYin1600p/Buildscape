...
public class RecipeToast implements Toast {
   ...
   public void update(final ToastManager manager, final long fullyVisibleForMs) {
      ...
      if (this.recipeItems.isEmpty()) {
         ...
         this.wantedVisibility = (double)(fullyVisibleForMs - this.lastChanged) >= 5000.0D * manager.getNotificationDisplayTimeMultiplier() ? Toast.Visibility.HIDE : Toast.Visibility.SHOW;
      }

      this.displayedRecipeIndex = (int)((double)fullyVisibleForMs / Math.max(1.0D, 5000.0D * manager.getNotificationDisplayTimeMultiplier() / (double)this.recipeItems.size()) % (double)this.recipeItems.size());
   }
