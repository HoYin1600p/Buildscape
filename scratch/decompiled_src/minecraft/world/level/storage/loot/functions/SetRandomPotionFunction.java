...
public class SetRandomPotionFunction extends LootItemConditionalFunction {
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> commonFields(i).and(RegistryCodecs.holderSet(Registries.POTION).optionalFieldOf("options").forGetter((f) -> f.options)).apply(i, SetRandomPotionFunction::new));
   private final Optional options;

   private SetRandomPotionFunction(final Optional condition, final Optional options) {
      super(condition);
   ...
   public static LootItemConditionalFunction.Builder fromTagKey(final HolderSet tagKey) {
