...
public class IllusionerRenderer extends IllagerRenderer {
   ...
   public IllusionerRenderer(final EntityRendererProvider.Context context) {
      ...
      this.addLayer(new ItemInHandLayer(this, this) {
         {
   ...
   public void submit(final IllusionerRenderState state, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final CameraRenderState camera) {
      if (state.isInvisible) {
         ...
         for(int i = 0; i < offsets.length; ++i) {
            poseStack.pushPose();
            poseStack.translate(offsets[i].x + (double)Mth.cos((double)((float)i + state.ageInTicks * 0.5F)) * 0.025D, offsets[i].y + (double)Mth.cos((double)((float)i + state.ageInTicks * 0.75F)) * 0.0125D, offsets[i].z + (double)Mth.cos((double)((float)i + state.ageInTicks * 0.7F)) * 0.025D);
   ...
   protected AABB getBoundingBoxForCulling(final Illusioner entity, final float partialTicks) {
      return super.getBoundingBoxForCulling(entity, partialTicks).inflate(3.0D, 0.0D, 3.0D);
   }
}
