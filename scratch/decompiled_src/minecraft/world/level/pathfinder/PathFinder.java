...
public class PathFinder {
   ...
   public @Nullable Path findPath(final PathNavigationRegion level, final Mob entity, final Set targets, final float maxPathLength, final int reachRange, final float maxVisitedNodesMultiplier) {
      ...
      if (from == null) {
         ...
         Map tos = (Map)targets.stream().collect(Collectors.toMap((pos) -> this.nodeEvaluator.getTarget((double)pos.getX(), (double)pos.getY(), (double)pos.getZ()), Function.identity()));
   ...
   private @Nullable Path findPath(final Node from, final Map targetMap, final float maxPathLength, final int reachRange, final float maxVisitedNodesMultiplier) {
      ...
      Set closedSet = (Set<Node>)(captureDebug ? new HashSet() : Set.of());
      int count = 0;
      Set reachedTargets = Sets.newHashSetWithExpectedSize(targets.size());
      ...
      Optional optPath = !reachedTargets.isEmpty() ? reachedTargets.stream().map((target) -> this.reconstructPath(target.getBestNode(), (BlockPos)targetMap.get(target), true)).min(Comparator.comparingInt(Path::getNodeCount)) : targets.stream().map((target) -> this.reconstructPath(target.getBestNode(), (BlockPos)targetMap.get(target), false)).min(Comparator.comparingDouble(Path::getDistToTarget).thenComparingInt(Path::getNodeCount));
      profiler.pop();
      if (optPath.isEmpty()) {
         return null;
      } else {
