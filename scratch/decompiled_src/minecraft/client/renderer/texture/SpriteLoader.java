...
public class SpriteLoader {
   ...
   private SpriteLoader.Preparations stitch(final List sprites, final int maxMipmapLevels, final Executor executor) {
      try (Zone ignored = Profiler.get().zone((Supplier)(() -> "stitch " + String.valueOf(this.location)))) {
         ...
         Stitcher stitcher = new Stitcher(maxTextureSize, maxTextureSize, mipLevel, anisotropyBit);
         ...
         try {
            ...
            category.setDetail("Sprites", var21.getAllSprites().stream().map((s) -> String.format(Locale.ROOT, "%s[%dx%d]", s.name(), s.width(), s.height())).collect(Collectors.joining(",")));
         ...
         Map result = this.getStitchedSprites(stitcher, width, height);
         TextureAtlasSprite missingSprite = (TextureAtlasSprite)result.get(MissingTextureAtlasSprite.getLocation());
         CompletableFuture readyForUpload = CompletableFuture.runAsync(() -> result.values().forEach((s) -> s.contents().increaseMipLevel(mipLevel)), executor);
   ...
   private static CompletableFuture runSpriteSuppliers(final SpriteResourceLoader resourceLoader, final List sprites, final Executor executor) {
      List spriteFutures = sprites.stream().map((supplier) -> CompletableFuture.supplyAsync(() -> supplier.get(resourceLoader), executor)).toList();
