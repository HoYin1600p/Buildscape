...
public final class ShapedRecipePattern {
   ...
   public static final MapCodec MAP_CODEC = ShapedRecipePattern.Data.MAP_CODEC.flatXmap(ShapedRecipePattern::unpack, (pattern) -> (DataResult)pattern.data.map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Cannot encode unpacked recipe")));
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.VAR_INT, (e) -> e.width, ByteBufCodecs.VAR_INT, (e) -> e.height, Ingredient.OPTIONAL_CONTENTS_STREAM_CODEC.apply(ByteBufCodecs.list()), (e) -> e.ingredients, ShapedRecipePattern::createFromNetwork);
   ...
   private static DataResult unpack(final ShapedRecipePattern.Data data) {
      String[] shrunkPattern = shrink(data.pattern);
      int width = shrunkPattern[0].length();
      int height = shrunkPattern.length;
      List ingredients = new ArrayList(width * height);
      ...
      return !unusedSymbols.isEmpty() ? DataResult.error(() -> "Key defines symbols that aren't used in pattern: " + String.valueOf(unusedSymbols)) : DataResult.success(new ShapedRecipePattern(width, height, ingredients, Optional.of(data)));
   }
   ...
   public static record Data(Map key, List pattern) {
      private static final Codec PATTERN_CODEC = Codec.STRING.listOf().comapFlatMap((strings) -> {
         if (strings.size() > 3) {
      ...
      private static final Codec SYMBOL_CODEC = Codec.STRING.comapFlatMap((symbol) -> {
         if (symbol.length() != 1) {
      ...
      public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(ExtraCodecs.strictUnboundedMap(SYMBOL_CODEC, Ingredient.CODEC).fieldOf("key").forGetter((d) -> d.key), PATTERN_CODEC.fieldOf("pattern").forGetter((d) -> d.pattern)).apply(i, ShapedRecipePattern.Data::new));
   }
}
