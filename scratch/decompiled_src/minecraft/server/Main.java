...
public class Main {
   ...
   public static void main(final String[] args) {
      ...
      OptionSpec demo = parser.accepts("demo");
      OptionSpec bonusChest = parser.accepts("bonusChest");
      OptionSpec forceUpgrade = parser.accepts("forceUpgrade");
      OptionSpec eraseCache = parser.accepts("eraseCache");
      OptionSpec recreateRegionFiles = parser.accepts("recreateRegionFiles");
      OptionSpec safeMode = parser.accepts("safeMode", "Loads level with vanilla datapack only");
      OptionSpec help = parser.accepts("help").forHelp();
      OptionSpec universe = parser.accepts("universe").withRequiredArg().defaultsTo(".", new String[0]);
      OptionSpec worldName = parser.accepts("world").withRequiredArg();
      OptionSpec port = parser.accepts("port").withRequiredArg().ofType(Integer.class).defaultsTo(-1, new Integer[0]);
      OptionSpec serverId = parser.accepts("serverId").withRequiredArg();
      OptionSpec jfrProfilingOption = parser.accepts("jfrProfile");
      OptionSpec pidFile = parser.accepts("pidFile").withRequiredArg().withValuesConvertedBy(new PathConverter(new PathProperties[0]));
   ...
   private static WorldLoader.DataLoadOutput createNewWorldData(final DedicatedServerSettings settings, final WorldLoader.DataLoadContext context, final Registry datapackDimensions, final boolean demoMode, final boolean bonusChest) {
      ...
      if (demoMode) {
         ...
         createLevelSettings = new LevelSettings(properties.levelName, (GameType)properties.gameMode.get(), new LevelSettings.DifficultySettings((Difficulty)properties.difficulty.get(), properties.hardcore, false), false, context.dataConfiguration());
      ...
      return new WorldLoader.DataLoadOutput(new LevelDataAndDimensions.WorldDataAndGenSettings(primaryLevelData, new WorldGenSettings(worldOptions, dimensions)), finalDimensions.dimensionsRegistryAccess());
   }
