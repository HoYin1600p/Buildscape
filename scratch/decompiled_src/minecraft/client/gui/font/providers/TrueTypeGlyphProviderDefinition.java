...
public record TrueTypeGlyphProviderDefinition(Identifier location, float size, float oversample, TrueTypeGlyphProviderDefinition.Shift shift, String skip) implements GlyphProviderDefinition {
   private static final Codec SKIP_LIST_CODEC = Codec.withAlternative(Codec.STRING, Codec.STRING.listOf(), (list) -> String.join("", list));
      ...
      try {
         ...
         try {
            ...
            synchronized(FreeTypeUtil.LIBRARY_LOCK) {
               ...
               var22 = new TrueTypeGlyphProvider(fontData, face, this.size, this.oversample, this.shift.x, this.shift.y, this.skip);
            }
   ...
   public static record Shift(float x, float y) {
      ...
      public static final Codec CODEC = Codec.floatRange(-512.0F, 512.0F).listOf().comapFlatMap((input) -> Util.fixedSize(input, 2).map((floats) -> new TrueTypeGlyphProviderDefinition.Shift(floats.get(0), floats.get(1))), (shift) -> List.of(shift.x, shift.y));
   }
}
