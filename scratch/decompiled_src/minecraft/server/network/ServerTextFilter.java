...
public abstract class ServerTextFilter implements AutoCloseable {
   ...
   protected ServerTextFilter(final URL chatEndpoint, final ServerTextFilter.MessageEncoder chatEncoder, final ServerTextFilter.IgnoreStrategy chatIgnoreStrategy, final ExecutorService workerPool) {
   ...
   protected CompletableFuture requestMessageProcessing(final GameProfile sender, final String message, final ServerTextFilter.IgnoreStrategy ignoreStrategy, final Executor executor) {
   ...
   protected class PlayerContext implements TextFilter {
      ...
      public CompletableFuture processMessageBundle(final List messages) {
         List requests = (List)messages.stream().map((message) -> ServerTextFilter.this.requestMessageProcessing(this.profile, message, ServerTextFilter.this.chatIgnoreStrategy, this.streamExecutor)).collect(ImmutableList.toImmutableList());
