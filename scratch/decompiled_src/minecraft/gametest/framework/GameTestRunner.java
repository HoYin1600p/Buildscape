...
public class GameTestRunner {
   ...
   protected GameTestRunner(final GameTestRunner.GameTestBatcher batcher, final Collection batches, final MinecraftServer server, final GameTestTicker testTicker, final GameTestRunner.StructureSpawner existingStructureSpawner, final GameTestRunner.StructureSpawner newStructureSpawner, final boolean haltOnError, final boolean clearBetweenBatches) {
      this.server = server;
   ...
   private void runBatch(final int batchIndex) {
      if (batchIndex >= this.batches.size()) {
         ...
         Collection testInfosForThisBatch = this.createStructuresForBatch(currentBatch.gameTestInfos());
   ...
   public static class Builder {
      ...
      private Builder(final Collection batches, final MinecraftServer server) {
         this.batches = batches;
         this.server = server;
      }

      public static GameTestRunner.Builder fromBatches(final Collection batches, final MinecraftServer server) {
         return new GameTestRunner.Builder(batches, server);
      }

      public static GameTestRunner.Builder fromInfo(final Collection tests, final MinecraftServer server) {
