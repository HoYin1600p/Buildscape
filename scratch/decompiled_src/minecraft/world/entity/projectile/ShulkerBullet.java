...
public class ShulkerBullet extends Projectile {
   ...
   private void selectNextMoveDirection(final Direction.@Nullable Axis avoidAxis, final @Nullable Entity target) {
      ...
      if (target == null) {
         ...
         yOffset = (double)target.getBbHeight() * 0.5D;
      ...
      if (!targetPos.closerToCenterThan(this.position(), 2.0D)) {
      ...
      if (distance == 0.0D) {
         ...
         this.targetDeltaX = xa / distance * 0.15D;
         this.targetDeltaY = ya / distance * 0.15D;
         this.targetDeltaZ = za / distance * 0.15D;
      }
   ...
   public void tick() {
      ...
      if (!this.level().isClientSide()) {
         ...
         if (finalTarget == null || !finalTarget.isAlive() || finalTarget instanceof Player && finalTarget.isSpectator()) {
            ...
            this.setDeltaMovement(movement.add((this.targetDeltaX - movement.x) * 0.2D, (this.targetDeltaY - movement.y) * 0.2D, (this.targetDeltaZ - movement.z) * 0.2D));
         }
      ...
      if (this.level().isClientSide()) {
         this.level().addParticle(ParticleTypes.END_ROD, this.getX() - movement.x, this.getY() - movement.y + 0.15D, this.getZ() - movement.z, 0.0D, 0.0D, 0.0D);
   ...
   protected void onRedirectProjectile(final Projectile hitProjectile) {
      hitProjectile.deflect(ProjectileDeflection.MOMENTUM_DEFLECT, this, this.owner, true, 1.0D);
   }
