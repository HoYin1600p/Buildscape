...
public class PathfindToRaidGoal extends Goal {
   ...
   private void recruitNearby(final Raid raid) {
      if (raid.isActive()) {
         ...
         Set raidersToAdd = Sets.newHashSet();
         List raidersNearby = level.getEntitiesOfClass(Raider.class, this.mob.getBoundingBox().inflate(16.0D), (mob) -> !mob.hasActiveRaid() && Raids.canJoinRaid(mob));
