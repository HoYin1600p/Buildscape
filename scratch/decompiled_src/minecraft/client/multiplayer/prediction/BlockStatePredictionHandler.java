...
public class BlockStatePredictionHandler implements AutoCloseable {
   ...
   public void retainKnownServerState(final BlockPos pos, final BlockState state, final LocalPlayer player) {
      this.serverVerifiedStates.compute(pos.asLong(), (key, serverVerifiedState) -> serverVerifiedState != null ? serverVerifiedState.setSequence(this.currentSequenceNr) : new BlockStatePredictionHandler.ServerVerifiedState(this.currentSequenceNr, state, player.position()));
   }

   public boolean updateKnownServerState(final BlockPos pos, final BlockState blockState) {
      BlockStatePredictionHandler.ServerVerifiedState serverVerifiedState = (BlockStatePredictionHandler.ServerVerifiedState)this.serverVerifiedStates.get(pos.asLong());
