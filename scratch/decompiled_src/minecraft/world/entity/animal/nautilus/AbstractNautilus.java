...
public abstract class AbstractNautilus extends TamableAnimal implements PlayerRideableJumping, HasCustomInventoryScreen {
   ...
   private static final EntityDataAccessor DASH = SynchedEntityData.defineId(AbstractNautilus.class, EntityDataSerializers.BOOLEAN);
   ...
   private void spawnBubbles() {
      ...
      double bubbleProbability = Mth.clamp(speed * 2.0D, (double)0.15F, 1.0D);
      if ((double)this.random.nextFloat() < bubbleProbability) {
         ...
         double spread = this.random.nextDouble() * 0.8D * (1.0D + speed);
         double dx = ((double)this.random.nextFloat() - 0.5D) * spread;
         double dy = ((double)this.random.nextFloat() - 0.5D) * spread;
         double dz = ((double)this.random.nextFloat() - 0.5D) * spread;
         this.level().addParticle(ParticleTypes.BUBBLE, this.getX() - mouthDirectionVector.x * 1.1D, this.getY() - mouthDirectionVector.y + 0.25D, this.getZ() - mouthDirectionVector.z * 1.1D, dx, dy, dz);
      }
