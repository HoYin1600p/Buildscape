...
public class DataCommands {
   ...
   public static final List ALL_PROVIDERS = List.of(EntityDataAccessor.PROVIDER, BlockDataAccessor.PROVIDER, StorageDataAccessor.PROVIDER);
   ...
   public static void register(final CommandDispatcher dispatcher) {
      ...
      for(ArgProvider targetProvider : TARGET_PROVIDERS) {
         ((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)root.then(targetProvider.wrap(Commands.literal("merge"), (p) -> p.then(Commands.argument("nbt", CompoundTagArgument.compoundTag()).executes((c) -> mergeData((CommandSourceStack)c.getSource(), (DataAccessor)targetProvider.access(c), CompoundTagArgument.getCompoundTag(c, "nbt"))))))).then(targetProvider.wrap(Commands.literal("get"), (p) -> p.executes((c) -> getData((CommandSourceStack)c.getSource(), (DataAccessor)targetProvider.access(c))).then(((RequiredArgumentBuilder)Commands.argument("path", NbtPathArgument.nbtPath()).executes((c) -> getData((CommandSourceStack)c.getSource(), (DataAccessor)targetProvider.access(c), NbtPathArgument.getPath(c, "path")))).then(Commands.argument("scale", DoubleArgumentType.doubleArg()).executes((c) -> getNumeric((CommandSourceStack)c.getSource(), (DataAccessor)targetProvider.access(c), NbtPathArgument.getPath(c, "path"), DoubleArgumentType.getDouble(c, "scale")))))))).then(targetProvider.wrap(Commands.literal("remove"), (p) -> p.then(Commands.argument("path", NbtPathArgument.nbtPath()).executes((c) -> removeData((CommandSourceStack)c.getSource(), (DataAccessor)targetProvider.access(c), NbtPathArgument.getPath(c, "path"))))))).then(decorateModification((parent, rest) -> parent.then(Commands.literal("insert").then(Commands.argument("index", IntegerArgumentType.integer()).then(rest.create((context, target, targetPath, source) -> targetPath.insert(IntegerArgumentType.getInteger(context, "index"), target, source))))).then(Commands.literal("prepend").then(rest.create((context, target, targetPath, source) -> targetPath.insert(0, target, source)))).then(Commands.literal("append").then(rest.create((context, target, targetPath, source) -> targetPath.insert(-1, target, source)))).then(Commands.literal("set").then(rest.create((context, target, targetPath, source) -> targetPath.set(target, (Tag)Iterables.getLast(source))))).then(Commands.literal("merge").then(rest.create((context, target, targetPath, source) -> {
               ...
               Collection targets = targetPath.getOrCreate(target, CompoundTag::new);
               int changedCount = 0;
   ...
   private static List stringifyTagList(final List source, final DataCommands.StringProcessor stringProcessor) throws CommandSyntaxException {
   ...
   private static List getSingletonSource(final CommandContext context, final ArgProvider sourceProvider) throws CommandSyntaxException {
      DataAccessor source = (DataAccessor)sourceProvider.access(context);
      return Collections.singletonList(source.getData());
   }

   private static List resolveSourcePath(final CommandContext context, final ArgProvider sourceProvider) throws CommandSyntaxException {
      DataAccessor source = (DataAccessor)sourceProvider.access(context);
      NbtPathArgument.NbtPath sourcePath = NbtPathArgument.getPath(context, "sourcePath");
      return sourcePath.get(source.getData());
   }

   private static int manipulateData(final CommandContext context, final ArgProvider targetProvider, final DataCommands.DataManipulator manipulator, final List source) throws CommandSyntaxException {
      DataAccessor target = (DataAccessor)targetProvider.access(context);
   ...
   public static Tag getSingleTag(final NbtPathArgument.NbtPath path, final DataAccessor accessor) throws CommandSyntaxException {
      Collection tags = path.get(accessor.getData());
   ...
   private interface DataManipulatorDecorator {
      ArgumentBuilder create(DataCommands.DataManipulator manipulator);
   }
