...
public record EnchantmentsByCost(HolderSet enchantments, IntProvider cost) implements EnchantmentProvider {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(RegistryCodecs.holderSet(Registries.ENCHANTMENT).fieldOf("enchantments").forGetter(EnchantmentsByCost::enchantments), IntProviders.CODEC.fieldOf("cost").forGetter(EnchantmentsByCost::cost)).apply(i, EnchantmentsByCost::new));
