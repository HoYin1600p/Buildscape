...
public class Sheets {
   ...
   public static final List SHULKER_TEXTURE_LOCATION = (List)Arrays.stream(DyeColor.values()).sorted(Comparator.comparingInt(DyeColor::getId)).map(Sheets::createShulkerSprite).collect(ImmutableList.toImmutableList());
   ...
   private static final Map BANNER_SPRITES = new HashMap();
   private static final Map SHIELD_SPRITES = new HashMap();
   ...
   public static final MultiblockChestResources CHEST_REGULAR = ChestSpecialRenderer.REGULAR.map(CHEST_MAPPER::apply);
   public static final MultiblockChestResources CHEST_TRAPPED = ChestSpecialRenderer.TRAPPED.map(CHEST_MAPPER::apply);
   public static final MultiblockChestResources CHEST_CHRISTMAS = ChestSpecialRenderer.CHRISTMAS.map(CHEST_MAPPER::apply);
   public static final WeatheringCopperCollection.ByState CHEST_COPPER = ChestSpecialRenderer.COPPER.map((r) -> r.map(CHEST_MAPPER::apply));
   ...
   public static SpriteId getBannerSprite(final Holder pattern) {
      return (SpriteId)BANNER_SPRITES.computeIfAbsent(((BannerPattern)pattern.value()).assetId(), BANNER_MAPPER::apply);
   }

   public static SpriteId getShieldSprite(final Holder pattern) {
   ...
   public static SpriteId chooseSprite(final ChestRenderState.ChestMaterialType materialType, final ChestType type) {
      ...
      switch (materialType) {
         case ENDER_CHEST:
            ...
            break;
         case REGULAR:
            var10000 = (SpriteId)CHEST_REGULAR.select(type);
            break;
         case CHRISTMAS:
            var10000 = (SpriteId)CHEST_CHRISTMAS.select(type);
            break;
         case TRAPPED:
            var10000 = (SpriteId)CHEST_TRAPPED.select(type);
            break;
         case COPPER_UNAFFECTED:
            var10000 = (SpriteId)((MultiblockChestResources)CHEST_COPPER.unaffected()).select(type);
            break;
         case COPPER_EXPOSED:
            var10000 = (SpriteId)((MultiblockChestResources)CHEST_COPPER.exposed()).select(type);
            break;
         case COPPER_WEATHERED:
            var10000 = (SpriteId)((MultiblockChestResources)CHEST_COPPER.weathered()).select(type);
            break;
         case COPPER_OXIDIZED:
            var10000 = (SpriteId)((MultiblockChestResources)CHEST_COPPER.oxidized()).select(type);
            break;
         default:
