...
public class AcquirePoi {
   ...
   public static BehaviorControl create(final Predicate poiType, final MemoryModuleType memoryToValidate, final MemoryModuleType memoryToAcquire, final boolean onlyIfAdult, final Optional onPoiAcquisitionEvent, final BiPredicate validPoi) {
      ...
      Long2ObjectMap batchCache = new Long2ObjectOpenHashMap();
      OneShot acquirePoi = BehaviorBuilder.create((i) -> i.group(i.absent(memoryToAcquire)).apply(i, (toAcquire) -> (level, body, timestamp) -> {
               if (onlyIfAdult && body.isBaby()) {
                  ...
                  if (nextScheduledStart.longValue() == 0L) {
                     ...
                     Set poiPositions = (Set)poiManager.findAllClosestFirstWithType(poiType, cacheTest, body.blockPosition(), 48, PoiManager.Occupancy.HAS_SPACE).limit(5L).filter((px) -> validPoi.test(level, (BlockPos)px.getSecond())).collect(Collectors.toSet());
