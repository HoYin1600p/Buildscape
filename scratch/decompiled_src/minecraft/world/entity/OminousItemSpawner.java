...
public class OminousItemSpawner extends Entity {
   ...
   private static final EntityDataAccessor DATA_ITEM = SynchedEntityData.defineId(OminousItemSpawner.class, EntityDataSerializers.ITEM_STACK);
   ...
   private void spawnItem() {
      ...
      if (item instanceof ServerLevel level) {
   ...
   public void addParticles() {
      ...
      for(int i = 0; i < particleCount; ++i) {
         double radius = 0.4D;
         Vec3 flyFrom = new Vec3(this.getX() + 0.4D * (this.random.nextGaussian() - this.random.nextGaussian()), this.getY() + 0.4D * (this.random.nextGaussian() - this.random.nextGaussian()), this.getZ() + 0.4D * (this.random.nextGaussian() - this.random.nextGaussian()));
