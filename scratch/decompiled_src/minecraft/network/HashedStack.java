...
public interface HashedStack {
   ...
   static HashedStack create(final ItemStack itemStack, final HashedPatchMap.HashGenerator hasher) {
      return (HashedStack)(itemStack.isEmpty() ? EMPTY : new HashedStack.ActualItem(itemStack.typeHolder(), itemStack.getCount(), HashedPatchMap.create(itemStack.getComponentsPatch(), hasher)));
   }

   public static record ActualItem(Holder item, int count, HashedPatchMap components) implements HashedStack {
      public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.holderRegistry(Registries.ITEM), HashedStack.ActualItem::item, ByteBufCodecs.VAR_INT, HashedStack.ActualItem::count, HashedPatchMap.STREAM_CODEC, HashedStack.ActualItem::components, HashedStack.ActualItem::new);
