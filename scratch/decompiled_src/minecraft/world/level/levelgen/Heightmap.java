...
public class Heightmap {
   ...
   private static final Predicate MATERIAL_MOTION_BLOCKING = (state) -> state.is(BlockTags.BLOCKS_MOTION_IN_HEIGHTMAP);
   ...
   public static void primeHeightmaps(final ChunkAccess chunk, final Set types) {
      if (!types.isEmpty()) {
         int size = types.size();
         List allHeightmaps = new ObjectArrayList(size);

         for(Heightmap.Types type : types) {
            allHeightmaps.add(chunk.getOrCreateHeightmapUnprimed(type));
         }

         List remainingHeightmaps = new ObjectArrayList(size);
   ...
   public static enum Types implements StringRepresentable {
      ...
      public static final Codec CODEC = StringRepresentable.fromEnum(Heightmap.Types::values);
      ...
      private Types(final int id, final String serializationKey, final Heightmap.Usage usage, final Predicate isOpaque) {
         this.id = id;
