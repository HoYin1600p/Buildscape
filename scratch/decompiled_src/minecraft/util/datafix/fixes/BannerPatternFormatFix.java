...
public class BannerPatternFormatFix extends NamedEntityFix {
   ...
   private static Dynamic fixLayer(Dynamic dynamic) {
      dynamic = dynamic.renameAndFixField("Pattern", "pattern", (pattern) -> (Dynamic)DataFixUtils.orElse(pattern.asString().map((id) -> (String)PATTERN_ID_MAP.getOrDefault(id, id)).map(pattern::createString).result(), pattern));
