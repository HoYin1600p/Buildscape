...
public record PositionMoveRotation(Vec3 position, Vec3 deltaMovement, float yRot, float xRot) {
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(Vec3.STREAM_CODEC, PositionMoveRotation::position, Vec3.STREAM_CODEC, PositionMoveRotation::deltaMovement, ByteBufCodecs.FLOAT, PositionMoveRotation::yRot, ByteBufCodecs.FLOAT, PositionMoveRotation::xRot, PositionMoveRotation::new);
   ...
   public static PositionMoveRotation calculateAbsolute(final PositionMoveRotation source, final PositionMoveRotation change, final Set relatives) {
      double offsetX = relatives.contains(Relative.X) ? source.position.x : 0.0D;
      double offsetY = relatives.contains(Relative.Y) ? source.position.y : 0.0D;
      double offsetZ = relatives.contains(Relative.Z) ? source.position.z : 0.0D;
