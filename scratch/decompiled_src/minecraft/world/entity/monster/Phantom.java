...
public class Phantom extends Mob implements Enemy {
   ...
   public void tick() {
      ...
      if (this.level().isClientSide()) {
         ...
         this.level().addParticle(ParticleTypes.MYCELIUM, this.getX() + (double)c, this.getY() + (double)h, this.getZ() + (double)s, 0.0D, 0.0D, 0.0D);
         this.level().addParticle(ParticleTypes.MYCELIUM, this.getX() - (double)c, this.getY() + (double)h, this.getZ() - (double)s, 0.0D, 0.0D, 0.0D);
      }
   ...
   private class PhantomAttackPlayerTargetGoal extends Goal {
      ...
      public boolean canUse() {
         if (this.nextScanTick > 0) {
            ...
            List players = level.getNearbyPlayers(this.attackTargeting, Phantom.this, Phantom.this.getBoundingBox().inflate(16.0D, 64.0D, 16.0D));
            if (!players.isEmpty()) {
   ...
   private class PhantomMoveControl extends MoveControl {
      ...
      public void tick() {
         ...
         if (Math.abs(sd) > (double)1.0E-5F) {
            double yRelativeScale = 1.0D - Math.abs(tdy * (double)0.7F) / sd;
            tdx *= yRelativeScale;
            tdz *= yRelativeScale;
            ...
            Phantom.this.setDeltaMovement(movement.add((new Vec3(txd, tyd, tzd)).subtract(movement).scale(0.2D)));
         }
         ...
         if (target == null) {
            ...
            if (!this.canUse()) {
               ...
               if (Phantom.this.tickCount > this.catSearchTick) {
                  ...
                  List cats = Phantom.this.level().getEntitiesOfClass(Cat.class, Phantom.this.getBoundingBox().inflate(16.0D), EntitySelector.ENTITY_STILL_ALIVE);
