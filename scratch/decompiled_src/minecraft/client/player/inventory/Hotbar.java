...
public class Hotbar {
   ...
   public static final Codec CODEC = Codec.PASSTHROUGH.listOf().validate((list) -> Util.fixedSize(list, SIZE)).xmap(Hotbar::new, (hotbar) -> hotbar.items);
   private static final DynamicOps DEFAULT_OPS = NbtOps.INSTANCE;
   ...
   public void storeFrom(final Inventory inventory, final RegistryAccess lookupProvider) {
      ...
      for(int i = 0; i < SIZE; ++i) {
         ...
         Optional result = ItemStack.OPTIONAL_CODEC.encodeStart(registryOps, item).resultOrPartial((error) -> LOGGER.warn("Could not encode hotbar item: {}", error)).map((tag) -> new Dynamic(DEFAULT_OPS, tag));
