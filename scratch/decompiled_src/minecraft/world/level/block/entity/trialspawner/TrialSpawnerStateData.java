...
public class TrialSpawnerStateData {
   ...
   public void tryDetectPlayers(final ServerLevel level, final BlockPos pos, final TrialSpawner trialSpawner) {
      ...
      if (!isThrottled) {
         if (!trialSpawner.getState().equals(TrialSpawnerState.COOLDOWN) || !trialSpawner.isOminous()) {
            List inLineOfSightPlayers = trialSpawner.getPlayerDetector().detect(level, trialSpawner.getEntitySelector(), pos, (double)trialSpawner.getRequiredPlayerRange(), true);
            boolean becameOminous;
            if (!trialSpawner.isOminous() && !inLineOfSightPlayers.isEmpty()) {
               Optional playerWithOminousEffect = findPlayerWithOminousEffect(level, inLineOfSightPlayers);
            ...
            if (!trialSpawner.getState().equals(TrialSpawnerState.COOLDOWN) || becameOminous) {
               ...
               List foundPlayers = isSearchingForFirstPlayer ? inLineOfSightPlayers : trialSpawner.getPlayerDetector().detect(level, trialSpawner.getEntitySelector(), pos, (double)trialSpawner.getRequiredPlayerRange(), false);
   ...
   private static Optional findPlayerWithOminousEffect(final ServerLevel level, final List inLineOfSightPlayers) {
      ...
      for(UUID playerUuid : inLineOfSightPlayers) {
         ...
         if (player != null) {
            Holder trialOmen = MobEffects.TRIAL_OMEN;
   ...
   protected SpawnData getOrCreateNextSpawnData(final TrialSpawner trialSpawner, final RandomSource random) {
      if (this.nextSpawnData.isPresent()) {
         ...
         WeightedList spawnPotentials = trialSpawner.activeConfig().spawnPotentialsDefinition();
         Optional selected = spawnPotentials.isEmpty() ? this.nextSpawnData : spawnPotentials.getRandom(random);
   ...
   public WeightedList getDispensingItems(final ServerLevel level, final TrialSpawnerConfig config, final BlockPos pos) {
      if (this.dispensing != null) {
         ...
         ObjectArrayList lootDrops = lootTable.getRandomItems(params, simplePositionalSeed);
   ...
   public static record Packed(Set detectedPlayers, Set currentMobs, long cooldownEndsAt, long nextMobSpawnsAt, int totalMobsSpawned, Optional nextSpawnData, Optional ejectingLootTable) {
      public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(UUIDUtil.CODEC_SET.lenientOptionalFieldOf("registered_players", Set.of()).forGetter(TrialSpawnerStateData.Packed::detectedPlayers), UUIDUtil.CODEC_SET.lenientOptionalFieldOf("current_mobs", Set.of()).forGetter(TrialSpawnerStateData.Packed::currentMobs), Codec.LONG.lenientOptionalFieldOf("cooldown_ends_at", 0L).forGetter(TrialSpawnerStateData.Packed::cooldownEndsAt), Codec.LONG.lenientOptionalFieldOf("next_mob_spawns_at", 0L).forGetter(TrialSpawnerStateData.Packed::nextMobSpawnsAt), Codec.intRange(0, Integer.MAX_VALUE).lenientOptionalFieldOf("total_mobs_spawned", 0).forGetter(TrialSpawnerStateData.Packed::totalMobsSpawned), SpawnData.CODEC.lenientOptionalFieldOf("spawn_data").forGetter(TrialSpawnerStateData.Packed::nextSpawnData), LootTable.KEY_CODEC.lenientOptionalFieldOf("ejecting_loot_table").forGetter(TrialSpawnerStateData.Packed::ejectingLootTable)).apply(i, TrialSpawnerStateData.Packed::new));
   }
}
