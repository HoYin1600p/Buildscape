...
public class PortalShape {
   ...
   public static Optional findPortalShape(final LevelAccessor level, final BlockPos pos, final Predicate isValid, final Direction.Axis preferredAxis) {
      Optional firstAxis = Optional.of(findAnyShape(level, pos, preferredAxis)).filter(isValid);
   ...
   public static Vec3 getRelativePosition(final BlockUtil.FoundRectangle largestRectangleAround, final Direction.Axis axis, final Vec3 position, final EntityDimensions dimensions) {
      ...
      if (width > 0.0D) {
         double bottomStart = (double)bottomMin.get(axis) + (double)dimensions.width() / 2.0D;
         relativeRight = Mth.clamp(Mth.inverseLerp(position.get(axis) - bottomStart, 0.0D, width), 0.0D, 1.0D);
      } else {
         relativeRight = 0.5D;
      }
      ...
      if (height > 0.0D) {
         ...
         relativeUp = Mth.clamp(Mth.inverseLerp(position.get(heightAxis) - (double)bottomMin.get(heightAxis), 0.0D, height), 0.0D, 1.0D);
      } else {
         relativeUp = 0.0D;
      }

      Direction.Axis forwardAxis = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
      double relativeForward = position.get(forwardAxis) - ((double)bottomMin.get(forwardAxis) + 0.5D);
   ...
   public static Vec3 findCollisionFreePosition(final Vec3 bottomCenter, final ServerLevel serverLevel, final Entity entity, final EntityDimensions dimensions) {
      if (!(dimensions.width() > 4.0F) && !(dimensions.height() > 4.0F)) {
         ...
         Vec3 center = bottomCenter.add(0.0D, halfHeight, 0.0D);
         VoxelShape allowedCenters = Shapes.create(AABB.ofSize(center, (double)dimensions.width(), 0.0D, (double)dimensions.width()).expandTowards(0.0D, 1.0D, 0.0D).inflate(1.0E-6D));
         Optional collisionFreePosition = serverLevel.findFreePosition(entity, allowedCenters, center, (double)dimensions.width(), (double)dimensions.height(), (double)dimensions.width());
         Optional collisionFreeBottomCenter = collisionFreePosition.map((vec) -> vec.subtract(0.0D, halfHeight, 0.0D));
