...
public class SplashManager extends SimplePreparableReloadListener {
   ...
   public @Nullable SplashRenderer getSplash() {
      ...
      if (monthDay.equals(SpecialDates.CHRISTMAS)) {
         ...
         return this.user != null && RANDOM.nextInt(this.splashes.size()) == 42 ? new SplashRenderer(literalSplash(this.user.getName().toUpperCase(Locale.ROOT) + " IS YOU")) : new SplashRenderer((Component)this.splashes.get(RANDOM.nextInt(this.splashes.size())));
      }
   }
}
