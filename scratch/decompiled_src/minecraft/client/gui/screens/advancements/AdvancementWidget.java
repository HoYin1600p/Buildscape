...
public class AdvancementWidget {
   ...
   private static float getMaxWidth(final StringSplitter splitter, final List input) {
      return (float)input.stream().mapToDouble(splitter::stringWidth).max().orElse(0.0D);
   }
   ...
   private List findOptimalLines(final Component input, final int preferredWidth) {
      ...
      for(int testMargin : TEST_SPLIT_OFFSETS) {
         List testSplit = splitter.splitLines(input, preferredWidth - testMargin, Style.EMPTY);
   ...
   private void extractMultilineText(final GuiGraphicsExtractor graphics, final List lines, final int x, final int y, final int color) {
      Font font = this.minecraft.font;
