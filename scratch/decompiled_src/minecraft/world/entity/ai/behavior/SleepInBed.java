   ...
   protected boolean checkExtraStartConditions(final ServerLevel level, final LivingEntity body) {
      if (body.isPassenger()) {
         ...
         if (level.dimension() != target.dimension()) {
            ...
            Optional lastWokenMemory = brain.getMemory(MemoryModuleType.LAST_WOKEN);
            ...
            return target.pos().closerToCenterThan(body.position(), 2.0D) && blockState.is(BlockTags.VILLAGERS_CAN_SLEEP_ON_BED) && !blockState.getValue(AbstractBedBlock.OCCUPIED);
         }
      }
   }

   protected boolean canStillUse(final ServerLevel level, final LivingEntity body, final long timestamp) {
      Optional memory = body.getBrain().getMemory(MemoryModuleType.HOME);
      if (memory.isEmpty()) {
         return false;
      } else {
         BlockPos bedPos = ((GlobalPos)memory.get()).pos();
         return body.getBrain().isActive(Activity.REST) && body.getY() > (double)bedPos.getY() + 0.4D && bedPos.closerToCenterThan(body.position(), 1.14D);
      }
   }
   ...
   protected void start(final ServerLevel level, final LivingEntity body, final long timestamp) {
      if (timestamp > this.nextOkStartTime) {
         ...
         if (brain.hasMemoryValue(MemoryModuleType.DOORS_TO_CLOSE)) {
            Set doors = (Set)brain.getMemory(MemoryModuleType.DOORS_TO_CLOSE).get();
            Optional nearestEntities;
