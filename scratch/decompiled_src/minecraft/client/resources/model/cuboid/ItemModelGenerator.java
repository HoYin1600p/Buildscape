...
public class ItemModelGenerator implements UnbakedModel {
   ...
   private static QuadCollection bake(final TextureSlots textureSlots, final ModelBaker modelBaker, final ModelState modelState, final ModelDebugName name) {
      ...
      for(int layerIndex = 0; layerIndex < LAYERS.size(); ++layerIndex) {
         ...
         QuadCollection bakedLayer = (QuadCollection)modelBaker.compute(new ItemModelGenerator.ItemLayerKey(bakedMaterial, modelState, layerIndex));
         if (builder != null) {
   ...
   private static void checkTransition(final ItemModelGenerator.SideDirection facing, final Set sideFaces, final SpriteContents sprite, final int frame, final int x, final int y, final int width, final int height) {
   ...
   private static record ItemLayerKey(Material.Baked material, ModelState modelState, int layerIndex) implements ModelBaker.SharedOperationKey {
