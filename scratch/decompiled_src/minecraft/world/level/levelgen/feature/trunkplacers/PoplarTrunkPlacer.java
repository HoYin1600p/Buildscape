...
public class PoplarTrunkPlacer extends TrunkPlacer {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> trunkPlacerParts(i).and(i.group(IntProviders.codec(0, 8).fieldOf("trunk_height_above_branches").forGetter((t) -> t.trunkHeightAboveBranches), IntProviders.codec(1, 4).fieldOf("branch_amount").forGetter((t) -> t.branchAmount))).apply(i, PoplarTrunkPlacer::new));
   ...
   public List placeTrunk(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int treeHeight, final BlockPos origin, final TreeFeature tree) {
      ...
      for(int y = 0; y < treeHeight; ++y) {
         ...
         List directions = getShuffledBranchDirections(random);
   ...
   private static Function getSidewaysStateModifier(final Direction branchDirection) {
      return (state) -> (BlockState)state.trySetValue(RotatedPillarBlock.AXIS, branchDirection.getAxis());
   }

   private static List getShuffledBranchDirections(final RandomSource random) {
