...
public record ServerStatus(Component description, Optional players, Optional version, Optional favicon, boolean enforcesSecureChat) {
   ...
   public static record Players(int max, int online, List sample) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.INT.fieldOf("max").forGetter(ServerStatus.Players::max), Codec.INT.fieldOf("online").forGetter(ServerStatus.Players::online), NameAndId.CODEC.listOf().lenientOptionalFieldOf("sample", List.of()).forGetter(ServerStatus.Players::sample)).apply(i, ServerStatus.Players::new));
   }

   public static record Version(String name, int protocol) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.STRING.fieldOf("name").forGetter(ServerStatus.Version::name), Codec.INT.fieldOf("protocol").forGetter(ServerStatus.Version::protocol)).apply(i, ServerStatus.Version::new));
