         ...
         if (this.onlyAtNight && this.mob.level().isBrightOutside()) {
            ...
            if (!level.isCloseToVillage(pos, 6)) {
               ...
               if (landPos == null) {
                  ...
                  Optional target = level.getPoiManager().find((e) -> e.is(PoiTypeTags.VILLAGE), this::hasNotVisited, BlockPos.containing(landPos), 10, PoiManager.Occupancy.IS_OCCUPIED);
                  if (target.isEmpty()) {
                     return false;
                  } else {
