...
public class NoxiousGasCloudParticle extends NoRenderParticle {
   ...
   private static Vec3 pickRandomParticleSpawnPoint(final Level level, final BlockPos centerBlock) {
      ...
      Vec3 horizontalDirection = (new Vec3((double)(random.nextFloat() - 0.5F), 0.0D, (double)(random.nextFloat() - 0.5F))).normalize();
      float distance = random.nextFloat() * 3.0F;
      return Vec3.atCenterOf(centerBlock).add(horizontalDirection.scale((double)distance)).subtract(0.0D, 0.25D, 0.0D);
   }

   private static void spawnNoxiousGasParticle(final Level level, final Vec3 pos) {
      level.addAlwaysVisibleParticle(ParticleTypes.NOXIOUS_GAS, pos.x, pos.y, pos.z, 0.0D, 0.0D, 0.0D);
   }
