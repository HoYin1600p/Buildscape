...
public class ExplorerMapItemFix extends DataFix {
   ...
   private static Dynamic removeLegacyItemName(final Dynamic components, final String legacyNameKey) {
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder idFinder = DSL.fieldFinder("id", DSL.named(References.ITEM_NAME.typeName(), NamespacedSchema.namespacedString()));
      ...
      return this.fixTypeEverywhereTyped("ExplorerMapItemFix", itemStackType, (itemStack) -> {
         Optional id = itemStack.getOptional(idFinder).map(Pair::getSecond);
         if (id.filter((itemId) -> itemId.equals("minecraft:filled_map")).isEmpty()) {
            ...
            if (components.isEmpty()) {
               ...
               if (mapDecorations.isEmpty()) {
                  ...
                  Optional explorerDecoration = ((Dynamic)mapDecorations.get()).get("+").result();
