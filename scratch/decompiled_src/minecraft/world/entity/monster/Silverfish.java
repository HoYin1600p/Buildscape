...
public class Silverfish extends Monster {
   ...
   public static boolean checkSilverfishSpawnRules(final EntityType type, final LevelAccessor level, final EntitySpawnReason spawnReason, final BlockPos pos, final RandomSource random) {
      if (!checkAnyLightMonsterSpawnRules(type, level, spawnReason, pos, random)) {
         ...
         Player nearestPlayer = level.getNearestPlayer((double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D, 5.0D, true);
      ...
      public boolean canUse() {
         if (this.mob.getTarget() != null) {
            ...
            if (getServerLevel(this.mob).getGameRules().get(GameRules.MOB_GRIEFING) && random.nextInt(reducedTickDelay(10)) == 0) {
               ...
               BlockPos pos = BlockPos.containing(this.mob.getX(), this.mob.getY() + 0.5D, this.mob.getZ()).relative(this.selectedDirection);
