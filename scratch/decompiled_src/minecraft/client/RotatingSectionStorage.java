...
public class RotatingSectionStorage implements Iterable {
   private final RotatingSectionStorage.Node[] nodes;
   ...
   public RotatingSectionStorage(final int radius, final int minY, final int maxY, final RotatingSectionStorage.ValueCreator valueCreator) {
      this.radius = radius;
      this.minY = minY;
      this.maxY = maxY;
         ...
         for(int gridX = 0; gridX < this.sectionGridSizeXZ; ++gridX) {
            ...
            for(int gridZ = 0; gridZ < this.sectionGridSizeXZ; ++gridZ) {
               ...
               for(int gridY = 0; gridY < this.sectionGridSizeY; ++gridY) {
                  ...
                  RotatingSectionStorage.Value value = (T)((RotatingSectionStorage.Value)this.nodes[this.getSectionIndex(gridX, gridY, gridZ)].value);
   ...
   public RotatingSectionStorage.@Nullable Value getValueAt(final BlockPos pos) {
      return this.getValue(SectionPos.asLong(pos));
   }

   public RotatingSectionStorage.@Nullable Value getValue(final long sectionNode) {
   ...
   public RotatingSectionStorage.@Nullable Value getValue(final int sectionX, final int sectionY, final int sectionZ) {
      if (!this.containsSection(sectionX, sectionY, sectionZ)) {
         ...
         return (RotatingSectionStorage.Value)this.nodes[this.getSectionIndex(x, y, z)].value;
      }
   }
   ...
   public Iterator iterator() {
      return new Iterator() {
         ...
         public RotatingSectionStorage.Value next() {
            if (this.i >= RotatingSectionStorage.this.nodes.length) {
               throw new NoSuchElementException();
            } else {
               return (RotatingSectionStorage.Value)RotatingSectionStorage.this.nodes[this.i++].value;
            }
         }
      };
   }
   ...
   public void forEach(final Consumer action) {
      for(RotatingSectionStorage.Node node : this.nodes) {
   ...
   private static record Node(Object value) {
   }
   ...
   public interface ValueCreator {
      RotatingSectionStorage.Value createValue(int index, long sectionNode);
   }
}
