...
public class AreaEffectCloud extends Entity implements TraceableEntity {
      ...
      if (!isWaiting || !this.random.nextBoolean()) {
         ...
         for(int i = 0; i < particleCount; ++i) {
            ...
            if (particle.getType() == ParticleTypes.ENTITY_EFFECT) {
               if (isWaiting && this.random.nextBoolean()) {
                  this.level().addAlwaysVisibleParticle(DEFAULT_PARTICLE, x, y, z, 0.0D, 0.0D, 0.0D);
               } else {
                  this.level().addAlwaysVisibleParticle(particle, x, y, z, 0.0D, 0.0D, 0.0D);
               }
            } else if (isWaiting) {
               this.level().addAlwaysVisibleParticle(particle, x, y, z, 0.0D, 0.0D, 0.0D);
            } else {
               this.level().addAlwaysVisibleParticle(particle, x, y, z, (0.5D - this.random.nextDouble()) * 0.15D, (double)0.01F, (0.5D - this.random.nextDouble()) * 0.15D);
            }
         }
         ...
         if (!shouldWait) {
            ...
            if (this.tickCount % 5 == 0) {
               ...
               if (!this.potionContents.hasEffects()) {
                  ...
                  List allEffects = new ArrayList();
                  this.potionContents.forEachEffect(allEffects::add, this.potionDurationScale);
                  List entities = this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox());
                  if (!entities.isEmpty()) {
                           ...
                           if (dist <= (double)(radius * radius)) {
                              ...
                              for(MobEffectInstance effect : allEffects) {
                                 if (((MobEffect)effect.getEffect().value()).isInstantaneous()) {
                                    ((MobEffect)effect.getEffect().value()).applyInstantaneousEffect(serverLevel, this, this.getOwner(), entity, effect.getAmplifier(), 0.5D);
                                 } else {
   ...
   public @Nullable Object get(final DataComponentType type) {
      if (type == DataComponents.POTION_CONTENTS) {
         ...
         return type == DataComponents.POTION_DURATION_SCALE ? castComponentValue(type, Float.valueOf(this.potionDurationScale)) : super.get(type);
      }
   }
