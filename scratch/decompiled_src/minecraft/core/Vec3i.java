...
public class Vec3i implements Comparable {
   public static final Codec CODEC = Codec.INT_STREAM.comapFlatMap((input) -> Util.fixedSize(input, 3).map((ints) -> new Vec3i(ints[0], ints[1], ints[2])), (pos) -> IntStream.of(new int[]{pos.getX(), pos.getY(), pos.getZ()}));
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.VAR_INT, Vec3i::getX, ByteBufCodecs.VAR_INT, Vec3i::getY, ByteBufCodecs.VAR_INT, Vec3i::getZ, Vec3i::new);
