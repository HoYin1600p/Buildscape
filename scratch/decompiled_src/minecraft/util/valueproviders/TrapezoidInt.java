...
public record TrapezoidInt(int minInclusive, int maxInclusive, int plateau) implements IntProvider {
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.INT.fieldOf("min").forGetter(TrapezoidInt::minInclusive), Codec.INT.fieldOf("max").forGetter(TrapezoidInt::maxInclusive), Codec.INT.fieldOf("plateau").forGetter(TrapezoidInt::plateau)).apply(i, TrapezoidInt::new)).validate((c) -> {
