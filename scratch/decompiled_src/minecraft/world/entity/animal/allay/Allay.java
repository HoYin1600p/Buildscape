...
public class Allay extends PathfinderMob implements InventoryCarrier, VibrationSystem {
   ...
   private static final EntityDataAccessor DATA_DANCING = SynchedEntityData.defineId(Allay.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor DATA_CAN_DUPLICATE = SynchedEntityData.defineId(Allay.class, EntityDataSerializers.BOOLEAN);
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(MemoryModuleType.LIKED_PLAYER, MemoryModuleType.LIKED_NOTEBLOCK_POSITION, MemoryModuleType.LIKED_NOTEBLOCK_COOLDOWN_TICKS), List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.NEAREST_PLAYERS, SensorType.HURT_BY, SensorType.NEAREST_ITEMS), (var0) -> AllayAi.getActivities());
   public static final ImmutableList THROW_SOUND_PITCHES = ImmutableList.of(0.5625F, 0.625F, 0.75F, 0.9375F, 1.0F, 1.0F, 1.125F, 1.25F, 1.5F, 1.875F, 2.0F, 2.25F, new Float[]{2.5F, 3.0F, 3.75F, 4.0F});
   ...
   private boolean isLikedPlayer(final @Nullable Entity other) {
      if (!(other instanceof Player player)) {
         ...
         Optional likedPlayer = this.getBrain().getMemory(MemoryModuleType.LIKED_PLAYER);
   ...
   public boolean wantsToPickUp(final ServerLevel level, final ItemStack itemStack) {
      ItemStack itemInHand = this.getItemInHand(InteractionHand.MAIN_HAND);
      return !itemInHand.isEmpty() && level.getGameRules().get(GameRules.MOB_GRIEFING) && this.inventory.canAddItem(itemStack) && this.allayConsidersItemEqual(itemInHand, itemStack);
   }
   ...
   private void spawnHeartParticle() {
      double xd = this.random.nextGaussian() * 0.02D;
      double yd = this.random.nextGaussian() * 0.02D;
      double zd = this.random.nextGaussian() * 0.02D;
      this.level().addParticle(ParticleTypes.HEART, this.getRandomX(1.0D), this.getRandomY() + 0.5D, this.getRandomZ(1.0D), xd, yd, zd);
   }
   ...
   private class VibrationUser implements VibrationSystem.User {
      ...
      public boolean canReceiveVibration(final ServerLevel level, final BlockPos pos, final Holder event, final GameEvent.Context context) {
         if (Allay.this.isNoAi()) {
            ...
            Optional maybeGlobalPos = Allay.this.getBrain().getMemory(MemoryModuleType.LIKED_NOTEBLOCK_POSITION);
