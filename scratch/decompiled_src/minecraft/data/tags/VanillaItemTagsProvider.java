...
public class VanillaItemTagsProvider extends TagsProvider {
   ...
   protected BlockItemTagAppender tag(final TagKey tag) {
      return new BlockItemTagAppender(this, super.tag(tag)) {
         {
            Objects.requireNonNull(this$0);
         }

         protected ResourceKey convertElement(final BlockItemId element) {
            return element.item();
         }
      };
   }
   ...
   private static ColorCollection toIds(final ColorCollection ids) {
      return ids.map(BlockItemId::item);
   }

   private static WeatheringCopperCollection toIds(final WeatheringCopperCollection ids) {
      return ids.map(BlockItemId::item);
   }
}
