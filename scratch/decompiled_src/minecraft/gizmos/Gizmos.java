...
public class Gizmos {
   ...
   public static GizmoProperties billboardTextOverBlock(final String text, final BlockPos pos, final int row, final int color, final float scale) {
      double firstRowStartPosition = 1.3D;
      double rowHeight = 0.2D;
      GizmoProperties properties = billboardText(text, Vec3.atLowerCornerWithOffset(pos, 0.5D, 1.3D + (double)row * 0.2D, 0.5D), TextGizmo.Style.forColorAndCentered(color).withScale(scale));
   ...
   public static GizmoProperties billboardTextOverMob(final Entity entity, final int row, final String text, final int color, final float scale) {
      double firstRowStartPosition = 2.4D;
      double rowHeight = 0.25D;
      double x = (double)entity.getBlockX() + 0.5D;
      double y = entity.getY() + 2.4D + (double)row * 0.25D;
      double z = (double)entity.getBlockZ() + 0.5D;
   ...
   public static class TemporaryCollection implements AutoCloseable {
      private final @Nullable GizmoCollector old = (GizmoCollector)Gizmos.collector.get();
