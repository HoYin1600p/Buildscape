...
public record TestData(Object environment, ResourceKey dimension, Identifier structure, int maxTicks, int setupTicks, boolean required, Rotation rotation, boolean manualOnly, int maxAttempts, int requiredSuccesses, boolean skyAccess, int padding) {
   ...
   public TestData map(final Function mapper) {
      return new TestData(mapper.apply(this.environment), this.dimension, this.structure, this.maxTicks, this.setupTicks, this.required, this.rotation, this.manualOnly, this.maxAttempts, this.requiredSuccesses, this.skyAccess, this.padding);
   }
}
