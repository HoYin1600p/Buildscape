...
public class IOWorker implements AutoCloseable, ChunkScanAccess {
   ...
   private CompletableFuture getOrCreateOldDataForRegion(final int regionX, final int regionZ) {
      ...
      synchronized(this.regionCacheForBlender) {
         CompletableFuture result = (CompletableFuture)this.regionCacheForBlender.getAndMoveToFirst(regionPos);
         if (result == null) {
   ...
   public CompletableFuture synchronize(final boolean flush) {
      CompletableFuture currentWrites = this.submitTask(() -> CompletableFuture.allOf((CompletableFuture[])this.pendingWrites.values().stream().map((store) -> store.result).toArray((x$0) -> new CompletableFuture[x$0]))).thenCompose(Function.identity());
   ...
   private CompletableFuture submitThrowingTask(final IOWorker.ThrowingSupplier task) {
