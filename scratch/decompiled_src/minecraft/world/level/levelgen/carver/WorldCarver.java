...
public interface WorldCarver {
   Codec DIRECT_CODEC = BuiltInRegistries.CARVER_TYPE.byNameCodec().dispatch(WorldCarver::codec, Function.identity());
   Codec CODEC = RegistryCodecs.holder(Registries.CARVER, DIRECT_CODEC);
   Codec LIST_CODEC = RegistryCodecs.holderSet(Registries.CARVER, DIRECT_CODEC);

   default int getRange() {
      return 4;
   }

   static void carveEllipsoid(final ChunkPos chunkPos, final double x, final double y, final double z, final double horizontalRadius, final double verticalRadius, final CarverOutput output, final WorldCarver.CarveSkipChecker skipChecker) {
