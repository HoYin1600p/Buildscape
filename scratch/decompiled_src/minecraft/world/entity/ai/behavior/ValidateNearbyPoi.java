...
public class ValidateNearbyPoi {
   ...
   public static BehaviorControl create(final Predicate poiType, final MemoryModuleType memoryType) {
      return BehaviorBuilder.create((i) -> i.group(i.present(memoryType)).apply(i, (memory) -> (level, body, timestamp) -> {
               ...
               if (level.dimension() == globalPos.dimension() && pos.closerToCenterThan(body.position(), 16.0D)) {
   ...
   private static boolean bedIsOccupiedByVillager(final ServerLevel poiLevel, final BlockPos poiPos) {
      List villagers = poiLevel.getEntitiesOfClass(Villager.class, new AABB(poiPos), LivingEntity::isSleeping);
