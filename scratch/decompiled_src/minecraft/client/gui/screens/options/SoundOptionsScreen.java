...
public class SoundOptionsScreen extends OptionsSubScreen {
   ...
   private OptionInstance[] getAllSoundOptionsExceptMaster() {
      return (OptionInstance[])Arrays.stream(SoundSource.values()).filter((s) -> s != SoundSource.MASTER).map(this.options::getSoundSourceOptionInstance).toArray((x$0) -> new OptionInstance[x$0]);
   }
}
