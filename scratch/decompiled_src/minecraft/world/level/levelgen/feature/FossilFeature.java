...
public record FossilFeature(List fossilStructures, List overlayStructures, Holder fossilProcessors, Holder overlayProcessors, int maxEmptyCornersAllowed) implements Feature {
   ...
   public boolean place(final WorldGenLevel level, final ChunkGenerator chunkGenerator, final RandomSource random, final BlockPos origin) {
      ...
      if (countEmptyCorners(level, fossilBase.getBoundingBox(settings, targetPos)) > this.maxEmptyCornersAllowed()) {
         ...
         ((StructureProcessorList)this.fossilProcessors().value()).list().forEach(settings::addProcessor);
         fossilBase.placeInWorld(level, targetPos, targetPos, settings, random, 260);
         settings.clearProcessors();
         ((StructureProcessorList)this.overlayProcessors().value()).list().forEach(settings::addProcessor);
