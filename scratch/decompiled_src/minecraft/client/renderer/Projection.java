...
public class Projection {
   ...
   public Matrix4f getMatrix(final Matrix4f dest) {
      if (!this.isMatrixDirty) {
         ...
         return this.projectionType == ProjectionType.PERSPECTIVE ? dest.set(this.matrix.setPerspective(this.perspectiveFov * ((float)Math.PI / 180F), this.width / this.height, near, far, zZeroToOne)) : dest.set(this.matrix.setOrtho(0.0F, this.width, this.orthoInvertY ? this.height : 0.0F, this.orthoInvertY ? 0.0F : this.height, near, far, zZeroToOne));
      }
   }
