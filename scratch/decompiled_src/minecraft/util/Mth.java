...
public class Mth {
   ...
   private static final float[] SIN = (float[])Util.make(new float[65536], (sin) -> {
      for(int i = 0; i < sin.length; ++i) {
         sin[i] = (float)Math.sin((double)i / 10430.378350470453D);
      }
   ...
   public static float sin(final double i) {
      return SIN[(int)((long)(i * 10430.378350470453D) & 65535L)];
   }

   public static float cos(final double i) {
      return SIN[(int)((long)(i * 10430.378350470453D + 16384.0D) & 65535L)];
   }
   ...
   public static double clampedLerp(final double factor, final double min, final double max) {
      if (factor < 0.0D) {
         ...
         return factor > 1.0D ? max : lerp(factor, min, max);
      }
   }
   ...
   public static double wrapDegrees(final double angle) {
      double normalizedAngle = angle % 360.0D;
   ...
   public static boolean rayIntersectsAABB(final Vec3 rayStart, final Vec3 rayDir, final AABB aabb) {
      double centerX = (aabb.minX + aabb.maxX) * 0.5D;
      double boxExtentX = (aabb.maxX - aabb.minX) * 0.5D;
      double diffX = rayStart.x - centerX;
      if (Math.abs(diffX) > boxExtentX && diffX * rayDir.x >= 0.0D) {
         return false;
      } else {
         double centerY = (aabb.minY + aabb.maxY) * 0.5D;
         double boxExtentY = (aabb.maxY - aabb.minY) * 0.5D;
         double diffY = rayStart.y - centerY;
         if (Math.abs(diffY) > boxExtentY && diffY * rayDir.y >= 0.0D) {
            return false;
         } else {
            double centerZ = (aabb.minZ + aabb.maxZ) * 0.5D;
            double boxExtentZ = (aabb.maxZ - aabb.minZ) * 0.5D;
            double diffZ = rayStart.z - centerZ;
            if (Math.abs(diffZ) > boxExtentZ && diffZ * rayDir.z >= 0.0D) {
               return false;
            } else {
   ...
   public static double atan2(double y, double x) {
      ...
      if (Double.isNaN(d2)) {
         ...
         double d = (6.0D + sd * sd) * sd * 0.16666666666666666D;
         double theta = phi + d;
         if (steep) {
   ...
   public static double wobble(final double coord) {
      return coord + (2.0D * RandomSource.createThreadLocalInstance((long)floor(coord * 3000.0D)).nextDouble() - 1.0D) * 1.0E-7D / 2.0D;
   }
