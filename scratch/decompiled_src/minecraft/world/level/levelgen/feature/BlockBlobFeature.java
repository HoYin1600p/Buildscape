...
public record BlockBlobFeature(BlockState state, BlockPredicate canPlaceOn) implements Feature {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(BlockState.CODEC.fieldOf("state").forGetter(BlockBlobFeature::state), BlockPredicate.CODEC.fieldOf("can_place_on").forGetter(BlockBlobFeature::canPlaceOn)).apply(i, BlockBlobFeature::new));
