...
public class TemptingSensor extends Sensor {
   ...
   public static TemptingSensor forAnimal() {
      return new TemptingSensor((BiPredicate)((m, i) -> {
   ...
   protected void doTick(final ServerLevel level, final PathfinderMob body) {
      ...
      List players = (List)level.players().stream().filter(EntitySelector.NO_SPECTATORS).filter((playerx) -> targeting.test(level, body, playerx)).filter((p) -> this.playerHoldingTemptation(body, p)).filter((playerx) -> !body.hasPassenger(playerx)).sorted(Comparator.comparingDouble(body::distanceToSqr)).collect(Collectors.toList());
      if (!players.isEmpty()) {
