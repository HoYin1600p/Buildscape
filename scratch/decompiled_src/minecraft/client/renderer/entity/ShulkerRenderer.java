...
public class ShulkerRenderer extends MobRenderer {
   private static final Identifier DEFAULT_TEXTURE_LOCATION = Sheets.DEFAULT_SHULKER_TEXTURE_LOCATION.texture().withPath((UnaryOperator)((path) -> "textures/" + path + ".png"));
   private static final Identifier[] TEXTURE_LOCATION = (Identifier[])Sheets.SHULKER_TEXTURE_LOCATION.stream().map((location) -> location.texture().withPath((UnaryOperator)((path) -> "textures/" + path + ".png"))).toArray((x$0) -> new Identifier[x$0]);
