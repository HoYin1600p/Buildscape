...
public class TeamCommand {
   ...
   private static final CommandResponseTracker.Messages RESPONSE_LEAVE = CommandResponseTracker.messages((holder, var1) -> Component.translatable("commands.team.leave.success.single", holder.getFeedbackDisplayName()), (holderCount, var1) -> Component.translatable("commands.team.leave.success.multiple", holderCount));
   private static final CommandResponseTracker.MessagesWithArg RESPONSE_JOIN = CommandResponseTracker.messages((holder, var1, team) -> Component.translatable("commands.team.join.success.single", holder.getFeedbackDisplayName(), team.getFormattedDisplayName()), (holderCount, var1, team) -> Component.translatable("commands.team.join.success.multiple", holderCount, team.getFormattedDisplayName()));
   ...
   private static int leaveTeam(final CommandSourceStack source, final Collection members) throws CommandSyntaxException {
   ...
   private static int joinTeam(final CommandSourceStack source, final PlayerTeam team, final Collection members) throws CommandSyntaxException {
      CommandResponseTracker tracker = CommandResponseTracker.create();
      Scoreboard scoreboard = source.getServer().getScoreboard();
   ...
   private static int setColor(final CommandSourceStack source, final PlayerTeam team, final TeamColor color) throws CommandSyntaxException {
      Optional currentColor = team.getColor();
   ...
   private static int emptyTeam(final CommandSourceStack source, final PlayerTeam team) throws CommandSyntaxException {
      Scoreboard scoreboard = source.getServer().getScoreboard();
      Collection members = Lists.newArrayList(team.getPlayers());
      if (members.isEmpty()) {
   ...
   private static int listTeams(final CommandSourceStack source) {
      Collection teams = source.getServer().getScoreboard().getPlayerTeams();
      if (teams.isEmpty()) {
