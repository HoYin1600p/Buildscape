...
public record FloatWithAlpha(float value, float alpha) {
   ...
   public static final Codec CODEC = Codec.either(Codec.FLOAT, FULL_CODEC).xmap((either) -> (FloatWithAlpha)either.map(FloatWithAlpha::new, (p) -> p), (parameter) -> parameter.alpha() == 1.0F ? Either.left(parameter.value()) : Either.right(parameter));
