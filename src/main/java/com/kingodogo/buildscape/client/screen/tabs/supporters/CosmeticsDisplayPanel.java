...
public class CosmeticsDisplayPanel extends BasePanel {
    ...
    // Rotation state for all items (like pillars) - each item has its own rotation timer
    // Key: cosmeticId, Value: animation start time in milliseconds
    ...
    public void init() {
        ...
        // Layout: gap + item + gap + item + gap + item + gap + item + gap + scrollbar = full width
        ...
        // Calculate item size to fit 4 items with 4 gaps (between items) in available width
        ...
        // Calculate visible rows based on panel height (account for title and filter buttons)
        int titleHeight = 15;
     ...
     * @param speedMultiplier Multiplier for animation speed (1.0 = normal, >1.0 = faster)
     */
    ...
    private boolean matchesFilter(String cosmeticId) {
        ...
        com.kingodogo.buildscape.cosmetics.CosmeticManager.CosmeticMetadata metadata = 
                com.kingodogo.buildscape.cosmetics.CosmeticManager.getInstance().getMetadata(cosmeticId);
        
        switch (currentFilter) {
            case WINGS:
                if (metadata != null && metadata.type == com.kingodogo.buildscape.cosmetics.CosmeticManager.CosmeticType.WINGS) {
                    return true;
                }
                return idLower.contains("elytra") || idLower.contains("wing");
            case PARTICLES:
                if (metadata != null && metadata.type == com.kingodogo.buildscape.cosmetics.CosmeticManager.CosmeticType.PARTICLE_TRAIL) {
                    return true;
                }
    ...
    public void updateCosmeticList() {
        ...
        for (String cosmeticId : filteredCosmeticIds) {
            ...
            // Even if stack is null, the registry will cache the null result to avoid repeated lookups
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        int titleHeight = 15;
        // Add spacing below filter buttons so items don't appear right at the button edge
        ...
        // renderStartY and scissorHeight are in GUI-scaled coordinates, convert to actual pixels
        ...
        // Don't enable scissor yet - render buttons first, then enable scissor for items
        ...
        int buttonWidth = (width - PADDING * 2 - BUTTON_SPACING * (CosmeticType.values().length - 1)) / CosmeticType.values().length;
        ...
        for (CosmeticType type : CosmeticType.values()) {
            ...
            GuiComponent.fill(poseStack, buttonX, buttonY + BUTTON_HEIGHT - 1, buttonX + buttonWidth, buttonY + BUTTON_HEIGHT, borderColor); // Bottom
            GuiComponent.fill(poseStack, buttonX, buttonY, buttonX + 1, buttonY + BUTTON_HEIGHT, borderColor); // Left
            GuiComponent.fill(poseStack, buttonX + buttonWidth - 1, buttonY, buttonX + buttonWidth, buttonY + BUTTON_HEIGHT, borderColor); // Right
        ...
        for (int row = startRow; row < endRow; row++) {
            ...
            // Skip rendering if row is completely outside visible area (with some margin for smooth scrolling)
            ...
            for (int col = 0; col < itemsPerRow; col++) {
                ...
                if (isEquipped) {
                    ...
                    GuiComponent.fill(poseStack, itemX - 1, rowY + itemSize - 1, itemX + itemSize + 1, rowY + itemSize + 1, borderColor);
                    GuiComponent.fill(poseStack, itemX - 1, rowY - 1, itemX + 1, rowY + itemSize + 1, borderColor);
                    GuiComponent.fill(poseStack, itemX + itemSize - 1, rowY - 1, itemX + itemSize + 1, rowY + itemSize + 1, borderColor);
                } else if (isSelected) {
                ...
                // Use 3D preview for selected item, animated for animated entities, 2D for others
                ...
                com.kingodogo.buildscape.cosmetics.CosmeticManager.CosmeticMetadata headMetadata = 
                    com.kingodogo.buildscape.cosmetics.CosmeticManager.getInstance().getMetadata(cosmeticId);
                ...
                // For particle trails and HEAD cosmetics, always render (even if stack is null, we'll show placeholder)
                if (stack != null && !stack.isEmpty() || isParticleTrail || isHeadCosmetic) {
                    ...
                    if (isAnimated) {
                        if (!itemAnimationTimes.containsKey(cosmeticId)) {
                            // Initialize animation timer when first seen - use staggered start times for variety
                    ...
                    try {
                        if (isSelected && !isParticleTrail) {
                            ...
                            renderAnimatedEntity(poseStack, stack, cosmeticId, itemX + itemSize / 2, rowY + itemSize / 2, partialTick, false);
                        } else {
                    ...
                    // Render placeholder for invalid cosmetic - show that item exists but can't be resolved
        ...
        if (maxScroll > 0) {
            ...
            GuiComponent.fill(poseStack, scrollbarX, scrollbarY, scrollbarX + SCROLLBAR_WIDTH, scrollbarY + scrollbarHeight, 0x80000000);
            ...
            GuiComponent.fill(poseStack, scrollbarX, thumbY, scrollbarX + SCROLLBAR_WIDTH, thumbY + thumbHeight, 0xFF808080);
            GuiComponent.fill(poseStack, scrollbarX + 1, thumbY + 1, scrollbarX + SCROLLBAR_WIDTH - 1, thumbY + thumbHeight - 1, 0xFFC0C0C0);
        }
    ...
    private void renderAnimatedEntity(PoseStack poseStack, ItemStack stack, String cosmeticId, int centerX, int centerY, float partialTick, boolean isHovered) {
        ...
        if (idLower.contains("elytra") || idLower.contains("wing")) {
            ...
            // Scale factor: itemSize * 0.46875 gives good fit for wings (for 128px item size at scale 2, that's 60.0f)
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                float particleSize = 5.0f * (1.0f - trailProgress * 0.5f); // Smaller as they go up
                ...
                // Draw a simple star/cross pattern
            ...
            // Scale proportional to itemSize (which is calculated to fit 4 per row)
            // itemSize is in GUI-scaled coordinates, so it already scales with GUI scale
            // Scale factor: itemSize * 0.46875 gives good fit (for 128px item size at scale 2, that's 60.0f)
     ...
     */
    private void render3DItemLikePillar(PoseStack poseStack, ItemStack stack, String cosmeticId, int centerX, int centerY, float partialTick) {
        Level level = mc.level;
        if (level == null) {
            // Fallback to 2D if no level
            itemRenderer.renderGuiItem(stack, centerX - 8, centerY - 8);
        ...
        // itemSize is in GUI-scaled coordinates, so it already scales with GUI scale
        // Scale factor: itemSize * 0.390625 gives good fit (for 128px item size at scale 2, that's 50.0f)
     ...
     */
    private void render3DItemPreview(PoseStack poseStack, ItemStack stack, int centerX, int centerY, float partialTick) {
        Level level = mc.level;
        if (level == null) {
            // Fallback to 2D if no level
            itemRenderer.renderGuiItem(stack, centerX - 8, centerY - 8);
        ...
        // Apply scale proportional to itemSize (which is calculated to fit 4 per row)
        // itemSize is in GUI-scaled coordinates, so it already scales with GUI scale
        // Scale factor: itemSize * 0.625 gives good fit (for 128px item size at scale 2, that's 80.0f)
    ...
    protected boolean handleMouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (colorPicker != null && selectedCosmeticForColor != null) {
            ...
            // If click is outside color picker, close it
        ...
        if (col < 0 || col >= itemsPerRow) return false;
