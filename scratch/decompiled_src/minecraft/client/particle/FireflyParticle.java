...
public class FireflyParticle extends SingleQuadParticle {
   ...
   public static class FireflyProvider implements ParticleProvider {
      ...
      public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
         FireflyParticle particle = new FireflyParticle(level, x, y, z, 0.5D - random.nextDouble(), random.nextBoolean() ? yAux : -yAux, 0.5D - random.nextDouble(), this.sprite.get(random));
