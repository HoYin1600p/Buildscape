...
public class Mannequin extends Avatar {
   protected static final EntityDataAccessor DATA_PROFILE = SynchedEntityData.defineId(Mannequin.class, EntityDataSerializers.RESOLVABLE_PROFILE);
   private static final EntityDataAccessor DATA_IMMOVABLE = SynchedEntityData.defineId(Mannequin.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor DATA_DESCRIPTION = SynchedEntityData.defineId(Mannequin.class, EntityDataSerializers.OPTIONAL_COMPONENT);
   ...
   private static final Set VALID_POSES = Set.of(Pose.STANDING, Pose.CROUCHING, Pose.SWIMMING, Pose.FALL_FLYING, Pose.SLEEPING);
   public static final Codec POSE_CODEC = Pose.CODEC.validate((pose) -> VALID_POSES.contains(pose) ? DataResult.success(pose) : DataResult.error(() -> "Invalid pose: " + pose.getSerializedName()));
   private static final Codec LAYERS_CODEC = PlayerModelPart.CODEC.listOf().xmap((list) -> (byte)list.stream().mapToInt(PlayerModelPart::getMask).reduce(ALL_LAYERS, (a, b) -> a & ~b), (mask) -> Arrays.stream(PlayerModelPart.values()).filter((part) -> (mask & part.getMask()) == 0).toList());
   ...
   protected static EntityType.EntityFactory constructor = Mannequin::new;
