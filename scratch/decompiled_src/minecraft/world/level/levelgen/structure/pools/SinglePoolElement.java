...
public class SinglePoolElement extends StructurePoolElement {
   ...
   private static final Codec TEMPLATE_CODEC = Codec.of(SinglePoolElement::encodeTemplate, Identifier.CODEC.map(Either::left));
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(templateCodec(), processorsCodec(), projectionCodec(), overrideLiquidSettingsCodec()).apply(i, SinglePoolElement::new));
   ...
   public List getDataMarkers(final StructureTemplateManager structureTemplateManager, final BlockPos position, final Rotation rotation, final boolean absolute) {
      StructureTemplate template = this.getTemplate(structureTemplateManager);
      List structureBlocks = template.filterBlocks(position, (new StructurePlaceSettings()).setRotation(rotation), Blocks.STRUCTURE_BLOCK, absolute);
      List dataMarkers = Lists.newArrayList();
   ...
   public List getShuffledJigsawBlocks(final StructureTemplateManager structureTemplateManager, final BlockPos position, final Rotation rotation, final RandomSource random) {
      List jigsaws = this.getTemplate(structureTemplateManager).getJigsaws(position, rotation);
   ...
   protected StructurePlaceSettings getSettings(final Rotation rotation, final BoundingBox chunkBB, final LiquidSettings liquidSettings, final boolean keepJigsaws) {
      ...
      ((StructureProcessorList)this.processors.value()).list().forEach(settings::addProcessor);
