...
public record AmbientLeavesBlockSoundPlayer(Optional ambientSound, int chance, Optional satisfyingBlocks, int nearbySatisfyingBlocksRequired, int nearbySameLeavesRequired) {
   ...
   public static AmbientLeavesBlockSoundPlayer of(final Holder ambientSound, final TagKey satisfyingBlocks) {
   ...
   public void playAmbientLeavesSounds(final Level level, final BlockPos pos, final Block block, final RandomSource random) {
      if (this.ambientSound.isPresent() && random.nextInt(this.chance) == 0) {
         TagKey satisfyingBlocks = (TagKey)this.satisfyingBlocks.orElse((Object)null);
         int nearbyLogs = 0;
         int nearbyLeaves = 0;
