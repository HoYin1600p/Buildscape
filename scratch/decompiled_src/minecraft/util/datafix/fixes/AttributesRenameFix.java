...
public class AttributesRenameFix extends DataFix {
   ...
   public AttributesRenameFix(final Schema outputSchema, final String name, final UnaryOperator renames, final boolean oldDataComponentFormat) {
   ...
   private Typed fixEntity(final Typed entity) {
      return entity.update(DSL.remainderFinder(), (tag) -> tag.update("attributes", (attributeList) -> (Dynamic)DataFixUtils.orElse(attributeList.asStreamOpt().result().map((s) -> s.map(this::fixIdField)).map(attributeList::createList), attributeList)));
   }
