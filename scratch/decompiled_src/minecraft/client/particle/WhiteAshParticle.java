...
public class WhiteAshParticle extends BaseAshSmokeParticle {
   ...
   public static class Provider implements ParticleProvider {
      ...
      public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
         double xa = (double)random.nextFloat() * -1.9D * (double)random.nextFloat() * 0.1D;
         double ya = (double)random.nextFloat() * -0.5D * (double)random.nextFloat() * 0.1D * 5.0D;
         double za = (double)random.nextFloat() * -1.9D * (double)random.nextFloat() * 0.1D;
