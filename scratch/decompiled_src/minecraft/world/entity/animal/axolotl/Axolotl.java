...
public class Axolotl extends Animal implements Bucketable {
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.NEAREST_ADULT, SensorType.HURT_BY, SensorType.AXOLOTL_ATTACKABLES, SensorType.FOOD_TEMPTATIONS), (var0) -> AxolotlAi.getActivities());
   private static final EntityDataAccessor DATA_VARIANT = SynchedEntityData.defineId(Axolotl.class, EntityDataSerializers.INT);
   private static final EntityDataAccessor DATA_PLAYING_DEAD = SynchedEntityData.defineId(Axolotl.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor FROM_BUCKET = SynchedEntityData.defineId(Axolotl.class, EntityDataSerializers.BOOLEAN);
   ...
   private final ImmutableList ALL_ANIMATIONS = ImmutableList.of(this.swimAnimationState, this.walkAnimationState, this.walkUnderWaterAnimationState, this.idleUnderWaterAnimationState, this.idleUnderWaterOnGroundAnimationState, this.idleOnGroundAnimationState, this.playDeadAnimationState);
   ...
   public @Nullable AgeableMob getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Axolotl baby = (Axolotl)EntityTypes.AXOLOTL.create(level, EntitySpawnReason.BREEDING);
      if (baby != null) {
   ...
   protected void customServerAiStep(final ServerLevel level) {
      ...
      profiler.pop();
      if (!this.isNoAi()) {
         Optional playDeadTicks = this.getBrain().getMemory(MemoryModuleType.PLAY_DEAD_TICKS);
      ...
      if (target.isDeadOrDying()) {
         ...
         if (lastDamageSource != null) {
            ...
            if (entity instanceof Player) {
               ...
               List playersInRange = level.getEntitiesOfClass(Player.class, body.getBoundingBox().inflate(20.0D));
   ...
   public static enum AxolotlAnimationState {
      ...
      // $FF: synthetic method
      private static Axolotl.AxolotlAnimationState[] $values() {
         return new Axolotl.AxolotlAnimationState[]{PLAYING_DEAD, IN_WATER, ON_GROUND, IN_AIR};
      }
   }
   ...
   public static enum Variant implements StringRepresentable {
      ...
      private static final IntFunction BY_ID = ByIdMap.continuous(Axolotl.Variant::getId, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
      public static final StreamCodec STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, Axolotl.Variant::getId);
      public static final Codec CODEC = StringRepresentable.fromEnum(Axolotl.Variant::values);
      /** @deprecated */
      @Deprecated
      public static final Codec LEGACY_CODEC = Codec.INT.xmap(BY_ID::apply, Axolotl.Variant::getId);
      ...
      private static Axolotl.Variant getSpawnVariant(final RandomSource random, final boolean common) {
         Axolotl.Variant[] validVariants = (Axolotl.Variant[])Arrays.stream(values()).filter((v) -> v.common == common).toArray((x$0) -> new Axolotl.Variant[x$0]);
         return (Axolotl.Variant)Util.getRandom((Object[])validVariants, random);
      }
