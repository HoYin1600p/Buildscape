...
public class EntityArgument implements ArgumentType {
   ...
   public static Collection getEntities(final CommandContext context, final String name) throws CommandSyntaxException {
      Collection result = getOptionalEntities(context, name);
      if (result.isEmpty()) {
   ...
   public static Collection getPlayers(final CommandContext context, final String name) throws CommandSyntaxException {
      List players = ((EntitySelector)context.getArgument(name, EntitySelector.class)).findPlayers((CommandSourceStack)context.getSource());
      if (players.isEmpty()) {
   ...
   public CompletableFuture listSuggestions(final CommandContext contextBuilder, final SuggestionsBuilder builder) {
      Object reader = contextBuilder.getSource();
      if (reader instanceof SharedSuggestionProvider source) {
         ...
         return parser.fillSuggestions(builder, (suggestions) -> {
            Collection onlinePlayerNames = source.getOnlinePlayerNames();
            Iterable suggestedNames = (Iterable<String>)(this.playersOnly ? onlinePlayerNames : Iterables.concat(onlinePlayerNames, source.getSelectedEntities()));
