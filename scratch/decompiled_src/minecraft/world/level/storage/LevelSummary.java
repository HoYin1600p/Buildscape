...
public class LevelSummary implements Comparable {
   ...
   private Component createInfo() {
      if (this.isLocked()) {
         ...
         MutableComponent result = this.isHardcore() ? Component.empty().append(Component.translatable("gameMode.hardcore").withColor(-65536)) : Component.translatable("gameMode." + this.getGameMode().getName());
         if (this.hasCommands()) {
   ...
   public static enum BackupStatus {
      ...
      private static LevelSummary.BackupStatus[] $values() {
         return new LevelSummary.BackupStatus[]{NONE, DOWNGRADE, UPGRADE_TO_SNAPSHOT, FILE_FIXING_REQUIRED};
      }
   }
