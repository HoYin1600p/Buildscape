...
public class EnderDragon extends Mob implements Enemy {
   ...
   public void aiStep() {
      ...
      if (this.dragonFight == null) {
         Level flapSpeed = this.level();
         if (flapSpeed instanceof ServerLevel) {
            ServerLevel serverLevel = (ServerLevel)flapSpeed;
      ...
      if (this.isDeadOrDying()) {
         ...
         this.level().addParticle(ParticleTypes.EXPLOSION, this.getX() + (double)xo, this.getY() + 2.0D + (double)yo, this.getZ() + (double)zo, 0.0D, 0.0D, 0.0D);
      } else {
         this.checkCrystals();
         Vec3 movement = this.getDeltaMovement();
         float flapSpeed = 0.2F / ((float)movement.horizontalDistance() * 10.0F + 1.0F);
         flapSpeed *= (float)Math.pow(2.0D, movement.y);
         ...
         if (this.isNoAi()) {
            ...
            Level tilt = this.level();
            if (tilt instanceof ServerLevel) {
               ServerLevel level = (ServerLevel)tilt;
               ...
               if (targetLocation != null) {
                  ...
                  this.setDeltaMovement(this.getDeltaMovement().add(0.0D, ydd * 0.01D, 0.0D));
                  this.setYRot(Mth.wrapDegrees(this.getYRot()));
                  ...
                  float span = (float)(2.0D / (distToTarget + 1.0D));
                  float speed = 0.06F;
                  this.moveRelative(0.06F * (dot * span + (1.0F - span)), new Vec3(0.0D, 0.0D, -1.0D));
                  if (this.inWall) {
                  ...
                  double slide = 0.8D + 0.15D * (actual.dot(dir) + 1.0D) / 2.0D;
            ...
            this.tickPart(this.body, (double)(ss1 * 0.5F), 0.0D, (double)(-cc1 * 0.5F));
            this.tickPart(this.wing1, (double)(cc1 * 4.5F), 2.0D, (double)(ss1 * 4.5F));
            this.tickPart(this.wing2, (double)(cc1 * -4.5F), 2.0D, (double)(ss1 * -4.5F));
            Level cc2 = this.level();
            if (cc2 instanceof ServerLevel) {
               ServerLevel serverLevel = (ServerLevel)cc2;
               if (!this.wasHurtRecently()) {
                  this.knockBack(serverLevel, serverLevel.getEntities(this, this.wing1.getBoundingBox().inflate(4.0D, 2.0D, 4.0D).move(0.0D, -2.0D, 0.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.knockBack(serverLevel, serverLevel.getEntities(this, this.wing2.getBoundingBox().inflate(4.0D, 2.0D, 4.0D).move(0.0D, -2.0D, 0.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.hurt(serverLevel, serverLevel.getEntities(this, this.head.getBoundingBox().inflate(1.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.hurt(serverLevel, serverLevel.getEntities(this, this.neck.getBoundingBox().inflate(1.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
               }
            }
            ...
            for(int i = 0; i < 3; ++i) {
               ...
               this.tickPart(part, (double)(-(ss1 * 1.5F + ss * dd) * ccTilt), p0.y() - p1.y() - (double)((dd + 1.5F) * ssTilt) + 1.5D, (double)((cc1 * 1.5F + cc * dd) * ccTilt));
            }
   ...
   private void checkCrystals() {
      ...
      if (this.random.nextInt(10) == 0) {
         List crystals = this.level().getEntitiesOfClass(EndCrystal.class, this.getBoundingBox().inflate(32.0D));
   ...
   private void knockBack(final ServerLevel serverLevel, final List entities) {
      double xm = (this.body.getBoundingBox().minX + this.body.getBoundingBox().maxX) / 2.0D;
      double zm = (this.body.getBoundingBox().minZ + this.body.getBoundingBox().maxZ) / 2.0D;

      for(Entity entity : entities) {
         if (entity instanceof LivingEntity livingTarget) {
            double xd = entity.getX() - xm;
            double zd = entity.getZ() - zm;
            double dd = Math.max(xd * xd + zd * zd, 0.1D);
            entity.push(xd / dd * 4.0D, (double)0.2F, zd / dd * 4.0D);
   ...
   protected void tickDeath() {
      ...
      if (this.dragonDeathTime >= 180 && this.dragonDeathTime <= 200) {
         ...
         float yo = (this.random.nextFloat() - 0.5F) * 4.0F;
         float zo = (this.random.nextFloat() - 0.5F) * 8.0F;
         this.level().addParticle(ParticleTypes.EXPLOSION_EMITTER, this.getX() + (double)xo, this.getY() + 2.0D + (double)yo, this.getZ() + (double)zo, 0.0D, 0.0D, 0.0D);
      }
      ...
      if (var10 instanceof ServerLevel level) {
      ...
      Vec3 deathMove = new Vec3(0.0D, (double)0.1F, 0.0D);
   ...
   public Vec3 getHeadLookVector(final float a) {
      ...
      EnderDragonPhase phase = phaseInstance.getPhase();
      Vec3 result;
