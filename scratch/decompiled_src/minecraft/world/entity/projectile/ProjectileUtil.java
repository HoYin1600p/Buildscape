...
public final class ProjectileUtil {
   ...
   public static Either getHitEntitiesAlong(final Entity attacker, final AttackRange attackRange, final Predicate matching, final ClipContext.Block blockClipType) {
      ...
      Vec3 to = eyePosition.add(look.scale((double)attackRange.effectiveMaxRange(attacker) + Math.max(0.0D, movementComponent)));
   ...
   private static HitResult getHitResult(final Vec3 from, final Entity source, final Predicate matching, final Vec3 delta, final Level level, final float entityMargin, final ClipContext.Block clipType) {
      ...
      HitResult entityHit = getEntityHitResult(level, source, from, to, source.getBoundingBox().expandTowards(delta).inflate(1.0D), matching, entityMargin);
      if (entityHit != null) {
         hitResult = entityHit;
      }
   ...
   private static Either getHitEntitiesAlong(final Entity source, final Vec3 origin, final Vec3 from, final Predicate matching, Vec3 to, final float entityMargin, final ClipContext.Block clipType) {
      ...
      AABB searchArea = AABB.ofSize(from, (double)entityMargin, (double)entityMargin, (double)entityMargin).expandTowards(to.subtract(from)).inflate(1.0D);
      Collection entityHit = getManyEntityHitResult(level, source, from, to, searchArea, matching, entityMargin, clipType, true, true);
   ...
   public static @Nullable EntityHitResult getEntityHitResult(final Entity except, final Vec3 from, final Vec3 to, final AABB box, final Predicate matching, final double maxValue) {
      ...
      for(Entity entity : level.getEntities(except, box, matching)) {
         ...
         Optional clipPoint = bb.clip(from, to);
         if (bb.contains(from)) {
            if (nearest >= 0.0D && entity.canBePickedFromInside()) {
               hovered = entity;
   ...
   public static @Nullable EntityHitResult getEntityHitResult(final Level level, final Entity source, final Vec3 from, final Vec3 to, final AABB targetSearchArea, final Predicate matching, final float entityMargin) {
      ...
      Optional nearestLocation = Optional.empty();
      ...
      for(Entity entity : level.getEntities(source, targetSearchArea, matching)) {
         ...
         Optional location = bb.clip(from, to);
   ...
   public static Collection getManyEntityHitResult(final Level level, final Entity source, final Vec3 from, final Vec3 to, final AABB targetSearchArea, final Predicate matching, final boolean includeFromEntity, final boolean projectSurfaceHitLocation) {
   ...
   public static Collection getManyEntityHitResult(final Level level, final Entity source, final Vec3 from, final Vec3 to, final AABB targetSearchArea, final Predicate matching, final float entityMargin, final ClipContext.Block clipType, final boolean includeFromEntity, final boolean projectSurfaceHitLocation) {
         ...
         if (includeFromEntity && entityBB.contains(from)) {
            ...
            if (exactHit.isPresent()) {
               ...
               if (!outsideHit.isEmpty()) {
                  ...
                  Optional surfaceHit = entity.getBoundingBox().clip(outsideHitPosition, towardsTarget);
