...
public class MultiVariantGenerator implements BlockModelDefinitionGenerator {
   ...
   private static Set validateAndExpandProperties(final Set seenProperties, final Block block, final PropertyDispatch generator) {
      List addedProperties = generator.getDefinedProperties();
      ...
      Set newSeenProperties = new HashSet(seenProperties);
   ...
   public MultiVariantGenerator with(final PropertyDispatch newStage) {
      Set newSeenProperties = validateAndExpandProperties(this.seenProperties, this.block, newStage);
      List newEntries = this.entries.stream().flatMap((entry) -> entry.apply(newStage)).toList();
   ...
   public MultiVariantGenerator with(final VariantMutator singleMutator) {
      List newEntries = this.entries.stream().flatMap((entry) -> entry.apply(singleMutator)).toList();
   ...
   public static class Empty {
      ...
      public MultiVariantGenerator with(final PropertyDispatch newStage) {
         Set newSeenProperties = MultiVariantGenerator.validateAndExpandProperties(Set.of(), this.block, newStage);
         List newEntries = newStage.getEntries().entrySet().stream().map((e) -> new MultiVariantGenerator.Entry((PropertyValueList)e.getKey(), (MultiVariant)e.getValue())).toList();
         return new MultiVariantGenerator(this.block, newEntries, newSeenProperties);
      }
   }
