...
public class LootItemFunctions {
   public static final Codec TYPED_CODEC = BuiltInRegistries.LOOT_FUNCTION_TYPE.byNameCodec().dispatch(LootItemFunction::codec, (c) -> c);
   public static final Codec DIRECT_CODEC = Codec.lazyInitialized(() -> Codec.either(TYPED_CODEC, SequenceFunction.INLINE_CODEC).xmap((typedOrList) -> (LootItemFunction)typedOrList.map((f) -> f, (f) -> f), (function) -> {
   ...
   public static final Codec CODEC = RegistryCodecs.holder(Registries.ITEM_MODIFIER, DIRECT_CODEC);
   public static final Codec LIST_CODEC = RegistryCodecs.holderSet(Registries.ITEM_MODIFIER, TYPED_CODEC);
   ...
   public static MapCodec bootstrap(final Registry registry) {
      ...
      return (MapCodec)Registry.register(registry, "discard", DiscardItem.MAP_CODEC);
   }
}
