...
public class RuinedPortalStructure extends Structure {
   ...
   public Optional findGenerationPoint(final Structure.GenerationContext context) {
      ...
      if (chosenSetup == null) {
         ...
         int projectedY = findSuitableY(random, chunkGenerator, setup.placement(), airPocket, surfaceY, boundingBox.getYSpan(), boundingBox, heightAccessor, randomState);
         BlockPos origin = new BlockPos(basePosition.getX(), projectedY, basePosition.getZ());
         return Optional.of(new Structure.GenerationStub(origin, (Consumer)((builder) -> {
            RuinedPortalPiece.Properties properties = new RuinedPortalPiece.Properties(setup.canBeCold() && isCold(origin, context.biomeResolver().getNoiseBiome(QuartPos.fromBlock(origin.getX()), QuartPos.fromBlock(origin.getY()), QuartPos.fromBlock(origin.getZ())), chunkGenerator.getSeaLevel()), setup.mossiness(), airPocket, setup.overgrown(), setup.vines(), setup.replaceWithBlackstone());
            builder.addPiece(new RuinedPortalPiece(context.registryAccess(), context.structureTemplateManager(), origin, setup.placement(), properties, templateLocation, template, rotation, mirror, pivot));
         })));
      }
   }
   ...
   private static int findSuitableY(final RandomSource random, final ChunkGenerator generator, final RuinedPortalPiece.VerticalPlacement verticalPlacement, final boolean airPocket, final int surfaceYAtCenter, final int ySpan, final BoundingBox boundingBox, final LevelHeightAccessor heightAccessor, final RandomState randomState) {
      ...
      List bottomCorners = ImmutableList.of(new BlockPos(boundingBox.minX(), 0, boundingBox.minZ()), new BlockPos(boundingBox.maxX(), 0, boundingBox.minZ()), new BlockPos(boundingBox.minX(), 0, boundingBox.maxZ()), new BlockPos(boundingBox.maxX(), 0, boundingBox.maxZ()));
      List columns = (List)bottomCorners.stream().map((p) -> generator.getBaseColumn(p.getX(), p.getZ(), heightAccessor, randomState)).collect(Collectors.toList());
