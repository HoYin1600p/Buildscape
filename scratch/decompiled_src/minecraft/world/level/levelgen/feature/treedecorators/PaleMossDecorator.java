...
public class PaleMossDecorator extends TreeDecorator {
   ...
   public void place(final TreeDecorator.Context context) {
      ...
      List logs = Util.shuffledCopy(context.logs(), random);
      if (!logs.isEmpty()) {
   ...
   private static void addMossHanger(BlockPos pos, final TreeDecorator.Context context) {
      while(context.isAir(pos.below()) && !((double)context.random().nextFloat() < 0.5D)) {
         context.setBlock(pos, (BlockState)Blocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, Boolean.valueOf(false)));
         pos = pos.below();
      }

      context.setBlock(pos, (BlockState)Blocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, Boolean.valueOf(true)));
   }
}
