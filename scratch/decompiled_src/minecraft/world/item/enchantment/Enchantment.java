...
public record Enchantment(Component description, Enchantment.EnchantmentDefinition definition, HolderSet exclusiveSet, DataComponentMap effects) {
   ...
   public static final Codec CODEC = RegistryCodecs.holder(Registries.ENCHANTMENT);
   public static final StreamCodec STREAM_CODEC = ByteBufCodecs.holderRegistry(Registries.ENCHANTMENT);
   ...
   public static Enchantment.EnchantmentDefinition definition(final HolderSet supportedItems, final HolderSet primaryItems, final int weight, final int maxLevel, final Enchantment.Cost minCost, final Enchantment.Cost maxCost, final int anvilCost, final EquipmentSlotGroup... slots) {
   ...
   public static Enchantment.EnchantmentDefinition definition(final HolderSet supportedItems, final int weight, final int maxLevel, final Enchantment.Cost minCost, final Enchantment.Cost maxCost, final int anvilCost, final EquipmentSlotGroup... slots) {
   ...
   public Map getSlotItems(final LivingEntity entity) {
      Map itemStacks = Maps.newEnumMap(EquipmentSlot.class);
   ...
   public static void doPostAttack(final TargetedConditionalEffect effect, final ServerLevel serverLevel, final int enchantmentLevel, final EnchantedItemInUse item, final Entity victim, final DamageSource damageSource) {
      if (effect.matches(damageContext(serverLevel, enchantmentLevel, victim, damageSource))) {
         ...
         switch (effect.affected()) {
            case ATTACKER:
               var10000 = damageSource.getEntity();
               break;
            case DAMAGING_ENTITY:
               var10000 = damageSource.getDirectEntity();
               break;
            case VICTIM:
               var10000 = victim;
               break;
   ...
   private static void applyEffects(final List effects, final LootContext filterData, final Enchantment.GenericAction action) {
   ...
   private static void applyEffects(final List effects, final LootContext filterData, final MutableFloat value, final Enchantment.FloatAction action) {
   ...
   public void runLocationChangedEffects(final ServerLevel serverLevel, final int enchantmentLevel, final EnchantedItemInUse item, final LivingEntity entity) {
      ...
      if (slot != null) {
         Map activeLocationDependentEffects = entity.activeLocationDependentEnchantments(slot);
         if (!this.matchingSlot(slot)) {
            Set activeEffects = (Set)activeLocationDependentEffects.remove(this);
            if (activeEffects != null) {
               activeEffects.forEach((effectx) -> effectx.onDeactivated(item, entity, entity.position(), enchantmentLevel));
            }

         } else {
            Set activeEffects = (Set)activeLocationDependentEffects.get(this);
   ...
   public void stopLocationBasedEffects(final int enchantmentLevel, final EnchantedItemInUse item, final LivingEntity entity) {
      EquipmentSlot slot = item.inSlot();
      if (slot != null) {
         Set activeEffects = (Set)entity.activeLocationDependentEnchantments(slot).remove(this);
         if (activeEffects != null) {
   ...
   public static class Builder {
      ...
      public Enchantment.Builder exclusiveWith(final HolderSet set) {
         this.exclusiveSet = set;
         return this;
      }

      public Enchantment.Builder withEffect(final DataComponentType type, final Object effect, final LootItemCondition.Builder condition) {
         this.getEffectsList(type).add(new ConditionalEffect(effect, Optional.of(Holder.direct(condition.build()))));
         return this;
      }

      public Enchantment.Builder withEffect(final DataComponentType type, final Object effect) {
         this.getEffectsList(type).add(new ConditionalEffect(effect, Optional.empty()));
         return this;
      }

      public Enchantment.Builder withEffect(final DataComponentType type, final EnchantmentTarget enchanted, final EnchantmentTarget affected, final Object effect, final LootItemCondition.Builder condition) {
      ...
      public Enchantment.Builder withEffect(final DataComponentType type, final EnchantmentTarget enchanted, final EnchantmentTarget affected, final Object effect) {
         this.getEffectsList(type).add(new TargetedConditionalEffect(enchanted, affected, effect, Optional.empty()));
         return this;
      }

      public Enchantment.Builder withEffect(final DataComponentType type, final EnchantmentAttributeEffect effect) {
         this.getEffectsList(type).add(effect);
         return this;
      }

      public Enchantment.Builder withSpecialEffect(final DataComponentType type, final Object effect) {
         this.effectMapBuilder.set(type, effect);
         return this;
      }

      public Enchantment.Builder withEffect(final DataComponentType type) {
   ...
   public static record Cost(int base, int perLevelAboveFirst) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.INT.fieldOf("base").forGetter(Enchantment.Cost::base), Codec.INT.fieldOf("per_level_above_first").forGetter(Enchantment.Cost::perLevelAboveFirst)).apply(i, Enchantment.Cost::new));
   ...
   public static record EnchantmentDefinition(HolderSet supportedItems, Optional primaryItems, int weight, int maxLevel, Enchantment.Cost minCost, Enchantment.Cost maxCost, int anvilCost, List slots) {
      public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(RegistryCodecs.holderSet(Registries.ITEM).fieldOf("supported_items").forGetter(Enchantment.EnchantmentDefinition::supportedItems), RegistryCodecs.holderSet(Registries.ITEM).optionalFieldOf("primary_items").forGetter(Enchantment.EnchantmentDefinition::primaryItems), ExtraCodecs.intRange(1, 1024).fieldOf("weight").forGetter(Enchantment.EnchantmentDefinition::weight), ExtraCodecs.intRange(1, 255).fieldOf("max_level").forGetter(Enchantment.EnchantmentDefinition::maxLevel), Enchantment.Cost.CODEC.fieldOf("min_cost").forGetter(Enchantment.EnchantmentDefinition::minCost), Enchantment.Cost.CODEC.fieldOf("max_cost").forGetter(Enchantment.EnchantmentDefinition::maxCost), ExtraCodecs.NON_NEGATIVE_INT.fieldOf("anvil_cost").forGetter(Enchantment.EnchantmentDefinition::anvilCost), EquipmentSlotGroup.CODEC.listOf().fieldOf("slots").forGetter(Enchantment.EnchantmentDefinition::slots)).apply(i, Enchantment.EnchantmentDefinition::new));
   }
   ...
   private interface FloatAction {
      ...
      default Enchantment.GenericAction asGeneric(final MutableFloat v) {
