...
public class ColorSwatchButton extends Button {
    ...
    public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        // Render color swatch instead of default button background
        int swatchX = this.x;
        int swatchY = this.y;
        ...
        if (!this.active) {
            // Darken the color when disabled/locked
            int r = (color >> 16) & 0xFF;
            int g = (color >> 8) & 0xFF;
            int b = color & 0xFF;
            // Darken by 60%
        ...
        // Draw border (darker when disabled)
        ...
        // Highlight selected swatch (only if active)
        if (isSelected && this.active) {
            // Draw selection indicator (slightly larger border)
