...
public class StrollThroughVillageGoal extends Goal {
   ...
   public void tick() {
      if (this.wantedPos != null) {
         ...
         if (navigation.isDone() && !this.wantedPos.closerToCenterThan(this.mob.position(), 10.0D)) {
            ...
            longDistanceTarget = distance.scale(0.4D).add(longDistanceTarget);
            ...
            if (!navigation.moveTo((double)pathTarget.getX(), (double)pathTarget.getY(), (double)pathTarget.getZ(), 1.0D)) {
               this.moveRandomly();
            }
         }
   ...
   private void moveRandomly() {
      ...
      this.mob.getNavigation().moveTo((double)pathTarget.getX(), (double)pathTarget.getY(), (double)pathTarget.getZ(), 1.0D);
   }
}
