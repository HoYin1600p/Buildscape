...
public class Goat extends Animal {
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.NEAREST_PLAYERS, SensorType.NEAREST_ITEMS, SensorType.NEAREST_ADULT, SensorType.HURT_BY, SensorType.FOOD_TEMPTATIONS), (var0) -> GoatAi.getActivities());
   public static final int GOAT_FALL_DAMAGE_REDUCTION = 10;
   public static final double GOAT_SCREAMING_CHANCE = 0.02D;
   public static final double UNIHORN_CHANCE = (double)0.1F;
   private static final EntityDataAccessor DATA_IS_SCREAMING_GOAT = SynchedEntityData.defineId(Goat.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor DATA_HAS_LEFT_HORN = SynchedEntityData.defineId(Goat.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor DATA_HAS_RIGHT_HORN = SynchedEntityData.defineId(Goat.class, EntityDataSerializers.BOOLEAN);
   ...
   public ItemStack createHorn() {
      ...
      TagKey key = this.isScreamingGoat() ? InstrumentTags.SCREAMING_GOAT_HORNS : InstrumentTags.REGULAR_GOAT_HORNS;
   ...
   protected void ageBoundaryReached() {
      this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.isBaby() ? 1.0D : 2.0D);
   }
   ...
   public @Nullable Goat getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Goat newGoat = (Goat)EntityTypes.GOAT.create(level, EntitySpawnReason.BREEDING);
      if (newGoat != null) {
         boolean var10000;
         label22: {
            label21: {
   ...
   protected void playEatingSound() {
      this.level().playSound((Entity)null, this, this.isScreamingGoat() ? SoundEvents.GOAT_SCREAMING_EAT : SoundEvents.GOAT_EAT, SoundSource.NEUTRAL, 1.0F, Mth.randomBetween(this.level().getRandom(), 0.8F, 1.2F));
   }
   ...
   public SpawnGroupData finalizeSpawn(final ServerLevelAccessor level, final DifficultyInstance difficulty, final EntitySpawnReason spawnReason, final @Nullable SpawnGroupData groupData) {
      ...
      this.setScreamingGoat(random.nextDouble() < 0.02D);
      this.ageBoundaryReached();
      if (!this.isBaby() && (double)random.nextFloat() < (double)0.1F) {
         EntityDataAccessor hornToRemove = random.nextBoolean() ? DATA_HAS_LEFT_HORN : DATA_HAS_RIGHT_HORN;
