...
public class DimensionArgument implements ArgumentType {
   private static final Collection EXAMPLES = (Collection)Stream.of(Level.OVERWORLD, Level.NETHER).map((key) -> key.identifier().toString()).collect(Collectors.toList());
   ...
   public static ServerLevel getDimension(final CommandContext context, final String name) throws CommandSyntaxException {
      ...
      ResourceKey key = ResourceKey.create(Registries.DIMENSION, location);
