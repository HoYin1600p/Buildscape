   ...
   public InteractionResult useOn(final UseOnContext context) {
      ...
      if (player != null && player.isFallFlying()) {
         ...
         if (level instanceof ServerLevel) {
            ...
            Projectile.spawnProjectile(new FireworkRocketEntity(level, context.getPlayer(), clickLocation.x + (double)direction.getStepX() * 0.15D, clickLocation.y + (double)direction.getStepY() * 0.15D, clickLocation.z + (double)direction.getStepZ() * 0.15D, itemStack), serverLevel, itemStack);
            itemStack.shrink(1);
         }
      ...
      if (player.isFallFlying()) {
         ...
         if (level instanceof ServerLevel) {
            ...
            if (player.dropAllLeashConnections((Player)null)) {
               level.playSound((Entity)null, player, SoundEvents.LEAD_BREAK, SoundSource.NEUTRAL, 1.0F, 1.0F);
            }
