...
public interface SelectableSlotContainer {
   ...
   private static Optional<Vec2> getRelativeHitCoordinatesForBlockFace(final BlockHitResult hitResult, final Direction blockFacing) {
      ...
      if (blockFacing != hitDirection) {
         ...
         Optional<Vec2> var10000;
         switch (hitDirection) {
            case NORTH:
               var10000 = Optional.of(new Vec2((float)(1.0D - relativeX), (float)relativeY));
               break;
            case SOUTH:
            ...
            case WEST:
               ...
               break;
            case EAST:
               var10000 = Optional.of(new Vec2((float)(1.0D - relativeZ), (float)relativeY));
               break;
            case DOWN:
            case UP:
            default:
