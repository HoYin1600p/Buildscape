...
public class InteractWithDoor {
   ...
   public static BehaviorControl create() {
      ...
      return BehaviorBuilder.create((i) -> i.group(i.present(MemoryModuleType.PATH), i.registered(MemoryModuleType.DOORS_TO_CLOSE), i.registered(MemoryModuleType.NEAREST_LIVING_ENTITIES)).apply(i, (pathMemory, doorsMemory, nearestEntities) -> (level, body, timestamp) -> {
               ...
               Optional doors = i.tryGet(doorsMemory);
   ...
   private static boolean areOtherMobsComingThroughDoor(final LivingEntity body, final BlockPos doorPos, final Optional nearestEntities) {
      return nearestEntities.isEmpty() ? false : ((List)nearestEntities.get()).stream().filter((otherMob) -> otherMob.getType() == body.getType()).filter((otherMob) -> doorPos.closerToCenterThan(otherMob.position(), 2.0D)).anyMatch((otherMob) -> isMobComingThroughDoor(otherMob.getBrain(), doorPos));
   }
   ...
   private static Optional rememberDoorToClose(final MemoryAccessor doorsMemory, final Optional doors, final ServerLevel level, final BlockPos doorPos) {
      ...
      return Optional.of((Set)doors.map((set) -> {
         ...
         Set set = Sets.newHashSet(new GlobalPos[]{globalDoorPos});
         doorsMemory.set(set);
         return set;
      }));
   }
