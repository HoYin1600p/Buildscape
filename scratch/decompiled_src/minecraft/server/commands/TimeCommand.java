...
public class TimeCommand {
   ...
   private static ArgumentBuilder addClockNodes(final CommandBuildContext context, final ArgumentBuilder node, final TimeCommand.ClockGetter clockGetter) {
   ...
   private static CompletableFuture suggestTimelines(final CommandSourceStack source, final SuggestionsBuilder builder, final Holder clock) {
      Stream timelines = source.registryAccess().lookupOrThrow(Registries.TIMELINE).listElements().filter((timeline) -> ((Timeline)timeline.value()).clock().equals(clock)).map(Holder.Reference::key);
   ...
   private static int setTotalTicks(final CommandSourceStack source, final Holder clock, final int totalTicks) throws CommandSyntaxException {
      ServerClockManager clockManager = source.getServer().clockManager();
   ...
   private static int setPaused(final CommandSourceStack source, final Holder clock, final boolean paused) throws CommandSyntaxException {
      ServerClockManager clockManager = source.getServer().clockManager();
      String clockName = clock.getRegisteredName();
   ...
   private static int setRate(final CommandSourceStack source, final Holder clock, final float rate) throws CommandSyntaxException {
      ServerClockManager clockManager = source.getServer().clockManager();
   ...
   private static Holder getDefaultClock(final CommandSourceStack source) throws CommandSyntaxException {
      Holder dimensionType = source.getLevel().dimensionTypeRegistration();
