...
public class TestInstanceBlockEditScreen extends Screen {
   ...
   private boolean sendToServer(final ServerboundTestInstanceBlockActionPacket.Action action) {
      Optional id = Optional.ofNullable(Identifier.tryParse(this.idEdit.getValue()));
      Optional key = id.map((i) -> ResourceKey.create(Registries.TEST_INSTANCE, i));
   ...
   public void setStatus(final Component status, final Optional size) {
      ...
      this.blockEntity.errorMessage().ifPresent((error) -> description.append(Component.translatable("test_instance.description.failed", Component.empty().withStyle(ChatFormatting.RED).append(error))).append("\n\n"));
