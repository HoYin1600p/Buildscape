...
public class EntitySectionStorage {
   ...
   public void forEachAccessibleNonEmptySection(final AABB bb, final AbortableIterationConsumer output) {
      int xMin = SectionPos.posToSectionCoord(bb.minX - 2.0D);
      int yMin = SectionPos.posToSectionCoord(bb.minY - 4.0D);
      int zMin = SectionPos.posToSectionCoord(bb.minZ - 2.0D);
      int xMax = SectionPos.posToSectionCoord(bb.maxX + 2.0D);
      int yMax = SectionPos.posToSectionCoord(bb.maxY + 0.0D);
      int zMax = SectionPos.posToSectionCoord(bb.maxZ + 2.0D);
      ...
      for(int x = xMin; x <= xMax; ++x) {
         ...
         while(it.hasNext()) {
            ...
            if (y >= yMin && y <= yMax && z >= zMin && z <= zMax) {
               EntitySection entitySection = (EntitySection)this.sections.get(sectionKey);
