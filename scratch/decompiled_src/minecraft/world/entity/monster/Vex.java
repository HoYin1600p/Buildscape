...
public class Vex extends Monster implements TraceableEntity, OwnableEntity {
   ...
   private class VexChargeAttackGoal extends Goal {
      ...
      public boolean canUse() {
         ...
         if (target != null && target.isAlive() && !Vex.this.getMoveControl().hasWanted() && Vex.this.random.nextInt(reducedTickDelay(7)) == 0) {
            return Vex.this.distanceToSqr(target) > 4.0D;
         } else {
            return false;
         }
      }
   ...
   private class VexMoveControl extends MoveControl {
      public VexMoveControl(final Mob vex) {
         Objects.requireNonNull(Vex.this);
         super(vex);
      }
      ...
      public void tick() {
         if (this.operation == MoveControl.Operation.MOVE_TO) {
            ...
            if (deltaLength < Vex.this.getBoundingBox().getSize()) {
               ...
               Vex.this.setDeltaMovement(Vex.this.getDeltaMovement().add(delta.scale(this.speedModifier * 0.05D / deltaLength)));
      ...
      public void tick() {
         ...
         for(int attempts = 0; attempts < 3; ++attempts) {
            ...
            if (Vex.this.level().isEmptyBlock(testPos)) {
               Vex.this.moveControl.setWantedPosition((double)testPos.getX() + 0.5D, (double)testPos.getY() + 0.5D, (double)testPos.getZ() + 0.5D, 0.25D);
               if (Vex.this.getTarget() == null) {
                  Vex.this.getLookControl().setLookAt((double)testPos.getX() + 0.5D, (double)testPos.getY() + 0.5D, (double)testPos.getZ() + 0.5D, 180.0F, 20.0F);
               }
               break;
            }
         }
