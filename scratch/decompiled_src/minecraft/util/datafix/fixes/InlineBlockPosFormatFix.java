...
public class InlineBlockPosFormatFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      List blockAttachedFinders = List.of(this.entityFinder("minecraft:item_frame"), this.entityFinder("minecraft:glow_item_frame"), this.entityFinder("minecraft:painting"), this.entityFinder("minecraft:leash_knot"));
   ...
   private Dynamic fixPlayer(Dynamic tag) {
      ...
      Optional enteredNetherPos = tag.get("enteredNetherPosition").result();
      if (enteredNetherPos.isPresent()) {
         tag = tag.remove("enteredNetherPosition").set("entered_nether_pos", tag.createList(Stream.of(tag.createDouble(((Dynamic)enteredNetherPos.get()).get("x").asDouble(0.0D)), tag.createDouble(((Dynamic)enteredNetherPos.get()).get("y").asDouble(0.0D)), tag.createDouble(((Dynamic)enteredNetherPos.get()).get("z").asDouble(0.0D)))));
      }
