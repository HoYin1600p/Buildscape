...
public class GuardianRenderer extends MobRenderer {
      ...
      if (super.shouldRender(entity, culler, camX, camY, camZ, partialTicks)) {
         ...
         if (entity.hasActiveAttackTarget()) {
            ...
            if (lookAtEntity != null) {
               Vec3 targetPos = this.getPosition(lookAtEntity, (double)lookAtEntity.getBbHeight() * 0.5D, 1.0F);
   ...
   public void extractRenderState(final Guardian entity, final GuardianRenderState state, final float partialTicks) {
      ...
      if (targetEntity != null) {
         ...
         state.attackTargetPosition = this.getPosition(targetEntity, (double)targetEntity.getBbHeight() * 0.5D, partialTicks);
      } else {
