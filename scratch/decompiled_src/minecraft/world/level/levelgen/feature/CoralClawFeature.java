...
public record CoralClawFeature(Holder feature) implements Feature {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(PlacedFeature.CODEC.fieldOf("feature").forGetter(CoralClawFeature::feature)).apply(i, CoralClawFeature::new));
   ...
   public boolean place(final WorldGenLevel level, final ChunkGenerator chunkGenerator, final RandomSource random, final BlockPos origin) {
      if (!((PlacedFeature)this.feature.value()).place(level, chunkGenerator, random, origin)) {
         ...
         List possibleDirections = Util.toShuffledList(Stream.of(clawDirection, clawDirection.getClockWise(), clawDirection.getCounterClockWise()), random);
