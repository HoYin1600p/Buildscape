...
public class CreateWorldScreen extends Screen {
   ...
   public static void openFresh(final Minecraft minecraft, final Runnable onClose, final CreateWorldCallback createWorld) {
      ...
      Function worldGenSettings = (context) -> new WorldGenSettings(WorldOptions.defaultWithRandomSeed(), WorldPresets.createNormalWorldDimensions(context.datapackWorldRegistries()));
   ...
   public static void testWorld(final Minecraft minecraft, final Runnable onClose) {
      ...
      Function worldGenSettings = (context) -> new WorldGenSettings(WorldOptions.testWorldWithRandomSeed(), WorldPresets.createTestWorldDimensions(context.datapackWorldRegistries()));
   ...
   private static void openCreateWorldScreen(final Minecraft minecraft, final Runnable onClose, final Function worldGenSettings, final WorldCreationContextMapper worldCreationContext, final ResourceKey worldPreset, final CreateWorldCallback createWorld) {
      ...
      CompletableFuture loadResult = WorldLoader.load(loadConfig, (context) -> new WorldLoader.DataLoadOutput(new DataPackReloadCookie((WorldGenSettings)worldGenSettings.apply(context), context.dataConfiguration()), context.datapackDimensions()), (resources, managers, registries, cookie) -> {
         resources.close();
   ...
   protected void init() {
      this.tabNavigationBar = MenuTabBar.builder(this.tabManager, this.width).addTabs(new CreateWorldScreen.GameTab(this), new CreateWorldScreen.WorldTab(), new CreateWorldScreen.MoreTab()).build();
   ...
   private void onCreate() {
      ...
      LayeredRegistryAccess finalLayers = context.worldgenRegistries().replaceFrom(RegistryLayer.DIMENSIONS, finalDimensions.dimensionsRegistryAccess());
   ...
   private void createWorldAndCleanup(final LayeredRegistryAccess finalLayers, final LevelDataAndDimensions.WorldDataAndGenSettings worldDataAndGenSettings, final Optional gameRules) {
   ...
   private boolean createNewWorld(final LayeredRegistryAccess finalLayers, final LevelDataAndDimensions.WorldDataAndGenSettings worldDataAndGenSettings, final Optional gameRules) {
      String worldFolder = this.uiState.getTargetFolder();
      WorldCreationContext context = this.uiState.getSettings();
      queueLoadScreen(this.minecraft, PREPARING_WORLD_DATA);
      Optional newWorldAccess = createNewWorldDirectory(this.minecraft, worldFolder, this.tempDataPackDir);
   ...
   private void openExperimentsScreen(final WorldDataConfiguration dataConfiguration) {
      Pair settings = this.getDataPackSelectionSettings(dataConfiguration);
      if (settings != null) {
   ...
   private void tryApplyNewDataPacks(final PackRepository packRepository, final boolean isDataPackScreen, final Consumer onAbort) {
      List newEnabled = ImmutableList.copyOf(packRepository.getSelectedIds());
      List newDisabled = (List)packRepository.getAvailableIds().stream().filter((id) -> !newEnabled.contains(id)).collect(ImmutableList.toImmutableList());
   ...
   private void applyNewPackConfig(final PackRepository packRepository, final WorldDataConfiguration newConfig, final Consumer onAbort) {
      ...
      WorldLoader.load(config, (context) -> {
         if (context.datapackWorldRegistries().lookupOrThrow(Registries.WORLD_PRESET).listElements().findAny().isEmpty()) {
            ...
            DynamicOps writeOps = existingContext.worldgenLoadContext().createSerializationContext(JsonOps.INSTANCE);
            DataResult encoded = WorldGenSettings.CODEC.encodeStart(writeOps, new WorldGenSettings(existingContext.options(), existingContext.selectedDimensions())).setLifecycle(Lifecycle.stable());
            DynamicOps readOps = context.datapackWorldRegistries().createSerializationContext(JsonOps.INSTANCE);
         ...
         settings.validate();
         return settings;
      }).thenAcceptAsync(this.uiState::setSettings, this.minecraft).handleAsync((nothing, throwable) -> {
         if (throwable != null) {
   ...
   private void removeTempDataPackDir() {
      if (this.tempDataPackDir != null && Files.exists(this.tempDataPackDir, new LinkOption[0])) {
         try {
            Stream files = Files.walk(this.tempDataPackDir);
   ...
   private static Optional createNewWorldDirectory(final Minecraft minecraft, final String worldFolder, final @Nullable Path tempDataPackDir) {
      try {
         ...
         try {
            Stream files = Files.walk(tempDataPackDir);
   ...
   public static Path createTempDataPackDirFromExistingWorld(final Path sourcePackDir, final Minecraft minecraft) {
      MutableObject tempDataPackDir = new MutableObject();

      try {
         Stream dataPackContents = Files.walk(sourcePackDir);
   ...
   private class GameTab extends GridLayoutTab {
      ...
      private GameTab(final CreateWorldScreen this$0) {
         ...
         this.nameEdit = new EditBox(this$0.font, 208, 20, Component.translatable("selectWorld.enterName"));
         this.nameEdit.setValue(this$0.uiState.getName());
         this.nameEdit.setResponder(this$0.uiState::setName);
         this$0.uiState.addListener((uiState) -> this.nameEdit.setTooltip(Tooltip.create(Component.translatable("selectWorld.targetFolder", Component.literal(uiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)))));
         this$0.setInitialFocus(this.nameEdit);
         helper.addChild(CommonLayouts.labeledElement(this$0.font, this.nameEdit, CreateWorldScreen.NAME_LABEL), helper.newCellSettings().alignHorizontallyCenter());
         CycleButton gameModeButton = (CycleButton)helper.addChild(CycleButton.builder((selectedGameMode) -> selectedGameMode.displayName, this$0.uiState.getGameMode()).withValues(WorldCreationUiState.SelectedGameMode.SURVIVAL, WorldCreationUiState.SelectedGameMode.HARDCORE, WorldCreationUiState.SelectedGameMode.CREATIVE).create(0, 0, 210, 20, CreateWorldScreen.GAME_MODEL_LABEL, (button, gameMode) -> this$0.uiState.setGameMode(gameMode)), buttonLayoutSettings);
         this$0.uiState.addListener((data) -> {
         ...
         CycleButton difficultyButton = (CycleButton)helper.addChild(CycleButton.builder(Difficulty::getDisplayName, this$0.uiState.getDifficulty()).withValues(Difficulty.values()).create(0, 0, 210, 20, Component.translatable("options.difficulty"), (button, value) -> this$0.uiState.setDifficulty(value)), buttonLayoutSettings);
         this$0.uiState.addListener((d) -> {
         ...
         CycleButton allowCommandsButton = (CycleButton)helper.addChild(CycleButton.onOffBuilder(this$0.uiState.isAllowCommands()).withTooltip((state) -> Tooltip.create(CreateWorldScreen.ALLOW_COMMANDS_INFO)).create(0, 0, 210, 20, ALLOW_COMMANDS, (b, state) -> this$0.uiState.setAllowCommands(state)));
         this$0.uiState.addListener((d) -> {
         ...
         if (!SharedConstants.getCurrentVersion().stable()) {
            helper.addChild(Button.builder(CreateWorldScreen.EXPERIMENTS_LABEL, (button) -> this$0.openExperimentsScreen(this$0.uiState.getSettings().dataConfiguration())).width(210).build());
         }
   ...
   private class WorldTab extends GridLayoutTab {
      ...
      private WorldTab() {
         ...
         CycleButton typeButton = (CycleButton)helper.addChild(CycleButton.builder(WorldCreationUiState.WorldTypeEntry::describePreset, CreateWorldScreen.this.uiState.getWorldType()).withValues(this.createWorldTypeValueSupplier()).create(0, 0, 150, 20, Component.translatable("selectWorld.mapType"), (button, newPreset) -> CreateWorldScreen.this.uiState.setWorldType(newPreset)));
         ...
         switchGridBuilder.addSwitch(GENERATE_STRUCTURES, CreateWorldScreen.this.uiState::isGenerateStructures, CreateWorldScreen.this.uiState::setGenerateStructures).withIsActiveCondition(() -> !CreateWorldScreen.this.uiState.isDebug()).withInfo(GENERATE_STRUCTURES_INFO);
         switchGridBuilder.addSwitch(BONUS_CHEST, CreateWorldScreen.this.uiState::isBonusChest, CreateWorldScreen.this.uiState::setBonusChest).withIsActiveCondition(() -> !CreateWorldScreen.this.uiState.isHardcore() && !CreateWorldScreen.this.uiState.isDebug());
