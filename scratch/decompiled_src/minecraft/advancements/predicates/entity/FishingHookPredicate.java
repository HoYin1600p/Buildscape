...
public record FishingHookPredicate(Optional inOpenWater) implements EntitySubPredicate {
   ...
   public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.BOOL.optionalFieldOf("in_open_water").forGetter(FishingHookPredicate::inOpenWater)).apply(i, FishingHookPredicate::new));
   ...
   public boolean matches(final Entity entity, final ServerLevel level, final @Nullable Vec3 position) {
      if (this.inOpenWater.isEmpty()) {
         ...
         return this.inOpenWater.get() == hook.isOpenWaterFishing();
      } else {
         return false;
      }
   }
