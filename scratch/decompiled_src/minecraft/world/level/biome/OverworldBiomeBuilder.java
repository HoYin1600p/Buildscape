...
public final class OverworldBiomeBuilder {
   ...
   private void addDebugBiomes(final Consumer biomes) {
      ...
      SplineFunction.Coordinate continents = new SplineFunction.Coordinate(new DensityFunctions.HolderHolder(densityFunctions.getOrThrow((ResourceKey)NoiseRouterData.OVERWORLD_FUNCTIONS.continents())));
      SplineFunction.Coordinate erosion = new SplineFunction.Coordinate(new DensityFunctions.HolderHolder(densityFunctions.getOrThrow((ResourceKey)NoiseRouterData.OVERWORLD_FUNCTIONS.erosion())));
      ...
      if (erosionOffsetSpline instanceof CubicSpline.Multipoint multipoint) {
         ResourceKey biome = Biomes.DESERT;
   ...
   private void addPeaks(final Consumer biomes, final Climate.Parameter weirdness) {
      for(int temperatureIndex = 0; temperatureIndex < this.temperatures.length; ++temperatureIndex) {
         ...
         for(int humidityIndex = 0; humidityIndex < this.humidities.length; ++humidityIndex) {
            ...
            ResourceKey middleBiome = this.pickMiddleBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey middleBiomeOrBadlandsIfHot = this.pickMiddleBiomeOrBadlandsIfHot(temperatureIndex, humidityIndex, weirdness);
            ResourceKey middleBiomeOrBadlandsIfHotOrSlopeIfCold = this.pickMiddleBiomeOrBadlandsIfHotOrSlopeIfCold(temperatureIndex, humidityIndex, weirdness);
            ResourceKey plateauBiome = this.pickPlateauBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey shatteredBiome = this.pickShatteredBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey shatteredBiomeOrWindsweptSavanna = this.maybePickWindsweptSavannaBiome(temperatureIndex, humidityIndex, weirdness, shatteredBiome);
            ResourceKey peakBiome = this.pickPeakBiome(temperatureIndex, humidityIndex, weirdness);
   ...
   private void addHighSlice(final Consumer biomes, final Climate.Parameter weirdness) {
      for(int temperatureIndex = 0; temperatureIndex < this.temperatures.length; ++temperatureIndex) {
         ...
         for(int humidityIndex = 0; humidityIndex < this.humidities.length; ++humidityIndex) {
            ...
            ResourceKey plateauBiome = this.pickPlateauBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey shatteredBiome = this.pickShatteredBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey middleBiomeOrWindsweptSavanna = this.maybePickWindsweptSavannaBiome(temperatureIndex, humidityIndex, weirdness, middleBiome);
            ResourceKey slopeBiome = this.pickSlopeBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey peakBiome = this.pickPeakBiome(temperatureIndex, humidityIndex, weirdness);
   ...
   private void addMidSlice(final Consumer biomes, final Climate.Parameter weirdness) {
      ...
      for(int temperatureIndex = 0; temperatureIndex < this.temperatures.length; ++temperatureIndex) {
         ...
         for(int humidityIndex = 0; humidityIndex < this.humidities.length; ++humidityIndex) {
            ...
            ResourceKey shatteredBiome = this.pickShatteredBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey plateauBiome = this.pickPlateauBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey beachBiome = this.pickBeachBiome(temperatureIndex, humidityIndex);
            ResourceKey middleBiomeOrWindsweptSavanna = this.maybePickWindsweptSavannaBiome(temperatureIndex, humidityIndex, weirdness, middleBiome);
            ResourceKey shatteredCoastBiome = this.pickShatteredCoastBiome(temperatureIndex, humidityIndex, weirdness);
            ResourceKey slopeBiome = this.pickSlopeBiome(temperatureIndex, humidityIndex, weirdness);
   ...
   private ResourceKey pickMiddleBiome(final int temperatureIndex, final int humidityIndex, final Climate.Parameter weirdness) {
      if (weirdness.max() < 0L) {
         ...
         ResourceKey variant = this.MIDDLE_BIOMES_VARIANT[temperatureIndex][humidityIndex];
   ...
   private ResourceKey pickShatteredCoastBiome(final int temperatureIndex, final int humidityIndex, final Climate.Parameter weirdness) {
      ResourceKey beachOrMiddleBiome = weirdness.max() >= 0L ? this.pickMiddleBiome(temperatureIndex, humidityIndex, weirdness) : this.pickBeachBiome(temperatureIndex, humidityIndex);
   ...
   private ResourceKey pickPlateauBiome(final int temperatureIndex, final int humidityIndex, final Climate.Parameter weirdness) {
      if (weirdness.max() >= 0L) {
         ResourceKey variant = this.PLATEAU_BIOMES_VARIANT[temperatureIndex][humidityIndex];
         if (variant != null) {
            return variant;
         }
      }
   ...
   private ResourceKey pickShatteredBiome(final int temperatureIndex, final int humidityIndex, final Climate.Parameter weirdness) {
      ResourceKey biome = this.SHATTERED_BIOMES[temperatureIndex][humidityIndex];
