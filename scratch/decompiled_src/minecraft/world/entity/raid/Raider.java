...
public abstract class Raider extends PatrollingMonster {
   ...
   private static final Predicate ALLOWED_ITEMS = (e) -> !e.hasPickUpDelay() && e.isAlive() && ItemStack.matches(e.getItem(), Raid.getOminousBannerInstance(e.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
   ...
   public void aiStep() {
      Level currentRaid = this.level();
      if (currentRaid instanceof ServerLevel level) {
         if (this.isAlive()) {
   ...
   public void die(final DamageSource source) {
      Level killer = this.level();
      if (killer instanceof ServerLevel serverLevel) {
   ...
   protected static class HoldGroundAttackGoal extends Goal {
      ...
      public void start() {
         ...
         for(Raider entity : getServerLevel(this.mob).getNearbyEntities(Raider.class, this.shoutTargeting, this.mob, this.mob.getBoundingBox().inflate(8.0D, 8.0D, 8.0D))) {
   ...
   public class ObtainRaidLeaderBannerGoal extends Goal {
      ...
      public boolean canUse() {
         if (this.cannotPickUpBanner()) {
            ...
            for(ItemEntity banner : this.mob.level().getEntitiesOfClass(ItemEntity.class, this.mob.getBoundingBox().inflate(followRange, 8.0D, followRange), Raider.ALLOWED_ITEMS)) {
   ...
   private static class RaiderMoveThroughVillageGoal extends Goal {
      ...
      private boolean hasSuitablePoi() {
         ...
         Optional homePos = level.getPoiManager().getRandom((p) -> p.is(PoiTypes.HOME), this::hasNotVisited, PoiManager.Occupancy.ANY, pos, 48, this.raider.random);
         if (homePos.isEmpty()) {
            return false;
         } else {
