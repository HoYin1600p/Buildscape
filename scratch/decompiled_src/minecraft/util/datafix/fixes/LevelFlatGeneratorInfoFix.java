...
public class LevelFlatGeneratorInfoFix extends DataFix {
   ...
   private Dynamic fix(final Dynamic input) {
      return input.get("generatorName").asString("").equalsIgnoreCase("flat") ? input.update("generatorOptions", (options) -> (Dynamic)DataFixUtils.orElse(options.asString().map(this::fixString).map(options::createString).result(), options)) : input;
   }
   ...
   String fixString(final String generatorOptions) {
      if (generatorOptions.isEmpty()) {
         ...
         Iterator parts = SPLITTER.split(generatorOptions).iterator();
         ...
         if (version >= 0 && version <= 3) {
            ...
            result.append((String)StreamSupport.stream(LAYER_SPLITTER.split(layerInfo).spliterator(), false).map((layerString) -> {
               List list = heightSplitter.splitToList(layerString);
               int height;
               String layerType;
               if (list.size() == 2) {
               ...
               List layerParts = BLOCK_SPLITTER.splitToList(layerType);
