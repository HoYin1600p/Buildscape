...
public record RuleBasedStateProvider(@Nullable BlockStateProvider fallback, List rules) implements BlockStateProvider {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(BlockStateProvider.CODEC.optionalFieldOf("fallback").forGetter((provider) -> Optional.ofNullable(provider.fallback)), RuleBasedStateProvider.Rule.CODEC.listOf().fieldOf("rules").forGetter((p) -> p.rules)).apply(i, RuleBasedStateProvider::new));

   private RuleBasedStateProvider(final Optional fallback, final List rules) {
   ...
   public static RuleBasedStateProvider.Builder builder() {
      return new RuleBasedStateProvider.Builder((BlockStateProvider)null);
   }
   ...
   public static record Rule(BlockPredicate ifTrue, BlockStateProvider then) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(BlockPredicate.CODEC.fieldOf("if_true").forGetter(RuleBasedStateProvider.Rule::ifTrue), BlockStateProvider.CODEC.fieldOf("then").forGetter(RuleBasedStateProvider.Rule::then)).apply(i, RuleBasedStateProvider.Rule::new));
   }
}
