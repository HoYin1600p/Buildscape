...
public class ChatComponent {
   ...
   private Predicate visibleMessageFilter = (var0) -> true;
   ...
   public void setVisibleMessageFilter(final Predicate visibleMessageFilter) {
   ...
   public void extractRenderState(final GuiGraphicsExtractor graphics, final Font font, final int ticks, final int mouseX, final int mouseY, final ChatComponent.DisplayMode displayMode, final boolean changeCursorOnInsertions) {
      graphics.pose().pushMatrix();
      this.extractRenderState((ChatComponent.ChatGraphicsAccess)(displayMode.foreground ? new ChatComponent.DrawingFocusedGraphicsAccess(graphics, font, mouseX, mouseY, changeCursorOnInsertions) : new ChatComponent.DrawingBackgroundGraphicsAccess(graphics)), graphics.guiHeight(), ticks, displayMode);
      graphics.pose().popMatrix();
   }

   public void captureClickableText(final ActiveTextCollector activeTextCollector, final int screenHeight, final int ticks, final ChatComponent.DisplayMode displayMode) {
   ...
   private void extractRenderState(final ChatComponent.ChatGraphicsAccess graphics, final int screenHeight, final int ticks, final ChatComponent.DisplayMode displayMode) {
      ...
      if (total > 0 || isRestricted) {
         ...
         final int entryHeight = (int)((double)messageHeight * (chatLineSpacing + 1.0D));
         final int entryBottomToMessageY = (int)Math.round(8.0D * (chatLineSpacing + 1.0D) - 4.0D * chatLineSpacing);
   ...
   private void addMessageToDisplayQueue(final GuiMessage message) {
      ...
      List lines = message.splitLines(this.minecraft.font, maxWidth);
   ...
   private ChatComponent.@Nullable DelayedMessageDeletion deleteMessageOrDelay(final MessageSignature signature) {
      int time = this.minecraft.gui.hud.getGuiTicks();
      ListIterator iterator = this.allMessages.listIterator();
   ...
   public static int getWidth(final double pct) {
      int max = 320;
      int min = 40;
      return Mth.floor(pct * 280.0D + 40.0D);
   }

   public static int getHeight(final double pct) {
      int max = 180;
      int min = 20;
      return Mth.floor(pct * 160.0D + 20.0D);
   }
   ...
   public ChatScreen createScreen(final ChatComponent.ChatMethod chatMethod, final ChatScreen.ChatConstructor chat) {
   ...
   private interface AlphaCalculator {
      ...
      static ChatComponent.AlphaCalculator timeBased(final int currentTickTime) {
         return (message) -> {
            ...
            double t = (double)tickDelta / 200.0D;
            t = 1.0D - t;
            t *= 10.0D;
   ...
   public static enum DisplayMode {
      ...
      // $FF: synthetic method
      private static ChatComponent.DisplayMode[] $values() {
         return new ChatComponent.DisplayMode[]{BACKGROUND, FOREGROUND, FOREGROUND_RESTRICTED};
      }
   }
   ...
   private static class DrawingFocusedGraphicsAccess implements ChatComponent.ChatGraphicsAccess, Consumer {
      private final GuiGraphicsExtractor graphics;
