...
public class TrialSpawnerConfigs {
   ...
   private static SpawnData customSpawnDataWithEquipment(final EntityType type, final Consumer tagModifier, final @Nullable ResourceKey equipmentLootTable) {
      ...
      Optional table = Optional.ofNullable(equipmentLootTable).map((lootTable) -> new EquipmentTable(lootTable, 0.0F));
      return new SpawnData(tag, Optional.empty(), table);
   }

   private static void register(final BootstrapContext context, final TrialSpawnerConfigs.Keys keys, final TrialSpawnerConfig normalConfig, final TrialSpawnerConfig ominousConfig) {
