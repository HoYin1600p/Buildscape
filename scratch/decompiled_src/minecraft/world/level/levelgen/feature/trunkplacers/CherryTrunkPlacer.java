...
public class CherryTrunkPlacer extends TrunkPlacer {
   private static final Codec BRANCH_START_CODEC = UniformInt.MAP_CODEC.codec().validate((u) -> u.maxInclusive() - u.minInclusive() < 1 ? DataResult.error(() -> "Need at least 2 blocks variation for the branch starts to fit both branches") : DataResult.success(u));
   ...
   public List placeTrunk(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int treeHeight, final BlockPos origin, final TreeFeature tree) {
      ...
      Function sidewaysStateModifier = (state) -> (BlockState)state.trySetValue(RotatedPillarBlock.AXIS, treeDirection.getAxis());
   ...
   private FoliagePlacer.FoliageAttachment generateBranch(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int treeHeight, final BlockPos origin, final TreeFeature tree, final Function sidewaysStateModifier, final Direction branchDirection, final int offsetFromOrigin, final boolean middleContinuesUpwards, final BlockPos.MutableBlockPos logPos) {
