...
public class Cat extends TamableAnimal {
   ...
   private static final EntityDataAccessor DATA_VARIANT_ID = SynchedEntityData.defineId(Cat.class, EntityDataSerializers.CAT_VARIANT);
   ...
   private static final EntityDataAccessor DATA_SOUND_VARIANT_ID = SynchedEntityData.defineId(Cat.class, EntityDataSerializers.CAT_SOUND_VARIANT);
   private static final ResourceKey DEFAULT_VARIANT = CatVariants.BLACK;
   ...
   private Cat.@Nullable CatAvoidEntityGoal avoidPlayersGoal;
   ...
   protected void registerGoals() {
      this.temptGoal = new Cat.CatTemptGoal(this, 0.6D, (i) -> i.is(ItemTags.CAT_FOOD), true);
      this.goalSelector.addGoal(1, new FloatGoal(this));
      this.goalSelector.addGoal(1, new TamableAnimal.TamableAnimalPanicGoal(1.5D));
      this.goalSelector.addGoal(2, new SitWhenOrderedToGoal(this));
      this.goalSelector.addGoal(3, new Cat.CatRelaxOnOwnerGoal(this));
      this.goalSelector.addGoal(4, this.temptGoal);
      this.goalSelector.addGoal(5, new CatLieOnBlockGoal(this, 1.1D, 8));
   ...
   protected void defineSynchedData(final SynchedEntityData.Builder entityData) {
      ...
      Registry catSoundVariants = this.registryAccess().lookupOrThrow(Registries.CAT_SOUND_VARIANT);
      entityData.define(DATA_VARIANT_ID, VariantUtils.getDefaultOrAny(this.registryAccess(), DEFAULT_VARIANT));
      entityData.define(DATA_SOUND_VARIANT_ID, (Holder)catSoundVariants.get(CatSoundVariants.CLASSIC).or(catSoundVariants::getAny).orElseThrow());
   ...
   private void handleLieDown() {
      ...
      if (this.isLying()) {
         ...
         for(Player player : this.level().getEntitiesOfClass(Player.class, (new AABB(catPos)).inflate(2.0D, 2.0D, 2.0D))) {
   ...
   public @Nullable Cat getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Cat baby = (Cat)EntityTypes.CAT.create(level, EntitySpawnReason.BREEDING);
   ...
   private static class CatRelaxOnOwnerGoal extends Goal {
         ...
         if (!this.cat.isTame()) {
            ...
            if (owner instanceof Player) {
               ...
               if (ownerPosState.is(BlockTags.CATS_CAN_LIE_ON)) {
                  this.goalPos = (BlockPos)ownerPosState.getOptionalValue(BedBlock.FACING).map((bedDir) -> ownerPos.relative(bedDir.getOpposite())).orElseGet(ownerPos::immutable);
      ...
      private boolean spaceIsOccupied() {
         for(Cat otherCat : this.cat.level().getEntitiesOfClass(Cat.class, (new AABB(this.goalPos)).inflate(2.0D))) {
      ...
      public void stop() {
         ...
         if (this.ownerPlayer.getSleepTimer() >= 100 && this.cat.level().getRandom().nextFloat() < this.cat.level().environmentAttributes().getValue(EnvironmentAttributes.CAT_WAKING_UP_GIFT_CHANCE, this.cat.position())) {
