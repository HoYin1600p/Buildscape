            ...
            if (node != null) {
               ...
               if (sectionPath) {
                  ...
                  for(int i = 0; i < DIRECTIONS.length; ++i) {
                     if (node.hasSourceDirection(i)) {
                        ...
                        Gizmos.line(Vec3.atLowerCornerWithOffset(renderOffset, 8.0D, 8.0D, 8.0D), Vec3.atLowerCornerWithOffset(renderOffset, (double)(8 - 16 * direction.getStepX()), (double)(8 - 16 * direction.getStepY()), (double)(8 - 16 * direction.getStepZ())), ARGB.opaque(color));
                     }
                  }
               }
               ...
               if (sectionVisibility && section.getSectionMesh().hasRenderableLayers()) {
                  ...
                  if (c > 0) {
                     float delta = 0.5F;
                     float a = 0.2F;
                     Gizmos.cuboid(section.getBoundingBox().deflate(0.5D), GizmoStyle.fill(ARGB.colorFromFloat(0.2F, 0.9F, 0.9F, 0.0F)));
                  }
               }
            }
         }
