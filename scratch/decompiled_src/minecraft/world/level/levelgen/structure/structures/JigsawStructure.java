...
public final class JigsawStructure extends Structure {
   ...
   public static record MaxDistance(int horizontal, int vertical) {
      ...
      private static final Codec FULL_CODEC = RecordCodecBuilder.create((i) -> i.group(HORIZONTAL_VALUE_CODEC.fieldOf("horizontal").forGetter(JigsawStructure.MaxDistance::horizontal), ExtraCodecs.intRange(1, DimensionType.Y_SIZE).optionalFieldOf("vertical", DimensionType.Y_SIZE).forGetter(JigsawStructure.MaxDistance::vertical)).apply(i, JigsawStructure.MaxDistance::new));
      public static final Codec CODEC = Codec.either(FULL_CODEC, HORIZONTAL_VALUE_CODEC).xmap((either) -> (JigsawStructure.MaxDistance)either.map(Function.identity(), JigsawStructure.MaxDistance::new), (distance) -> distance.horizontal == distance.vertical ? Either.right(distance.horizontal) : Either.left(distance));
