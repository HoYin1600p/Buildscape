...
public class LoomScreen extends AbstractContainerScreen {
      ...
      if (this.displayPatterns) {
         ...
         for(int row = 0; row < 4; ++row) {
            for(int column = 0; column < 4; ++column) {
               ...
               int actualRow = row + this.startRow;
               int index = actualRow * 4 + column;
               if (xx >= 0.0D && yy >= 0.0D && xx < 14.0D && yy < 14.0D && ((LoomMenu)this.menu).clickMenuButton(this.minecraft.player, index)) {
   ...
   public boolean mouseDragged(final MouseButtonEvent event, final double dx, final double dy) {
      ...
      if (this.scrolling && this.displayPatterns && offscreenRows > 0) {
         ...
         this.startRow = Math.max((int)((double)(this.scrollOffs * (float)offscreenRows) + 0.5D), 0);
         return true;
      } else {
