...
public class FireworkRocketEntity extends Projectile implements ItemSupplier {
      ...
      if (this.isAttachedToEntity()) {
         ...
         if (this.attachedToEntity != null) {
            ...
            if (this.attachedToEntity.isFallFlying()) {
               ...
               double power = 1.5D;
               double powerAdd = 0.1D;
               Vec3 movement = this.attachedToEntity.getDeltaMovement();
               this.attachedToEntity.setDeltaMovement(movement.add(lookAngle.x * 0.1D + (lookAngle.x * 1.5D - movement.x) * 0.5D, lookAngle.y * 0.1D + (lookAngle.y * 1.5D - movement.y) * 0.5D, lookAngle.z * 0.1D + (lookAngle.z * 1.5D - movement.z) * 0.5D));
         ...
         if (!this.isShotAtAngle()) {
            double horizontalAcceleration = this.horizontalCollision ? 1.0D : 1.15D;
            this.setDeltaMovement(this.getDeltaMovement().multiply(horizontalAcceleration, 1.0D, horizontalAcceleration).add(0.0D, 0.04D, 0.0D));
         }
   ...
   private void dealExplosionDamage(final ServerLevel level) {
      ...
      List explosions = this.getExplosions();
      ...
      if (damageAmount > 0.0F) {
         ...
         for(LivingEntity target : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(5.0D))) {
            if (target != this.attachedToEntity && !(this.distanceToSqr(target) > 25.0D)) {
               boolean canSee = false;

               for(int testStep = 0; testStep < 2; ++testStep) {
                  Vec3 to = new Vec3(target.getX(), target.getY(0.5D * (double)testStep), target.getZ());
               ...
               if (canSee) {
                  float damage = damageAmount * (float)Math.sqrt((5.0D - (double)this.distanceTo(target)) / 5.0D);
