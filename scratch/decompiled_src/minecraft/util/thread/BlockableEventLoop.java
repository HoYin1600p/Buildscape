...
public abstract class BlockableEventLoop implements Executor, TaskScheduler, ProfilerMeasured {
   ...
   private static volatile @Nullable Supplier delayedCrash;
   ...
   public void execute(final Runnable command) {
      Runnable task = (R)this.wrapRunnable(command);
      if (this.scheduleExecutables()) {
         this.schedule(task);
      } else {
         this.doRunTask(task);
   ...
   protected boolean pollTask() {
      ...
      Runnable task = (R)((Runnable)this.pendingRunnables.peek());
      if (task == null) {
         return false;
   ...
   private void throwDelayedException() {
      if (this.propagatesCrashes) {
         Supplier delayedCrash = BlockableEventLoop.delayedCrash;
