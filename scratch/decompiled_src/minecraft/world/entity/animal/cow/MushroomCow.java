...
public class MushroomCow extends AbstractCow implements Shearable {
   private static final EntityDataAccessor DATA_TYPE = SynchedEntityData.defineId(MushroomCow.class, EntityDataSerializers.INT);
   ...
   public InteractionResult mobInteract(final Player player, final InteractionHand hand) {
      ...
      if (itemStack.is(Items.BOWL) && !this.isBaby()) {
         ...
         Optional effectsFromItemStack = this.getEffectsFromItemStack(itemStack);
         if (effectsFromItemStack.isEmpty()) {
            ...
            if (this.stewEffects != null) {
               ...
               for(int i = 0; i < 4; ++i) {
                  this.level().addParticle(particle, this.getX() + this.random.nextDouble() / 2.0D, this.getY(0.5D), this.getZ() + this.random.nextDouble() / 2.0D, 0.0D, this.random.nextDouble() / 5.0D, 0.0D);
               }
      ...
      this.convertTo(EntityTypes.COW, ConversionParams.single(this, false, false), (cow) -> {
         ...
         this.dropFromShearingLootTable(level, BuiltInLootTables.SHEAR_MOOSHROOM, tool, (l, drop) -> {
            for(int i = 0; i < drop.getCount(); ++i) {
               l.addFreshEntity(new ItemEntity(this.level(), this.getX(), this.getY(1.0D), this.getZ(), drop.copyWithCount(1)));
            }
   ...
   public @Nullable MushroomCow getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      MushroomCow baby = (MushroomCow)EntityTypes.MOOSHROOM.create(level, EntitySpawnReason.BREEDING);
      if (baby != null) {
   ...
   public static enum Variant implements StringRepresentable {
      ...
      public static final Codec CODEC = StringRepresentable.fromEnum(MushroomCow.Variant::values);
      private static final IntFunction BY_ID = ByIdMap.continuous(MushroomCow.Variant::id, values(), ByIdMap.OutOfBoundsStrategy.CLAMP);
      public static final StreamCodec STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, MushroomCow.Variant::id);
