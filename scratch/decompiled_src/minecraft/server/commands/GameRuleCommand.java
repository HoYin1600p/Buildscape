...
public class GameRuleCommand {
   ...
   public static void register(final CommandDispatcher dispatcher, final CommandBuildContext context) {
      ...
      (new GameRules(context.enabledFeatures())).visitGameRuleTypes(new GameRuleTypeVisitor() {
         public void visit(final GameRule gameRule) {
            LiteralArgumentBuilder unqualified = Commands.literal(gameRule.id());
            LiteralArgumentBuilder qualified = Commands.literal(gameRule.getIdentifier().toString());
   ...
   private static int setRule(final CommandContext context, final GameRule gameRule) throws CommandSyntaxException {
      CommandSourceStack source = (CommandSourceStack)context.getSource();
      Object value = (T)context.getArgument("value", gameRule.valueClass());
   ...
   private static int queryRule(final CommandSourceStack source, final GameRule gameRule) {
      Object value = (T)source.getLevel().getGameRules().get(gameRule);
