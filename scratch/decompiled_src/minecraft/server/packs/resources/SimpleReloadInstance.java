...
public class SimpleReloadInstance implements ReloadInstance {
   ...
   public static ReloadInstance of(final ResourceManager resourceManager, final List listeners, final Executor taskExecutor, final Executor mainThreadExecutor, final CompletableFuture initialTask) {
      SimpleReloadInstance result = new SimpleReloadInstance(listeners);
   ...
   protected void startTasks(final Executor taskExecutor, final Executor mainThreadExecutor, final ResourceManager resourceManager, final List listeners, final SimpleReloadInstance.StateFactory stateFactory, final CompletableFuture initialTask) {
   ...
   protected CompletableFuture prepareTasks(final Executor taskExecutor, final Executor mainThreadExecutor, final ResourceManager resourceManager, final List listeners, final SimpleReloadInstance.StateFactory stateFactory, final CompletableFuture initialTask) {
      ...
      for(PreparableReloadListener listener : listeners) {
         ...
         CompletableFuture state = stateFactory.create(sharedState, barrierForCurrentTask, listener, countingTaskExecutor, countingReloadExecutor);
         allSteps.add(state);
         barrier = state;
      }
   ...
   protected interface StateFactory {
      SimpleReloadInstance.StateFactory SIMPLE = (currentReload, previousStep, listener, taskExecutor, reloadExecutor) -> listener.reload(currentReload, taskExecutor, previousStep, reloadExecutor);
