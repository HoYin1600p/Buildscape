...
public abstract class TrackedWaypoint implements Waypoint {
   ...
   private TrackedWaypoint(final Either identifier, final Waypoint.Icon icon, final TrackedWaypoint.Type type) {
   ...
   private static TrackedWaypoint read(final ByteBuf buf) {
      FriendlyByteBuf byteBuf = new FriendlyByteBuf(buf);
      Either identifier = byteBuf.readEither(UUIDUtil.STREAM_CODEC, FriendlyByteBuf::readUtf);
      Waypoint.Icon icon = (Waypoint.Icon)Waypoint.Icon.STREAM_CODEC.decode(byteBuf);
      TrackedWaypoint.Type type = (TrackedWaypoint.Type)byteBuf.readEnum(TrackedWaypoint.Type.class);
   ...
   private static class AzimuthWaypoint extends TrackedWaypoint {
      ...
      public TrackedWaypoint.PitchDirection pitchDirectionToCamera(final Level level, final TrackedWaypoint.Projector projector, final PartialTickSupplier partialTickSupplier) {
         double horizon = projector.projectHorizonToScreen();
         if (horizon < -1.0D) {
            return TrackedWaypoint.PitchDirection.DOWN;
         } else {
            return horizon > 1.0D ? TrackedWaypoint.PitchDirection.UP : TrackedWaypoint.PitchDirection.NONE;
         }
      }
   ...
   private static class ChunkWaypoint extends TrackedWaypoint {
      ...
      public TrackedWaypoint.PitchDirection pitchDirectionToCamera(final Level level, final TrackedWaypoint.Projector projector, final PartialTickSupplier partialTickSupplier) {
         ...
         if (onScreenHorizon < -1.0D) {
            return TrackedWaypoint.PitchDirection.DOWN;
         } else {
            return onScreenHorizon > 1.0D ? TrackedWaypoint.PitchDirection.UP : TrackedWaypoint.PitchDirection.NONE;
         }
      }
   ...
   private static class Vec3iWaypoint extends TrackedWaypoint {
      ...
      private Vec3 position(final Level level, final PartialTickSupplier partialTick) {
         return (Vec3)this.identifier.left().map(level::getEntity).map((e) -> e.blockPosition().distManhattan(this.vector) > 3 ? null : e.getEyePosition(partialTick.apply(e))).orElseGet(() -> Vec3.atCenterOf(this.vector));
      }
      ...
      public TrackedWaypoint.PitchDirection pitchDirectionToCamera(final Level level, final TrackedWaypoint.Projector projector, final PartialTickSupplier partialTickSupplier) {
         ...
         boolean isBehindCamera = pointOnScreen.z > 1.0D;
         double yInFrontOfCamera = isBehindCamera ? -pointOnScreen.y : pointOnScreen.y;
         if (yInFrontOfCamera < -1.0D) {
            return TrackedWaypoint.PitchDirection.DOWN;
         } else if (yInFrontOfCamera > 1.0D) {
            return TrackedWaypoint.PitchDirection.UP;
         } else {
            if (isBehindCamera) {
               if (pointOnScreen.y > 0.0D) {
                  return TrackedWaypoint.PitchDirection.UP;
               }
