...
public class CreateBuffetWorldScreen extends Screen {
   ...
   protected void init() {
      ...
      this.list = (CreateBuffetWorldScreen.BiomeList)this.layout.addToContents(biomeList);
      ...
      this.list.setSelected((CreateBuffetWorldScreen.BiomeList.Entry)this.list.children().stream().filter((e) -> Objects.equals(e.biome, this.biome)).findFirst().orElse((Object)null));
   ...
   private class BiomeList extends ObjectSelectionList {
      ...
      private void filterEntries(final String filter) {
         ...
         List list = CreateBuffetWorldScreen.this.biomes.listElements().map((x$0) -> new CreateBuffetWorldScreen.BiomeList.Entry(x$0)).sorted(Comparator.comparing((e) -> e.name.getString(), localeCollator)).filter((entry) -> filter.isEmpty() || entry.name.getString().toLowerCase(Locale.ROOT).contains(lowercaseFilter)).toList();
