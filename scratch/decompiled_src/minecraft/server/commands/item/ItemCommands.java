...
public class ItemCommands {
   ...
   public static final List ALL_PROVIDERS = List.of(EntityItemAccessor.PROVIDER, BlockItemAccessor.PROVIDER);
   public static final List TARGET_PROVIDERS = ArgProvider.buildList("target", ALL_PROVIDERS);
   public static final List SOURCE_PROVIDERS = ArgProvider.buildList("source", ALL_PROVIDERS);
   ...
   public static void register(final CommandDispatcher dispatcher, final CommandBuildContext context) {
      ...
      for(ArgProvider targetProvider : TARGET_PROVIDERS) {
         ((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)root.then(wrapSetItems(context, Commands.literal("replace"), targetProvider, ItemCommands::replaceItemProvider))).then(wrapSetItems(context, Commands.literal("fill"), targetProvider, ItemCommands::fillItemProvider))).then(wrapSetItems(context, Commands.literal("override"), targetProvider, ItemCommands::overrideItemProvider))).then(targetProvider.wrap(Commands.literal("modify"), (p) -> p.then(Commands.argument("slots", SlotSourceArgument.slotSource(context)).then(Commands.argument("modifier", ResourceOrIdArgument.lootModifier(context)).executes((c) -> modifyItems((CommandSourceStack)c.getSource(), (ItemAccessor)targetProvider.access(c), SlotSourceArgument.getSlotSource(c, "slots"), ResourceOrIdArgument.getLootModifier(c, "modifier")))))));
      }

      dispatcher.register(root);
   }

   private static ArgumentBuilder wrapSetItems(final CommandBuildContext context, final ArgumentBuilder builder, final ArgProvider targetProvider, final ItemCommands.ItemDistributor distributor) {
      return targetProvider.wrap(builder, (t) -> {
         ArgumentBuilder slotsNode = Commands.argument("slots", SlotSourceArgument.slotSource(context));

         for(ArgProvider sourceProvider : SOURCE_PROVIDERS) {
            slotsNode.then(sourceProvider.wrap(Commands.literal("from"), (p) -> p.then(((RequiredArgumentBuilder)Commands.argument("sourceSlots", SlotSourceArgument.slotSource(context)).executes((c) -> setItems((CommandSourceStack)c.getSource(), (ItemAccessor)targetProvider.access(c), SlotSourceArgument.getSlotSource(c, "slots"), distributor, getItems((CommandSourceStack)c.getSource(), (ItemAccessor)sourceProvider.access(c), SlotSourceArgument.getSlotSource(c, "sourceSlots"), (Holder)null)))).then(Commands.argument("modifier", ResourceOrIdArgument.lootModifier(context)).executes((c) -> setItems((CommandSourceStack)c.getSource(), (ItemAccessor)targetProvider.access(c), SlotSourceArgument.getSlotSource(c, "slots"), distributor, getItems((CommandSourceStack)c.getSource(), (ItemAccessor)sourceProvider.access(c), SlotSourceArgument.getSlotSource(c, "sourceSlots"), ResourceOrIdArgument.getLootModifier(c, "modifier"))))))));
         }

         slotsNode.then(Commands.literal("with").then(((RequiredArgumentBuilder)Commands.argument("item", ItemArgument.item(context)).executes((c) -> setItem((CommandSourceStack)c.getSource(), (ItemAccessor)targetProvider.access(c), SlotSourceArgument.getSlotSource(c, "slots"), distributor, ItemArgument.getItem(c, "item").createItemStack(1)))).then(Commands.argument("count", IntegerArgumentType.integer(1, 99)).executes((c) -> setItem((CommandSourceStack)c.getSource(), (ItemAccessor)targetProvider.access(c), SlotSourceArgument.getSlotSource(c, "slots"), distributor, ItemArgument.getItem(c, "item").createItemStack(IntegerArgumentType.getInteger(c, "count")))))));
         return t.then(slotsNode);
      });
   }

   public static ItemProvider replaceItemProvider(final Collection items) {
      return ItemProvider.of(items);
   }

   public static ItemProvider fillItemProvider(final Collection items) {
      return ItemProvider.cycle(items);
   }

   public static ItemProvider overrideItemProvider(final Collection items) {
      return ItemProvider.of(items).orElseProvide(ItemStack.EMPTY);
   }

   private static int setItems(final CommandSourceStack source, final ItemAccessor accessor, final SlotSourceArgument.Result slotSource, final ItemProvider items, final @Nullable ItemStack knownItem) throws CommandSyntaxException {
   ...
   private static int setItems(final CommandSourceStack source, final ItemAccessor accessor, final SlotSourceArgument.Result slotSource, final ItemCommands.ItemDistributor distributor, final Collection items) throws CommandSyntaxException {
      return setItems(source, accessor, slotSource, distributor.apply(items), (ItemStack)null);
   }

   private static int setItem(final CommandSourceStack source, final ItemAccessor accessor, final SlotSourceArgument.Result slotSource, final ItemCommands.ItemDistributor distributor, final ItemStack item) throws CommandSyntaxException {
      return setItems(source, accessor, slotSource, distributor.apply(List.of(item)), item);
   }

   private static int modifyItems(final CommandSourceStack source, final ItemAccessor accessor, final SlotSourceArgument.Result slotSource, final Holder modifier) throws CommandSyntaxException {
      MutableInt selectedCount = new MutableInt();
      SlotSelector slotSelector = SlotSelector.tracking(SlotSelector.NON_EMPTY_SLOTS, selectedCount);
      CountingModifier countingModifier = new CountingModifier(applyModifier(source, (LootItemFunction)modifier.value()));
      CommandResponseTracker tracker = CommandResponseTracker.create();
      accessor.setItems(source, slotSource.value(), (target, slots) -> {
   ...
   private static Collection getItems(final CommandSourceStack source, final ItemAccessor accessor, final SlotSourceArgument.Result slotSource, final @Nullable Holder modifier) throws CommandSyntaxException {
      SlotCollection slots = accessor.getSlots(source, slotSource.value());
      Collection items = modifier != null ? slots.itemCopies().map(applyModifier(source, (LootItemFunction)modifier.value())).toList() : slots.itemCopies().toList();
      if (items.isEmpty()) {
   ...
   private static UnaryOperator applyModifier(final CommandSourceStack source, final LootItemFunction modifier) {
   ...
   private interface ItemDistributor {
      ItemProvider apply(Collection items);
   }
}
