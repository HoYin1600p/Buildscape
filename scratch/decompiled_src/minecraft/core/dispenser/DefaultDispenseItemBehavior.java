...
public class DefaultDispenseItemBehavior implements DispenseItemBehavior {
   ...
   public static void spawnItem(final Level level, final ItemStack itemStack, final int accuracy, final Direction direction, final Position position) {
      ...
      double pow = random.nextDouble() * 0.1D + 0.2D;
      itemEntity.setDeltaMovement(random.triangle((double)direction.getStepX() * pow, 0.0172275D * (double)accuracy), random.triangle(0.2D, 0.0172275D * (double)accuracy), random.triangle((double)direction.getStepZ() * pow, 0.0172275D * (double)accuracy));
