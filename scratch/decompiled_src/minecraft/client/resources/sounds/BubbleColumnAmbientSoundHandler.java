...
public class BubbleColumnAmbientSoundHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      BlockState state = (BlockState)level.getBlockStatesIfLoaded(this.player.getBoundingBox().inflate(0.0D, (double)-0.4F, 0.0D).deflate(1.0E-6D)).filter((s) -> s.is(Blocks.BUBBLE_COLUMN)).findFirst().orElse((Object)null);
      if (state != null) {
