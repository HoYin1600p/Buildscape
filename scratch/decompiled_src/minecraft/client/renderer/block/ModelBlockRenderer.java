...
public class ModelBlockRenderer {
   ...
   private void configureTintCache(final BlockState blockState) {
      List tintSources = this.blockColors.getTintSources(blockState);
   ...
   private void tesselateAmbientOcclusion(final BlockQuadOutput output, final float x, final float y, final float z, final List parts, final BlockAndTintGetter level, final BlockState state, final BlockPos pos) {
      int cacheValid = 0;
      ...
      for(BlockStateModelPart part : parts) {
         for(Direction direction : DIRECTIONS) {
            ...
            if (!validCacheForDirection || shouldRenderFace) {
               List culledQuads = part.getQuads(direction);
   ...
   private void tesselateFlat(final BlockQuadOutput output, final float x, final float y, final float z, final List parts, final BlockAndTintGetter level, final BlockState state, final BlockPos pos) {
      int cacheValid = 0;
      int shouldRenderFaceCache = 0;
