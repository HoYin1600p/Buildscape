...
public class RangeSelectItemModel implements ItemModel {
   ...
   public static record Entry(float threshold, ItemModel.Unbaked model) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.FLOAT.fieldOf("threshold").forGetter(RangeSelectItemModel.Entry::threshold), ItemModels.CODEC.fieldOf("model").forGetter(RangeSelectItemModel.Entry::model)).apply(i, RangeSelectItemModel.Entry::new));
      public static final Comparator BY_THRESHOLD = Comparator.comparingDouble(RangeSelectItemModel.Entry::threshold);
   }

   public static record Unbaked(Optional transformation, RangeSelectItemModelProperty property, float scale, List entries, Optional fallback) implements ItemModel.Unbaked {
      public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Transformation.EXTENDED_CODEC.optionalFieldOf("transformation").forGetter(RangeSelectItemModel.Unbaked::transformation), RangeSelectItemModelProperties.MAP_CODEC.forGetter(RangeSelectItemModel.Unbaked::property), Codec.FLOAT.optionalFieldOf("scale", 1.0F).forGetter(RangeSelectItemModel.Unbaked::scale), RangeSelectItemModel.Entry.CODEC.listOf().fieldOf("entries").forGetter(RangeSelectItemModel.Unbaked::entries), ItemModels.CODEC.optionalFieldOf("fallback").forGetter(RangeSelectItemModel.Unbaked::fallback)).apply(i, RangeSelectItemModel.Unbaked::new));
      ...
      public ItemModel bake(final ItemModel.BakingContext context, final Matrix4fc transformation) {
         ...
         List mutableEntries = new ArrayList(this.entries);
