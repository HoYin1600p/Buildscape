   ...
   protected TypeRewriteRule makeRule() {
      return this.writeFixAndRead("Food to consumable fix", this.getInputSchema().getType(References.DATA_COMPONENTS), this.getOutputSchema().getType(References.DATA_COMPONENTS), (components) -> {
         ...
         if (foodComponent.isPresent()) {
            ...
            Stream onConsumeEffects = effects.map((effect) -> effect.emptyMap().set("type", effect.createString("minecraft:apply_effects")).set("effects", effect.createList(effect.get("effect").result().stream())).set("probability", effect.createFloat(effect.get("probability").asFloat(1.0F))));
