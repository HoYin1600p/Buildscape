...
public record DimensionPadding(int bottom, int top) {
   ...
   public static final Codec CODEC = Codec.either(ExtraCodecs.NON_NEGATIVE_INT, RECORD_CODEC).xmap((e) -> (DimensionPadding)e.map(DimensionPadding::new, Function.identity()), (padding) -> padding.hasEqualTopAndBottom() ? Either.left(padding.bottom) : Either.right(padding));
