...
public class LocalPlayer extends AbstractClientPlayer {
   ...
   private final ItemActivation itemActivation = new ItemActivation();
   ...
   private void sendPosition() {
      ...
      if (this.isControlledCamera()) {
         ...
         boolean move = Mth.lengthSquared(deltaX, deltaY, deltaZ) > Mth.square(2.0E-4D) || this.positionReminder >= 20;
         boolean rot = deltaYRot != 0.0D || deltaXRot != 0.0D;
         if (move && rot) {
   ...
   private void moveTowardsClosestSpace(final double x, final double z) {
      ...
      if (this.suffocatesAt(pos)) {
         ...
         for(Direction direction : directions) {
            double axisDistance = direction.getAxis().choose(xd, 0.0D, zd);
            double distanceToEdge = direction.getAxisDirection() == Direction.AxisDirection.POSITIVE ? 1.0D - axisDistance : axisDistance;
         ...
         if (dir != null) {
            ...
            if (dir.getAxis() == Direction.Axis.X) {
               this.setDeltaMovement(0.1D * (double)dir.getStepX(), oldMovement.y, oldMovement.z);
            } else {
               this.setDeltaMovement(oldMovement.x, oldMovement.y, 0.1D * (double)dir.getStepZ());
            }
         }
   ...
   private boolean suffocatesAt(final BlockPos pos) {
      ...
      AABB testArea = (new AABB((double)pos.getX(), boundingBox.minY, (double)pos.getZ(), (double)pos.getX() + 1.0D, boundingBox.maxY, (double)pos.getZ() + 1.0D)).deflate(1.0E-7D);
   ...
   public void aiStep() {
      ...
      if (abilities.flying && this.isControlledCamera()) {
         ...
         if (inputYa != 0) {
            this.setDeltaMovement(this.getDeltaMovement().add(0.0D, (double)((float)inputYa * abilities.getFlyingSpeed() * 3.0F), 0.0D));
         }
      }
   ...
   protected void updateAutoJump(final float xa, final float za) {
      if (this.canAutoJump()) {
         ...
         Vec3 moveEnd = moveBegin.add((double)xa, 0.0D, (double)za);
         Vec3 moveDiff = new Vec3((double)xa, 0.0D, (double)za);
         ...
         if (!(facingVsMovingDotProduct2 < -0.15F)) {
            ...
            if (aboveBlock1.getCollisionShape(this.level(), ceilingPos, context).isEmpty()) {
               ...
               if (aboveBlock2.getCollisionShape(this.level(), ceilingPos, context).isEmpty()) {
                  ...
                  AABB testBox = (new AABB(moveBegin, segEnd.add(0.0D, (double)playerHeight, 0.0D))).inflate((double)playerWidth, 0.0D, (double)playerWidth);
                  Vec3 segBegin = moveBegin.add(0.0D, (double)0.51F, 0.0D);
                  segEnd = segEnd.add(0.0D, (double)0.51F, 0.0D);
                  Vec3 rightDir = moveDir.cross(new Vec3(0.0D, 1.0D, 0.0D));
                  ...
                  Iterable collisions = this.level().getCollisions(this, testBox);
   ...
   public Vec3 getRopeHoldPosition(final float partialTickTime) {
      if (this.minecraft.options.getCameraType().isFirstPerson()) {
         ...
         double handDir = this.getMainArm() == HumanoidArm.RIGHT ? -1.0D : 1.0D;
         Vec3 offset = new Vec3(0.39D * handDir, -0.6D, 0.3D);
   ...
   private static HitResult pick(final Entity cameraEntity, final double blockInteractionRange, final double entityInteractionRange, final float partialTicks) {
      ...
      float overlap = 1.0F;
      AABB box = cameraEntity.getBoundingBox().expandTowards(direction.scale(maxDistance)).inflate(1.0D, 1.0D, 1.0D);
   ...
   public void setActivePostEffects(final List postEffects) {
      this.postEffects.clear();
      this.postEffects.addAll(postEffects);
   }

   public List getActivePostEffects() {
