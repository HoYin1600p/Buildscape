...
public class TranslatableContents implements ComponentContents {
   ...
   private static final Codec ARG_CODEC = Codec.either(PRIMITIVE_ARG_CODEC, ComponentSerialization.CODEC).xmap((e) -> e.map((o) -> o, (component) -> Objects.requireNonNullElse(component.tryCollapseToString(), component)), (o) -> {
      Either var10000;
   ...
   public Optional visit(final FormattedText.StyledContentConsumer output, final Style currentStyle) {
      ...
      for(FormattedText part : this.decomposedParts) {
         Optional result = part.visit(output, currentStyle);
         if (result.isPresent()) {
            return result;
         }
      }
   ...
   public Optional visit(final FormattedText.ContentConsumer output) {
      ...
      for(FormattedText part : this.decomposedParts) {
         Optional result = part.visit(output);
         if (result.isPresent()) {
            return result;
         }
      }
