...
- Added 2 custom Music Discs: `Celebration` (`music_disc_celebration`) rewarded for completing `Buildscape Museum` and `Snowfall` (`music_disc_snowfall`) rewarded for completing `A Very Buildscape Christmas` by @KyroXova
...
- Added directional fluid outflow from open pipe endpoints pouring flowing fluid into the surrounding world with seamless vanilla water flow blending and preserved flow distances by @KyroXova
...
- Fixed dedicated server crash vulnerabilities across `CascadeBlockEntity` and `PillarBlockEntity` by isolating client-only particle spawning handlers and stripping invalid `@OnlyIn(Dist.CLIENT)` annotations by @KyroXova
- Hardened network packet validation and security across `RemovePillarPacket` (operator permissions check), `HammerReplacePacket` (mayBuild & blockActionRestricted checks), `CyclePillarPatternPacket` (interaction distance & chunk validation), and `TreeChopPacket` (multi-tick scheduled break safety) by @KyroXova
- Fixed `AdvancementWidgetMixin` icon cycling to strictly target `buildscape` namespace advancements, preventing visual overrides on vanilla or third-party advancements by @KyroXova
- Fixed `ComposterBlockMixin` shape update logic so snowy grass composter planters properly revert to regular grass when the snow layer above is broken by @KyroXova
