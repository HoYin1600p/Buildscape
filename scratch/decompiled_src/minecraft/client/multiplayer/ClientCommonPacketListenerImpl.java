...
public abstract class ClientCommonPacketListenerImpl implements ClientCommonPacketListener {
   ...
   public void onPacketError(final Packet packet, final Exception cause) {
      ...
      Optional report = this.storeDisconnectionReport(packet, cause);
      Optional bugReportLink = this.serverLinks.findKnownType(ServerLinks.KnownLinkType.BUG_REPORT).map(ServerLinks.Entry::link);
   ...
   public DisconnectionDetails createDisconnectionInfo(final Component reason, final Throwable cause) {
      Optional report = this.storeDisconnectionReport((Packet)null, cause);
      Optional bugReportUrl = this.serverLinks.findKnownType(ServerLinks.KnownLinkType.BUG_REPORT).map(ServerLinks.Entry::link);
   ...
   private Optional storeDisconnectionReport(final @Nullable Packet packet, final Throwable cause) {
      ...
      List extraComments = (List)bugReportLink.map((link) -> List.of("Server bug reporting link: " + String.valueOf(link.link()))).orElse(List.of());
   ...
   private Screen addOrUpdatePackPrompt(final UUID packId, final URL url, final String hash, final boolean required, final @Nullable Component prompt) {
      ...
      if (currentScreen instanceof ClientCommonPacketListenerImpl.PackConfirmScreen promptScreen) {
         return promptScreen.update(this.minecraft, packId, url, hash, required, prompt);
      } else {
         return new ClientCommonPacketListenerImpl.PackConfirmScreen(this.minecraft, currentScreen, List.of(new ClientCommonPacketListenerImpl.PackConfirmScreen.PendingRequest(packId, url, hash)), required, prompt);
      }
   }
   ...
   private class PackConfirmScreen extends ConfirmScreen {
      ...
      public ClientCommonPacketListenerImpl.PackConfirmScreen update(final Minecraft minecraft, final UUID id, final URL url, final String hash, final boolean required, final @Nullable Component prompt) {
         List extendedRequests = ImmutableList.builderWithExpectedSize(this.requests.size() + 1).addAll(this.requests).add(new ClientCommonPacketListenerImpl.PackConfirmScreen.PendingRequest(id, url, hash)).build();
