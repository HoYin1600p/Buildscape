...
public class BlockPos extends Vec3i {
   public static final Codec CODEC = Codec.INT_STREAM.comapFlatMap((input) -> Util.fixedSize(input, 3).map((ints) -> new BlockPos(ints[0], ints[1], ints[2])), (pos) -> IntStream.of(new int[]{pos.getX(), pos.getY(), pos.getZ()})).stable();
   ...
   public static Iterable withinManhattan(final BlockPos origin, final int reach) {
      return manhattanOrdered(origin, reach, reach, reach, reach);
   }

   public static Iterable withinClippedManhattan(final BlockPos origin, final int reachX, final int reachY, final int reachZ) {
      return manhattanOrdered(origin, reachX, reachY, reachZ, Math.max(Math.max(reachX, reachY), reachZ));
   }

   public static Iterable withinBoxByManhattanDistance(final BlockPos origin, final int reachX, final int reachY, final int reachZ) {
      return manhattanOrdered(origin, reachX, reachY, reachZ, reachX + reachY + reachZ);
   }

   private static Iterable manhattanOrdered(final BlockPos origin, final int reachX, final int reachY, final int reachZ, final int maxDepth) {
   ...
   public static Iterable neighborColumn(final int startX, final int startY, final int startZ, final int endY) {
   ...
   public static Iterable betweenCornersInDirection(final int firstCornerX, final int firstCornerY, final int firstCornerZ, final int secondCornerX, final int secondCornerY, final int secondCornerZ, final Vec3 direction) {
      ...
      int startCornerX = direction.x >= 0.0D ? minCornerX : maxCornerX;
      int startCornerY = direction.y >= 0.0D ? minCornerY : maxCornerY;
      int startCornerZ = direction.z >= 0.0D ? minCornerZ : maxCornerZ;
      List axes = Direction.axisStepOrder(direction);
      ...
      Direction firstVisitDir = direction.get(firstVisitAxis) >= 0.0D ? firstVisitAxis.getPositive() : firstVisitAxis.getNegative();
      Direction secondVisitDir = direction.get(secondVisitAxis) >= 0.0D ? secondVisitAxis.getPositive() : secondVisitAxis.getNegative();
      Direction thirdVisitDir = direction.get(thirdVisitAxis) >= 0.0D ? thirdVisitAxis.getPositive() : thirdVisitAxis.getNegative();
   ...
   public static class MutableBlockPos extends BlockPos {
      ...
      public BlockPos.MutableBlockPos clamp(final Direction.Axis axis, final int minimum, final int maximum) {
         ...
         switch (axis) {
            case X:
               var10000 = this.set(Mth.clamp(this.getX(), minimum, maximum), this.getY(), this.getZ());
               break;
            case Y:
               var10000 = this.set(this.getX(), Mth.clamp(this.getY(), minimum, maximum), this.getZ());
               break;
            case Z:
               var10000 = this.set(this.getX(), this.getY(), Mth.clamp(this.getZ(), minimum, maximum));
               break;
            default:
               throw new MatchException((String)null, (Throwable)null);
         }
