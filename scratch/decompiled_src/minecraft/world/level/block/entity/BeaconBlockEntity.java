...
public class BeaconBlockEntity extends BlockEntity implements MenuProvider, Nameable, BeaconBeamOwner {
   ...
   public static final List BEACON_EFFECTS = List.of(List.of(MobEffects.SPEED, MobEffects.HASTE), List.of(MobEffects.RESISTANCE, MobEffects.JUMP_BOOST), List.of(MobEffects.STRENGTH), List.of(MobEffects.REGENERATION));
   private static final Set VALID_EFFECTS = (Set)BEACON_EFFECTS.stream().flatMap(Collection::stream).collect(Collectors.toSet());
      ...
      if (entity.lastCheckY >= lastSetBlock) {
         ...
         if (!level.isClientSide()) {
            ...
            if (!wasActive && isActive) {
               ...
               for(ServerPlayer player : level.getEntitiesOfClass(ServerPlayer.class, (new AABB((double)x, (double)y, (double)z, (double)x, (double)(y - 4), (double)z)).inflate(10.0D, 5.0D, 10.0D))) {
   ...
   public static boolean validateEffects(final @Nullable Holder primary, final @Nullable Holder secondary, final int levels) {
   ...
   private static int getRequiredLevelsFor(final @Nullable Holder effect) {
      if (effect == null) {
         return 0;
      } else {
         for(int i = 0; i < BEACON_EFFECTS.size(); ++i) {
            List effectsForLevel = (List)BEACON_EFFECTS.get(i);
   ...
   private static void applyEffects(final Level level, final BlockPos worldPosition, final int levels, final @Nullable Holder primaryPower, final @Nullable Holder secondaryPower) {
      if (!level.isClientSide() && primaryPower != null) {
         ...
         AABB bb = (new AABB(worldPosition)).inflate(range).expandTowards(0.0D, (double)level.getHeight(), 0.0D);
         List players = level.getEntitiesOfClass(Player.class, bb);
   ...
   private static @Nullable Holder loadEffect(final ValueInput input, final String field) {
      return (Holder)input.read(field, BuiltInRegistries.MOB_EFFECT.holderByNameCodec()).filter(VALID_EFFECTS::contains).orElse((Object)null);
   }
