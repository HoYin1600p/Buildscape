...
public class Blaze extends Monster {
   ...
   protected void customServerAiStep(final ServerLevel level) {
      ...
      if (target != null && target.getEyeY() > this.getEyeY() + (double)this.allowedHeightOffset && this.canAttack(target)) {
         ...
         this.setDeltaMovement(this.getDeltaMovement().add(0.0D, ((double)0.3F - movement.y) * (double)0.3F, 0.0D));
         this.needsSync = true;
      }
      ...
      public void tick() {
         ...
         if (target != null) {
            ...
            if (distance < 4.0D) {
               ...
               double yd = target.getY(0.5D) - this.blaze.getY(0.5D);
               ...
               if (this.attackTime <= 0) {
                  ...
                  if (this.attackStep > 1) {
                     double sqd = Math.sqrt(Math.sqrt(distance)) * 0.5D;
                     if (!this.blaze.isSilent()) {
                        this.blaze.level().levelEvent((Entity)null, 1018, this.blaze.blockPosition(), 0);
                     }

                     for(int i = 0; i < 1; ++i) {
                        Vec3 direction = new Vec3(this.blaze.getRandom().triangle(xd, 2.297D * sqd), yd, this.blaze.getRandom().triangle(zd, 2.297D * sqd));
                        SmallFireball entity = new SmallFireball(this.blaze.level(), this.blaze, direction.normalize());
                        entity.setPos(entity.getX(), this.blaze.getY(0.5D) + 0.5D, entity.getZ());
