...
public class SpriteContents implements AutoCloseable, Stitcher.Entry {
   ...
   public SpriteContents(final Identifier name, final FrameSize frameSize, final NativeImage image, final Optional animationInfo, final List additionalMetadata, final Optional textureInfo) {
      ...
      this.animatedTexture = (SpriteContents.AnimatedTexture)animationInfo.map((animation) -> this.createAnimatedTexture(frameSize, image.getWidth(), image.getHeight(), animation)).orElse((Object)null);
   ...
   public void increaseMipLevel(final int mipLevel) {
      try {
         ...
         frameCategory.setDetail("Original image size", (CrashReportDetail)(() -> this.originalImage.getWidth() + "x" + this.originalImage.getHeight()));
   ...
   private SpriteContents.@Nullable AnimatedTexture createAnimatedTexture(final FrameSize frameSize, final int fullWidth, final int fullHeight, final AnimationMetadataSection metadata) {
      ...
      if (metadata.frames().isEmpty()) {
         ...
         List metadataFrames = (List)metadata.frames().get();
         ...
         for(Iterator iterator = frames.iterator(); iterator.hasNext(); ++index) {
   ...
   private class AnimatedTexture {
      ...
      public SpriteContents.AnimationState createAnimationState(final GpuBufferSlice uboSlice, final int spriteUboSize) {
         ...
         Int2ObjectMap frameTexturesByIndex = new Int2ObjectOpenHashMap();
         GpuBufferSlice[] spriteUbosByMip = new GpuBufferSlice[SpriteContents.this.byMipLevel.length];
         CommandEncoder encoder = device.createCommandEncoder();
         List stagingBuffers = encoder.transientMemory().multiUploadStaging(Arrays.stream(SpriteContents.this.byMipLevel).map(NativeImage::getPixelBytes).toList(), 1L, 16);

         for(int i = 0; i < this.uniqueFrames.size(); ++i) {
            int frame = this.uniqueFrames.getInt(i);
            GpuTexture texture = device.createTexture(() -> String.valueOf(SpriteContents.this.name) + " animation frame " + frame, 5, GpuFormat.RGBA8_UNORM, SpriteContents.this.width, SpriteContents.this.height, 1, SpriteContents.this.byMipLevel.length);
         ...
         return SpriteContents.this.new AnimationState(SpriteContents.this, this, frameTexturesByIndex, spriteUbosByMip);
      }
   ...
   public class AnimationState implements AutoCloseable {
      ...
      private AnimationState(final SpriteContents this$0, final SpriteContents.AnimatedTexture animationInfo, final Int2ObjectMap frameTexturesByIndex, final GpuBufferSlice[] spriteUbosByMip) {
      ...
      public void drawToAtlas(final RenderPass renderPass, final GpuBufferSlice ubo) {
         ...
         float frameProgress = (float)this.subFrame / (float)((SpriteContents.FrameInfo)this.animationInfo.frames.get(this.frame)).time;
