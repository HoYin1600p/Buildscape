...
public class Minecart extends AbstractMinecart {
   ...
   public void tick() {
      ...
      super.tick();
      double tickDiff = ((double)this.getYRot() - lastKnownYRot) % 360.0D;
      if (this.level().isClientSide() && lastKnownPos.distanceTo(this.position()) > 0.01D) {
   ...
   protected void positionRider(final Entity passenger, final Entity.MoveFunction moveFunction) {
      ...
      if (this.level().isClientSide() && passenger instanceof Player player) {
         if (player.shouldRotateWithMinecart() && useExperimentalMovement(this.level())) {
            float yRot = (float)Mth.rotLerp(0.5D, (double)this.playerRotationOffset, (double)this.rotationOffset);
