...
public class ServerPlaceRecipe {
   ...
   private final ServerPlaceRecipe.CraftingMenuAccess menu;
   ...
   public static RecipeBookMenu.PostPlaceAction placeRecipe(final ServerPlaceRecipe.CraftingMenuAccess menu, final int gridWidth, final int gridHeight, final List inputGridSlots, final List slotsToClear, final Inventory inventory, final RecipeHolder recipe, final boolean useMaxItems, final boolean allowDroppingItemsToClear) {
      ServerPlaceRecipe placer = new ServerPlaceRecipe(menu, inventory, useMaxItems, gridWidth, gridHeight, inputGridSlots, slotsToClear);
   ...
   private ServerPlaceRecipe(final ServerPlaceRecipe.CraftingMenuAccess menu, final Inventory inventory, final boolean useMaxItems, final int gridWidth, final int gridHeight, final List inputGridSlots, final List slotsToClear) {
      this.menu = menu;
   ...
   private void placeRecipe(final RecipeHolder recipe, final StackedItemContents availableItems) {
      ...
      List itemsUsedPerIngredient = new ArrayList();
      if (availableItems.canCraft(recipe.value(), amountToCraft, itemsUsedPerIngredient::add)) {
         ...
         PlaceRecipeHelper.placeRecipe(this.gridWidth, this.gridHeight, recipe.value(), recipe.value().placementInfo().slotsToIngredientIndex(), (ingredientIndex, gridIndex, gridXPos, gridYPos) -> {
            if (ingredientIndex != -1) {
               ...
               Holder itemUsed = (Holder)itemsUsedPerIngredient.get(ingredientIndex);
   ...
   private boolean testClearGrid() {
      List freeSlots = Lists.newArrayList();
