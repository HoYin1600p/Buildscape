...
public class BendingTrunkPlacer extends TrunkPlacer {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> trunkPlacerParts(i).and(i.group(ExtraCodecs.POSITIVE_INT.optionalFieldOf("min_height_for_leaves", 1).forGetter((c) -> c.minHeightForLeaves), IntProviders.codec(1, 64).fieldOf("bend_length").forGetter((c) -> c.bendLength))).apply(i, BendingTrunkPlacer::new));
   ...
   public List placeTrunk(final WorldGenLevel level, final BiConsumer trunkSetter, final RandomSource random, final int treeHeight, final BlockPos origin, final TreeFeature tree) {
      ...
      List foliagePoints = Lists.newArrayList();
