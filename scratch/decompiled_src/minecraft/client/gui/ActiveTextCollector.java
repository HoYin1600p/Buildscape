...
public interface ActiveTextCollector {
   ...
   default void defaultScrollingHelper(final Component message, final int centerX, final int left, final int right, final int top, final int bottom, final int lineWidth, final int lineHeight, final ActiveTextCollector.Parameters parameters) {
      ...
      if (lineWidth > availableMessageWidth) {
         ...
         double period = Math.max((double)maxPosition * 0.5D, 3.0D);
         double alpha = Math.sin((Math.PI / 2D) * Math.cos((Math.PI * 2D) * time / period)) / 2.0D + 0.5D;
         double pos = Mth.lerp(alpha, 0.0D, (double)maxPosition);
   ...
   public static class ClickableStyleFinder implements ActiveTextCollector {
      ...
      private final Consumer styleScanner = (style) -> {
