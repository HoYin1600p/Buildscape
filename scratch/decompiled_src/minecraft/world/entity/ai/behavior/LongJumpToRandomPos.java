...
public class LongJumpToRandomPos extends Behavior {
   ...
   protected List jumpCandidates = Lists.newArrayList();
   protected Optional initialPosition = Optional.empty();
   ...
   protected void pickCandidate(final ServerLevel level, final Mob body, final long timestamp) {
      while(true) {
         if (!this.jumpCandidates.isEmpty()) {
            Optional optionalPosition = this.getJumpCandidate(level);
   ...
   protected Optional getJumpCandidate(final ServerLevel level) {
      Optional randomItem = WeightedRandom.getRandomItem(level.getRandom(), this.jumpCandidates, LongJumpToRandomPos.PossibleJump::weight);
   ...
   protected @Nullable Vec3 calculateOptimalJumpVector(final Mob body, final Vec3 targetPos) {
      List allowedAngles = Lists.newArrayList(ALLOWED_ANGLES);
      ...
      for(int angle : allowedAngles) {
         Optional velocityVector = LongJumpUtil.calculateJumpVectorForAngle(body, targetPos, maxJumpVelocity, angle, true);
