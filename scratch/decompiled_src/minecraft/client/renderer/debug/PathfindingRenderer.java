...
public class PathfindingRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public static void renderPath(final Path path, final float maxNodeDistance, final boolean renderOpenAndClosedSets, final boolean renderGroundLabels, final double camX, final double camY, final double camZ) {
      ...
      if (distanceToCamera(pos, camX, camY, camZ) <= 80.0F) {
         Gizmos.cuboid(new AABB((double)((float)pos.getX() + 0.25F), (double)((float)pos.getY() + 0.25F), (double)pos.getZ() + 0.25D, (double)((float)pos.getX() + 0.75F), (double)((float)pos.getY() + 0.75F), (double)((float)pos.getZ() + 0.75F)), GizmoStyle.fill(ARGB.colorFromFloat(0.5F, 0.0F, 1.0F, 0.0F)));
      ...
      if (renderGroundLabels) {
         for(int i = 0; i < path.getNodeCount(); ++i) {
            Node n = path.getNode(i);
            if (distanceToCamera(n.asBlockPos(), camX, camY, camZ) <= 80.0F) {
               Gizmos.billboardText(String.valueOf(n.type), new Vec3((double)n.x + 0.5D, (double)n.y + 0.75D, (double)n.z + 0.5D), TextGizmo.Style.whiteAndCentered().withScale(0.32F)).setAlwaysOnTop();
               Gizmos.billboardText(String.format(Locale.ROOT, "%.2f", n.costMalus), new Vec3((double)n.x + 0.5D, (double)n.y + 0.25D, (double)n.z + 0.5D), TextGizmo.Style.whiteAndCentered().withScale(0.32F)).setAlwaysOnTop();
            }
         }
      }
      ...
      if (path.getNodeCount() >= 2) {
         ...
         for(int i = 1; i < path.getNodeCount(); ++i) {
            ...
            if (distanceToCamera(n.asBlockPos(), camX, camY, camZ) > 80.0F) {
               ...
               Gizmos.arrow(last.add(0.5D, 0.5D, 0.5D), n.asVec3().add(0.5D, 0.5D, 0.5D), color);
               last = n.asVec3();
            }
         }
