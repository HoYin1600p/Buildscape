...
public class FocusableTextWidget extends MultiLineTextWidget {
   ...
   private FocusableTextWidget(final Component message, final Font font, final int padding, final int maxWidth, final FocusableTextWidget.BackgroundFill backgroundFill, final boolean alwaysShowBorder) {
      super(message, font);
      this.active = true;
   ...
   public boolean keyPressed(final KeyEvent event) {
      if (this.isActive() && event.isSelection()) {
         Optional clickableStyle = this.getMessage().visit((style, text) -> style.getClickEvent() != null ? Optional.of(style) : Optional.empty(), Style.EMPTY);
