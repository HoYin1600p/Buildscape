...
public class MouseHandler {
   ...
   public void onButton(final long handle, final MouseButtonInfo rawButtonInfo, final @MouseButtonInfo.Action int action) {
      ...
      if (handle != 0L && handle == window.handle()) {
         ...
         if (this.minecraft.gui.overlay() == null) {
            if (pressed && this.minecraft.handleGlobalKeyPress(Type.MOUSE.getOrCreate(buttonInfo.button()), buttonInfo.hasControlDownWithQuirk())) {
               return;
            }
         ...
         if (this.minecraft.gui.screen() == null && this.minecraft.gui.overlay() == null) {
            if (buttonInfo.button() == 1) {
               this.isLeftPressed = pressed;
            } else if (buttonInfo.button() == 2) {
               this.isMiddlePressed = pressed;
            } else if (buttonInfo.button() == 3) {
               this.isRightPressed = pressed;
            }

            InputConstants.Key mouseKey = Type.MOUSE.getOrCreate(buttonInfo.button());
   ...
   private MouseButtonInfo simulateRightClick(final MouseButtonInfo info, final boolean pressed) {
      if (InputQuirks.SIMULATE_RIGHT_CLICK_WITH_LONG_LEFT_CLICK && info.button() == 1) {
         if (pressed) {
            if ((info.modifiers() & 192) == 192) {
               ++this.fakeRightMouse;
               return new MouseButtonInfo(3, info.modifiers());
            }
   ...
   public void onDrop(final long handle, final List rawPaths) {
      this.minecraft.getFramerateLimitTracker().onInputReceived();
      List files = new ArrayList(rawPaths.size());
      int failedCount = 0;
   ...
   public void handleAccumulatedMovement() {
      ...
      if (this.minecraft.isWindowActive()) {
         Screen screen = this.minecraft.gui.screen();
         boolean mouseMoved = this.accumulatedDX != 0.0D || this.accumulatedDY != 0.0D;
         if (mouseMoved) {
            this.minecraft.getFramerateLimitTracker().onInputReceived();
         }
   ...
   private void turnPlayer(final double mousea) {
      ...
      double sens = sensitivityMod * 8.0D;
      double xo;
      double yo;
   ...
   public void grabMouse() {
      if (this.minecraft.isWindowActive()) {
         if (!this.mouseGrabbed) {
            ...
            this.xpos = (double)this.minecraft.getWindow().getScreenWidth() / 2.0D;
            this.ypos = (double)this.minecraft.getWindow().getScreenHeight() / 2.0D;
   ...
   public void drawDebugMouseInfo(final Font font, final GuiGraphicsExtractor graphics) {
      ...
      double y = this.getScaledYPos(window) - 8.0D;
