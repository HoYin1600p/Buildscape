...
public class BellBlockEntity extends BlockEntity {
   ...
   private static void tick(final Level level, final BlockPos pos, final BlockState state, final BellBlockEntity entity, final BellBlockEntity.ResonationEndAction onResonationEnd) {
      if (entity.shaking) {
         ++entity.ticks;
      }
   ...
   private void updateEntities() {
      ...
      if (!this.level.isClientSide()) {
         for(LivingEntity entity : this.nearbyEntities) {
            if (entity.isAlive() && !entity.isRemoved() && blockPos.closerToCenterThan(entity.position(), 32.0D)) {
   ...
   private static boolean areRaidersNearby(final BlockPos bellPos, final List nearbyEntities) {
      for(LivingEntity entity : nearbyEntities) {
         if (entity.isAlive() && !entity.isRemoved() && bellPos.closerToCenterThan(entity.position(), 32.0D) && entity.is(EntityTypeTags.RAIDERS)) {
            return true;
         }
      }
   ...
   private static void showBellParticles(final Level level, final BlockPos bellPos, final List nearbyEntities) {
      ...
      int nearbyRaiderCount = (int)nearbyEntities.stream().filter((p) -> bellPos.closerToCenterThan(p.position(), 48.0D)).count();
      nearbyEntities.stream().filter((e) -> isRaiderWithinRange(bellPos, e)).forEach((entity) -> {
         ...
         double x3 = (double)((float)bellPos.getX() + 0.5F) + 1.0D / distBtwn * (entity.getX() - (double)bellPos.getX());
         double z3 = (double)((float)bellPos.getZ() + 0.5F) + 1.0D / distBtwn * (entity.getZ() - (double)bellPos.getZ());
         int particleCount = Mth.clamp((nearbyRaiderCount - 21) / -2, 3, 15);

         for(int i = 0; i < particleCount; ++i) {
            int color = particleColor.addAndGet(5);
            level.addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, color), x3, (double)((float)bellPos.getY() + 0.5F), z3, 0.0D, 0.0D, 0.0D);
         }
   ...
   private static boolean isRaiderWithinRange(final BlockPos blockPos, final LivingEntity entity) {
      return entity.isAlive() && !entity.isRemoved() && blockPos.closerToCenterThan(entity.position(), 48.0D) && entity.is(EntityTypeTags.RAIDERS);
   }
