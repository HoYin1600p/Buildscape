...
public class FeatureCountTracker {
   ...
   private static final LoadingCache data = CacheBuilder.newBuilder().weakKeys().expireAfterAccess(5L, TimeUnit.MINUTES).build(new CacheLoader() {
   ...
   public static void featurePlaced(final ServerLevel level, final Feature feature, final Optional topFeature) {
      try {
         ((FeatureCountTracker.LevelData)data.get(level)).featureData().computeInt(new FeatureCountTracker.FeatureData(feature, topFeature), (f, old) -> old == null ? 1 : old + 1);
   ...
   public static void logCounts() {
      ...
      data.asMap().forEach((level, featureCounts) -> {
         ...
         featureCounts.featureData().forEach((data, count) -> LOGGER.debug("{} {} {} {} {}", new Object[]{prefix, String.format(Locale.ROOT, "%10d", count), String.format(Locale.ROOT, "%10f", (double)count / (double)chunks), data.topFeature().flatMap(featureRegistry::getResourceKey).map(ResourceKey::identifier), data.feature()}));
      });
   }

   private static record FeatureData(Feature feature, Optional topFeature) {
   }
