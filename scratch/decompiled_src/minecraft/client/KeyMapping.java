...
public class KeyMapping implements Comparable {
   ...
   private static void forAllKeyMappings(final InputConstants.Key key, final Consumer operation) {
      List keyMappings = (List)MAP.get(key);
   ...
   public KeyMapping(final String name, final int keysym, final KeyMapping.Category category) {
      this(name, Type.KEYBOARD, keysym, category);
   }
   ...
   protected boolean shouldSetOnIngameFocus() {
      return this.key.getType() == Type.KEYBOARD && this.key.getValue() != InputConstants.UNKNOWN.getValue();
   }
   ...
   public boolean matches(final KeyEvent event) {
      return this.key.getType() == Type.KEYBOARD && this.key.getValue() == event.key();
   }
