...
public class ParticleUnflatteningFix extends DataFix {
   ...
   private Dynamic updateItem(final Dynamic result, final String contents) {
      ...
      if (tagPartStart == -1) {
         ...
         Dynamic itemTag = parseTag(result.getOps(), contents.substring(tagPartStart));
         if (itemTag != null) {
   ...
   private Dynamic updateBlock(final Dynamic result, final String contents) {
      ...
      if (statePartStart == -1) {
         ...
         Map properties = parseBlockProperties(result, contents.substring(statePartStart));
   ...
   private Dynamic updateDust(final Dynamic result, final String contents) {
      try {
         StringReader reader = new StringReader(contents);
         Dynamic vector = readVector(result, reader);
         reader.expect(' ');
   ...
   private Dynamic updateDustTransition(final Dynamic result, final String contents) {
      try {
         StringReader reader = new StringReader(contents);
         Dynamic from = readVector(result, reader);
         reader.expect(' ');
         float scale = reader.readFloat();
         reader.expect(' ');
         Dynamic to = readVector(result, reader);
   ...
   private Dynamic updateVibration(final Dynamic result, final String contents) {
      try {
         ...
         Dynamic blockPos = result.createIntList(IntStream.of(new int[]{Mth.floor(destX), Mth.floor(destY), Mth.floor(destZ)}));
