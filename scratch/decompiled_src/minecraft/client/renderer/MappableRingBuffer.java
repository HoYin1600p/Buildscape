...
public class MappableRingBuffer implements AutoCloseable {
   ...
   public MappableRingBuffer(final Supplier label, final @Usage int usage, final int size) {
      ...
      if ((usage & 1) == 0 && (usage & 2) == 0) {
         ...
         for(int i = 0; i < 3; ++i) {
            int finalI = i;
            this.buffers[i] = device.createBuffer(() -> (String)label.get() + " #" + finalI, usage, (long)size);
            this.fences[i] = null;
         }
