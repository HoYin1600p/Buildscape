...
public record WrittenBookContent(Filterable title, String author, int generation, List pages, boolean resolved) implements BookContent, TooltipProvider {
   ...
   public static final Codec PAGES_CODEC = pagesCodec(CONTENT_CODEC);
   ...
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(Filterable.streamCodec(ByteBufCodecs.stringUtf8(32)), WrittenBookContent::title, ByteBufCodecs.STRING_UTF8, WrittenBookContent::author, ByteBufCodecs.VAR_INT, WrittenBookContent::generation, Filterable.streamCodec(ComponentSerialization.STREAM_CODEC).apply(ByteBufCodecs.list()), WrittenBookContent::pages, ByteBufCodecs.BOOL, WrittenBookContent::resolved, WrittenBookContent::new);
   ...
   public @Nullable WrittenBookContent resolve(final ResolutionContext context, final HolderLookup.Provider registries) {
      if (this.resolved) {
         ...
         for(Filterable page : this.pages) {
            Optional resolvedPage = resolvePage(context, registries, page);
   ...
   private static Optional resolvePage(final ResolutionContext context, final HolderLookup.Provider registries, final Filterable page) {
   ...
   private static boolean isPageTooLarge(final Component page, final HolderLookup.Provider registries) {
      DataResult json = ComponentSerialization.CODEC.encodeStart(registries.createSerializationContext(JsonOps.INSTANCE), page);
