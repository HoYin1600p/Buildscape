...
public class ClientConfigurationPacketListenerImpl extends ClientCommonPacketListenerImpl implements ClientConfigurationPacketListener, TickablePacketListener {
   ...
   public void handleSelectKnownPacks(final ClientboundSelectKnownPacks packet) {
      ...
      List selected = this.knownPacks.trySelectingPacks(packet.knownPacks());
   ...
   private static RegistryAccess.Frozen filterRegistries(final RegistryAccess.Frozen original, final Stream keysToInclude) {
      List filteredRegistries = keysToInclude.map(original::lookupOrThrow).toList();
