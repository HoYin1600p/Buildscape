...
public class UpgradeStatusTranslator {
   ...
   public static record Messages(Component upgrading, Component finished) {
      public static UpgradeStatusTranslator.Messages create(final String type) {
         return new UpgradeStatusTranslator.Messages(Component.translatable("optimizeWorld.stage.upgrading." + type), Component.translatable("optimizeWorld.stage.finished." + type));
      }
      ...
      public Component forStatus(final UpgradeProgress.Status status) {
         ...
         switch (status) {
            case UPGRADING:
               var10000 = this.upgrading;
               break;
            case FINISHED:
               var10000 = this.finished;
               break;
            default:
               throw new IllegalStateException("Invalid Status received: " + String.valueOf(status));
         }
