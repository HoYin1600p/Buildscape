...
public class BlockUtil {
   public static BlockUtil.FoundRectangle getLargestRectangleAround(final BlockPos center, final Direction.Axis axis1, final int limit1, final Direction.Axis axis2, final int limit2, final Predicate test) {
      ...
      boundsByAxis1[negativeDelta1] = new BlockUtil.IntBounds(getLimit(test, pos.set(center), negativeDirection2, limit2), getLimit(test, pos.set(center), positiveDirection2, limit2));
      ...
      for(int i = 1; i <= negativeDelta1; ++i) {
         ...
         boundsByAxis1[centerIndex1 - i] = new BlockUtil.IntBounds(getLimit(test, pos.set(center).move(negativeDirection1, i), negativeDirection2, lastBounds.min), getLimit(test, pos.set(center).move(negativeDirection1, i), positiveDirection2, lastBounds.max));
      }

      for(int i = 1; i <= positiveDelta1; ++i) {
         BlockUtil.IntBounds lastBounds = boundsByAxis1[centerIndex1 + i - 1];
         boundsByAxis1[centerIndex1 + i] = new BlockUtil.IntBounds(getLimit(test, pos.set(center).move(positiveDirection1, i), negativeDirection2, lastBounds.min), getLimit(test, pos.set(center).move(positiveDirection1, i), positiveDirection2, lastBounds.max));
      }
      ...
      for(int i2 = centerIndex2; i2 >= 0; --i2) {
         ...
         Pair rectangle = getMaxRectangleLocation(columns);
