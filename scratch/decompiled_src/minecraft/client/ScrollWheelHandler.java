...
public class ScrollWheelHandler {
   ...
   public Vector2i onMouseScroll(final double scaledXScrollOffset, final double scaledYScrollOffset) {
      if (this.accumulatedScrollX != 0.0D && Math.signum(scaledXScrollOffset) != Math.signum(this.accumulatedScrollX)) {
         this.accumulatedScrollX = 0.0D;
      }

      if (this.accumulatedScrollY != 0.0D && Math.signum(scaledYScrollOffset) != Math.signum(this.accumulatedScrollY)) {
