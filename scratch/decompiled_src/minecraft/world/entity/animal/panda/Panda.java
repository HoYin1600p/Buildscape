...
public class Panda extends Animal {
   ...
   public @Nullable AgeableMob getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      Panda baby = (Panda)EntityTypes.PANDA.create(level, EntitySpawnReason.BREEDING);
      if (baby != null) {
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(4, new TemptGoal(this, 1.0D, (i) -> i.is(ItemTags.PANDA_FOOD), false));
   ...
   private void addEatingParticles() {
      if (this.getEatCounter() % 5 == 0) {
         ...
         if (!heldItem.isEmpty()) {
            ...
            for(int i = 0; i < 6; ++i) {
               Vec3 velocity = (new Vec3(((double)this.random.nextFloat() - 0.5D) * 0.1D, (double)this.random.nextFloat() * 0.1D + 0.1D, ((double)this.random.nextFloat() - 0.5D) * 0.1D)).xRot(-this.getXRot() * ((float)Math.PI / 180F)).yRot(-this.getYRot() * ((float)Math.PI / 180F));
               Vec3 position = (new Vec3(((double)this.random.nextFloat() - 0.5D) * 0.8D, (double)(-this.random.nextFloat()) * 0.6D - 0.3D, 1.0D + ((double)this.random.nextFloat() - 0.5D) * 0.4D)).yRot(-this.yBodyRot * ((float)Math.PI / 180F)).add(this.getX(), this.getEyeY() + 1.0D, this.getZ());
               this.level().addParticle(breakParticle, position.x, position.y, position.z, velocity.x, velocity.y + 0.05D, velocity.z);
            }
         }
      }
      ...
      if (this.rollCounter > 32) {
         ...
         if (!this.level().isClientSide()) {
            ...
            if (this.rollCounter == 1) {
               ...
               this.rollDelta = new Vec3(movement.x + (double)(-Mth.sin((double)angle) * multiplier), 0.0D, movement.z + (double)(Mth.cos((double)angle) * multiplier));
               ...
               this.setDeltaMovement(0.0D, this.onGround() ? 0.27D : movement.y, 0.0D);
            }
         }
   ...
   private void afterSneeze() {
      ...
      level.addParticle(ParticleTypes.SNEEZE, this.getX() - (double)(this.getBbWidth() + 1.0F) * 0.5D * (double)Mth.sin((double)(this.yBodyRot * ((float)Math.PI / 180F))), this.getEyeY() - (double)0.1F, this.getZ() + (double)(this.getBbWidth() + 1.0F) * 0.5D * (double)Mth.cos((double)(this.yBodyRot * ((float)Math.PI / 180F))), movement.x, 0.0D, movement.z);
      ...
      if (this.isScared()) {
         ...
         if (this.canAgeUp()) {
            ...
            if (!this.level().isClientSide() && this.getAge() == 0 && this.canFallInLove()) {
               ...
               Level pandasCurrentItem = this.level();
               if (!(pandasCurrentItem instanceof ServerLevel)) {
                  return InteractionResult.PASS;
               }

               ServerLevel level = (ServerLevel)pandasCurrentItem;
   ...
   public static enum Gene implements StringRepresentable {
      ...
      public static final Codec CODEC = StringRepresentable.fromEnum(Panda.Gene::values);
      private static final IntFunction BY_ID = ByIdMap.continuous(Panda.Gene::getId, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
         ...
         if (this.mob.getRandom().nextFloat() >= this.probability) {
            ...
            if (this.lookAt == null) {
               ...
               if (this.lookAtType == Player.class) {
                  ...
                  this.lookAt = level.getNearestEntity(this.mob.level().getEntitiesOfClass(this.lookAtType, this.mob.getBoundingBox().inflate((double)this.lookDistance, 3.0D, (double)this.lookDistance), (entity) -> true), this.lookAtContext, this.mob, this.mob.getX(), this.mob.getEyeY(), this.mob.getZ());
               }
            }
      ...
      public boolean canUse() {
         if ((this.panda.isBaby() || this.panda.isPlayful()) && this.panda.onGround()) {
            if (!this.panda.canPerformAction()) {
               ...
               int xStep = (double)Math.abs(xDir) > 0.5D ? Mth.sign((double)xDir) : 0;
               int zStep = (double)Math.abs(zDir) > 0.5D ? Mth.sign((double)zDir) : 0;
   ...
   private class PandaSitGoal extends Goal {
      ...
      public void start() {
         if (Panda.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
            List items = Panda.this.level().getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Panda::canPickUpAndEat);
            if (!items.isEmpty()) {
