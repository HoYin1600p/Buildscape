...
public record Rotations(float x, float y, float z) {
   public static final Codec CODEC = Codec.FLOAT.listOf().comapFlatMap((input) -> Util.fixedSize(input, 3).map((floats) -> new Rotations(floats.get(0), floats.get(1), floats.get(2))), (rotations) -> List.of(rotations.x(), rotations.y(), rotations.z()));
