...
public class VanillaRegistries {
   ...
   private static final RegistrySetBuilder RELOADABLE_BUILDER = (new RegistrySetBuilder()).add(Registries.LOOT_TABLE, VanillaLootTableProvider.create()).add(Registries.PREDICATE, LootPredicates::bootstrap).add(Registries.ADVANCEMENT, VanillaAdvancementProvider.create()).add(Registries.NUMBER_PROVIDER, NumberProviders::bootstrap).add(VanillaRecipeProvider.create());
   ...
   public static void validateThatAllBiomeFeaturesHaveBiomeFilter(final HolderLookup.Provider registries) {
      ...
      HolderLookup.RegistryLookup placedFeatures = registries.lookupOrThrow(Registries.PLACED_FEATURE);
      biomes.listElements().forEach((biome) -> {
         Identifier biomeKey = biome.key().identifier();
         List biomeFeatures = ((Biome)biome.value()).getGenerationSettings().features();
         biomeFeatures.stream().flatMap(HolderSet::stream).forEach((feature) -> feature.unwrap().ifLeft((key) -> {
               Holder.Reference value = placedFeatures.getOrThrow(key);
