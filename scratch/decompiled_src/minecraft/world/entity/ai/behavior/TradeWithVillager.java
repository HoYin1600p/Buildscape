...
public class TradeWithVillager extends Behavior {
   ...
   protected void tick(final ServerLevel level, final Villager body, final long timestamp) {
      ...
      if (!(body.distanceToSqr(target) > 5.0D)) {
   ...
   private static Set figureOutWhatIAmWillingToTrade(final Villager myBody, final Villager target) {
      ImmutableSet targetItems = ((VillagerProfession)target.getVillagerData().profession().value()).requestedItems();
      ImmutableSet selfItems = ((VillagerProfession)myBody.getVillagerData().profession().value()).requestedItems();
      return (Set)targetItems.stream().filter((entry) -> !selfItems.contains(entry)).collect(Collectors.toSet());
   }

   private static void throwHalfStack(final Villager villager, final Predicate predicate, final LivingEntity target) {
