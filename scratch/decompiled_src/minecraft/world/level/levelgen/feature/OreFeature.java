...
public class OreFeature extends AbstractOreFeature {
   public static final MapCodec CODEC = makeCodec(OreFeature::new);

   public OreFeature(final List targetStates, final int size, final float discardChanceOnAirExposure) {
      super(targetStates, size, discardChanceOnAirExposure);
   }

   public OreFeature(final List targetStates, final int size) {
   ...
   protected boolean doPlace(final WorldGenLevel level, final RandomSource random, final double x0, final double x1, final double z0, final double z1, final double y0, final double y1, final int xStart, final int yStart, final int zStart, final int sizeXZ, final int sizeY) {
      ...
      for(int i = 0; i < this.size; ++i) {
         ...
         double ss = random.nextDouble() * (double)this.size / 16.0D;
         double r = ((double)(Mth.sin((double)((float)Math.PI * step)) + 1.0F) * ss + 1.0D) / 2.0D;
         data[i * 4 + 0] = xx;
         data[i * 4 + 1] = yy;
         data[i * 4 + 2] = zz;
         data[i * 4 + 3] = r;
         ...
         for(int i = 0; i < this.size; ++i) {
            ...
            if (!(r < 0.0D)) {
               ...
               for(int x = xMin; x <= xMax; ++x) {
                  double xd = ((double)x + 0.5D - xx) / r;
                  if (xd * xd < 1.0D) {
                     for(int y = yMin; y <= yMax; ++y) {
                        double yd = ((double)y + 0.5D - yy) / r;
                        if (xd * xd + yd * yd < 1.0D) {
                           for(int z = zMin; z <= zMax; ++z) {
                              double zd = ((double)z + 0.5D - zz) / r;
                              if (xd * xd + yd * yd + zd * zd < 1.0D && !level.isOutsideBuildHeight(y)) {
