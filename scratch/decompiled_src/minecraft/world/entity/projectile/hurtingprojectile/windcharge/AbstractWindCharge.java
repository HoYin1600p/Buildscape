...
public abstract class AbstractWindCharge extends AbstractHurtingProjectile implements ItemSupplier {
   ...
   protected AbstractWindCharge(final EntityType type, final double x, final double y, final double z, final Vec3 direction, final Level level) {
   ...
   protected void onHitEntity(final EntityHitResult hitResult) {
      ...
      Level owner = this.level();
      if (owner instanceof ServerLevel serverLevel) {
   ...
   protected void onHitBlock(final BlockHitResult hitResult) {
      ...
      if (!this.level().isClientSide()) {
         ...
         Vec3 scaledNormal = Vec3.atLowerCornerOf(collisionNormal).multiply(0.25D, 0.25D, 0.25D);
