...
public class ArmadilloAi {
   ...
   private static final OneShot ARMADILLO_ROLLING_OUT = BehaviorBuilder.create((i) -> i.group(i.absent(MemoryModuleType.DANGER_DETECTED_RECENTLY)).apply(i, (location) -> (level, body, timestamp) -> {
            if (body.isScared()) {
               body.rollOut();
               return true;
            } else {
   ...
   private static ActivityData initCoreActivity() {
      return ActivityData.create(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new ArmadilloAi.ArmadilloPanic(2.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink() {
   ...
   private static ActivityData initIdleActivity() {
      return ActivityData.create(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityTypes.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityTypes.ARMADILLO, 1.0F, 1)), Pair.of(2, new RunOne(ImmutableList.of(Pair.of(new FollowTemptation((armadillo) -> 1.25F, (armadillo) -> armadillo.isBaby() ? 1.0D : 2.0D), 1), Pair.of(BabyFollowAdult.create(ADULT_FOLLOW_RANGE, 1.25F), 1)))), Pair.of(3, new RandomLookAround(UniformInt.of(150, 250), 30.0F, 0.0F, 0.0F)), Pair.of(4, new RunOne(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(RandomStroll.stroll(1.0F), 1), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 1), Pair.of(new DoNothing(30, 60), 1))))));
   }

   private static ActivityData initScaredActivity() {
      return ActivityData.create(Activity.PANIC, ImmutableList.of(Pair.of(0, new ArmadilloAi.ArmadilloBallUp())), Set.of(Pair.of(MemoryModuleType.DANGER_DETECTED_RECENTLY, MemoryStatus.VALUE_PRESENT), Pair.of(MemoryModuleType.IS_PANICKING, MemoryStatus.VALUE_ABSENT)));
   }
