...
public record EntityTagPredicate(Optional anyOf, Optional allOf, Optional noneOf) implements EntitySubPredicate {
   ...
   public boolean matches(final Set tags) {
   ...
   private static boolean containsAtLeastOne(final Set provided, final List tags) {
      for(String tag : tags) {
   ...
   private static boolean containsAllOf(final Set provided, final List tags) {
      for(String tag : tags) {
         if (!provided.contains(tag)) {
            return false;
         }
