...
public class MobEffectIdFix extends DataFix {
   ...
   private static Dynamic updateMobEffectIdField(final Dynamic input, final String oldFieldName, final Dynamic output, final String newFieldName) {
      Optional mappedId = getAndConvertMobEffectId(input, oldFieldName);
   ...
   private static Dynamic updateMobEffectInstance(Dynamic input) {
      ...
      Optional hiddenEffect = input.get("HiddenEffect").result().map(MobEffectIdFix::updateMobEffectInstance);
   ...
   private static Dynamic updateMobEffectInstanceList(final Dynamic input, final String oldField, final String newField) {
      Optional newValue = input.get(oldField).asStreamOpt().result().map((effects) -> input.createList(effects.map(MobEffectIdFix::updateMobEffectInstance)));
   ...
   private TypeRewriteRule blockEntityFixer() {
      ...
      return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", blockEntityType, (input) -> this.updateNamedChoice(input, References.BLOCK_ENTITY, "minecraft:beacon", (tag) -> {
   ...
   private static Dynamic fixMooshroomTag(Dynamic entityTag) {
      ...
      Dynamic entry = updateSuspiciousStewEntry(entityTag, initialEntry);
   ...
   private static Dynamic fixSuspiciousStewTag(final Dynamic tag) {
      Optional effectsList = tag.get("Effects").asStreamOpt().result().map((list) -> tag.createList(list.map(MobEffectIdFix::updateSuspiciousStewEntry)));
      return tag.replaceField("Effects", "effects", effectsList);
   }

   private TypeRewriteRule itemStackFixer() {
      OpticFinder idF = DSL.fieldFinder("id", DSL.named(References.ITEM_NAME.typeName(), NamespacedSchema.namespacedString()));
