...
public record EquipmentTable(ResourceKey lootTable, Map slotDropChances) {
   public static final Codec DROP_CHANCES_CODEC = Codec.either(Codec.FLOAT, Codec.unboundedMap(EquipmentSlot.CODEC, Codec.FLOAT)).xmap((either) -> (Map)either.map(EquipmentTable::createForAllSlots, Function.identity()), (provider) -> {
   ...
   public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(LootTable.KEY_CODEC.fieldOf("loot_table").forGetter(EquipmentTable::lootTable), DROP_CHANCES_CODEC.optionalFieldOf("slot_drop_chances", Map.of()).forGetter(EquipmentTable::slotDropChances)).apply(i, EquipmentTable::new));
