...
public class Drowned extends Zombie implements RangedAttackMob, NeutralMob {
   ...
   protected void addBehaviourGoals() {
      ...
      this.targetSelector.addGoal(1, (new HurtByTargetGoal(this, Drowned.class)).setAlertOthers(ZombifiedPiglin.class).setAlertCondition(() -> !this.shouldBehaveNeutrally()));
   ...
   public void performRangedAttack(final LivingEntity target, final float power) {
      ...
      if (var15 instanceof ServerLevel serverLevel) {
         Projectile.spawnProjectileUsingShoot(trident, serverLevel, tridentItemStack, xd, yd + distanceToTarget * (double)0.2F, zd, 1.6F, this.rangedAttackUncertainty(serverLevel));
      }
      ...
      public void tick() {
         ...
         if (((Drowned)this.mob).wantsToSwim() && ((Drowned)this.mob).isInWater()) {
            if (target != null && target.getY() > ((Drowned)this.mob).getY() || ((Drowned)this.mob).isSearchingForLand()) {
               ((Drowned)this.mob).setDeltaMovement(((Drowned)this.mob).getDeltaMovement().add(0.0D, 0.002D, 0.0D));
            }
            ...
            ((Drowned)this.mob).setDeltaMovement(((Drowned)this.mob).getDeltaMovement().add((double)newSpeed * xd * 0.005D, (double)newSpeed * yd * 0.1D, (double)newSpeed * zd * 0.005D));
         } else {
            if (!((Drowned)this.mob).onGround()) {
               ((Drowned)this.mob).setDeltaMovement(((Drowned)this.mob).getDeltaMovement().add(0.0D, -0.008D, 0.0D));
            }
