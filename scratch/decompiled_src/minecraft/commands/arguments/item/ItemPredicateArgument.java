...
public class ItemPredicateArgument extends ParserBasedArgument {
   ...
   private static final Map PSEUDO_COMPONENTS = (Map)Stream.of(new ItemPredicateArgument.ComponentWrapper(COUNT_ID, (itemStack) -> true, MinMaxBounds.Ints.CODEC.map((range) -> (itemStack) -> range.matches(itemStack.getCount())))).collect(Collectors.toUnmodifiableMap(ItemPredicateArgument.ComponentWrapper::id, (e) -> e));
   private static final Map PSEUDO_PREDICATES = (Map)Stream.of(new ItemPredicateArgument.PredicateWrapper(COUNT_ID, MinMaxBounds.Ints.CODEC.map((range) -> (itemStack) -> range.matches(itemStack.getCount())))).collect(Collectors.toUnmodifiableMap(ItemPredicateArgument.PredicateWrapper::id, (e) -> e));

   private static ItemPredicateArgument.PredicateWrapper createComponentExistencePredicate(final Holder.Reference componentId) {
      Predicate componentExists = (itemStack) -> itemStack.has((DataComponentType)componentId.value());
   ...
   public static ItemPredicateArgument.Result getItemPredicate(final CommandContext context, final String name) {
   ...
   private static record ComponentWrapper(Identifier id, Predicate presenceChecker, Decoder valueChecker) {
      public static ItemPredicateArgument.ComponentWrapper create(final ImmutableStringReader reader, final Identifier id, final DataComponentType type) throws CommandSyntaxException {
         ...
         if (codec == null) {
            ...
            return new ItemPredicateArgument.ComponentWrapper(id, (itemStack) -> itemStack.has(type), codec.map((expected) -> (itemStack) -> {
                  Object actual = (T)itemStack.get(type);
   ...
   private static class Context implements ComponentPredicateParser.Context {
      ...
      public Predicate forElementType(final ImmutableStringReader reader, final Identifier id) throws CommandSyntaxException {
         Holder.Reference item = (Holder.Reference)this.items.get(ResourceKey.create(Registries.ITEM, id)).orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_ITEM.createWithContext(reader, id));
      ...
      public Predicate forTagType(final ImmutableStringReader reader, final Identifier id) throws CommandSyntaxException {
         HolderSet tag = (HolderSet)this.items.get(TagKey.create(Registries.ITEM, id)).orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_TAG.createWithContext(reader, id));
      ...
      public Predicate createComponentTest(final ImmutableStringReader reader, final ItemPredicateArgument.ComponentWrapper componentType, final Dynamic value) throws CommandSyntaxException {
         return componentType.decode(reader, RegistryOps.injectRegistryContext(value, this.registries));
      }

      public Predicate createComponentTest(final ImmutableStringReader reader, final ItemPredicateArgument.ComponentWrapper componentType) {
      ...
      public ItemPredicateArgument.PredicateWrapper lookupPredicateType(final ImmutableStringReader reader, final Identifier componentId) throws CommandSyntaxException {
         ...
         return wrapper != null ? wrapper : (ItemPredicateArgument.PredicateWrapper)this.predicates.get(ResourceKey.create(Registries.DATA_COMPONENT_PREDICATE_TYPE, componentId)).map(ItemPredicateArgument.PredicateWrapper::new).or(() -> this.components.get(ResourceKey.create(Registries.DATA_COMPONENT_TYPE, componentId)).map(ItemPredicateArgument::createComponentExistencePredicate)).orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_PREDICATE.createWithContext(reader, componentId));
      }

      public Predicate createPredicateTest(final ImmutableStringReader reader, final ItemPredicateArgument.PredicateWrapper predicateType, final Dynamic value) throws CommandSyntaxException {
   ...
   private static record PredicateWrapper(Identifier id, Decoder type) {
      public PredicateWrapper(final Holder.Reference holder) {
         this(holder.key().identifier(), ((DataComponentPredicate.Type)holder.value()).codec().map((v) -> v::matches));
      }
