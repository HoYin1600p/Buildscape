...
public final class Interval {
   ...
   public static Interval encapsulating(final List intervals) {
   ...
   private static Interval powInfiniteExponent(final float base, final Interval exponent) {
      if (Float.isInfinite(exponent.min()) && Float.isInfinite(exponent.max())) {
         ...
         return base < 1.0F ? of((float)Math.pow((double)base, (double)exponent.max()), Float.POSITIVE_INFINITY) : of(0.0F, (float)Math.pow((double)base, (double)exponent.max()));
      } else {
         return base < 1.0F ? of(0.0F, (float)Math.pow((double)base, (double)exponent.min())) : of((float)Math.pow((double)base, (double)exponent.min()), Float.POSITIVE_INFINITY);
      }
   }
