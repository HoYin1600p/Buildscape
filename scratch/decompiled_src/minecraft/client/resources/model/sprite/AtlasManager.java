...
public class AtlasManager implements AutoCloseable, PreparableReloadListener, SpriteGetter {
   ...
   private static final List KNOWN_ATLASES = List.of(new AtlasManager.AtlasConfig(Sheets.BANNER_SHEET, AtlasIds.BANNER_PATTERNS, false), new AtlasManager.AtlasConfig(TextureAtlas.LOCATION_BLOCKS, AtlasIds.BLOCKS, true), new AtlasManager.AtlasConfig(TextureAtlas.LOCATION_ITEMS, AtlasIds.ITEMS, false), new AtlasManager.AtlasConfig(Sheets.CHEST_SHEET, AtlasIds.CHESTS, false), new AtlasManager.AtlasConfig(Sheets.DECORATED_POT_SHEET, AtlasIds.DECORATED_POT, false), new AtlasManager.AtlasConfig(Sheets.GUI_SHEET, AtlasIds.GUI, false, Set.of(GuiMetadataSection.TYPE)), new AtlasManager.AtlasConfig(Sheets.MAP_DECORATIONS_SHEET, AtlasIds.MAP_DECORATIONS, false), new AtlasManager.AtlasConfig(Sheets.PAINTINGS_SHEET, AtlasIds.PAINTINGS, false), new AtlasManager.AtlasConfig(TextureAtlas.LOCATION_PARTICLES, AtlasIds.PARTICLES, false), new AtlasManager.AtlasConfig(Sheets.SHIELD_SHEET, AtlasIds.SHIELD_PATTERNS, false), new AtlasManager.AtlasConfig(Sheets.SHULKER_SHEET, AtlasIds.SHULKER_BOXES, false), new AtlasManager.AtlasConfig(Sheets.CELESTIAL_SHEET, AtlasIds.CELESTIALS, false));
   public static final PreparableReloadListener.StateKey PENDING_STITCH = new PreparableReloadListener.StateKey();
   private final Map atlasByTexture = new HashMap();
   private final Map atlasById = new HashMap();
   private Map spriteLookup = Map.of();
   ...
   public void prepareSharedState(final PreparableReloadListener.SharedState currentReload) {
      ...
      List pendingStitches = new ArrayList(atlasCount);
      Map pendingStitchById = new HashMap(atlasCount);
      List readyForUploads = new ArrayList(atlasCount);
      this.atlasById.forEach((atlasId, atlasEntry) -> {
         CompletableFuture stitchingDone = new CompletableFuture();
   ...
   public CompletableFuture reload(final PreparableReloadListener.SharedState currentReload, final Executor taskExecutor, final PreparableReloadListener.PreparationBarrier preparationBarrier, final Executor reloadExecutor) {
      ...
      return pendingStitches.allReadyToUpload.thenCompose(preparationBarrier::wait).thenAcceptAsync((unused) -> this.updateSpriteMaps(pendingStitches), reloadExecutor);
   }
   ...
   private void updateSpriteMaps(final AtlasManager.PendingStitchResults pendingStitches) {
      ...
      Map globalSpriteLookup = new HashMap();
   ...
   private static record PendingStitch(AtlasManager.AtlasEntry entry, CompletableFuture preparations) {
