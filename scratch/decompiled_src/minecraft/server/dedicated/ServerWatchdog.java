...
public class ServerWatchdog implements Runnable {
   ...
   private static final Comparator THREAD_INFO_COMPARATOR = Comparator.comparing(ThreadInfo::isDaemon).thenComparing(ThreadInfo::getThreadState).thenComparing(ThreadInfo::getThreadName);
   ...
   public void run() {
      while(this.server.isRunning()) {
         ...
         if (deltaNanos > this.maxTickTimeNanos) {
            ...
            serverStats.setDetail("Level stats", (CrashReportDetail)(() -> (String)Streams.stream(this.server.getAllLevels()).map((level) -> String.valueOf(level.dimension().identifier()) + ": " + level.getWatchdogStats()).collect(Collectors.joining(",\n"))));
