...
public class GizmoFeatureRenderer extends RenderTypeFeatureRenderer {
   public static final FeatureRendererType TYPE = FeatureRendererType.create("Gizmo");
   ...
   private void buildTexts(final List texts, final CameraRenderState camera, final Font font) {
   ...
   private void buildLines(final List lines, final CameraRenderState camera, final Matrix4fc modelViewMatrix, final boolean opaque) {
      if (!lines.isEmpty()) {
         ...
         for(DrawableGizmoPrimitives.Line line : lines) {
            start.set(line.start().x() - camX, line.start().y() - camY, line.start().z() - camZ, 1.0D);
            end.set(line.end().x() - camX, line.end().y() - camY, line.end().z() - camZ, 1.0D);
   ...
   private void buildTriangleFans(final List triangleFans, final CameraRenderState camera) {
   ...
   private void buildQuads(final List quads, final CameraRenderState camera) {
      if (!quads.isEmpty()) {
   ...
   private void buildPoints(final List points, final CameraRenderState camera) {
      if (!points.isEmpty()) {
