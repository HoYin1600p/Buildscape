...
public class WorldBorder extends SavedData {
   ...
   public static final Codec CODEC = WorldBorder.Settings.CODEC.xmap(WorldBorder::new, WorldBorder.Settings::new);
   public static final SavedDataType TYPE = new SavedDataType(Identifier.withDefaultNamespace("world_border"), WorldBorder::new, CODEC, DataFixTypes.SAVED_DATA_WORLD_BORDER);
   ...
   public boolean isInsideCloseToBorder(final Entity source, final AABB boundingBox) {
      double bbMax = Math.max(Mth.absMax(boundingBox.getXsize(), boundingBox.getZsize()), 1.0D);
      return this.getDistanceToBorder(source) < bbMax * 2.0D && this.isWithinBounds(source.getX(), source.getZ(), bbMax);
   }
   ...
   private class MovingBorderExtent implements WorldBorder.BorderExtent {
      ...
      private double calculateSize() {
         ...
         return progress < 1.0D ? Mth.lerp(progress, this.from, this.to) : this.to;
      }
   ...
   public static record Settings(double centerX, double centerZ, double damagePerBlock, double safeZone, int warningBlocks, int warningTime, double size, long lerpTime, double lerpTarget) {
      ...
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.doubleRange(-2.9999984E7D, 2.9999984E7D).fieldOf("center_x").forGetter(WorldBorder.Settings::centerX), Codec.doubleRange(-2.9999984E7D, 2.9999984E7D).fieldOf("center_z").forGetter(WorldBorder.Settings::centerZ), Codec.DOUBLE.fieldOf("damage_per_block").forGetter(WorldBorder.Settings::damagePerBlock), Codec.DOUBLE.fieldOf("safe_zone").forGetter(WorldBorder.Settings::safeZone), Codec.INT.fieldOf("warning_blocks").forGetter(WorldBorder.Settings::warningBlocks), Codec.INT.fieldOf("warning_time").forGetter(WorldBorder.Settings::warningTime), Codec.DOUBLE.fieldOf("size").forGetter(WorldBorder.Settings::size), Codec.LONG.fieldOf("lerp_time").forGetter(WorldBorder.Settings::lerpTime), Codec.DOUBLE.fieldOf("lerp_target").forGetter(WorldBorder.Settings::lerpTarget)).apply(i, WorldBorder.Settings::new));
