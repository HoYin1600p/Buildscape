...
public class ServerExplosion implements Explosion {
            ...
            for(int zz = 0; zz < 16; ++zz) {
               if (xx == 0 || xx == 15 || yy == 0 || yy == 15 || zz == 0 || zz == 15) {
                  ...
                  for(float stepSize = 0.3F; remainingPower > 0.0F; remainingPower -= 0.22500001F) {
                     ...
                     Optional resistance = this.damageCalculator.getBlockExplosionResistance(this, this.level, pos, block, fluid);
   ...
   private void hurtEntities() {
      if (!(this.radius < 1.0E-5F)) {
         ...
         int x0 = Mth.floor(this.center.x - (double)doubleRadius - 1.0D);
         int x1 = Mth.floor(this.center.x + (double)doubleRadius + 1.0D);
         int y0 = Mth.floor(this.center.y - (double)doubleRadius - 1.0D);
         int y1 = Mth.floor(this.center.y + (double)doubleRadius + 1.0D);
         int z0 = Mth.floor(this.center.z - (double)doubleRadius - 1.0D);
         int z1 = Mth.floor(this.center.z + (double)doubleRadius + 1.0D);
         ...
         for(Entity entity : this.level.getEntities(this.source, new AABB((double)x0, (double)y0, (double)z0, (double)x1, (double)y1, (double)z1))) {
            if (!entity.ignoreExplosion(this)) {
               ...
               if (!(dist > 1.0D)) {
                  ...
                  double knockbackPower = (1.0D - dist) * (double)exposure * (double)knockbackMultiplier * (1.0D - knockbackResistance);
   ...
   public int explode() {
      ...
      List toBlow = this.calculateExplodedPositions();
      this.hurtEntities();
