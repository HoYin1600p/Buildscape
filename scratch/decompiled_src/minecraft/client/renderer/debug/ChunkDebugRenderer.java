...
public class ChunkDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public void emitGizmos(final double camX, final double camY, final double camZ, final DebugValueAccess debugValues, final Frustum frustum, final float partialTicks) {
      ...
      if (time - this.lastUpdateTime > 3.0E9D) {
         this.lastUpdateTime = time;
         IntegratedServer server = this.minecraft.getSingleplayerServer();
         if (server != null) {
            this.data = new ChunkDebugRenderer.ChunkData(this, server, camX, camZ);
         } else {
            this.data = null;
         }
      }
      ...
      if (this.data != null) {
         Map serverData = (Map)this.data.serverData.getNow((Object)null);
         double y = this.minecraft.gameRenderer.mainCamera().position().y * 0.85D;
   ...
   private final class ChunkData {
      ...
      private ChunkData(final ChunkDebugRenderer this$0, final IntegratedServer server, final double camX, final double camZ) {
         Objects.requireNonNull(this$0);
         super();
         ClientLevel clientLevel = this$0.minecraft.level;
