...
public class VaultSharedData {
   ...
   private Set connectedPlayers = new ObjectLinkedOpenHashSet();
   private double connectedParticlesRange = VaultConfig.DEFAULT.deactivationRange();
   boolean isDirty;

   public VaultSharedData(final ItemStack displayItem, final Set connectedPlayers, final double connectedParticlesRange) {
   ...
   void updateConnectedPlayersWithinRange(final ServerLevel serverLevel, final BlockPos pos, final VaultServerData serverData, final VaultConfig config, final double limit) {
      Set currentConnectedPlayers = (Set)config.playerDetector().detect(serverLevel, config.entitySelector(), pos, limit, false).stream().filter((uuid) -> !serverData.getRewardedPlayers().contains(uuid)).collect(Collectors.toSet());
