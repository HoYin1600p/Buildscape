...
public class LevelExtractor implements ResourceManagerReloadListener {
   ...
   public void extract(final DeltaTracker deltaTracker, final Camera camera, final float deltaPartialTick) {
      this.levelRenderState.worldPartialTicks = deltaPartialTick;
      ...
      this.extractPlayerState(camera, deltaPartialTick, this.levelRenderState.playerRenderState);
   ...
   private void extractVisibleBlockEntities(final Camera camera, final float deltaPartialTick, final LevelRenderState levelRenderState) {
      ...
      long chunkFadeDuration = Util.toMillis(this.minecraft.options.chunkSectionFadeInTime().get());
      Iterator iterator = this.levelRenderer.visibleSections().iterator();

      while(iterator.hasNext()) {
         SectionRenderDispatcher.RenderSection section = (SectionRenderDispatcher.RenderSection)iterator.next();
         List renderableBlockEntities = section.getSectionMesh().getRenderableBlockEntities();
   ...
   private void extractBlockDestroyAnimation(final Camera camera, final LevelRenderState levelRenderState) {
      ...
      while(var10.hasNext()) {
         Long2ObjectMap.Entry entry = (Long2ObjectMap.Entry)var10.next();
         BlockPos pos = BlockPos.of(entry.getLongKey());
         if (!(pos.distToCenterSqr(camX, camY, camZ) > 1024.0D)) {
   ...
   private void extractBlockOutline(final Camera camera, final LevelRenderState levelRenderState) {
      ...
      HitResult pos = this.minecraft.hitResult;
   ...
   private void extractPlayerState(final Camera camera, final float deltaPartialTick, final PlayerRenderState state) {
      state.reset();
      LocalPlayer player = this.minecraft.player;
      if (player != null) {
         state.hasPlayer = true;
         EntityRenderState entityRenderState = this.extractEntity(player, deltaPartialTick);
         if (entityRenderState instanceof AvatarRenderState) {
            ...
            player.firstPersonHandsAndItems().extractRenderState(player, deltaPartialTick, state.firstPersonHandsAndItems);
            state.portalEffectIntensity = Mth.lerp(deltaPartialTick, player.oPortalEffectIntensity, player.portalEffectIntensity);
            state.nauseaEffectIntensity = player.getEffectBlendFactor(MobEffects.NAUSEA, deltaPartialTick);
            state.spinningEffectAngle = player.getSpinningEffectAngle(deltaPartialTick);
            state.isUnderWater = player.isUnderWater();
            state.eyePositionY = player.getEyePosition(deltaPartialTick).y;
   ...
   public @Nullable String entityStatistics() {
      return this.level == null ? null : "E: " + this.levelRenderState.lastEntityRenderStateCount + "/" + this.level.getEntityCount() + ", SD: " + this.level.getServerSimulationDistance();
   }

   @VisibleForDebug
   public double totalSections() {
      return this.sectionUpdateTracker == null ? 0.0D : (double)this.sectionUpdateTracker.size();
   }
