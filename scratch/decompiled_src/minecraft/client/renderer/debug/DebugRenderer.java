...
public class DebugRenderer {
   ...
   public static Optional getTargetedEntity(final @Nullable Entity cameraEntity, final int maxTargetingRange) {
      if (cameraEntity == null) {
         ...
         AABB box = cameraEntity.getBoundingBox().expandTowards(pick).inflate(1.0D);
   ...
   private static Vec3 mixColor(final float hueShift) {
      ...
      switch (region) {
         case 0:
            var10000 = new Vec3(1.0D, (double)progress, 0.0D);
            break;
         case 1:
            var10000 = new Vec3((double)(1.0F - progress), 1.0D, 0.0D);
            break;
         case 2:
            var10000 = new Vec3(0.0D, 1.0D, (double)progress);
            break;
         case 3:
            var10000 = new Vec3(0.0D, 1.0D - (double)progress, 1.0D);
            break;
         case 4:
            var10000 = new Vec3((double)progress, 0.0D, 1.0D);
            break;
         case 5:
            var10000 = new Vec3(1.0D, 0.0D, 1.0D - (double)progress);
            break;
         default:
   ...
   private static Vec3 shiftHue(final float r, final float g, final float b, final float hs) {
      ...
      double max = Math.max(Math.max(1.0D, combined.x), Math.max(combined.y, combined.z));
