...
public class SheepColorSpawnRules {
   ...
   private static SheepColorSpawnRules.SheepColorSpawnConfiguration getSheepColorConfiguration(final Holder biome) {
   ...
   private static SheepColorSpawnRules.SheepColorProvider weighted(final WeightedList elements) {
      if (elements.isEmpty()) {
         throw new IllegalArgumentException("List must be non-empty");
      } else {
         return (random) -> ((SheepColorSpawnRules.SheepColorProvider)elements.getRandomOrThrow(random)).get(random);
      }
   }
