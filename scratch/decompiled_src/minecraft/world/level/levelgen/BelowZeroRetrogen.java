...
public final class BelowZeroRetrogen {
   ...
   private static final Codec BITSET_CODEC = Codec.LONG_STREAM.xmap((longStream) -> BitSet.valueOf(longStream.toArray()), (bitSet) -> LongStream.of(bitSet.toLongArray()));
   private static final Codec NON_EMPTY_CHUNK_STATUS = BuiltInRegistries.CHUNK_STATUS.byNameCodec().comapFlatMap((status) -> status == ChunkStatus.EMPTY ? DataResult.error(() -> "target_status cannot be empty") : DataResult.success(status), Function.identity());
   public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(NON_EMPTY_CHUNK_STATUS.fieldOf("target_status").forGetter(BelowZeroRetrogen::targetStatus), BITSET_CODEC.lenientOptionalFieldOf("missing_bedrock").forGetter((b) -> b.missingBedrock.isEmpty() ? Optional.empty() : Optional.of(b.missingBedrock))).apply(i, BelowZeroRetrogen::new));
   private static final Set RETAINED_RETROGEN_BIOMES = Set.of(Biomes.LUSH_CAVES, Biomes.DRIPSTONE_CAVES, Biomes.DEEP_DARK);
   ...
   public static BiomeResolver getBiomeResolver(final BiomeResolver biomeResolver, final ChunkAccess protoChunk) {
      if (!protoChunk.isUpgrading()) {
         ...
         Predicate retainedBiomes = RETAINED_RETROGEN_BIOMES::contains;
         return (quartX, quartY, quartZ) -> {
            Holder noiseBiome = biomeResolver.getNoiseBiome(quartX, quartY, quartZ);
