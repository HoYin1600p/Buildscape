...
public class AnimalPanic extends Behavior {
   ...
   public AnimalPanic(final float speedMultiplier, final int flyHeight) {
      this(speedMultiplier, (var0) -> DamageTypeTags.PANIC_CAUSES, (mob) -> AirAndWaterRandomPos.getPos(mob, 5, 4, flyHeight, mob.getViewVector(0.0F).x, mob.getViewVector(0.0F).z, (double)((float)Math.PI / 2F)));
   }
   ...
   protected boolean checkExtraStartConditions(final ServerLevel level, final PathfinderMob body) {
      return body.getBrain().getMemory(MemoryModuleType.HURT_BY).map((d) -> d.is((TagKey)this.panicCausingDamageTypes.apply(body))).orElse(false) || body.getBrain().hasMemoryValue(MemoryModuleType.IS_PANICKING);
   }
   ...
   private @Nullable Vec3 getPanicPos(final PathfinderMob body, final ServerLevel level) {
      if (body.isOnFire()) {
         Optional nearestWater = this.lookForWater(level, body).map(Vec3::atBottomCenterOf);
   ...
   private Optional lookForWater(final LevelReader level, final Entity mob) {
