...
public class SimpleSoundInstance extends AbstractSoundInstance {
   public SimpleSoundInstance(final SoundEvent sound, final SoundSource source, final float volume, final float pitch, final RandomSource random, final BlockPos pos) {
      this(sound, source, volume, pitch, random, (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D);
   }
   ...
   public static SimpleSoundInstance forUI(final SoundEvent sound, final float pitch, final float volume) {
      return new SimpleSoundInstance(sound.location(), SoundSource.UI, volume, pitch, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true);
   }

   public static SimpleSoundInstance forMusic(final SoundEvent sound) {
      return new SimpleSoundInstance(sound.location(), SoundSource.MUSIC, 1.0F, 1.0F, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true);
   }
   ...
   public static SimpleSoundInstance forLocalAmbience(final SoundEvent sound, final float pitch, final float volume) {
      return new SimpleSoundInstance(sound.location(), SoundSource.AMBIENT, volume, pitch, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true);
   }
