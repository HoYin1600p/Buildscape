...
public abstract class ProjectileWeaponItem extends Item {
   ...
   public static final Predicate ARROW_OR_FIREWORK = ARROW_ONLY.or((itemStack) -> itemStack.is(Items.FIREWORK_ROCKET));
   ...
   protected void shoot(final ServerLevel level, final LivingEntity shooter, final InteractionHand hand, final ItemStack weapon, final List projectiles, final float power, final float uncertainty, final boolean isCrit, final @Nullable LivingEntity targetOverride) {
      ...
      for(int i = 0; i < projectiles.size(); ++i) {
         ...
         if (!projectile.isEmpty()) {
            ...
            direction = -direction;
            int index = i;
            Projectile.spawnProjectile(this.createProjectile(level, shooter, weapon, projectile, isCrit), level, projectile, (projectileEntity) -> this.shootProjectile(shooter, projectileEntity, index, power, uncertainty, angle, targetOverride));
   ...
   protected static List draw(final ItemStack weapon, final ItemStack projectile, final LivingEntity shooter) {
      if (projectile.isEmpty()) {
         ...
         Level projectileCopy = shooter.level();
         int var10000;
         if (projectileCopy instanceof ServerLevel) {
            ServerLevel serverLevel = (ServerLevel)projectileCopy;
         ...
         List drawn = new ArrayList(numProjectiles);
