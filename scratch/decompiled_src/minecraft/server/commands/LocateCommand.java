...
public class LocateCommand {
   ...
   private static Optional getHolders(final ResourceOrTagKeyArgument.Result resourceOrTag, final Registry registry) {
      return (Optional)resourceOrTag.unwrap().map((id) -> registry.get(id).map((xva$0) -> HolderSet.direct(xva$0)), registry::get);
   }
   ...
   private static int locateStructure(final CommandSourceStack source, final ResourceOrTagKeyArgument.Result resourceOrTag) throws CommandSyntaxException {
      Registry registry = source.getLevel().registryAccess().lookupOrThrow(Registries.STRUCTURE);
      HolderSet target = (HolderSet)getHolders(resourceOrTag, registry).orElseThrow(() -> ERROR_STRUCTURE_INVALID.create(resourceOrTag.asPrintable()));
      BlockPos sourcePos = BlockPos.containing(source.getPosition());
      ServerLevel serverLevel = source.getLevel();
      Stopwatch stopwatch = Stopwatch.createStarted(Util.TICKER);
      Pair nearest = serverLevel.getChunkSource().getGenerator().findNearestMapStructure(serverLevel, target, sourcePos, 100, false);
      stopwatch.stop();
      if (nearest == null) {
   ...
   private static int locateBiome(final CommandSourceStack source, final ResourceOrTagArgument.Result elementOrTag) throws CommandSyntaxException {
      BlockPos sourcePos = BlockPos.containing(source.getPosition());
      Stopwatch stopwatch = Stopwatch.createStarted(Util.TICKER);
      Pair nearest = source.getLevel().findClosestBiome3d(elementOrTag, sourcePos, 6400, 32, 64);
      stopwatch.stop();
      if (nearest == null) {
   ...
   private static int locatePoi(final CommandSourceStack source, final ResourceOrTagArgument.Result resourceOrTag) throws CommandSyntaxException {
      ...
      ServerLevel serverLevel = source.getLevel();
      Stopwatch stopwatch = Stopwatch.createStarted(Util.TICKER);
      Optional closestWithType = serverLevel.getPoiManager().findClosestWithType(resourceOrTag, sourcePos, 256, PoiManager.Occupancy.ANY);
      stopwatch.stop();
   ...
   private static int showLocateResult(final CommandSourceStack source, final BlockPos sourcePos, final Pair found, final String successMessageKey, final boolean includeY, final String foundName, final Duration taskDuration) {
      ...
      Component coordinates = ComponentUtils.wrapInSquareBrackets(Component.translatable("chat.coordinates", foundPos.getX(), displayedY, foundPos.getZ())).withStyle((s) -> s.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent.SuggestCommand("/tp @s " + foundPos.getX() + " " + displayedY + " " + foundPos.getZ())).withHoverEvent(new HoverEvent.ShowText(Component.translatable("chat.coordinates.tooltip"))));
