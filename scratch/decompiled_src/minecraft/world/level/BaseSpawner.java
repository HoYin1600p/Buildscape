...
public abstract class BaseSpawner {
   ...
   public void setEntityData(final TypedEntityData entityData, final @Nullable Level level, final RandomSource random, final BlockPos pos) {
   ...
   public void clientTick(final Level level, final BlockPos pos) {
      if (!this.isNearPlayer(level, pos)) {
         ...
         level.addParticle(ParticleTypes.FLAME, xP, yP, zP, 0.0D, 0.0D, 0.0D);
         ...
         if (this.spawnDelay > 0) {
            ...
            for(int c = 0; c < this.spawnCount; ++c) {
               try (ProblemReporter.ScopedCollector reporter = new ProblemReporter.ScopedCollector(this::toString, LOGGER)) {
                  ...
                  Vec3 spawnPos = (Vec3)input.read("Pos", Vec3.CODEC).orElseGet(() -> new Vec3((double)pos.getX() + (random.nextDouble() - random.nextDouble()) * (double)this.spawnRange + 0.5D, (double)(pos.getY() + random.nextInt(3) - 1), (double)pos.getZ() + (random.nextDouble() - random.nextDouble()) * (double)this.spawnRange + 0.5D));
