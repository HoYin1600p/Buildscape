...
public abstract class DisplayRenderer extends EntityRenderer {
   ...
   protected AABB getBoundingBoxForCulling(final Display entity, final float partialTicks) {
   ...
   private Quaternionf calculateOrientation(final Display.RenderState renderState, final DisplayEntityRenderState state, final Quaternionf output) {
      ...
      switch (renderState.billboardConstraints()) {
         case FIXED:
            var10000 = output.rotationYXZ(-0.017453292F * state.entityYRot, ((float)Math.PI / 180F) * state.entityXRot, 0.0F);
            break;
         case HORIZONTAL:
            var10000 = output.rotationYXZ(-0.017453292F * state.entityYRot, ((float)Math.PI / 180F) * transformXRot(state.cameraXRot), 0.0F);
            break;
         case VERTICAL:
            var10000 = output.rotationYXZ(-0.017453292F * transformYRot(state.cameraYRot), ((float)Math.PI / 180F) * state.entityXRot, 0.0F);
            break;
         case CENTER:
            var10000 = output.rotationYXZ(-0.017453292F * transformYRot(state.cameraYRot), ((float)Math.PI / 180F) * transformXRot(state.cameraXRot), 0.0F);
            break;
         default:
   ...
   public static class TextDisplayRenderer extends DisplayRenderer {
      ...
      private Display.TextDisplay.CachedInfo splitLines(final Component input, final int width) {
         List lines = this.font.split(input, width);
         ...
         for(Display.TextDisplay.CachedLine line : cachedInfo.lines()) {
            ...
            switch (alignment) {
               case LEFT:
                  ...
                  break;
               case RIGHT:
                  var10000 = (float)(width - line.width());
                  break;
               case CENTER:
                  var10000 = (float)width / 2.0F - (float)line.width() / 2.0F;
                  break;
               default:
                  throw new MatchException((String)null, (Throwable)null);
            }
