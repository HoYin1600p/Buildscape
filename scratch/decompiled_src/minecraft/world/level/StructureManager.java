...
public class StructureManager {
   ...
   public List startsForStructure(final ChunkPos pos, final Predicate matcher) {
      Map allReferences = this.level.getChunk(pos.x(), pos.z(), ChunkStatus.STRUCTURE_REFERENCES).getAllReferences();
   ...
   public StructureStart getStructureWithPieceAt(final BlockPos blockPos, final Predicate predicate) {
      Registry structures = this.registryAccess().lookupOrThrow(Registries.STRUCTURE);

      for(StructureStart structureStart : this.startsForStructure(ChunkPos.containing(blockPos), (s) -> structures.get(structures.getId(s)).map(predicate::test).orElse(false))) {
