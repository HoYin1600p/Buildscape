...
public class Hud {
   ...
   private static final Comparator SCORE_DISPLAY_ORDER = Comparator.comparing(PlayerScoreEntry::value).reversed().thenComparing(PlayerScoreEntry::owner, String.CASE_INSENSITIVE_ORDER);
   ...
   private Pair contextualInfoBar = Pair.of(Hud.ContextualInfo.EMPTY, ContextualBar.EMPTY);
   private final Map contextualInfoBars;
   ...
   private void extractScoreboardSidebar(final GuiGraphicsExtractor graphics, final DeltaTracker deltaTracker) {
      ...
      if (playerTeam != null) {
         Optional teamColor = playerTeam.getColor();
   ...
   private int getVisibleVehicleHeartRows(final int hearts) {
      return (int)Math.ceil((double)hearts / 10.0D);
   }
   ...
   private void extractHearts(final GuiGraphicsExtractor graphics, final Player player, final int xLeft, final int yLineBase, final int healthRowHeight, final int heartOffsetIndex, final float maxHealth, final int currentHealth, final int oldHealth, final int absorption, final boolean blink) {
      ...
      int healthContainerCount = Mth.ceil((double)maxHealth / 2.0D);
      int absorptionContainerCount = Mth.ceil((double)absorption / 2.0D);
   ...
   private void extractHeart(final GuiGraphicsExtractor graphics, final Hud.HeartType type, final int xo, final int yo, final boolean isHardcore, final boolean blinks, final boolean half) {
   ...
   private static enum ContextualInfo {
      ...
      private static Hud.ContextualInfo[] $values() {
         return new Hud.ContextualInfo[]{EMPTY, EXPERIENCE, LOCATOR, JUMPABLE_VEHICLE};
      }
   }
   ...
   private static enum HeartType {
      ...
      // $FF: synthetic method
      private static Hud.HeartType[] $values() {
         return new Hud.HeartType[]{CONTAINER, NORMAL, POISIONED, WITHERED, ABSORBING, FROZEN};
      }
   }
}
