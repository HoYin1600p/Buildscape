...
public class HelpCommand {
   ...
   public static void register(final CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("help").executes((s) -> {
         Map usage = dispatcher.getSmartUsage(dispatcher.getRoot(), (CommandSourceStack)s.getSource());
         ...
         ParseResults command = dispatcher.parse(StringArgumentType.getString(s, "command"), (CommandSourceStack)s.getSource());
         if (command.getContext().getNodes().isEmpty()) {
            throw ERROR_FAILED.create();
         } else {
            Map usage = dispatcher.getSmartUsage(((ParsedCommandNode)Iterables.getLast(command.getContext().getNodes())).getNode(), (CommandSourceStack)s.getSource());
