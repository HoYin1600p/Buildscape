...
public class PathPackResources extends AbstractPackMetadataResources implements PackResources {
   ...
   public static @Nullable IoSupplier getResource(final Path topDir, final Identifier location) {
   ...
   public static void listPath(final String namespace, final Path topDir, final List decomposedPrefixPath, final PackResources.ResourceOutput output) {
      Path targetPath = FileUtil.resolvePath(topDir, decomposedPrefixPath);

      try {
         Stream files = Files.find(targetPath, Integer.MAX_VALUE, PathPackResources::isRegularFile, new FileVisitOption[0]);
   ...
   public static Set getNamespaces(final Path rootDir) {
      Set namespaces = new HashSet();

      try {
         DirectoryStream directDirs = Files.newDirectoryStream(rootDir);
   ...
   public static class PathResourcesSupplier implements Pack.ResourcesSupplier {
      ...
      public Stream openResources(final PackLocationInfo location, final Pack.Metadata metadata) {
         ...
         if (overlays.isEmpty()) {
            ...
            List overlayResources = new ArrayList(overlays.size());
