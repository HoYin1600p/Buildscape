...
public class PlayerSensor extends Sensor {
   ...
   protected void doTick(final ServerLevel level, final LivingEntity body) {
      List players = (List)level.players().stream().filter(EntitySelector.NO_SPECTATORS).filter((player) -> body.closerThan(player, this.getFollowDistance(body))).sorted(Comparator.comparingDouble(body::distanceToSqr)).collect(Collectors.toList());
      Brain brain = body.getBrain();
      brain.setMemory(MemoryModuleType.NEAREST_PLAYERS, players);
      List visiblePlayers = (List)players.stream().filter((livingEntity) -> isEntityTargetable(level, body, livingEntity)).collect(Collectors.toList());
      brain.setMemory(MemoryModuleType.NEAREST_VISIBLE_PLAYER, visiblePlayers.isEmpty() ? null : (Player)visiblePlayers.get(0));
      List visibleAttackablePlayers = visiblePlayers.stream().filter((livingEntity) -> isEntityAttackable(level, body, livingEntity)).toList();
