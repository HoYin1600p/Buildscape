...
public class HappyGhast extends Animal {
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.HURT_BY, SensorType.FOOD_TEMPTATIONS, SensorType.NEAREST_ADULT_ANY_TYPE, SensorType.NEAREST_PLAYERS), (var0) -> HappyGhastAi.getActivities());
   ...
   private static final EntityDataAccessor IS_LEASH_HOLDER = SynchedEntityData.defineId(HappyGhast.class, EntityDataSerializers.BOOLEAN);
   private static final EntityDataAccessor STAYS_STILL = SynchedEntityData.defineId(HappyGhast.class, EntityDataSerializers.BOOLEAN);
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(4, new TemptGoal.ForNonPathfinders(this, 1.0D, (itemStack) -> !this.isWearingBodyArmor() && !this.isBaby() ? itemStack.is(ItemTags.HAPPY_GHAST_TEMPT_ITEMS) : itemStack.is(ItemTags.HAPPY_GHAST_FOOD), false, 7.0D));
   ...
   public @Nullable AgeableMob getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      return (AgeableMob)EntityTypes.HAPPY_GHAST.create(level, EntitySpawnReason.BREEDING);
   }
   ...
   public @Nullable LivingEntity getControllingPassenger() {
      ...
      return (LivingEntity)(this.isWearingBodyArmor() && !this.isOnStillTimeout() && firstPassenger instanceof Player ? (Player)firstPassenger : super.getControllingPassenger());
   }
   ...
   private void continuousHeal() {
      Level isFastHealing = this.level();
      if (isFastHealing instanceof ServerLevel level) {
   ...
   private boolean scanPlayerAboveGhast() {
      ...
      AABB ghastDetectionBox = new AABB(happyGhastBb.minX - 1.0D, happyGhastBb.maxY - (double)1.0E-5F, happyGhastBb.minZ - 1.0D, happyGhastBb.maxX + 1.0D, happyGhastBb.maxY + happyGhastBb.getYsize() / 2.0D, happyGhastBb.maxZ + 1.0D);
