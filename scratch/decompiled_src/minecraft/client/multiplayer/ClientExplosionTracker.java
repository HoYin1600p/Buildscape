   ...
   public void tick(final ClientLevel level) {
      if (Minecraft.getInstance().options.particles().get() != ParticleStatus.ALL) {
         ...
         for(int i = 0; i < totalParticles; ++i) {
            WeightedRandom.getRandomItem(level.getRandom(), this.explosions, totalBlocks, ClientExplosionTracker.ExplosionInfo::blockCount).ifPresent((info) -> this.addParticle(level, info));
         }
