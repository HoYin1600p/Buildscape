...
public class Connection extends SimpleChannelInboundHandler {
   ...
   private void validateListener(final ProtocolInfo protocol, final PacketListener packetListener) {
      ...
      if (listenerFlow != this.receiving) {
         throw new IllegalStateException("Trying to set listener for wrong side: connection is " + String.valueOf(this.receiving) + ", but listener is " + String.valueOf(listenerFlow));
      } else {
         ConnectionProtocol listenerProtocol = packetListener.protocol();
         if (protocol.id() != listenerProtocol) {
            throw new IllegalStateException("Listener protocol (" + String.valueOf(listenerProtocol) + ") does not match requested one " + String.valueOf(protocol));
         }
      }
   }
