...
public class CreativeModeInventoryScreen extends AbstractContainerScreen {
   ...
   private boolean tryRebuildTabContents(final @Nullable SessionSearchTrees searchTrees, final FeatureFlagSet enabledFeatures, final boolean hasPermissions, final HolderLookup.Provider holders) {
      if (!CreativeModeTabs.tryRebuildTabContents(enabledFeatures, hasPermissions, holders)) {
         ...
         if (searchTrees != null) {
            List creativeSearchItems = List.copyOf(CreativeModeTabs.searchTab().getDisplayItems());
   ...
   private void refreshCurrentTabContents(final Collection displayList) {
      int oldRowIndex = ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getRowIndexForScroll(this.scrollOffs);
      ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).items.clear();
      if (selectedTab.getType() == CreativeModeTab.Type.SEARCH) {
         this.refreshSearchResults();
      } else {
         ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).items.addAll(displayList);
      }

      this.scrollOffs = ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getScrollForRowIndex(oldRowIndex);
      ...
      if (containerInput != ContainerInput.THROW || this.minecraft.player.canDropItems()) {
         ...
         if (slot == null && selectedTab.getType() != CreativeModeTab.Type.INVENTORY && containerInput != ContainerInput.QUICK_CRAFT) {
            if (!((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried().isEmpty() && this.hasClickedOutside) {
               ...
               if (buttonNum == 0) {
                  this.minecraft.player.drop(((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried(), true, Prediction.PREDICTED);
               ...
               if (buttonNum == 1) {
                  ItemStack removedItem = ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried().split(1);
            ...
            if (slot == this.destroyItemSlot && quickKey) {
               ...
               } else if (containerInput == ContainerInput.THROW && slotId == -999 && !((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried().isEmpty()) {
                  this.minecraft.player.drop(((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried(), true, Prediction.PREDICTED);
                  this.minecraft.gameMode.handleCreativeModeItemDrop(((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried());
                  ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).setCarried(ItemStack.EMPTY);
               } else {
                  this.minecraft.player.inventoryMenu.clicked(slot == null ? slotId : ((CreativeModeInventoryScreen.SlotWrapper)slot).target.index, buttonNum, containerInput, this.minecraft.player);
                  this.minecraft.player.inventoryMenu.broadcastChanges();
               }
            } else if (containerInput != ContainerInput.QUICK_CRAFT && slot.container == CONTAINER) {
               ItemStack carried = ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried();
               ...
               if (containerInput == ContainerInput.CLONE) {
                  if (((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried().isEmpty() && slot.hasItem()) {
                     ...
                     ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).setCarried(item.copyWithCount(item.getMaxStackSize()));
                  }
               ...
               if (!carried.isEmpty() && !clicked.isEmpty() && ItemStack.isSameItemSameComponents(carried, clicked)) {
                  ...
                  ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).setCarried(clicked.copyWithCount(count));
               } else if (buttonNum == 0) {
                  ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).setCarried(ItemStack.EMPTY);
               ...
               ItemStack oldItemStack = slot == null ? ItemStack.EMPTY : ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getSlot(slot.index).getItem();
               ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).clicked(slot == null ? slotId : slot.index, buttonNum, containerInput, this.minecraft.player);
               if (AbstractContainerMenu.getQuickcraftHeader(buttonNum) == 2) {
                  for(int i = 0; i < 9; ++i) {
                     this.minecraft.gameMode.handleCreativeModeItemAdd(((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getSlot(45 + i).getItem(), 36 + i);
                  }
               } else if (slot != null && Inventory.isHotbarSlot(slot.getContainerSlot()) && selectedTab.getType() != CreativeModeTab.Type.INVENTORY) {
                  if (containerInput == ContainerInput.THROW && !oldItemStack.isEmpty() && !((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).getCarried().isEmpty()) {
   ...
   private void refreshSearchResults() {
      ...
      if (searchTerm.isEmpty()) {
         ...
         if (connection != null) {
            ...
            ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).items.addAll(tree.search(searchTerm.toLowerCase(Locale.ROOT)));
         }
      }
   ...
   private void selectTab(final CreativeModeTab tab) {
            ...
            if (hotbar.isEmpty()) {
               for(int i = 0; i < 9; ++i) {
                  if (i == hotbarIndex) {
                     ...
                     ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).items.add(placeholder);
                  } else {
      ...
      if (selectedTab.getType() == CreativeModeTab.Type.INVENTORY) {
         ...
         for(int i = 0; i < invMenu.slots.size(); ++i) {
            ...
            Slot slot = new CreativeModeInventoryScreen.SlotWrapper((Slot)invMenu.slots.get(i), i, x, y);
   ...
   public boolean mouseScrolled(final double x, final double y, final double scrollX, final double scrollY) {
      if (super.mouseScrolled(x, y, scrollX, scrollY)) {
         ...
         this.scrollOffs = ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).subtractInputFromScroll(this.scrollOffs, scrollY);
         ((CreativeModeInventoryScreen.ItemPickerMenu)this.menu).scrollTo(this.scrollOffs);
         return true;
      }
   }
   ...
   public List getTooltipFromContainerItem(final ItemStack itemStack) {
      ...
      List originalLines = itemStack.getTooltipLines(Item.TooltipContext.of(this.minecraft.level), this.minecraft.player, tooltipStyle);
      if (originalLines.isEmpty()) {
         return originalLines;
      } else if (isSingleCategoryTab && isCreativeSlot) {
         return originalLines;
      } else {
         List linesToDisplay = Lists.newArrayList(originalLines);
   ...
   public static void handleHotbarLoadOrSave(final Minecraft minecraft, final int index, final boolean isLoadPressed, final boolean isSavePressed) {
      ...
      if (isLoadPressed) {
         List originalItems = hotbar.load(registries);
   ...
   public static class ItemPickerMenu extends AbstractContainerMenu {
      ...
      protected int getRowIndexForScroll(final float scrollOffs) {
         return Math.max((int)((double)(scrollOffs * (float)this.calculateRowCount()) + 0.5D), 0);
      }
