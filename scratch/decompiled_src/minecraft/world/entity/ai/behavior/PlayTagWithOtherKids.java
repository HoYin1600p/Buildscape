...
public class PlayTagWithOtherKids {
   ...
   public static BehaviorControl create() {
      return BehaviorBuilder.create((i) -> i.group(i.present(MemoryModuleType.VISIBLE_VILLAGER_BABIES), i.absent(MemoryModuleType.WALK_TARGET), i.registered(MemoryModuleType.LOOK_TARGET), i.registered(MemoryModuleType.INTERACTION_TARGET)).apply(i, (babies, walkTarget, lookTarget, interactionTarget) -> (level, me, timestamp) -> {
               if (level.getRandom().nextInt(10) != 0) {
                  ...
                  Optional otherKidChasingMe = friendsNearby.stream().filter((friend) -> isFriendChasingMe(me, friend)).findAny();
                  if (!otherKidChasingMe.isPresent()) {
                     Optional otherKidBeingChased = findSomeoneBeingChased(friendsNearby);
   ...
   private static Optional findSomeoneBeingChased(final List friendsNearby) {
      Map chasedKids = checkHowManyChasersEachFriendHas(friendsNearby);
      return chasedKids.entrySet().stream().sorted(Comparator.comparingInt(Map.Entry::getValue)).filter((entry) -> entry.getValue() > 0 && entry.getValue() <= 5).map(Map.Entry::getKey).findFirst();
   }

   private static Map checkHowManyChasersEachFriendHas(final List friendsNearby) {
      Map chasedKids = Maps.newHashMap();
