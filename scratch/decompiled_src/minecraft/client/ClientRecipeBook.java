...
public class ClientRecipeBook extends RecipeBook {
   ...
   public void rebuildCollections() {
      Map recipeListsByCategory = categorizeAndGroupRecipes(this.known.values());
      Map byCategory = new HashMap();
      ImmutableList.Builder all = ImmutableList.builder();
      recipeListsByCategory.forEach((category, categoryRecipes) -> byCategory.put(category, (List)categoryRecipes.stream().map(RecipeCollection::new).peek(all::add).collect(ImmutableList.toImmutableList())));
   ...
   private static Map categorizeAndGroupRecipes(final Iterable recipes) {
      ...
      Table multiItemGroups = HashBasedTable.create();
      ...
      for(RecipeDisplayEntry entry : recipes) {
         ...
         if (groupId.isEmpty()) {
            ...
            List groupRecipes = (List)multiItemGroups.get(category, groupId.getAsInt());
