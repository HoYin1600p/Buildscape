...
public final class MinecraftServerStatistics implements DynamicMBean {
   ...
   private MinecraftServerStatistics(final MinecraftServer server) {
      this.server = server;
      MBeanAttributeInfo[] mBeanAttributeInfos = (MBeanAttributeInfo[])this.attributeDescriptionByName.values().stream().map(MinecraftServerStatistics.AttributeDescription::asMBeanAttributeInfo).toArray((x$0) -> new MBeanAttributeInfo[x$0]);
   ...
   public @Nullable Object getAttribute(final String attribute) {
      MinecraftServerStatistics.AttributeDescription attributeDescription = (MinecraftServerStatistics.AttributeDescription)this.attributeDescriptionByName.get(attribute);
   ...
   public AttributeList getAttributes(final String[] attributes) {
      List attributeList = (List)Arrays.stream(attributes).map(this.attributeDescriptionByName::get).filter(Objects::nonNull).map((attributeDescription) -> new Attribute(attributeDescription.name, attributeDescription.getter.get())).collect(Collectors.toList());
