...
public class Camel extends AbstractHorse {
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.HURT_BY, SensorType.FOOD_TEMPTATIONS, SensorType.NEAREST_ADULT), (var0) -> CamelAi.getActivities());
   public static final EntityDataAccessor DASH = SynchedEntityData.defineId(Camel.class, EntityDataSerializers.BOOLEAN);
   public static final EntityDataAccessor LAST_POSE_CHANGE_TICK = SynchedEntityData.defineId(Camel.class, EntityDataSerializers.LONG);
   ...
   protected void executeRidersJump(final float amount, final Vec3 input) {
      ...
      this.addDeltaMovement(this.getLookAngle().multiply(1.0D, 0.0D, 1.0D).normalize().scale((double)(22.2222F * amount) * this.getAttributeValue(Attributes.MOVEMENT_SPEED) * (double)this.getBlockSpeedFactor()).add(0.0D, (double)(1.4285F * amount) * jumpMomentum, 0.0D));
   ...
   public @Nullable Camel getBreedOffspring(final ServerLevel level, final AgeableMob partner) {
      return (Camel)EntityTypes.CAMEL.create(level, EntitySpawnReason.BREEDING);
   }
   ...
   protected Vec3 getPassengerAttachmentPoint(final Entity passenger, final EntityDimensions dimensions, final float scale) {
      ...
      return (new Vec3(0.0D, (double)height, (double)(offset * scale))).yRot(-this.getYRot() * ((float)Math.PI / 180F));
   }
   ...
   private double getBodyAnchorAnimationYOffset(final boolean isFront, final float partialTicks, final EntityDimensions dimensions, final float scale) {
      double ageSitYOffset = this.isBaby() ? 0.09375D : 0.375D;
   ...
   public Vec3 getLeashOffset(final float partialTicks) {
      ...
      return new Vec3(0.0D, this.getBodyAnchorAnimationYOffset(true, partialTicks, dimensions, scale) - (double)(0.2F * scale), (double)(dimensions.width() * 0.56F));
   }
   ...
   private static class CamelMoveControl extends MoveControl {
      public CamelMoveControl(final Camel camel) {
         super(camel);
      }
