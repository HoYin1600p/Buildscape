...
public class GossipContainer {
   public static final Codec CODEC = GossipContainer.GossipEntry.CODEC.listOf().xmap(GossipContainer::new, (container) -> container.unpack().toList());
   ...
   private Collection selectGossipsForTransfer(final RandomSource random, final int maxCount) {
      List entries = this.unpack().toList();
      if (entries.isEmpty()) {
   ...
   public void transferFrom(final GossipContainer source, final RandomSource random, final int maxCount) {
      Collection newGossips = source.selectGossipsForTransfer(random, maxCount);
