...
public class Warden extends Monster implements VibrationSystem {
   ...
   private static final EntityDataAccessor CLIENT_ANGER_LEVEL = SynchedEntityData.defineId(Warden.class, EntityDataSerializers.INT);
   ...
   private static final Brain.Provider BRAIN_PROVIDER = Brain.provider(List.of(MemoryModuleType.NEAREST_VISIBLE_NEMESIS, MemoryModuleType.RECENT_PROJECTILE, MemoryModuleType.TOUCH_COOLDOWN, MemoryModuleType.VIBRATION_COOLDOWN), List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR), WardenAi::getActivities);
   ...
   public boolean hurtServer(final ServerLevel level, final DamageSource source, final float damage) {
      ...
      if (!this.isNoAi() && !this.isDiggingOrEmerging()) {
         ...
         if (this.brain.getMemory(MemoryModuleType.ATTACK_TARGET).isEmpty() && attacker instanceof LivingEntity) {
            ...
            if (source.isDirect() || this.closerThan(livingAttacker, 5.0D)) {
      ...
      public void onReceiveVibration(final ServerLevel level, final BlockPos pos, final Holder event, final @Nullable Entity sourceEntity, final @Nullable Entity projectileOwner, final float receivingDistance) {
         if (!Warden.this.isDeadOrDying()) {
            ...
            if (!Warden.this.getAngerLevel().isAngry()) {
               Optional activeEntity = Warden.this.angerManagement.getActiveEntity();
