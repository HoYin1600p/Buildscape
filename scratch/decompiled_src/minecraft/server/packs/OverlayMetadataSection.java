...
public record OverlayMetadataSection(List overlays) {
   ...
   public static final MetadataSectionType CLIENT_TYPE = new MetadataSectionType("overlays", codecForPackType(PackType.CLIENT_RESOURCES));
   public static final MetadataSectionType SERVER_TYPE = new MetadataSectionType("overlays", codecForPackType(PackType.SERVER_DATA));
   ...
   public static record OverlayEntry(InclusiveRange format, String overlay) {
      private static Codec listCodecForPackType(final PackType packType) {
         ...
         return OverlayMetadataSection.OverlayEntry.IntermediateEntry.CODEC.listOf().flatXmap((list) -> PackFormat.validateHolderList(list, lastPreMinorVersion, (entry, formats) -> new OverlayMetadataSection.OverlayEntry(formats, entry.overlay())), (list) -> DataResult.success(list.stream().map((entry) -> new OverlayMetadataSection.OverlayEntry.IntermediateEntry(PackFormat.IntermediaryFormat.fromRange(entry.format(), lastPreMinorVersion), entry.overlay())).toList()));
      }
