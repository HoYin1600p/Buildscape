...
public class PathAllowList implements PathMatcher {
      ...
      return (PathMatcher)this.compiledPaths.computeIfAbsent(fileSystem.provider().getScheme(), (scheme) -> {
         ...
         switch (compiledMatchers.size()) {
            case 0:
               ...
               break;
            case 1:
               var10000 = (PathMatcher)compiledMatchers.get(0);
               break;
            default:
               var10000 = (path) -> {
   ...
   public static record ConfigEntry(PathAllowList.EntryType type, String pattern) {
      ...
      public static Optional parse(final String definition) {
            ...
            if (!definition.startsWith("[")) {
               ...
               if (split == -1) {
                  ...
                  switch (type) {
                     case "glob":
                     case "regex":
                        var10000 = Optional.of(new PathAllowList.ConfigEntry(PathAllowList.EntryType.FILESYSTEM, type + ":" + contents));
                        break;
                     case "prefix":
                        var10000 = Optional.of(new PathAllowList.ConfigEntry(PathAllowList.EntryType.PREFIX, contents));
                        break;
                     default:
