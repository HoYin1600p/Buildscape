...
public record TeleportRandomlyConsumeEffect(float diameter) implements ConsumeEffect {
   ...
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.FLOAT, TeleportRandomlyConsumeEffect::diameter, TeleportRandomlyConsumeEffect::new);
   ...
   public boolean apply(final Level level, final ItemStack stack, final LivingEntity user) {
      ...
      for(int attempt = 0; attempt < 16; ++attempt) {
         double xx = user.getX() + (user.getRandom().nextDouble() - 0.5D) * (double)this.diameter;
         double yy = Mth.clamp(user.getY() + (user.getRandom().nextDouble() - 0.5D) * (double)this.diameter, (double)level.getMinY(), (double)(level.getMinY() + ((ServerLevel)level).getLogicalHeight() - 1));
         double zz = user.getZ() + (user.getRandom().nextDouble() - 0.5D) * (double)this.diameter;
         if (user.isPassenger()) {
            user.stopRiding();
         }
