...
public class LeadItem extends Item {
   ...
   public static InteractionResult bindPlayerMobs(final Player player, final Level level, final BlockPos pos) {
      List entitiesToLeash = Leashable.leashableInArea(level, Vec3.atCenterOf(pos), (l) -> l.getLeashHolder() == player);
      if (entitiesToLeash.isEmpty()) {
         return InteractionResult.PASS;
      } else {
         Optional existingKnot = LeashFenceKnotEntity.getKnot(level, pos);
