...
public class VillagerTrade implements Validatable {
   ...
   private final Optional givenItemModifier;
   private final Holder maxUses;
   private final Holder xp;
   private final Holder reputationDiscount;
   private final Optional doubleTradePriceEnchantments;

   private VillagerTrade(final TradeCost wants, final Optional additionalWants, final ItemStackTemplate gives, final Holder maxUses, final Holder xp, final Holder reputationDiscount, final Optional merchantPredicate, final Optional givenItemModifier, final Optional doubleTradePriceEnchantments) {
      this.wants = wants;
   ...
   public @Nullable MerchantOffer getOffer(final LootContext lootContext) {
      if (this.merchantPredicate.isPresent() && !((LootItemCondition)((Holder)this.merchantPredicate.get()).value()).test(lootContext)) {
         ...
         if (this.doubleTradePriceEnchantments.isPresent()) {
            HolderSet enchantments = (HolderSet)this.doubleTradePriceEnchantments.get();
         ...
         if (itemCost.count() < 1) {
            ...
            Optional additionalItemCost = this.additionalWants.map((tradeCost) -> tradeCost.toItemCost(lootContext, 0));
   ...
   public static VillagerTrade.Builder builder(final TradeCost wants, final ItemStackTemplate gives, final int maxUses, final int xp, final float reputationDiscount) {
      return new VillagerTrade.Builder(wants, gives, ConstantValue.exactly((float)maxUses), ConstantValue.exactly((float)xp), ConstantValue.exactly(reputationDiscount));
   }

   public static VillagerTrade.Builder builder(final TradeCost wants, final TradeCost additionalWants, final ItemStackTemplate gives, final int maxUses, final int xp, final float reputationDiscount) {
      return (new VillagerTrade.Builder(wants, gives, ConstantValue.exactly((float)maxUses), ConstantValue.exactly((float)xp), ConstantValue.exactly(reputationDiscount))).additionalWants(additionalWants);
   }
   ...
   public static class Builder {
      ...
      private final Holder xp;
      private final Holder reputationDiscount;
      private Optional additionalWants = Optional.empty();
      private Optional merchantPredicate = Optional.empty();
      private Optional doubleTradePriceEnchantments = Optional.empty();
      private final ImmutableList.Builder givenItemModifiers = ImmutableList.builder();

      public Builder(final TradeCost wants, final ItemStackTemplate gives, final Holder maxUses, final Holder xp, final Holder reputationDiscount) {
         this.wants = wants;
         this.gives = gives;
         this.maxUses = maxUses;
         this.xp = xp;
      ...
      public VillagerTrade.Builder merchantPredicate(final Holder merchantPredicate) {
      ...
      @SafeVarargs
      public final VillagerTrade.Builder addModifiers(final Holder... functions) {
         return this.addModifiers(Arrays.asList(functions));
      }

      public VillagerTrade.Builder addModifiers(final Collection function) {
         this.givenItemModifiers.addAll(function);
         return this;
      }

      public VillagerTrade.Builder doubleTradePriceEnchantments(final HolderSet doubleTradePriceEnchantments) {
