...
public class StructureGridSpawner implements GameTestRunner.StructureSpawner {
   ...
   private final Function firstTestNorthWestCorner;
   private final boolean clearOnBatch;
   private final Map grids = new HashMap();

   public StructureGridSpawner(final Function firstTestNorthWestCorner, final int testsPerRow, final boolean clearOnBatch) {
   ...
   private StructureGridSpawner.DimensionGridState gridFor(final GameTestInfo testInfo) {
      return (StructureGridSpawner.DimensionGridState)this.grids.computeIfAbsent(testInfo.getTest().info().dimension(), (dimension) -> new StructureGridSpawner.DimensionGridState((BlockPos)this.firstTestNorthWestCorner.apply(dimension)));
   }
   ...
   private static final class DimensionGridState {
      ...
      private final Collection testsInLastBatch = new ArrayList();
