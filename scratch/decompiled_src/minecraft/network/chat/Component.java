...
public interface Component extends Message, FormattedText {
   ...
   default Optional visit(final FormattedText.StyledContentConsumer output, final Style parentStyle) {
      ...
      Optional selfResult = this.getContents().visit(output, selfStyle);
      if (selfResult.isPresent()) {
         return selfResult;
      } else {
         for(Component sibling : this.getSiblings()) {
            Optional result = sibling.visit(output, selfStyle);
            if (result.isPresent()) {
               return result;
            }
         }
   ...
   default Optional visit(final FormattedText.ContentConsumer output) {
      Optional selfResult = this.getContents().visit(output);
      if (selfResult.isPresent()) {
         return selfResult;
      } else {
         for(Component sibling : this.getSiblings()) {
            Optional result = sibling.visit(output);
            if (result.isPresent()) {
               return result;
            }
         }
   ...
   default boolean contains(final Component other) {
      if (this.equals(other)) {
         ...
         List flat = this.toFlatList();
         List otherFlat = other.toFlatList(this.getStyle());
   ...
   static MutableComponent nbt(final CompilableString nbtPath, final boolean interpreting, final boolean plain, final Optional separator, final DataSource dataSource) {
      return MutableComponent.create(new NbtContents(nbtPath, interpreting, plain, separator, dataSource));
   }

   static MutableComponent score(final CompilableString pattern, final String objective) {
   ...
   static MutableComponent selector(final CompilableString pattern, final Optional separator) {
