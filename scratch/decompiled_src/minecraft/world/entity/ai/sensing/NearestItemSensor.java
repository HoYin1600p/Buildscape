...
public class NearestItemSensor extends Sensor {
   ...
   protected void doTick(final ServerLevel level, final Mob body) {
      ...
      List items = level.getEntitiesOfClass(ItemEntity.class, body.getBoundingBox().inflate(32.0D, 16.0D, 32.0D), (item) -> true);
      items.sort(Comparator.comparingDouble(body::distanceToSqr));
      Optional nearestVisibleLovedItem = items.stream().filter((itemEntity) -> body.wantsToPickUp(level, itemEntity.getItem())).filter((itemEntity) -> itemEntity.closerThan(body, 32.0D)).filter(body::hasLineOfSight).findFirst();
