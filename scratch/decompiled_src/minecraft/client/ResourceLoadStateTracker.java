...
public class ResourceLoadStateTracker {
   ...
   public void startReload(final ResourceLoadStateTracker.ReloadReason reloadReason, final List packs) {
      ++this.reloadCount;
      if (this.reloadState != null && !this.reloadState.finished) {
         LOGGER.warn("Reload already ongoing, replacing");
      }

      this.reloadState = new ResourceLoadStateTracker.ReloadState(reloadReason, (List)packs.stream().map(PackResources::packId).collect(ImmutableList.toImmutableList()));
   }
   ...
   private static class ReloadState {
      ...
      private ReloadState(final ResourceLoadStateTracker.ReloadReason reloadReason, final List packs) {
