   ...
   public static class LongDistancePatrolGoal extends Goal {
      ...
      public void tick() {
         ...
         if (navigation.isDone()) {
            ...
            if (this.mob.isPatrolling() && companions.isEmpty()) {
            ...
            } else if (patrolLeader && this.mob.getPatrolTarget().closerToCenterThan(this.mob.position(), 10.0D)) {
               ...
               longDistanceTarget = distance.yRot(90.0F).scale(0.4D).add(longDistanceTarget);
               Vec3 moveTarget = longDistanceTarget.subtract(selfVector).normalize().scale(10.0D).add(selfVector);
      ...
      private List findPatrolCompanions() {
         return this.mob.level().getEntitiesOfClass(PatrollingMonster.class, this.mob.getBoundingBox().inflate(16.0D), (mob) -> mob.canJoinPatrol() && !mob.is(this.mob));
      }
