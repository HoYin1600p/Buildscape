...
public class GuardianModel extends EntityModel {
   ...
   public void setupAnim(final GuardianRenderState state) {
      ...
      if (state.lookAtPosition != null && state.lookDirection != null) {
         ...
         viewVector = new Vec3(viewVector.x, 0.0D, viewVector.z);
         Vec3 delta = (new Vec3(state.eyePosition.x - state.lookAtPosition.x, 0.0D, state.eyePosition.z - state.lookAtPosition.z)).normalize().yRot(((float)Math.PI / 2F));
