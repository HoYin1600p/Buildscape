...
public enum DragonRespawnStage implements StringRepresentable {
   START("start") {
      public void tick(final ServerLevel level, final EnderDragonFight fight, final List crystals, final int time) {
   ...
   SUMMONING_PILLARS("summoning_pillars") {
      public void tick(final ServerLevel level, final EnderDragonFight fight, final List crystals, final int time) {
         ...
         if (startOfBeam || endOfBeam) {
            List spikes = EndSpikeFeature.getSpikesForLevel(level);
            int index = time / 40;
   ...
   public static final Codec CODEC = StringRepresentable.fromEnum(DragonRespawnStage::values);
   ...
   public abstract void tick(ServerLevel level, EnderDragonFight fight, List crystals, int time);
