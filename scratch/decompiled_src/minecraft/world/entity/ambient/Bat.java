...
public class Bat extends AmbientCreature {
   ...
   protected void customServerAiStep(final ServerLevel level) {
      ...
      if (this.isResting()) {
         ...
         Vec3 newMovement = movement.add((Math.signum(dx) * 0.5D - movement.x) * (double)0.1F, (Math.signum(dy) * (double)0.7F - movement.y) * (double)0.1F, (Math.signum(dz) * 0.5D - movement.z) * (double)0.1F);
