...
public class Scoreboard {
   ...
   protected List packPlayerScores() {
      return this.playerScores.entrySet().stream().flatMap((playerEntry) -> {
         ...
         return ((PlayerScores)playerEntry.getValue()).listRawScores().entrySet().stream().map((entry) -> new Scoreboard.PackedScore(player, ((Objective)entry.getKey()).getName(), ((Score)entry.getValue()).pack()));
      }).toList();
   }
   ...
   protected Map packDisplaySlots() {
      Map displaySlots = new EnumMap(DisplaySlot.class);
   ...
   public static record PackedScore(String owner, String objective, Score.Packed score) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.STRING.fieldOf("Name").forGetter(Scoreboard.PackedScore::owner), Codec.STRING.fieldOf("Objective").forGetter(Scoreboard.PackedScore::objective), Score.Packed.MAP_CODEC.forGetter(Scoreboard.PackedScore::score)).apply(i, Scoreboard.PackedScore::new));
   }
}
