...
public class ChunkTaskPriorityQueue {
   ...
   private final List queuesPerPriority = IntStream.range(0, PRIORITY_LEVEL_COUNT).mapToObj((priority) -> new Long2ObjectLinkedOpenHashMap()).toList();
   ...
   protected void resortChunkTasks(final int oldPriority, final ChunkPos pos, final int newPriority) {
      if (oldPriority < PRIORITY_LEVEL_COUNT) {
         Long2ObjectLinkedOpenHashMap oldQueue = (Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get(oldPriority);
   ...
   public ChunkTaskPriorityQueue.@Nullable TasksForChunk pop() {
      if (!this.hasWork()) {
         ...
         Long2ObjectLinkedOpenHashMap queue = (Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get(index);
