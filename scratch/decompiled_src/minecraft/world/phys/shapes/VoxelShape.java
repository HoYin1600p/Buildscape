...
public abstract class VoxelShape {
   ...
   public VoxelShape move(final double dx, final double dy, final double dz) {
      return (VoxelShape)(this.isEmpty() ? Shapes.empty() : new ArrayVoxelShape(this.shape, (DoubleList)(new OffsetDoubleList(this.getCoords(Direction.Axis.X), dx)), (DoubleList)(new OffsetDoubleList(this.getCoords(Direction.Axis.Y), dy)), (DoubleList)(new OffsetDoubleList(this.getCoords(Direction.Axis.Z), dz))));
   }
   ...
   public @Nullable BlockHitResult clip(final Vec3 from, final Vec3 to, final BlockPos pos) {
      if (this.isEmpty()) {
         ...
         if (diff.lengthSqr() < 1.0E-7D) {
            ...
            Vec3 testPoint = from.add(diff.scale(0.001D));
   ...
   private VoxelShape calculateFace(final Direction direction) {
      ...
      if (this.isCubeLikeAlong(axis)) {
         ...
         int index = this.findIndex(axis, sign == Direction.AxisDirection.POSITIVE ? 0.9999999D : 1.0E-7D);
   ...
   private boolean isCubeLikeAlong(final Direction.Axis axis) {
      ...
      return coords.size() == 2 && DoubleMath.fuzzyEquals(coords.getDouble(0), 0.0D, 1.0E-7D) && DoubleMath.fuzzyEquals(coords.getDouble(1), 1.0D, 1.0E-7D);
   }
   ...
   protected double collideX(final AxisCycle transform, final AABB moving, double distance) {
      if (this.isEmpty()) {
         ...
         int aMin = this.findIndex(aAxis, minA + 1.0E-7D);
         int aMax = this.findIndex(aAxis, maxA - 1.0E-7D);
         int bMin = Math.max(0, this.findIndex(bAxis, moving.min(bAxis) + 1.0E-7D));
         int bMax = Math.min(this.shape.getSize(bAxis), this.findIndex(bAxis, moving.max(bAxis) - 1.0E-7D) + 1);
         int cMin = Math.max(0, this.findIndex(cAxis, moving.min(cAxis) + 1.0E-7D));
         int cMax = Math.min(this.shape.getSize(cAxis), this.findIndex(cAxis, moving.max(cAxis) - 1.0E-7D) + 1);
