...
public class EntityBlockStateFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function minecartUpdater = (input) -> this.updateBlockToBlockState(input, "DisplayTile", "DisplayData", "DisplayState");
      Function arrowUpdater = (input) -> this.updateBlockToBlockState(input, "inTile", "inData", "inBlockState");
      Type oldProjectileType = DSL.and(DSL.optional(DSL.field("inTile", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())))), DSL.remainderType());
      Function removeInTile = (input) -> input.update(oldProjectileType.finder(), DSL.remainderType(), Pair::getSecond);
   ...
   private Typed updateFallingBlock(final Typed input) {
      Type oldType = DSL.optional(DSL.field("Block", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString()))));
      Type newType = DSL.optional(DSL.field("BlockState", DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType())));
      Dynamic tag = (Dynamic)input.get(DSL.remainderFinder());
      return input.update(oldType.finder(), newType, (tile) -> {
         int block = tile.map((l) -> (Integer)((Either)l.getSecond()).map((l2) -> l2, EntityBlockStateFix::getBlockId), (r) -> {
   ...
   private Typed updateBlockToBlockState(final Typed input, final String oldFieldName, final String dataName, final String newFieldName) {
      Type oldType = DSL.field(oldFieldName, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
      Type newType = DSL.field(newFieldName, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
