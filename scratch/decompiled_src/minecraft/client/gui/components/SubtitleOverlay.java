      ...
      if (this.isListening) {
         ...
         if (!this.audibleSubtitles.isEmpty()) {
            ...
            while(iterator.hasNext()) {
               ...
               subtitle.purgeOldInstances(3000.0D * displayTimeMultiplier);
            ...
            for(SubtitleOverlay.Subtitle subtitle : this.audibleSubtitles) {
               ...
               if (closestRecentLocation != null) {
                  ...
                  boolean inView = forwardness > 0.5D;
                  int halfWidth = width / 2;
                  int height = 9;
                  int halfHeight = height / 2;
                  float scale = 1.0F;
                  int textWidth = this.minecraft.font.width(text);
                  int brightness = Mth.floor(Mth.clampedLerp((float)(Util.getMillis() - closestRecentLocation.time) / (float)(3000.0D * displayTimeMultiplier), 255.0F, 75.0F));
   ...
   private static class Subtitle {
      ...
      public SubtitleOverlay.@Nullable SoundPlayedAt getClosest(final Vec3 position) {
         if (this.playedAt.isEmpty()) {
            ...
            return this.playedAt.size() == 1 ? (SubtitleOverlay.SoundPlayedAt)this.playedAt.getFirst() : (SubtitleOverlay.SoundPlayedAt)this.playedAt.stream().min(Comparator.comparingDouble((soundPlayedAt) -> soundPlayedAt.location().distanceTo(position))).orElse((Object)null);
         }
      }
