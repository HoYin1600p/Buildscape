...
public class ServerCommandSuggestionsProvider {
   ...
   private void tryProcessRequest() {
      if (this.hasBudget) {
         ServerCommandSuggestionsProvider.Request request = (ServerCommandSuggestionsProvider.Request)this.lastRequest.getAndSet((Object)null);
         if (request != null) {
            ...
            try {
               ...
               ParseResults parse = dispatcher.parse(reader, this.player.createCommandSourceStack());
   ...
   public CompletableFuture request(final String command) {
      ServerCommandSuggestionsProvider.Request request = new ServerCommandSuggestionsProvider.Request(command);
      ServerCommandSuggestionsProvider.Request oldRequest = (ServerCommandSuggestionsProvider.Request)this.lastRequest.getAndSet(request);
      if (oldRequest == null) {
         this.trySchedule();
      }
