...
public class SoundEngine {
   ...
   private final Multimap instanceBySource = HashMultimap.create();
   private final Object2FloatMap gainBySource = (Object2FloatMap)Util.make(new Object2FloatOpenHashMap(), (map) -> map.defaultReturnValue(1.0F));
   private final List tickingSounds = Lists.newArrayList();
   private final Map queuedSounds = Maps.newHashMap();
   private final Map soundDeleteTime = Maps.newHashMap();
   private final List listeners = Lists.newArrayList();
   private final List queuedTickableSounds = Lists.newArrayList();
   private final List preloadQueue = Lists.newArrayList();
   ...
   private void tickInGameSound() {
      ...
      Iterator iterator = this.instanceToChannel.entrySet().iterator();
      ...
      Iterator queueIterator = this.queuedSounds.entrySet().iterator();

      while(queueIterator.hasNext()) {
         Map.Entry next = (Map.Entry)queueIterator.next();
   ...
   public boolean isActive(final SoundInstance instance) {
      if (!this.loaded) {
         ...
         return this.soundDeleteTime.containsKey(instance) && this.soundDeleteTime.get(instance) <= this.tickCount ? true : this.instanceToChannel.containsKey(instance);
      }
   }
   ...
   public SoundEngine.PlayResult play(final SoundInstance instance) {
      if (!this.loaded) {
         ...
         if (sound == SoundManager.INTENTIONALLY_EMPTY_SOUND) {
            ...
            CompletableFuture handleFuture = this.channelAccess.createHandle(sound.shouldStream() ? Pool.STREAMING : Pool.STATIC);
