...
public class ExperienceCommand {
   ...
   private static int addExperience(final CommandSourceStack source, final Collection players, final int amount, final ExperienceCommand.Type type) throws CommandSyntaxException {
   ...
   private static int setExperience(final CommandSourceStack source, final Collection players, final int amount, final ExperienceCommand.Type type) throws CommandSyntaxException {
      CommandResponseTracker tracker = CommandResponseTracker.create();
   ...
   private static enum Type {
      ...
      public final ToIntFunction query;
      public final String queryTranslationKey;
      public final CommandResponseTracker.MessagesWithArg addResponse;
      public final CommandResponseTracker.MessagesWithArg setResponse;

      private Type(final String name, final BiConsumer add, final BiPredicate set, final ToIntFunction query) {
         this.add = add;
         this.addResponse = CommandResponseTracker.messages((player, var2x, amount) -> Component.translatable("commands.experience.add." + name + ".success.single", amount, player.getDisplayName()), (playerCount, var2x, amount) -> Component.translatable("commands.experience.add." + name + ".success.multiple", amount, playerCount));
         this.set = set;
         this.setResponse = CommandResponseTracker.messages(ExperienceCommand.ERROR_SET_POINTS_INVALID, (player, var2x, amount) -> Component.translatable("commands.experience.set." + name + ".success.single", amount, player.getDisplayName()), (playerCount, var2x, amount) -> Component.translatable("commands.experience.set." + name + ".success.multiple", amount, playerCount));
         this.query = query;
