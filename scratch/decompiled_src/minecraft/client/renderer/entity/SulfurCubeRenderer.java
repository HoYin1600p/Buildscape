...
public class SulfurCubeRenderer extends AbstractCubeMobRenderer {
   ...
   public void submit(final SulfurCubeRenderState state, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final CameraRenderState camera) {
      this.model = (EntityModel)(state.isBaby ? this.smallModel : this.normalModel);
   ...
   protected void scale(final SulfurCubeRenderState state, final PoseStack poseStack) {
      ...
      super.scale((SlimeRenderState)state, poseStack);
   ...
   public void extractRenderState(final SulfurCube entity, final SulfurCubeRenderState state, final float partialTicks) {
      super.extractRenderState((AbstractCubeMob)entity, (SlimeRenderState)state, partialTicks);
      ...
      if (!containedBlock.isEmpty()) {
         Item blockItemState = containedBlock.getItem();
         if (blockItemState instanceof BlockItem) {
            BlockItem blockItem = (BlockItem)blockItemState;
