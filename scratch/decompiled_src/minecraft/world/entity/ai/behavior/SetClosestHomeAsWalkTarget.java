   ...
   public static BehaviorControl create(final float speedModifier) {
      ...
      return BehaviorBuilder.create((i) -> i.group(i.absent(MemoryModuleType.WALK_TARGET), i.absent(MemoryModuleType.HOME)).apply(i, (walkTarget, home) -> (level, body, timestamp) -> {
               if (level.getGameTime() - lastUpdate.longValue() < 20L) {
                  ...
                  Optional closest = poiManager.findClosest((p) -> p.is(PoiTypes.HOME), body.blockPosition(), 48, PoiManager.Occupancy.ANY);
                  if (!closest.isEmpty() && !(((BlockPos)closest.get()).distSqr(body.blockPosition()) <= 4.0D)) {
                     ...
                     Set pois = (Set)poiManager.findAllWithType((p) -> p.is(PoiTypes.HOME), cacheTest, body.blockPosition(), 48, PoiManager.Occupancy.ANY).collect(Collectors.toSet());
                     Path path = AcquirePoi.findPathToPois(body, pois);
                     if (path != null && path.canReach()) {
                        BlockPos targetPos = path.getTarget();
                        Optional type = poiManager.getType(targetPos);
                        if (type.isPresent()) {
