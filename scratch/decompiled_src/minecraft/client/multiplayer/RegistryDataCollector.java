...
public class RegistryDataCollector {
   ...
   private RegistryAccess loadNewElementsAndTags(final ResourceProvider knownDataSource, final RegistryDataCollector.ContentsCollector contentsCollector, final boolean tagsForSynchronizedRegistriesOnly) {
      LayeredRegistryAccess base = ClientRegistryLayer.createRegistryAccess();
      RegistryAccess.Frozen loadingContext = base.getAccessForLoading(ClientRegistryLayer.REMOTE);
      Map entriesToLoad = new HashMap();
      ...
      List pendingStaticTags = new ArrayList();
      if (this.tagCollector != null) {
      ...
      List contextRegistriesWithTags = TagLoader.buildUpdatedLookups(loadingContext, pendingStaticTags);
   ...
   private static void loadOnlyTags(final RegistryDataCollector.TagCollector tagCollector, final RegistryAccess.Frozen originalRegistries, final boolean includeSharedRegistries) {
