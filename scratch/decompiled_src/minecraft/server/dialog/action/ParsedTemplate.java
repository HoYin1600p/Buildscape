...
public class ParsedTemplate {
   public static final Codec CODEC = Codec.STRING.comapFlatMap(ParsedTemplate::parse, (t) -> t.raw);
   public static final Codec VARIABLE_CODEC = Codec.STRING.validate((s) -> StringTemplate.isValidVariableName(s) ? DataResult.success(s) : DataResult.error(() -> s + " is not a valid input name"));
   ...
   public String instantiate(final Map arguments) {
      List values = this.parsed.variables().stream().map((k) -> (String)arguments.getOrDefault(k, "")).toList();
