...
public class SonicBoom extends Behavior {
   ...
   protected void tick(final ServerLevel level, final Warden body, final long timestamp) {
      ...
      if (!body.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY) && !body.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
         ...
         body.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).filter(body::canTargetEntity).filter((target) -> body.closerThan(target, 15.0D, 20.0D)).ifPresent((target) -> {
            ...
            if (target.hurtServer(level, level.damageSources().sonicBoom(body), 10.0F)) {
               double knockbackVertical = 0.5D * (1.0D - target.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
               double knockbackHorizontal = 2.5D * (1.0D - target.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
