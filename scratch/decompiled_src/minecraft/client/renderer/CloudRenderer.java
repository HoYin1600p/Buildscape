...
public class CloudRenderer extends SimplePreparableReloadListener implements AutoCloseable {
   ...
   protected void apply(final Optional preparations, final ResourceManager manager, final ProfilerFiller profiler) {
      this.texture = (CloudRenderer.TextureData)preparations.orElse((Object)null);
   ...
   public void prepare(final int color, final CloudStatus cloudStatus, final float bottomY, final int range, final Vec3 cameraPosition, final long gameTime, final float partialTicks) {
      if (this.texture != null) {
         ...
         double textureWidthBlocks = (double)this.texture.width * 12.0D;
         double textureHeightBlocks = (double)this.texture.height * 12.0D;
         ...
         int cellX = Mth.floor(cloudX / 12.0D);
         int cellZ = Mth.floor(cloudZ / 12.0D);
         ...
         if (this.needsRebuild || cellX != this.prevCellX || cellZ != this.prevCellZ || relativeCameraPos != this.prevRelativeCameraPos || cloudStatus != this.prevCloudStatus) {
            ...
            this.utb.rotate();
            GpuBufferSlice.MappedView view = this.utb.currentBuffer().map(false, true);
   ...
   public void renderOit(final CloudStatus cloudStatus, final OitStage stage, final OitRenderPassProvider.Parameters params) {
      if (this.texture != null && this.quadCount != 0) {
         ...
         RenderPass renderPass = OitRenderPassProvider.createRenderPass(stage, () -> "Clouds", params);
   ...
   private void buildMesh(final CloudRenderer.RelativeCameraPos relativePos, final ByteBuffer faceBuffer, final int centerCellX, final int centerCellZ, final boolean extrude, final int radiusCells) {
      if (this.texture != null) {
   ...
   private void tryBuildCell(final CloudRenderer.RelativeCameraPos relativePos, final ByteBuffer faceBuffer, final int cellX, final int cellZ, final boolean extrude, final int relativeCellX, final int textureWidth, final int relativeCellZ, final int textureHeight, final long[] cells) {
