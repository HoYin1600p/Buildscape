...
public class DecoratedPotRenderer implements BlockEntityRenderer {
   private static final Map TRANSFORMATIONS = Util.makeEnumMap(Direction.class, DecoratedPotRenderer::createModelTransformation);
   ...
   private final Map sideCache = new HashMap();
   ...
   private DecoratedPotRenderer.SideSprite getSideSprite(final Optional item) {
      if (item.isPresent()) {
         Holder pattern = (Holder)((ItemInstance)item.get()).get(DataComponents.PROVIDES_POTTERY_PATTERN);
         if (pattern != null) {
            return (DecoratedPotRenderer.SideSprite)this.sideCache.computeIfAbsent(((DecoratedPotPattern)pattern.value()).assetId(), (id) -> DecoratedPotRenderer.SideSprite.create(this.sprites, Sheets.DECORATED_POT_MAPPER.apply(id)));
         }
      }
