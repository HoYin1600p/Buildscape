...
public class SectionUpdateTracker {
   private final RotatingSectionStorage storage;

   public SectionUpdateTracker(final LevelHeightAccessor levelHeightAccessor, final int renderDistance) {
      this.storage = new RotatingSectionStorage(renderDistance, levelHeightAccessor.getMinSectionY(), levelHeightAccessor.getMaxSectionY(), (index, sectionNode) -> new SectionUpdateTracker.SectionDirtyState(true, false, sectionNode));
   }

   public void setDirty(final int sectionX, final int sectionY, final int sectionZ, final boolean playerChanged) {
      SectionUpdateTracker.SectionDirtyState section = (SectionUpdateTracker.SectionDirtyState)this.storage.getValue(sectionX, sectionY, sectionZ);
      if (section != null) {
   ...
   public SectionUpdateTracker.@Nullable SectionDirtyState getDirtyState(final long sectionNode) {
      return (SectionUpdateTracker.SectionDirtyState)this.storage.getValue(sectionNode);
   }
