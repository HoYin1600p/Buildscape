...
public class PiglinBruteAi {
   ...
   private static ActivityData initIdleActivity() {
      return ActivityData.create(Activity.IDLE, 10, ImmutableList.of(StartAttacking.create(PiglinBruteAi::findNearestValidAttackTarget), createIdleLookBehaviors(), createIdleMovementBehaviors(), SetLookAndInteract.create(EntityTypes.PLAYER, 4)));
   }

   private static ActivityData initFightActivity(final PiglinBrute body) {
      return ActivityData.create(Activity.FIGHT, 10, ImmutableList.of(StopAttackingIfTargetInvalid.create((level, target) -> !isNearestValidAttackTarget(level, body, target)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0F), MeleeAttack.create(20)), MemoryModuleType.ATTACK_TARGET);
   }

   private static RunOne createIdleLookBehaviors() {
      return new RunOne(ImmutableList.of(Pair.of(SetEntityLookTarget.create(EntityTypes.PLAYER, 8.0F), 1), Pair.of(SetEntityLookTarget.create(EntityTypes.PIGLIN, 8.0F), 1), Pair.of(SetEntityLookTarget.create(EntityTypes.PIGLIN_BRUTE, 8.0F), 1), Pair.of(SetEntityLookTarget.create(8.0F), 1), Pair.of(new DoNothing(30, 60), 1)));
   }
   ...
   private static Optional findNearestValidAttackTarget(final ServerLevel level, final AbstractPiglin body) {
      ...
      if (angryAt.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(level, body, (LivingEntity)angryAt.get())) {
         ...
         Optional player = body.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER);
   ...
   protected static void maybePlayActivitySound(final PiglinBrute body) {
      if ((double)body.level().getRandom().nextFloat() < 0.0125D) {
         playActivitySound(body);
      }
