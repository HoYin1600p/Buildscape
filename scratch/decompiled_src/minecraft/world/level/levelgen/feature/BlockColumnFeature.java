...
public record BlockColumnFeature(List layers, Direction direction, BlockPredicate allowedPlacement, boolean prioritizeTip) implements Feature {
   ...
   public boolean place(final WorldGenLevel level, final ChunkGenerator chunkGenerator, final RandomSource random, final BlockPos origin) {
      ...
      for(int i = 0; i < layerCount; ++i) {
         layerHeights[i] = ((BlockColumnFeature.Layer)this.layers.get(i)).height().sample(random);
      ...
      if (totalHeight == 0) {
         ...
         for(int i = 0; i < layerCount; ++i) {
            ...
            if (count != 0) {
               BlockColumnFeature.Layer layer = (BlockColumnFeature.Layer)this.layers.get(i);
   ...
   public static record Layer(IntProvider height, BlockStateProvider state) {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(IntProviders.NON_NEGATIVE_CODEC.fieldOf("height").forGetter(BlockColumnFeature.Layer::height), BlockStateProvider.CODEC.fieldOf("provider").forGetter(BlockColumnFeature.Layer::state)).apply(i, BlockColumnFeature.Layer::new));
   }
}
