...
public record EndPodiumFeature(boolean active) implements Feature {
   ...
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.BOOL.optionalFieldOf("active", false).forGetter(EndPodiumFeature::active)).apply(i, EndPodiumFeature::new));
   ...
   public boolean place(final WorldGenLevel level, final ChunkGenerator chunkGenerator, final RandomSource random, final BlockPos origin) {
      for(BlockPos pos : BlockPos.betweenClosed(new BlockPos(origin.getX() - 4, origin.getY() - 1, origin.getZ() - 4), new BlockPos(origin.getX() + 4, origin.getY() + 32, origin.getZ() + 4))) {
         boolean insideRim = pos.closerThan(origin, 2.5D);
         if (insideRim || pos.closerThan(origin, 3.5D)) {
