...
public class ParticleResources implements PreparableReloadListener {
   ...
   private void registerProviders() {
      ...
      this.register(ParticleTypes.GUST_EMITTER_LARGE, new GustSeedParticle.Provider(3.0D, 7, 0));
      this.register(ParticleTypes.GUST_EMITTER_SMALL, new GustSeedParticle.Provider(1.0D, 3, 2));
   ...
   private void register(final ParticleType type, final ParticleResources.SpriteParticleRegistration provider) {
   ...
   public CompletableFuture reload(final PreparableReloadListener.SharedState currentReload, final Executor taskExecutor, final PreparableReloadListener.PreparationBarrier preparationBarrier, final Executor reloadExecutor) {
      ...
      CompletableFuture spriteSetsToLoad = CompletableFuture.supplyAsync(() -> PARTICLE_LISTER.listMatchingResources(manager), taskExecutor).thenCompose((definitionsToScan) -> {
         List loadTasks = new ArrayList(definitionsToScan.size());
      ...
      CompletableFuture pendingSprites = ((AtlasManager.PendingStitchResults)currentReload.get(AtlasManager.PENDING_STITCH)).get(AtlasIds.PARTICLES);
