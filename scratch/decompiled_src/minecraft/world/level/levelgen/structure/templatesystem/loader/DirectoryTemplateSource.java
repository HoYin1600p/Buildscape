...
public class DirectoryTemplateSource extends TemplateSource {
   ...
   public DirectoryTemplateSource(final DataFixer fixerUpper, final HolderGetter blockLookup, final Path sourceDir, final PackType packType, final FileToIdConverter fileToIdConverter, final boolean loadAsText) {
      this(fixerUpper, blockLookup, sourceDir.resolve(packType.getDirectory()), fileToIdConverter, loadAsText);
   }

   public DirectoryTemplateSource(final DataFixer fixerUpper, final HolderGetter blockLookup, final Path sourceDir, final FileToIdConverter fileToIdConverter, final boolean loadAsText) {
   ...
   public Optional load(final Identifier id) {
      if (!Files.isDirectory(this.sourceDir, new LinkOption[0])) {
         ...
         IoSupplier resource = PathPackResources.getResource(this.sourceDir, this.fileToIdConverter.idToFile(id));
