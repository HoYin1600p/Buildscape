...
public class PlayerTabOverlay {
   ...
   public void extractRenderState(final GuiGraphicsExtractor graphics, final int screenWidth, final Scoreboard scoreboard, final @Nullable Objective displayObjective) {
      List playerInfos = this.getPlayerInfos();
      List entriesToDisplay = new ArrayList(playerInfos.size());
      ...
      if (!this.healthStates.isEmpty()) {
         Set playerIds = (Set)playerInfos.stream().map((player) -> player.getProfile().id()).collect(Collectors.toSet());
   ...
   private void extractTablistScore(final Objective displayObjective, final int yo, final PlayerTabOverlay.ScoreDisplayEntry entry, final int left, final int right, final UUID profileId, final GuiGraphicsExtractor graphics) {
      if (displayObjective.getRenderType() == ObjectiveCriteria.RenderType.HEARTS) {
   ...
   private void extractTablistHearts(final int yo, final int left, final int right, final UUID profileId, final GuiGraphicsExtractor graphics, final int score) {
      PlayerTabOverlay.HealthState health = (PlayerTabOverlay.HealthState)this.healthStates.computeIfAbsent(profileId, (id) -> new PlayerTabOverlay.HealthState(score));
