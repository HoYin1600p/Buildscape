...
public class BabyFelineModel extends AbstractFelineModel {
   ...
   public void setupAnim(final FelineRenderState state) {
      ...
      if (state.lieDownAmount > 0.0F) {
         ...
         ++this.leftHindLeg.x;
         --this.leftHindLeg.z;
         this.tail1.xRot += Mth.rotLerp(state.lieDownAmountTail, this.tail1.xRot, (-(float)Math.PI / 6F));
         this.tail1.yRot += Mth.rotLerp(state.lieDownAmountTail, this.tail1.yRot, 0.0F);
         this.tail1.zRot += Mth.rotLerp(state.lieDownAmountTail, this.tail1.zRot, -0.17453292F);
         ++this.tail1.x;
         this.tail1.y += 0.5F;
         this.tail1.z -= 0.25F;
      }
