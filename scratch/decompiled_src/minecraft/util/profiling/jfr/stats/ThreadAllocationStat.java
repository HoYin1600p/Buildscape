...
public record ThreadAllocationStat(Instant timestamp, String threadName, long totalBytes) {
   ...
   public static ThreadAllocationStat.Summary summary(final List stats) {
      Map allocationsPerSecondByThread = new TreeMap();
      Map byThread = (Map)stats.stream().collect(Collectors.groupingBy((it) -> it.threadName));
