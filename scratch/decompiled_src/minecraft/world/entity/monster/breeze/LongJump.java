...
public class LongJump extends Behavior {
   ...
   private static @Nullable BlockPos snapToSurface(final LivingEntity entity, final Vec3 target) {
      ClipContext collisionBelow = new ClipContext(target, target.relative(Direction.DOWN, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, entity);
      ...
      if (surfaceBelow.getType() == HitResult.Type.BLOCK) {
         ...
         ClipContext collisionAbove = new ClipContext(target, target.relative(Direction.UP, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, entity);
   ...
   private static Optional calculateOptimalJumpVector(final Breeze body, final RandomSource random, final Vec3 targetPos) {
      for(int angle : Util.shuffledCopy(ALLOWED_ANGLES, random)) {
         ...
         Optional velocityVector = LongJumpUtil.calculateJumpVectorForAngle(body, targetPos, maxJumpVelocity, angle, false);
         if (velocityVector.isPresent()) {
            if (body.hasEffect(MobEffects.JUMP_BOOST)) {
               ...
               return velocityVector.map((v) -> v.add(0.0D, jumpEffectAmplifier, 0.0D));
            }
