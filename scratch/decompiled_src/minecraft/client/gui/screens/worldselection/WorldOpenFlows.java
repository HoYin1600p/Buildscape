...
public class WorldOpenFlows {
   ...
   public void createLevelFromExistingSettings(final LevelStorageSource.LevelStorageAccess levelSourceAccess, final ReloadableServerResources serverResources, final LayeredRegistryAccess registryAccess, final LevelDataAndDimensions.WorldDataAndGenSettings worldDataAndGenSettings, final Optional gameRules) {
      PackRepository packRepository = ServerPacksSource.createPackRepository(levelSourceAccess);
   ...
   public Pair recreateWorldData(final LevelStorageSource.LevelStorageAccess levelSourceAccess) throws Exception {
      ...
      if (DataFixers.getFileFixer().requiresFileFixing(dataVersion)) {
         ...
         return (Pair)this.loadWorldDataBlocking(packConfig, (context) -> {
            ...
            resources.close();
            DataResult existingGameRules = LevelStorageSource.readExistingSavedData(levelSourceAccess, registries.compositeAccess(), GameRuleMap.TYPE);
   ...
   private Object loadWorldDataBlocking(final WorldLoader.PackConfig packConfig, final WorldLoader.WorldDataSupplier worldDataGetter, final WorldLoader.ResultFactory worldDataSupplier) throws Exception {
      ...
      CompletableFuture resourceLoad = WorldLoader.load(config, worldDataGetter, worldDataSupplier, Util.backgroundExecutor(), this.minecraft);
   ...
   private @Nullable Dynamic tryFileFixAndReportErrors(final LevelStorageSource.LevelStorageAccess worldAccess, final Dynamic levelDataTag, final UpgradeProgress upgradeProgress, final Runnable cleanup) {
      try {
         ...
         this.minecraft.execute(() -> this.minecraft.setScreenAndShow(new AlertScreen(cleanup, Component.translatable("upgradeWorld.failed_cleanup.title"), Component.translatable("upgradeWorld.failed_cleanup.message", Component.literal(var10.newWorldFolderName()).withColor(-8355712)))));
         return null;
   ...
   private CompletableFuture loadBundledResourcePack(final DownloadedPackSource packSource, final LevelStorageSource.LevelStorageAccess levelSourceAccess) {
      ...
      if (Files.exists(mapResourceFile, new LinkOption[0]) && !Files.isDirectory(mapResourceFile, new LinkOption[0])) {
         ...
         CompletableFuture result = packSource.waitForPackFeedback(WORLD_PACK_ID);
