...
public class TagEntry {
   ...
   public static final Codec CODEC = Codec.either(ExtraCodecs.TAG_OR_ELEMENT_ID, FULL_CODEC).xmap((e) -> (TagEntry)e.map((l) -> new TagEntry(l, true), (r) -> r), (entry) -> entry.required ? Either.left(entry.elementOrTag()) : Either.right(entry));
   ...
   public boolean build(final TagEntry.Lookup lookup, final Consumer output) {
      if (this.tag) {
         Collection result = lookup.tag(this.id);
         if (result == null) {
         ...
         Object result = (T)lookup.element(this.id, this.required);
         if (result == null) {
            return !this.required;
         }
