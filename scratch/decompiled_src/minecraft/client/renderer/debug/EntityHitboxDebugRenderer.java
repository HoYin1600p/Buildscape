...
public class EntityHitboxDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
               ...
               if (this.minecraft.levelExtractor.isEntityVisible(entity, frustum, camX, camY, camZ, entityPartialTicks, chunkFadeDuration) && (entity != this.minecraft.getCameraEntity() || this.minecraft.options.getCameraType() != CameraType.FIRST_PERSON)) {
                  ...
                  if (SharedConstants.DEBUG_SHOW_LOCAL_SERVER_ENTITY_HIT_BOXES) {
                     ...
                     if (serverEntity != null) {
                        ...
                        Gizmos.billboardText("Missing Server Entity", entity.getPosition(entityPartialTicks).add(0.0D, entity.getBoundingBox().getYsize() + 1.5D, 0.0D), TextGizmo.Style.forColorAndCentered(-65536));
                     }
                  }
               }
            }
   ...
   private void showHitboxes(final Entity entity, final float partialTicks, final boolean isServerEntity) {
      ...
      if (vehicle != null) {
         ...
         Gizmos.cuboid(new AABB(position.x - (double)width, position.y, position.z - (double)width, position.x + (double)width, position.y + 0.0625D, position.z + (double)width), GizmoStyle.stroke(-256));
      }
      ...
      Vec3 eyePosition = currentPosition.add(0.0D, (double)entity.getEyeHeight(), 0.0D);
      Vec3 viewVector = entity.getViewVector(partialTicks);
      Gizmos.arrow(eyePosition, eyePosition.add(viewVector.scale(2.0D)), -16776961);
      if (isServerEntity) {
