...
public record ItemStackTemplate(Holder item, int count, DataComponentPatch components) implements ItemInstance {
   ...
   public static final Codec CODEC = Codec.withAlternative(MAP_CODEC.codec(), Item.CODEC, (item) -> new ItemStackTemplate((Item)item.value()));
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(Item.STREAM_CODEC, ItemStackTemplate::item, ByteBufCodecs.VAR_INT, ItemStackTemplate::count, DataComponentPatch.STREAM_CODEC, ItemStackTemplate::components, ItemStackTemplate::new);
