...
public class GolemRandomStrollInVillageGoal extends RandomStrollGoal {
   ...
   private @Nullable Vec3 getPositionTowardsVillagerWhoWantsGolem() {
      ...
      List villagers = level.getEntities(EntityTypes.VILLAGER, this.mob.getBoundingBox().inflate(32.0D), this::doesVillagerWantGolem);
   ...
   private @Nullable SectionPos getRandomVillageSection() {
      ServerLevel level = (ServerLevel)this.mob.level();
      List villageSections = (List)SectionPos.cube(SectionPos.of(this.mob), 2).filter((sectionPos) -> level.sectionsToVillage(sectionPos) == 0).collect(Collectors.toList());
   ...
   private @Nullable BlockPos getRandomPoiWithinSection(final SectionPos sectionPos) {
      ...
      List pois = (List)poiManager.getInRange((poiType) -> true, sectionPos.center(), 8, PoiManager.Occupancy.IS_OCCUPIED).map(PoiRecord::getPos).collect(Collectors.toList());
