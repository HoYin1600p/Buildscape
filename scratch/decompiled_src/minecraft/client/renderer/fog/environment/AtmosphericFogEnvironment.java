...
public class AtmosphericFogEnvironment extends FogEnvironment {
   ...
   public int getBaseColor(final ClientLevel level, final Camera camera, final int renderDistance, final float partialTicks) {
      ...
      if (renderDistance >= 4) {
         float sunAngle = camera.attributeProbe().getValue(EnvironmentAttributes.SUN_ANGLE, partialTicks) * ((float)Math.PI / 180F);
      ...
      skyColorMixFactor = 1.0F - (float)Math.pow((double)skyColorMixFactor, 0.25D);
   ...
   public void setupFog(final FogData fog, final Camera camera, final ClientLevel level, final float renderDistance, final DeltaTracker deltaTracker) {
      ...
      fog.cloudEnd = Math.min((float)(Minecraft.getInstance().options.cloudRange().get() * 16), camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_FOG_END_DISTANCE, partialTicks));
