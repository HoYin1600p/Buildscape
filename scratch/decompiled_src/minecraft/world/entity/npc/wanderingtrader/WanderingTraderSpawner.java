...
public class WanderingTraderSpawner implements CustomSpawner {
   ...
   private boolean spawn(final ServerLevel level) {
      ...
      if (player == null) {
         ...
         Optional poiPos = poiManager.find((p) -> p.is(PoiTypes.MEETING), (p) -> true, playerPos, 48, PoiManager.Occupancy.ANY);
         ...
         if (spawnPosition != null && this.hasEnoughSpace(level, spawnPosition)) {
            ...
            WanderingTrader trader = (WanderingTrader)EntityTypes.WANDERING_TRADER.spawn(level, spawnPosition, EntitySpawnReason.EVENT);
            if (trader != null) {
               for(int i = 0; i < 2; ++i) {
   ...
   private void tryToSpawnLlamaFor(final ServerLevel level, final WanderingTrader trader, final int radius) {
      ...
      if (spawnPosition != null) {
         TraderLlama llama = (TraderLlama)EntityTypes.TRADER_LLAMA.spawn(level, spawnPosition, EntitySpawnReason.EVENT);
         if (llama != null) {
