...
public class DefaultAttributes {
   ...
   public static void validate() {
      BuiltInRegistries.ENTITY_TYPE.stream().filter((entityType) -> entityType.getCategory() != MobCategory.MISC).filter((entityType) -> !hasSupplier(entityType)).map(BuiltInRegistries.ENTITY_TYPE::getKey).forEach((id) -> Util.logAndPauseIfInIde("Entity " + String.valueOf(id) + " has no attributes"));
   }
}
