...
public class MultifaceSpreader {
   ...
   public boolean canSpreadInAnyDirection(final BlockState state, final BlockGetter level, final BlockPos pos, final Direction startingFace) {
      return Direction.stream().anyMatch((spreadDirection) -> this.getSpreadFromFaceTowardDirection(state, level, pos, startingFace, spreadDirection, this.config::canSpreadInto).isPresent());
   }
   ...
   public long spreadAll(final BlockState state, final LevelAccessor level, final BlockPos pos, final boolean postProcess) {
      return Direction.stream().filter((faceDirection) -> this.config.canSpreadFrom(state, faceDirection)).map((faceDirection) -> this.spreadFromFaceTowardAllDirections(state, level, pos, faceDirection, postProcess)).reduce(0L, Long::sum);
   }
   ...
   public Optional spreadFromFaceTowardDirection(final BlockState state, final LevelAccessor level, final BlockPos pos, final Direction fromFace, final Direction spreadDirection, final boolean postProcess) {
      return this.getSpreadFromFaceTowardDirection(state, level, pos, fromFace, spreadDirection, this.config::canSpreadInto).flatMap((spreadPos) -> this.spreadToFace(level, spreadPos, postProcess));
   }

   public Optional getSpreadFromFaceTowardDirection(final BlockState state, final BlockGetter level, final BlockPos pos, final Direction startingFace, final Direction spreadDirection, final MultifaceSpreader.SpreadPredicate canSpreadInto) {
   ...
   public Optional spreadToFace(final LevelAccessor level, final MultifaceSpreader.SpreadPos spreadPos, final boolean postProcess) {
