...
public interface FileRelation {
   ...
   List getPaths(final Path basePath);
   ...
   private static List discoverDimensions(final Path basePath) {
      ...
      if (!Files.exists(dimensionsRoot, new LinkOption[0])) {
         ...
         try {
            Stream namespacePaths = Files.list(dimensionsRoot);

            List var4;
            try {
               List discoveredDimensions = namespacePaths.filter((x$0) -> Files.isDirectory(x$0, new LinkOption[0])).flatMap((path) -> directoriesInPath(path).stream()).toList();
   ...
   static List directoriesInPath(final Path path) {
      try {
         Stream dimensionPaths = Files.list(path);
   ...
   private static List getDefaultDimensions(final Path basePath) {
