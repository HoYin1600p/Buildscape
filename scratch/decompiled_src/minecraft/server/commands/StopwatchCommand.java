...
public class StopwatchCommand {
   ...
   public static final SuggestionProvider SUGGEST_STOPWATCHES = (c, p) -> SharedSuggestionProvider.suggestResource(((CommandSourceStack)c.getSource()).getServer().getStopwatches().ids(), p);

   public static void register(final CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("stopwatch").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.literal("create").then(Commands.argument("id", IdentifierArgument.id()).executes((c) -> createStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id")))))).then(Commands.literal("query").then(((RequiredArgumentBuilder)Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).then(Commands.argument("scale", DoubleArgumentType.doubleArg()).executes((c) -> queryStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"), DoubleArgumentType.getDouble(c, "scale"))))).executes((c) -> queryStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"), 1.0D))))).then(Commands.literal("restart").then(Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).executes((c) -> restartStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id")))))).then(Commands.literal("remove").then(Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).executes((c) -> removeStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"))))));
   }
