...
public class GuiRenderer implements AutoCloseable {
   ...
   private static final Comparator ELEMENT_SORT_COMPARATOR = Comparator.comparing(GuiElementRenderState::scissorArea, SCISSOR_COMPARATOR).thenComparing(GuiElementRenderState::pipeline, Comparator.comparing(RenderPipeline::getSortKey)).thenComparing(GuiElementRenderState::textureSetup, TEXTURE_COMPARATOR);
   ...
   public GuiRenderer(final GuiRenderState renderState, final FeatureRenderDispatcher featureRenderDispatcher, final List pictureInPictureRenderers) {
   ...
   private void clearUnusedOversizedItemRenderers() {
      Iterator oversizedItemRendererIterator = this.oversizedItemRenderers.entrySet().iterator();

      while(oversizedItemRendererIterator.hasNext()) {
         Map.Entry next = (Map.Entry)oversizedItemRendererIterator.next();
   ...
   private void draw() {
      if (!this.draws.isEmpty()) {
         ...
         if (this.draws.size() > this.firstDrawIndexAfterBlur) {
            RenderSystem.getDevice().createCommandEncoder().clearDepthTexture(mainRenderTarget.getDepthTexture(), 0.0D);
   ...
   private void executeDrawRange(final Supplier label, final RenderTarget mainRenderTarget, final GpuBufferSlice dynamicTransforms, final int startIndex, final int endIndex) {
      RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(label, mainRenderTarget.getColorTextureView(), Optional.empty(), mainRenderTarget.hasDepth() ? mainRenderTarget.getDepthTextureView() : null, OptionalDouble.empty());
   ...
   private void prepareItemElements() {
      Set itemsInFrame = this.renderState.getItemModelIdentities();
   ...
   private void preparePictureInPictureState(final PictureInPictureRenderState picturesInPictureState, final int guiScale) {
      PictureInPictureRenderer renderer = (PictureInPictureRenderer)this.pictureInPictureRenderers.get(picturesInPictureState.getClass());
      if (renderer != null) {
   ...
   private GuiItemAtlas prepareItemAtlas(final Set itemsInFrame, final int slotTextureSize) {
