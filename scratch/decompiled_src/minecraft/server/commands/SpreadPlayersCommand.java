...
public class SpreadPlayersCommand {
   ...
   private static int spreadPlayers(final CommandSourceStack source, final Vec2 center, final float spreadDistance, final float maxRange, final int maxHeight, final boolean respectTeams, final Collection entities) throws CommandSyntaxException {
      ...
      if (maxHeight < minY) {
         ...
         SpreadPlayersCommand.Position[] positions = createInitialPositions(random, respectTeams ? getNumberOfTeams(entities) : entities.size(), minX, minZ, maxX, maxZ);
   ...
   private static void spreadPositions(final Vec2 center, final double spreadDist, final ServerLevel level, final RandomSource random, final double minX, final double minZ, final double maxX, final double maxZ, final int maxHeight, final SpreadPlayersCommand.Position[] positions, final boolean respectTeams) throws CommandSyntaxException {
   ...
   private static double setPlayerPositions(final Collection entities, final ServerLevel level, final SpreadPlayersCommand.Position[] positions, final int maxHeight, final boolean respectTeams) {
      double avgDistance = 0.0D;
      int positionIndex = 0;
      Map teamPositions = Maps.newHashMap();
      ...
      for(Entity entity : entities) {
         ...
         entity.teleportTo(level, (double)Mth.floor(position.x) + 0.5D, (double)position.getSpawnY(level, maxHeight), (double)Mth.floor(position.z) + 0.5D, Set.of(), entity.getYRot(), entity.getXRot(), true);
      ...
      return entities.size() < 2 ? 0.0D : avgDistance / (double)entities.size();
   }

   private static SpreadPlayersCommand.Position[] createInitialPositions(final RandomSource random, final int count, final double minX, final double minZ, final double maxX, final double maxZ) {
