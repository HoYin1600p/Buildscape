...
public record NbtContents(CompilableString nbtPath, boolean interpreting, boolean plain, Optional separator, DataSource dataSource) implements ComponentContents {
   private static final Logger LOGGER = LogUtils.getLogger();
   public static final Codec NBT_PATH_CODEC = CompilableString.codec(new CompilableString.CommandParserHelper() {
   ...
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(NBT_PATH_CODEC.fieldOf("nbt").forGetter(NbtContents::nbtPath), Codec.BOOL.lenientOptionalFieldOf("interpret", false).forGetter(NbtContents::interpreting), Codec.BOOL.lenientOptionalFieldOf("plain", false).forGetter(NbtContents::plain), ComponentSerialization.CODEC.lenientOptionalFieldOf("separator").forGetter(NbtContents::separator), DataSources.CODEC.forGetter(NbtContents::dataSource)).apply(i, NbtContents::new)).validate((v) -> v.interpreting && v.plain ? DataResult.error(() -> "'interpret' and 'plain' flags can't be both on") : DataResult.success(v));
   ...
   public MutableComponent resolve(final ResolutionContext context, final int recursionDepth) throws CommandSyntaxException {
      ...
      if (source == null) {
         ...
         Stream elements = this.dataSource.getData(source).flatMap((t) -> {
            try {
         ...
         if (this.interpreting) {
            RegistryOps registryOps = source.registryAccess().createSerializationContext(NbtOps.INSTANCE);
            return (MutableComponent)elements.flatMap((tag) -> {
               try {
                  ...
                  LOGGER.warn("Failed to parse component: {}", tag, var5);
                  return Stream.of();
               }
