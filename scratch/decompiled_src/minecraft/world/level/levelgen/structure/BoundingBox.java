...
public class BoundingBox {
   ...
   public static final Codec CODEC = Codec.INT_STREAM.comapFlatMap((input) -> Util.fixedSize(input, 6).map((ints) -> new BoundingBox(ints[0], ints[1], ints[2], ints[3], ints[4], ints[5])), (bb) -> IntStream.of(new int[]{bb.minX, bb.minY, bb.minZ, bb.maxX, bb.maxY, bb.maxZ})).stable();
   public static final StreamCodec STREAM_CODEC = StreamCodec.composite(BlockPos.STREAM_CODEC, (box) -> new BlockPos(box.minX, box.minY, box.minZ), BlockPos.STREAM_CODEC, (box) -> new BlockPos(box.maxX, box.maxY, box.maxZ), (min, max) -> new BoundingBox(min.getX(), min.getY(), min.getZ(), max.getX(), max.getY(), max.getZ()));
   private int minX;
   private int minY;
   private int minZ;
   private int maxX;
