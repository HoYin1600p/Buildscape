...
public abstract class Structure {
   public static final Codec DIRECT_CODEC = BuiltInRegistries.STRUCTURE_TYPE.byNameCodec().dispatch(Structure::type, StructureType::codec);
   public static final Codec CODEC = RegistryCodecs.holder(Registries.STRUCTURE, DIRECT_CODEC);
   ...
   public StructureStart generate(final Holder selected, final ResourceKey dimension, final RegistryAccess registryAccess, final ChunkGenerator chunkGenerator, final BiomeResolver biomeResolver, final RandomState randomState, final StructureTemplateManager structureTemplateManager, final long seed, final ChunkPos sourceChunkPos, final int references, final LevelHeightAccessor heightAccessor, final Predicate validBiome) {
      ProfiledDuration profiled = JvmProfiler.INSTANCE.onStructureGenerate(sourceChunkPos, dimension, selected);
      Structure.GenerationContext context = new Structure.GenerationContext(registryAccess, chunkGenerator, biomeResolver, randomState, structureTemplateManager, seed, sourceChunkPos, heightAccessor, validBiome);
      Optional generation = this.findValidGenerationPoint(context);
   ...
   protected static Optional onTopOfChunkCenter(final Structure.GenerationContext context, final Heightmap.Types heightmap, final Consumer generator) {
   ...
   protected abstract Optional findGenerationPoint(final Structure.GenerationContext context);

   public Optional findValidGenerationPoint(final Structure.GenerationContext context) {
   ...
   public static record StructureSettings(HolderSet biomes, Map spawnOverrides, GenerationStep.Decoration step, TerrainAdjustment terrainAdaptation) {
      private static final Structure.StructureSettings DEFAULT = new Structure.StructureSettings(HolderSet.empty(), Map.of(), GenerationStep.Decoration.SURFACE_STRUCTURES, TerrainAdjustment.NONE);
      public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(RegistryCodecs.holderSet(Registries.BIOME).fieldOf("biomes").forGetter(Structure.StructureSettings::biomes), Codec.simpleMap(MobCategory.CODEC, StructureSpawnOverride.CODEC, StringRepresentable.keys(MobCategory.values())).fieldOf("spawn_overrides").forGetter(Structure.StructureSettings::spawnOverrides), GenerationStep.Decoration.CODEC.fieldOf("step").forGetter(Structure.StructureSettings::step), TerrainAdjustment.CODEC.optionalFieldOf("terrain_adaptation", DEFAULT.terrainAdaptation).forGetter(Structure.StructureSettings::terrainAdaptation)).apply(i, Structure.StructureSettings::new));
      ...
      public static class Builder {
         ...
         public Structure.StructureSettings.Builder spawnOverrides(final Map spawnOverrides) {
