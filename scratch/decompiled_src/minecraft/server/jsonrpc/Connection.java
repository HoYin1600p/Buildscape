...
public class Connection extends SimpleChannelInboundHandler {
   ...
   private @Nullable CompletableFuture sendRequest(final Holder.Reference methodHolder, final @Nullable Object params, final boolean expectReply) {
      OutgoingRpcMethod method = (OutgoingRpcMethod)methodHolder.value();
      if (this.minecraftApi.notificationManager().server() == null && !method.attributes().allowPreServerInit()) {
         ...
         List jsonParams = params != null ? List.of((JsonElement)Objects.requireNonNull(method.encodeParams(params))) : List.of();
         if (expectReply) {
   ...
   public @Nullable JsonElement dispatchIncomingRequest(final String method, final @Nullable JsonElement params) {
      ...
      if (identifier == null) {
         ...
         Optional incomingRpcMethod = BuiltInRegistries.INCOMING_RPC_METHOD.getOptional(identifier);
