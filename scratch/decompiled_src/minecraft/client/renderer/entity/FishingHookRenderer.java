...
public class FishingHookRenderer extends EntityRenderer {
   ...
   private Vec3 getPlayerHandPos(final Player owner, final float swing, final float partialTicks) {
      ...
      if (this.entityRenderDispatcher.options.getCameraType().isFirstPerson() && owner == Minecraft.getInstance().player) {
         float fov = (float)((Integer)this.entityRenderDispatcher.options.fov().get()).intValue();
         double viewBobbingScale = 960.0D / (double)fov;
         ...
         double rightOffset = (double)invert * 0.35D * (double)playerScale;
         double forwardOffset = 0.8D * (double)playerScale;
         float yOffset = owner.isCrouching() ? -0.1875F : 0.0F;
         return owner.getEyePosition(partialTicks).add(-cos * rightOffset - sin * forwardOffset, (double)yOffset - 0.45D * (double)playerScale, -sin * rightOffset + cos * forwardOffset);
      }
   }
   ...
   public void extractRenderState(final FishingHook entity, final FishingHookRenderState state, final float partialTicks) {
      ...
      if (owner == null) {
         ...
         Vec3 hookPos = entity.getPosition(partialTicks).add(0.0D, 0.25D, 0.0D);
