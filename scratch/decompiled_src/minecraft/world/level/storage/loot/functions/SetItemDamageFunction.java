...
public class SetItemDamageFunction extends LootItemConditionalFunction {
   ...
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> commonFields(i).and(i.group(NumberProviders.CODEC.fieldOf("damage").forGetter((f) -> f.damage), Codec.BOOL.optionalFieldOf("add", false).forGetter((f) -> f.add))).apply(i, SetItemDamageFunction::new));
   private final Holder damage;
   private final boolean add;

   private SetItemDamageFunction(final Optional condition, final Holder damage, final boolean add) {
      super(condition);
      this.damage = damage;
      this.add = add;
   }
   ...
   public static LootItemConditionalFunction.Builder setDamage(final Holder value) {
      return simpleBuilder((conditions) -> new SetItemDamageFunction(conditions, value, false));
   }

   public static LootItemConditionalFunction.Builder setDamage(final Holder value, final boolean add) {
