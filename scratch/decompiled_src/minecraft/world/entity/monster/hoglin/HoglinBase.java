...
public interface HoglinBase {
   ...
   static void throwTarget(final LivingEntity body, final LivingEntity target) {
      ...
      if (!(effectiveKnockbackPower <= 0.0D)) {
         ...
         Vec3 horizontalPushVector = (new Vec3(xd, 0.0D, zd)).normalize().scale(horizontalScale).yRot(horizontalPushAngle);
         double verticalScale = effectiveKnockbackPower * (double)random.nextFloat() * 0.5D;
