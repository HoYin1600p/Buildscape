...
public class WorldGenSettingsFix extends DataFix {
   ...
   private static Dynamic fix(final Dynamic input) {
      ...
      Optional legacyCustomOptions = (Optional)input.get("legacy_custom_options").asString().result().map(Optional::of).orElseGet(() -> name.equals(Optional.of("customized")) ? input.get("generatorOptions").asString().result() : Optional.empty());
      boolean caves = false;
      Dynamic generator;
      if (name.equals(Optional.of("customized"))) {
         ...
         switch ((String)name.get()) {
            case "flat":
               ...
               Map structureBuilder = fixFlatStructures(ops, flatSettings);
            ...
            case "buffet":
               ...
               Dynamic biomeSource = (Dynamic)settings.get("biome_source").result().orElseGet(() -> input.createMap(ImmutableMap.of(input.createString("type"), input.createString("minecraft:fixed"))));
               Dynamic fixedSource;
   ...
   private static Map fixFlatStructures(final DynamicOps ops, final OptionalDynamic settings) {
      ...
      Map structureConfig = Maps.newHashMap();
                     ...
                     switch (optionName) {
                        ...
                        case "separation":
                           if ("oceanmonument".equals(structureName)) {
                              WorldGenSettingsFix.StructureFeatureConfiguration config = (WorldGenSettingsFix.StructureFeatureConfiguration)structureConfig.getOrDefault("minecraft:monument", (WorldGenSettingsFix.StructureFeatureConfiguration)DEFAULTS.get("minecraft:monument"));
      ...
      structureBuilder.put(settings.createString("structures"), settings.createMap((Map)structureConfig.entrySet().stream().collect(Collectors.toMap((e) -> settings.createString((String)e.getKey()), (e) -> ((WorldGenSettingsFix.StructureFeatureConfiguration)e.getValue()).serialize(ops)))));
   ...
   private static void setSpacing(final Map structureConfig, final String structure, final String optionValue, final int min) {
      WorldGenSettingsFix.StructureFeatureConfiguration config = (WorldGenSettingsFix.StructureFeatureConfiguration)structureConfig.getOrDefault(structure, (WorldGenSettingsFix.StructureFeatureConfiguration)DEFAULTS.get(structure));
   ...
   private static final class StructureFeatureConfiguration {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.INT.fieldOf("spacing").forGetter((c) -> c.spacing), Codec.INT.fieldOf("separation").forGetter((c) -> c.separation), Codec.INT.fieldOf("salt").forGetter((c) -> c.salt)).apply(i, WorldGenSettingsFix.StructureFeatureConfiguration::new));
