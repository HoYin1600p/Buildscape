...
public class SectionBufferBuilderPool implements AutoCloseable {
   ...
   private final ArrayBlockingQueue freeBuffers;
   ...
   public static SectionBufferBuilderPool allocate(final int maxWorkers) {
      int maxBuffers = Math.max(1, (int)((double)Runtime.getRuntime().maxMemory() * 0.3D) / SectionBufferBuilderPack.TOTAL_BUFFERS_SIZE);
      int targetBufferCount = Math.max(1, Math.min(maxWorkers, maxBuffers));
      List buffers = new ArrayList(targetBufferCount);
