...
public class ShelfMushroomDecorator extends TreeDecorator {
   public static final MapCodec CODEC = Codec.floatRange(0.0F, 1.0F).fieldOf("probability").xmap(ShelfMushroomDecorator::new, (d) -> d.placementProbability);
   ...
   private static void placeOnStandingTree(final TreeDecorator.Context context, final List logs, final RandomSource random) {
   ...
   private static void placeOnFallenLog(final TreeDecorator.Context context, final List logs, final RandomSource random) {
   ...
   private static boolean isFallenLog(final List logs) {
   ...
   private static Direction[] perpendicularToFallenLog(final List logs) {
   ...
   private static void placeMushroom(final TreeDecorator.Context context, final BlockPos pos, final Direction facing, final RandomSource random) {
      context.setBlock(pos, (BlockState)((BlockState)Blocks.SHELF_MUSHROOM.defaultBlockState().setValue(ShelfMushroomBlock.AGE, Integer.valueOf(random.nextInt(2)))).setValue(ShelfMushroomBlock.FACING, facing));
   }
