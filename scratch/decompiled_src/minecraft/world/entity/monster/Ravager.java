...
public class Ravager extends Raider {
   ...
   private static final Predicate ROAR_TARGET_WITHOUT_GRIEFING = (entity) -> ROAR_TARGET_WITH_GRIEFING.test(entity) && !entity.is(EntityTypes.ARMOR_STAND);
   ...
   public void aiStep() {
      ...
      if (this.isAlive()) {
         if (this.isImmobile()) {
            ...
            double maxSpeed = this.getTarget() != null ? 0.35D : 0.3D;
            double baseValue = this.getAttribute(Attributes.MOVEMENT_SPEED).getBaseValue();
            this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(Mth.lerp(0.1D, baseValue, maxSpeed));
         }

         Level destroyedBlock = this.level();
         if (destroyedBlock instanceof ServerLevel) {
            ServerLevel serverLevel = (ServerLevel)destroyedBlock;
            if (this.horizontalCollision && serverLevel.getGameRules().get(GameRules.MOB_GRIEFING)) {
               boolean destroyedBlock = false;
               AABB bb = this.getBoundingBox().inflate(0.2D);
   ...
   private void stunEffect() {
      if (this.random.nextInt(6) == 0) {
         double headX = this.getX() - (double)this.getBbWidth() * Math.sin((double)(this.yBodyRot * ((float)Math.PI / 180F))) + (this.random.nextDouble() * 0.6D - 0.3D);
         double headY = this.getY() + (double)this.getBbHeight() - 0.3D;
         double headZ = this.getZ() + (double)this.getBbWidth() * Math.cos((double)(this.yBodyRot * ((float)Math.PI / 180F))) + (this.random.nextDouble() * 0.6D - 0.3D);
         this.level().addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0.49803922F, 0.5137255F, 0.57254905F), headX, headY, headZ, 0.0D, 0.0D, 0.0D);
      }
   ...
   private void roar() {
      if (this.isAlive()) {
         Level targetSelector = this.level();
         if (targetSelector instanceof ServerLevel) {
            ServerLevel level = (ServerLevel)targetSelector;
            Predicate targetSelector = level.getGameRules().get(GameRules.MOB_GRIEFING) ? ROAR_TARGET_WITH_GRIEFING : ROAR_TARGET_WITHOUT_GRIEFING;

            for(LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0D), targetSelector)) {
   ...
   private void applyRoarKnockbackClient() {
      for(LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0D), ROAR_TARGET_ON_CLIENT)) {
         this.strongKnockback(entity);
      }
   ...
   private void strongKnockback(final Entity entity) {
      ...
      double dd = Math.max(xd * xd + zd * zd, 0.001D);
      entity.push(xd / dd * 4.0D, 0.2D, zd / dd * 4.0D);
   }
   ...
   private void addRoarParticleEffects() {
      ...
      for(int i = 0; i < 40; ++i) {
         double velocityX = this.random.nextGaussian() * 0.2D;
         double velocityY = this.random.nextGaussian() * 0.2D;
         double velocityZ = this.random.nextGaussian() * 0.2D;
   ...
   protected AABB getAttackBoundingBox(final double horizontalExpansion) {
      ...
      return defaultBB.deflate(0.05D, 0.0D, 0.05D);
   }
}
