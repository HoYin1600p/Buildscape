...
public class SpearUseGoal extends Goal {
   ...
   private int getKineticWeaponUseDuration() {
      int durationTicks = Optional.ofNullable((KineticWeapon)this.mob.getMainHandItem().get(DataComponents.KINETIC_WEAPON)).map(KineticWeapon::computeDamageUseDuration).orElse(0);
   ...
   public void tick() {
      if (this.state != null) {
         ...
         if (this.state.tickAndCheckEngagement()) {
            ...
            this.state.awayPos = LandRandomPos.getPosAway(this.mob, Math.max(0.0D, (double)(9 + mountDistance) - distance), Math.max(1.0D, (double)(11 + mountDistance) - distance), 7, target.position());
