...
public record LargeDripstoneFeature(HolderSet replaceableBlocks, int floorToCeilingSearchRange, IntProvider columnRadius, FloatProvider heightScale, float maxColumnRadiusToCaveHeightRatio, FloatProvider stalactiteBluntness, FloatProvider stalagmiteBluntness, FloatProvider windSpeed, int minRadiusForWind, float minBluntnessForWind) implements Feature {
   ...
   public boolean place(final WorldGenLevel level, final ChunkGenerator chunkGenerator, final RandomSource random, final BlockPos origin) {
      if (!SpeleothemUtils.isEmptyOrWater(level, origin)) {
         ...
         Optional column = Column.scan(level, origin, this.floorToCeilingSearchRange, SpeleothemUtils::isEmptyOrWater, (state) -> SpeleothemUtils.isBaseOrLava(state, Blocks.DRIPSTONE_BLOCK, this.replaceableBlocks));
         if (!column.isEmpty()) {
            Object maxColumnRadiusBasedOnColumnHeight = column.get();
            if (maxColumnRadiusBasedOnColumnHeight instanceof Column.Range) {
               Column.Range columnRange = (Column.Range)maxColumnRadiusBasedOnColumnHeight;
               ...
               LargeDripstoneFeature.LargeDripstone stalactite = makeDripstone(origin.atY(columnRange.ceiling() - 1), false, random, radius, this.stalactiteBluntness, this.heightScale);
               LargeDripstoneFeature.LargeDripstone stalagmite = makeDripstone(origin.atY(columnRange.floor() + 1), true, random, radius, this.stalagmiteBluntness, this.heightScale);
   ...
   private static LargeDripstoneFeature.LargeDripstone makeDripstone(final BlockPos root, final boolean pointingUp, final RandomSource random, final int radius, final FloatProvider bluntness, final FloatProvider heightScale) {
   ...
   private static final class WindOffsetter {
      ...
      private WindOffsetter(final int originY, final RandomSource random, final FloatProvider windSpeedRange, final int maxOffset) {
         ...
         this.windSpeed = new Vec3((double)(Mth.cos((double)direction) * speed), 0.0D, (double)(Mth.sin((double)direction) * speed));
      }
