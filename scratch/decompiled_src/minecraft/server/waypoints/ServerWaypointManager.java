...
public class ServerWaypointManager implements WaypointManager {
   ...
   public void updateWaypoint(final WaypointTransmitter waypoint) {
      if (this.waypoints.contains(waypoint)) {
         Map playerConnection = Tables.transpose(this.connections).row(waypoint);
         ...
         while(var4.hasNext()) {
            Map.Entry waypointConnection = (Map.Entry)var4.next();
   ...
   public void updatePlayer(final ServerPlayer player) {
      Map waypointConnections = this.connections.row(player);
