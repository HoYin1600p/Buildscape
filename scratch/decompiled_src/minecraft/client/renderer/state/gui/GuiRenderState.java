...
public class GuiRenderState {
   ...
   private void navigateToAboveHighestElementWithIntersectingBounds(final ScreenRectangle bounds) {
      GuiRenderState.Node node;
      for(node = (GuiRenderState.Node)this.strata.getLast(); node.up != null; node = node.up) {
      }
   ...
   public void forEachElement(final Consumer consumer, final GuiRenderState.TraverseRange range) {
      this.traverse((node) -> {
   ...
   private void traverse(final Consumer consumer, final GuiRenderState.TraverseRange range) {
      int startIndex = 0;
   ...
   private void traverse(final GuiRenderState.Node node, final Consumer consumer) {
