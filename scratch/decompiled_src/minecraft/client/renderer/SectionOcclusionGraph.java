...
public class SectionOcclusionGraph {
   ...
   private volatile @Nullable BlockingQueue nextSectionsToPropagateFrom;
   ...
   public void invalidateIfNeeded(final CameraRenderState camera, final int fov) {
      ...
      double camX = Math.floor(cameraPos.x / 8.0D);
      double camY = Math.floor(cameraPos.y / 8.0D);
      double camZ = Math.floor(cameraPos.z / 8.0D);
   ...
   public void schedulePropagationFrom(final SectionRenderDispatcher.RenderSection section) {
      BlockingQueue nextSectionsToPropagateFrom = this.nextSectionsToPropagateFrom;
      if (nextSectionsToPropagateFrom != null) {
         nextSectionsToPropagateFrom.add(section);
      }

      BlockingQueue sectionsToPropagateFrom = ((SectionOcclusionGraph.GraphState)this.currentGraph.get()).sectionsToPropagateFrom;
   ...
   private void runPartialUpdate(final CameraRenderState camera, final LongSet loadedExpectedChunks) {
      ...
      if (!state.sectionsToPropagateFrom.isEmpty()) {
         ...
         Consumer onSectionAdded = (section) -> {
   ...
   private void initializeQueueForFullUpdate(final BlockPos cameraPosition, final Queue queue) {
      ...
      if (cameraSection == null) {
         ...
         queue.add(new SectionOcclusionGraph.Node(cameraSection, (Direction)null, 0));
      }

   }

   private void runUpdates(final SectionOcclusionGraph.GraphStorage storage, final Vec3 cameraPos, final Queue queue, final boolean smartCull, final Consumer onSectionAdded, final LongOpenHashSet emptySections, final LongOpenHashSet loadedChunks) {
            ...
            for(Direction direction : DIRECTIONS) {
               ...
               if (renderSectionAt != null && (!smartCull || !node.hasDirection(direction.getOpposite()))) {
                  ...
                  if (smartCull && distantFromCamera) {
                     ...
                     while(checkPos.distanceSquared(cameraPos.x, cameraPos.y, cameraPos.z) > 3600.0D) {
                        checkPos.add(step);
   ...
   private static record GraphState(SectionOcclusionGraph.GraphStorage storage, BlockingQueue sectionsToPropagateFrom) {
   ...
   private static class GraphStorage {
      ...
      public final Long2ObjectMap sectionsWaitingForChunkLoads;
