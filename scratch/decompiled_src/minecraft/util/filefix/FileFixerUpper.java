...
public class FileFixerUpper {
   ...
   private final List fileFixes;
   private final int latestFileFixerVersion;

   public FileFixerUpper(final DataFixerBuilder.Result dataFixer, final List fileFixes, final int latestFileFixerVersion) {
   ...
   private List startOrContinueFileFixing(final UpgradeProgress upgradeProgress, final int toVersion, final Path worldFolder, final Path tempWorld, final Path fileFixDirectory, final int loadedVersion) throws IOException {
   ...
   private List applyFileFixersOnCow(final UpgradeProgress upgradeProgress, final int loadedVersion, final int toVersion, final Path worldFolder, final Path fileFixDirectory, final Path tempWorld) throws IOException {
      CopyOnWriteFileSystem fs = CopyOnWriteFileSystem.create(worldFolder.getFileName().toString(), worldFolder, fileFixDirectory.resolve("cow"), fileFixDirectory::equals);
   ...
   public void applyFileFixers(final UpgradeProgress upgradeProgress, final int loadedVersion, final int toVersion, final Path basePath) throws IOException {
      List applicableFixers = this.getApplicableFixers(loadedVersion, toVersion);
   ...
   private List getApplicableFixers(final int fromVersion, final int toVersion) {
   ...
   private void countFiles(final List applicableFixers, final UpgradeProgress upgradeProgress) {
   ...
   @VisibleForTesting
   protected static void swapInFixedWorld(final LevelStorageSource.LevelStorageAccess worldAccess, final List moves, final Path fileFixDirectory, final Path tempWorld, final UpgradeProgress upgradeProgress) throws FileFixException {
      Path worldFolder = worldAccess.getLevelDirectory().path();
      ...
      if (!fileSystemCapabilities.atomicMove()) {
         ...
         if (fileSystemCapabilities.hardLinks()) {
            ...
            try {
               ...
               List failedMoves = CopyOnWriteFileSystem.tryRevertMoves(moves, moveOptions);
         ...
         try {
            worldAccess.releaseTemporarilyAndRun(() -> {
               ...
               try {
                  ...
                  LOGGER.warn("Failed to move outdated world folder out of the way; will try to delete instead: ", var10);

                  try {
                     deleteDirectory(worldFolder);
                  } catch (Exception var9) {
                     LOGGER.warn("Failed to delete outdated world folder: ", var10);
                     throw new FailedCleanupFileFixException(var10, tempWorldTopLevel.getFileName().toString(), fileSystemCapabilities);
                  }
               }
               ...
               try {
                  ...
                  LOGGER.warn("Failed to move in new world folder: ", var8);
                  throw new FailedCleanupFileFixException(var8, tempWorldTopLevel.getFileName().toString(), fileSystemCapabilities);
               }
   ...
   private static void writeMoves(final List moves, final Path fromDirectory, final Path toDirectory, final Path filePath) throws IOException {
      Codec codec = FileFixerUpper.UpgradeInProgress.codec(fromDirectory, toDirectory);
   ...
   private static List readMoves(final Path fromDirectory, final Path toDirectory, final Path filePath) throws IOException {
   ...
   public static class Builder {
      public final List fileFixes = new ArrayList();
      private final int currentVersion;
      private int latestFileFixerVersion;
      private final List knownSchemas = new ArrayList();
      ...
      public Schema addSchema(final DataFixerBuilder fixerUpper, final int version, final BiFunction factory) {
   ...
   public static record UpgradeInProgress(List moves) {
      public static Codec codec(final Path fromDirectory, final Path toDirectory) {
         return RecordCodecBuilder.create((i) -> i.group(FileMove.moveCodec(fromDirectory, toDirectory).listOf().fieldOf("moves").forGetter(FileFixerUpper.UpgradeInProgress::moves)).apply(i, FileFixerUpper.UpgradeInProgress::new));
      }
   }
}
