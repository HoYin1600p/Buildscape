   ...
   private void makeParticle(final int amount) {
      ...
      if (colorValue != -1 && amount > 0) {
         for(int i = 0; i < amount; ++i) {
            this.level().addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, colorValue), this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0.0D, 0.0D, 0.0D);
         }
   ...
   public void handleEntityEvent(final @EntityEvent.Value byte id) {
      if (id == 0) {
         ...
         if (colorValue != -1) {
            ...
            for(int i = 0; i < 20; ++i) {
               this.level().addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, red, green, blue), this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0.0D, 0.0D, 0.0D);
            }
         }
      } else {
