...
public class NautilusAi {
   ...
   protected static final TargetingConditions ATTACK_TARGET_CONDITIONS = TargetingConditions.forCombat().selector((target, level) -> (level.getGameRules().get(GameRules.MOB_GRIEFING) || !target.is(EntityTypes.ARMOR_STAND)) && level.getWorldBorder().isWithinBounds(target.getBoundingBox()));
   ...
   private static ActivityData initIdleActivity() {
      return ActivityData.create(Activity.IDLE, ImmutableList.of(Pair.of(1, new AnimalMakeLove(EntityTypes.NAUTILUS, 0.4F, 2)), Pair.of(2, new FollowTemptation((mob) -> 1.3F, (mob) -> mob.isBaby() ? 2.5D : 3.5D)), Pair.of(3, StartAttacking.create(NautilusAi::findNearestValidAttackTarget)), Pair.of(4, new GateBehavior(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableSet.of(), GateBehavior.OrderPolicy.ORDERED, GateBehavior.RunningPolicy.TRY_ALL, ImmutableList.of(Pair.of(RandomStroll.swim(1.0F), 2), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 3))))));
   }
   ...
   protected static Optional findNearestValidAttackTarget(final ServerLevel level, final AbstractNautilus body) {
      if (!BehaviorUtils.isBreeding(body) && body.isInWater() && !body.isBaby() && !body.isTame()) {
         Optional angryAt = BehaviorUtils.getLivingEntityFromUUIDMemory(body, MemoryModuleType.ANGRY_AT).filter((entity) -> entity.isInWater() && Sensor.isEntityAttackableIgnoringLineOfSight(level, body, entity));
         if (angryAt.isPresent()) {
            ...
            body.getBrain().setMemory(MemoryModuleType.ATTACK_TARGET_COOLDOWN, TIME_BETWEEN_NON_PLAYER_ATTACKS.sample(random));
            return random.nextFloat() < 0.5F ? Optional.empty() : ((NearestVisibleLivingEntities)body.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).orElse(NearestVisibleLivingEntities.empty())).findClosest(NautilusAi::isHostileTarget);
         }
      } else {
         return Optional.empty();
      }
