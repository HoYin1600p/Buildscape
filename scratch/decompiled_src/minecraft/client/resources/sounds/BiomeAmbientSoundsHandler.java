...
public class BiomeAmbientSoundsHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      Holder currentLoopSound = (Holder)ambientSounds.loop().orElse((Object)null);
      ...
      ambientSounds.mood().ifPresent((mood) -> {
         ...
         if (this.moodiness >= 1.0F) {
            double blockSampleX = (double)blockSamplingPos.getX() + 0.5D;
            double blockSampleY = (double)blockSamplingPos.getY() + 0.5D;
            double blockSampleZ = (double)blockSamplingPos.getZ() + 0.5D;
