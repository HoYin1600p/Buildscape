...
public record IsKeybindDown(KeyMapping keybind) implements ConditionalItemModelProperty {
   private static final Codec KEYBIND_CODEC = Codec.STRING.comapFlatMap((id) -> {
      KeyMapping mapping = KeyMapping.get(id);
      return mapping != null ? DataResult.success(mapping) : DataResult.error(() -> "Invalid keybind: " + id);
   }, KeyMapping::getName);
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(KEYBIND_CODEC.fieldOf("keybind").forGetter(IsKeybindDown::keybind)).apply(i, IsKeybindDown::new));
