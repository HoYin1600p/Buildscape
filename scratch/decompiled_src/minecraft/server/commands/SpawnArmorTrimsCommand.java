...
public class SpawnArmorTrimsCommand {
   private static final List VANILLA_TRIM_PATTERNS = List.of(TrimPatterns.SENTRY, TrimPatterns.DUNE, TrimPatterns.COAST, TrimPatterns.WILD, TrimPatterns.WARD, TrimPatterns.EYE, TrimPatterns.VEX, TrimPatterns.TIDE, TrimPatterns.SNOUT, TrimPatterns.RIB, TrimPatterns.SPIRE, TrimPatterns.WAYFINDER, TrimPatterns.SHAPER, TrimPatterns.SILENCE, TrimPatterns.RAISER, TrimPatterns.HOST, TrimPatterns.FLOW, TrimPatterns.BOLT);
   private static final List VANILLA_TRIM_MATERIALS = List.of(TrimMaterials.QUARTZ, TrimMaterials.IRON, TrimMaterials.NETHERITE, TrimMaterials.REDSTONE, TrimMaterials.COPPER, TrimMaterials.GOLD, TrimMaterials.EMERALD, TrimMaterials.DIAMOND, TrimMaterials.LAPIS, TrimMaterials.AMETHYST, TrimMaterials.RESIN);
   private static final ToIntFunction TRIM_PATTERN_ORDER = Util.createIndexLookup(VANILLA_TRIM_PATTERNS);
   private static final ToIntFunction TRIM_MATERIAL_ORDER = Util.createIndexLookup(VANILLA_TRIM_MATERIALS);
   ...
   private static int spawnArmorTrims(final CommandSourceStack source, final Player player, final Stream patterns) {
      ...
      List sortedPatterns = patterns.sorted(Comparator.comparing((h) -> TRIM_PATTERN_ORDER.applyAsInt(h.key()))).toList();
      List sortedMaterials = level.registryAccess().lookupOrThrow(Registries.TRIM_MATERIAL).listElements().sorted(Comparator.comparing((h) -> TRIM_MATERIAL_ORDER.applyAsInt(h.key()))).toList();
      List equippableItems = findEquippableItemsWithAssets(level.registryAccess().lookupOrThrow(Registries.ITEM));
      ...
      for(int materialIndex = 0; materialIndex < sortedMaterials.size(); ++materialIndex) {
         Holder.Reference material = (Holder.Reference)sortedMaterials.get(materialIndex);

         for(int patternIndex = 0; patternIndex < sortedPatterns.size(); ++patternIndex) {
            Holder.Reference pattern = (Holder.Reference)sortedPatterns.get(patternIndex);
            ArmorTrim trim = new ArmorTrim(material, pattern);

            for(int itemIndex = 0; itemIndex < equippableItems.size(); ++itemIndex) {
               Holder.Reference equippableItem = (Holder.Reference)equippableItems.get(itemIndex);
               double x = (double)origin.getX() + 0.5D - (double)itemIndex * 3.0D;
               double y = (double)origin.getY() + 0.5D + (double)materialIndex * 3.0D;
               double z = (double)origin.getZ() + 0.5D + (double)(patternIndex * 10);
