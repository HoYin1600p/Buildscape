...
public class Guardian extends Monster {
   ...
   protected void registerGoals() {
      MoveTowardsRestrictionGoal goal = new MoveTowardsRestrictionGoal(this, 1.0D);
      ...
      if (this.isAlive()) {
         if (this.level().isClientSide()) {
            ...
            if (!this.isInWater()) {
               ...
               if (movement.y > 0.0D && this.clientSideTouchedGround && !this.isSilent()) {
                  this.level().playLocalSound(this.getX(), this.getY(), this.getZ(), this.getFlopSound(), this.getSoundSource(), 1.0F, 1.0F, false);
               }

               this.clientSideTouchedGround = movement.y < 0.0D && this.level().loadedAndEntityCanStandOn(this.blockPosition().below(), this);
            ...
            if (this.isMoving() && this.isInWater()) {
               ...
               for(int i = 0; i < 2; ++i) {
                  this.level().addParticle(ParticleTypes.BUBBLE, this.getRandomX(0.5D) - viewVector.x * 1.5D, this.getRandomY() - viewVector.y * 1.5D, this.getRandomZ(0.5D) - viewVector.z * 1.5D, 0.0D, 0.0D, 0.0D);
               }
            }
            ...
            if (this.hasActiveAttackTarget()) {
               ...
               if (attackTarget != null) {
                  ...
                  double dy = attackTarget.getY(0.5D) - this.getEyeY();
                  ...
                  while(dist < dd) {
                     dist += 1.8D - at + this.random.nextDouble() * (1.7D - at);
                     this.level().addParticle(ParticleTypes.BUBBLE, this.getX() + dx * dist, this.getEyeY() + dy * dist, this.getZ() + dz * dist, 0.0D, 0.0D, 0.0D);
                  }
               }
            }
         }
   ...
   private static class GuardianAttackSelector implements TargetingConditions.Selector {
      ...
      public boolean test(final @Nullable LivingEntity target, final ServerLevel level) {
         return (target instanceof Player || target instanceof Squid || target instanceof Axolotl) && target.distanceToSqr(this.guardian) > 9.0D;
      }
   }
   ...
   private static class GuardianMoveControl extends MoveControl {
      ...
      public void tick() {
         if (this.operation == MoveControl.Operation.MOVE_TO && !((Guardian)this.mob).getNavigation().isDone()) {
            ...
            double push = Math.sin((double)(((Guardian)this.mob).tickCount + ((Guardian)this.mob).getId()) * 0.5D) * 0.05D;
            double cos = Math.cos((double)(((Guardian)this.mob).getYRot() * ((float)Math.PI / 180F)));
            double sin = Math.sin((double)(((Guardian)this.mob).getYRot() * ((float)Math.PI / 180F)));
            double yPush = Math.sin((double)(((Guardian)this.mob).tickCount + ((Guardian)this.mob).getId()) * 0.75D) * 0.05D;
            ((Guardian)this.mob).setDeltaMovement(((Guardian)this.mob).getDeltaMovement().add(push * cos, yPush * (sin + cos) * 0.25D + (double)newSpeed * yd * 0.1D, push * sin));
            LookControl control = ((Guardian)this.mob).getLookControl();
            double newLookX = ((Guardian)this.mob).getX() + xd * 2.0D;
            double newLookY = ((Guardian)this.mob).getEyeY() + yd / length;
            double newLookZ = ((Guardian)this.mob).getZ() + zd * 2.0D;
            ...
            ((Guardian)this.mob).getLookControl().setLookAt(Mth.lerp(0.125D, oldLookX, newLookX), Mth.lerp(0.125D, oldLookY, newLookY), Mth.lerp(0.125D, oldLookZ, newLookZ), 10.0F, 40.0F);
