...
public class EntityEquipment {
   public static final Codec CODEC = Codec.unboundedMap(EquipmentSlot.CODEC, ItemStack.CODEC).xmap((items) -> {
      EnumMap map = new EnumMap(EquipmentSlot.class);
      map.putAll(items);
      return new EntityEquipment(map);
   }, (equipment) -> {
      Map items = new EnumMap(equipment.items);
