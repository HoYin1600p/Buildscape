...
public class ScheduleCommand {
   ...
   private static final SuggestionProvider SUGGEST_SCHEDULE = (c, p) -> SharedSuggestionProvider.suggestResource(((CommandSourceStack)c.getSource()).getServer().getScheduledEvents().getEventsIds().stream().map(Identifier::parse), p);
   ...
   private static int schedule(final CommandSourceStack source, final Pair callback, final int time, final boolean replace) throws CommandSyntaxException {
      if (time == 0) {
         ...
         TimerQueue queue = source.getServer().getScheduledEvents();
