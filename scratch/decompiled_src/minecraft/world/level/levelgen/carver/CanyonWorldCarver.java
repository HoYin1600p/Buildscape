...
public record CanyonWorldCarver(float probability, HeightProvider y, FloatProvider verticalRotation, CanyonWorldCarver.Shape shape) implements WorldCarver {
   public static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.floatRange(0.0F, 1.0F).fieldOf("probability").forGetter((c) -> c.probability), HeightProvider.CODEC.fieldOf("y").forGetter((c) -> c.y), FloatProviders.CODEC.fieldOf("vertical_rotation").forGetter((c) -> c.verticalRotation), CanyonWorldCarver.Shape.CODEC.fieldOf("shape").forGetter((c) -> c.shape)).apply(i, CanyonWorldCarver::new));
   ...
   private void doCarve(final WorldGenerationContext context, final ChunkPos chunkPos, final long tunnelSeed, double x, double y, double z, final float thickness, float horizontalRotation, float verticalRotation, final int step, final int distance, final double yScale, final CarverOutput output) {
      ...
      for(int currentStep = step; currentStep < distance; ++currentStep) {
         double horizontalRadius = 1.5D + (double)(Mth.sin((double)((float)currentStep * (float)Math.PI / (float)distance)) * thickness);
   ...
   private boolean shouldSkip(final WorldGenerationContext context, final float[] widthFactorPerHeight, final double xd, final double yd, final double zd, final int y) {
      ...
      return (xd * xd + zd * zd) * (double)widthFactorPerHeight[yIndex - 1] + yd * yd / 6.0D >= 1.0D;
   }
