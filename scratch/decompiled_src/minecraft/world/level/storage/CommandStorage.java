...
public class CommandStorage {
   ...
   private CommandStorage.@Nullable Container getContainer(final String namespace) {
      ...
      if (container != null) {
         ...
         CommandStorage.Container newContainer = (CommandStorage.Container)this.savedDataStorage.get(CommandStorage.Container.type(namespace));
   ...
   private CommandStorage.Container getOrCreateContainer(final String namespace) {
      ...
      if (container != null) {
         ...
         CommandStorage.Container newContainer = (CommandStorage.Container)this.savedDataStorage.computeIfAbsent(CommandStorage.Container.type(namespace));
         this.namespaces.put(namespace, newContainer);
         return newContainer;
      }
   }
   ...
   private static class Container extends SavedData {
      public static final Codec CODEC = RecordCodecBuilder.create((i) -> i.group(Codec.unboundedMap(ExtraCodecs.RESOURCE_PATH_CODEC, CompoundTag.CODEC).fieldOf("contents").forGetter((container) -> container.storage)).apply(i, CommandStorage.Container::new));
      ...
      public static SavedDataType type(final String namespace) {
         return new SavedDataType(Identifier.fromNamespaceAndPath(namespace, "command_storage"), CommandStorage.Container::new, CODEC, DataFixTypes.SAVED_DATA_COMMAND_STORAGE);
      }
