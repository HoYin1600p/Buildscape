...
public class HumanoidModel extends EntityModel implements ArmedModel, HeadedModel {
   ...
   protected static final Map ADULT_ARMOR_PARTS_PER_SLOT = Maps.newEnumMap(Map.of(EquipmentSlot.HEAD, Set.of("head"), EquipmentSlot.CHEST, Set.of("body", "left_arm", "right_arm"), EquipmentSlot.LEGS, Set.of("left_leg", "right_leg", "body"), EquipmentSlot.FEET, Set.of("left_leg", "right_leg")));
   protected static final Map BABY_ARMOR_PARTS_PER_SLOT = Maps.newEnumMap(Map.of(EquipmentSlot.HEAD, Set.of("head"), EquipmentSlot.CHEST, Set.of("body", "left_arm", "right_arm"), EquipmentSlot.LEGS, Set.of("left_leg", "right_leg", "waist"), EquipmentSlot.FEET, Set.of("left_foot", "right_foot")));
   ...
   public static ArmorModelSet createBabyArmorMeshSet(final CubeDeformation innerDeformation, final CubeDeformation outerDeformation, final PartPose armOffset) {
   ...
   protected static ArmorModelSet createArmorMeshSet(final Function baseFactory, final Map partsPerSlot, final CubeDeformation innerDeformation, final CubeDeformation outerDeformation) {
   ...
   protected void setupSwimAnimation(final HumanoidRenderState state, final float animationPos, final float swimAmount) {
      if (!state.isFallFlying) {
   ...
   protected void setupAttackAnimation(final HumanoidRenderState state) {
      ...
      if (!(swingAnimation <= 0.0F) && swing != null) {
         ...
         this.body.yRot = Mth.sin((double)(Mth.sqrt(swingAnimation) * ((float)Math.PI * 2F))) * 0.2F;
         ...
         switch (swing.animation().type()) {
            case WHACK:
               ...
               attackArm.zRot += Mth.sin((double)(swingAnimation * (float)Math.PI)) * -0.4F;
               break;
            case STAB:
