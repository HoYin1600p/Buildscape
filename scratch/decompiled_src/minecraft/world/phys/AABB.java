...
public class AABB {
   ...
   private static @Nullable Direction clipPoint(final double[] scaleReference, final @Nullable Direction direction, final double da, final double db, final double dc, final double point, final double minB, final double maxB, final double minC, final double maxC, final Direction newDirection, final double fromA, final double fromB, final double fromC) {
      ...
      if (0.0D < s && s < scaleReference[0] && minB - 1.0E-7D < pb && pb < maxB + 1.0E-7D && minC - 1.0E-7D < pc && pc < maxC + 1.0E-7D) {
         scaleReference[0] = s;
         return newDirection;
      } else {
         return direction;
   ...
   public boolean collidedAlongVector(final Vec3 vector, final List aabbs) {
      ...
      for(AABB shapePart : aabbs) {
         AABB inflated = shapePart.inflate(this.getXsize() * 0.5D - 1.0E-7D, this.getYsize() * 0.5D - 1.0E-7D, this.getZsize() * 0.5D - 1.0E-7D);
   ...
   public double distanceToSqr(final Vec3 point) {
      double dx = Math.max(Math.max(this.minX - point.x, point.x - this.maxX), 0.0D);
      double dy = Math.max(Math.max(this.minY - point.y, point.y - this.maxY), 0.0D);
      double dz = Math.max(Math.max(this.minZ - point.z, point.z - this.maxZ), 0.0D);
      return Mth.lengthSquared(dx, dy, dz);
   }

   public double distanceToSqr(final AABB boundingBox) {
      double dx = Math.max(Math.max(this.minX - boundingBox.maxX, boundingBox.minX - this.maxX), 0.0D);
      double dy = Math.max(Math.max(this.minY - boundingBox.maxY, boundingBox.minY - this.maxY), 0.0D);
      double dz = Math.max(Math.max(this.minZ - boundingBox.maxZ, boundingBox.minZ - this.maxZ), 0.0D);
      return Mth.lengthSquared(dx, dy, dz);
   }
