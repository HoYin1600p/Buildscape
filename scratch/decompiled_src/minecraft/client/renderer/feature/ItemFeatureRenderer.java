...
public class ItemFeatureRenderer extends RenderTypeFeatureRenderer {
   public static final FeatureRendererType TYPE = FeatureRendererType.create("Item");
   ...
   private void prepareMainSubmit(final ItemFeatureRenderer.Submit submit) {
      ...
      for(BakedQuad quad : submit.quads()) {
         ...
         switch (foilType) {
            case NONE:
               var10000 = material.itemRenderType();
               break;
            case STANDARD:
               var10000 = material.itemGlintRenderType();
               break;
            case SPECIAL:
               var10000 = material.itemGlintSpecialRenderType();
               break;
            default:
   ...
   public static record Submit(PoseStack.Pose pose, ItemDisplayContext displayContext, int lightCoords, int overlayCoords, int outlineColor, int[] tintLayers, List quads, ItemStackRenderState.FoilType foilType) implements TranslucentSubmit {
