...
public class FollowPlayerRiddenEntityGoal extends Goal {
   ...
   public boolean canUse() {
      if (this.following != null && this.following.hasMovedHorizontallyRecently()) {
         ...
         for(Entity entity : this.mob.level().getEntitiesOfClass(this.entityTypeToFollow, this.mob.getBoundingBox().inflate(5.0D))) {
   ...
   public void tick() {
      ...
      if (--this.timeToRecalcPath <= 0) {
         ...
         if (this.currentGoal == FollowPlayerRiddenEntityGoal.FollowEntityGoal.GO_TO_ENTITY) {
            ...
            this.mob.getNavigation().moveTo((double)behindEntityPos.getX(), (double)behindEntityPos.getY(), (double)behindEntityPos.getZ(), 1.0D);
            ...
            this.mob.getNavigation().moveTo((double)goTo.getX(), (double)(goTo.getY() - 1), (double)goTo.getZ(), 1.0D);
   ...
   private static enum FollowEntityGoal {
      ...
      // $FF: synthetic method
      private static FollowPlayerRiddenEntityGoal.FollowEntityGoal[] $values() {
         return new FollowPlayerRiddenEntityGoal.FollowEntityGoal[]{GO_TO_ENTITY, GO_IN_ENTITY_DIRECTION};
      }
   }
}
