...
public class TpsDebugChart extends AbstractDebugChart {
   ...
   protected int getSampleHeight(final double nanos) {
      return (int)Math.round(toMilliseconds(nanos) * 60.0D / (double)((Float)this.msptSupplier.get()).floatValue());
   }

   protected int getSampleColor(final long nanos) {
      float mspt = this.msptSupplier.get();
      return this.getSampleColor(toMilliseconds((double)nanos), (double)mspt, -16711936, (double)mspt * 1.125D, -256, (double)mspt * 1.25D, -65536);
   }
