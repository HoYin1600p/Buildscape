...
public class OctreeDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void renderNode(final Octree.Node node, final int depth, final boolean fullyVisible, final MutableInt count, final boolean isClose) {
      ...
      long size = Math.round(xSize / 16.0D);
      if (size == 1L) {
         count.add(1);
      ...
      Gizmos.cuboid(aabb.deflate(0.1D * (double)depth), GizmoStyle.stroke(ARGB.colorFromFloat(fullyVisible ? 0.4F : 1.0F, getColorComponent(colorNum, 0.3F), getColorComponent(colorNum, 0.8F), getColorComponent(colorNum, 0.5F))));
   }
