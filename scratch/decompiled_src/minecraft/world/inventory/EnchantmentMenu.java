...
public class EnchantmentMenu extends AbstractContainerMenu {
      ...
      if (container == this.enchantSlots) {
         ...
         if (!itemStack.isEmpty() && itemStack.isEnchantable()) {
            this.access.execute((level, pos) -> {
               IdMap holders = level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).asHolderIdMap();
               int bookcases = 0;
               ...
               for(int i = 0; i < 3; ++i) {
                  if (this.costs[i] > 0) {
                     List list = this.getEnchantmentList(level.registryAccess(), itemStack, i, this.costs[i]);
                     if (!list.isEmpty()) {
      ...
      if (buttonId >= 0 && buttonId < this.costs.length) {
         ...
         if ((currency.isEmpty() || currency.getCount() < enchantmentCost) && !player.hasInfiniteMaterials()) {
            ...
            this.access.execute((level, pos) -> {
               ...
               List newEnchantment = this.getEnchantmentList(level.registryAccess(), itemStack, buttonId, this.costs[buttonId]);
               if (!newEnchantment.isEmpty()) {
                  ...
                  level.playSound((Entity)null, pos, SoundEvents.ENCHANTMENT_TABLE_USE, SoundSource.BLOCKS, 1.0F, level.getRandom().nextFloat() * 0.1F + 0.9F);
               }
   ...
   private List getEnchantmentList(final RegistryAccess access, final ItemStack itemStack, final int slot, final int enchantmentCost) {
      ...
      if (tag.isEmpty()) {
         ...
         List list = EnchantmentHelper.selectEnchantment(this.random, itemStack, enchantmentCost, ((HolderSet.Named)tag.get()).stream());
